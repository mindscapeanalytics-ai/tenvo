...
import {
    ...
    DollarSign, AlertTriangle, Droplets, Milk
} from 'lucide-react';
...
export function DomainDashboard({
    ...
    if (isEasyMode) {
        ...
        if (isWaterHisabRelevant(category)) {
            easyActions.push({ id: 'route-hisab', label: 'Water Route', desc: 'Daily rider sheet', icon: Droplets, color: 'bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200' });
        } else if (isMilkHisabRelevant(category)) {
            easyActions.push({ id: 'route-hisab', label: 'Milk Record', desc: 'Daily route entry', icon: Milk, color: 'bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200' });
        }
