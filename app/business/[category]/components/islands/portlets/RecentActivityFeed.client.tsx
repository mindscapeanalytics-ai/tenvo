...
/** Visible list height tuned for bottom-band stretch with ~6 activity rows */
const ACTIVITY_LIST_MAX_HEIGHT_CLASS = 'max-h-[min(14.5rem,32svh)]';
...
export const RecentActivityFeed = memo(function RecentActivityFeed({
    ...
    if (loading) {
        return (
            <Card className={cn('flex h-full min-h-[22rem] flex-col border border-slate-200 bg-white shadow-sm', className)}>
                ...
                <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                    <div
                        className={cn(
                            'min-h-0 flex-1 overflow-hidden px-3.5 py-1.5',
                            ACTIVITY_LIST_MAX_HEIGHT_CLASS
                        )}
                    >
                        <div className="flex flex-col divide-y divide-slate-100">
                            {[1, 2, 3, 4, 5, 6].map((i) => (
                                <div key={i} className="flex min-h-[2.75rem] items-center gap-3 py-2 animate-pulse">
                    ...
                    <div className="mt-auto shrink-0 border-t border-slate-100 px-3.5 pb-3 pt-2">
    ...
    if (activities.length === 0) {
        return (
            <Card className={cn('flex h-full min-h-[22rem] flex-col border border-slate-200 bg-white shadow-sm', className)}>
                ...
                <CardContent className="flex flex-1 items-center justify-center px-3.5 py-6 text-center text-xs italic text-slate-400">
    ...
    return (
        <Card className={cn('flex h-full min-h-[22rem] flex-col border border-slate-200 bg-white shadow-sm', className)}>
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                <ul
                    className={cn(
                        'min-h-0 flex-1 list-none overflow-y-auto overscroll-y-contain px-3.5 [scrollbar-gutter:stable]',
                        ACTIVITY_LIST_MAX_HEIGHT_CLASS
                    )}
                ...
                >
                    {activities.map((item) => (
                        <li
                            key={item.id}
                            className="flex min-h-[2.75rem] items-center gap-3 border-b border-slate-100 py-2 last:border-b-0"
                        >
                            <div
                                className={cn(
                ...
                <div className="mt-auto shrink-0 border-t border-slate-100 px-3.5 pb-2.5 pt-1.5">
                    <button
                        type="button"
                        onClick={onViewAll}
