-- Repair: 20260714062030_add_custom_domains was recorded as applied
-- with applied_steps_count = 0 while the table was never created (42P01).
-- Idempotent ensure for registration StorefrontSyncService.initializeStorefront.
...
CREATE UNIQUE INDEX IF NOT EXISTS "unique_business_custom_domain"
  ON "business_custom_domains"("business_id", "domain");

CREATE INDEX IF NOT EXISTS "idx_custom_domains_domain"
  ON "business_custom_domains"("domain" DESC);

CREATE INDEX IF NOT EXISTS "idx_custom_domains_business"
  ON "business_custom_domains"("business_id");

CREATE UNIQUE INDEX IF NOT EXISTS "idx_custom_domains_domain_lower"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'business_custom_domains_business_id_fkey'
  ) THEN
    ALTER TABLE "business_custom_domains"
      ADD CONSTRAINT "business_custom_domains_business_id_fkey"
