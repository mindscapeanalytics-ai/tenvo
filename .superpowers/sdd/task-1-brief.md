...
**Interfaces:**
- Produces:
  - `DOMAIN_KNOWLEDGE_OVERRIDE_KEYS` â€” allowlisted top-level keys
  - `LOCKED_DOMAIN_KNOWLEDGE_OVERRIDE_KEYS` â€” stripped keys
  - `domainKnowledgeOverrideSchema` â€” Zod object (passthrough false)
  - `parseDomainKnowledgeOverride(raw: unknown): { success: true, data: object } | { success: false, error: string, issues?: unknown }`
  - `extractDomainKnowledgeOverride(settings: unknown): object` â€” safe `{}` on corrupt
  - `applyDomainKnowledgeOverrides(base: object, patch: unknown): object` â€” pure merge
  - `mergeDomainKnowledgeIntoBusinessSettings(settings: unknown, patch: object | null): { nextSettings: object }` â€” `null` patch removes key
...
/**
 * @param {Record<string, any>} base â€” output of getDomainKnowledge (country-aware)
 * @param {unknown} patch
 */
...
if (!bad.success || bad.data.defaultTax != null) {
  // locked keys stripped before parse â€” defaultTax should be gone; units ok
}
