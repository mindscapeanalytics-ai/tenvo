...
import { normalizeBillingCurrency } from '@/lib/payments/stripeCatalog';
...
import { ADMIN_TABS, AdminMobileNav } from './AdminMobileNav';
import toast from 'react-hot-toast';

/** SaaS billing amounts are stored as minor units (paisa/cents). Default catalog currency is PKR. */
function formatPlatformMinorRevenue(amountMinor, currency = 'pkr') {
    const cur = normalizeBillingCurrency(currency).toUpperCase();
...
function isBusinessTrialing(biz) {
    if (!biz?.plan_expires_at) return false;
    const expires = new Date(biz.plan_expires_at);
    ...
    if (['active', 'past_due', 'unpaid'].includes(status)) return false;
    // No paid Stripe status: expiry window is treated as trial access.
    return true;
}

// --- Overview Panel ----------------------------------------------------------

function OverviewPanel({ stats, metrics, isLoading, onRefresh }) {
    if (isLoading) {
        return (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {[1, 2, 3, 4].map(i => (
                    <Card key={i} className="animate-pulse">
                        <CardContent className="p-4 sm:p-6"><div className="h-14 sm:h-16 bg-gray-100 rounded" /></CardContent>
                    </Card>
                ))}
            </div>
        );
    ...
    const totalBusinesses = totals.total_businesses ?? metrics?.totalBusinesses ?? 0;
    const totalUsers = totals.total_users ?? metrics?.totalUsers ?? 0;
    const activeMembers = totals.total_active_members ?? 0;

    const kpis = [
        { label: 'Total Businesses', value: totalBusinesses, hint: metrics?.activeBusinesses != null ? `${metrics.activeBusinesses} active` : null, icon: Building2, color: 'text-blue-600 bg-blue-50' },
        { label: 'Total Users', value: totalUsers, hint: metrics?.activeUsers != null ? `${metrics.activeUsers} active (24h)` : null, icon: Users, color: 'text-wine-600 bg-wine-50' },
        { label: 'Monthly Revenue', value: formatPlatformMinorRevenue(monthlyRevenue), hint: 'SaaS billing this month', icon: TrendingUp, color: 'text-emerald-600 bg-emerald-50' },
        { label: 'Active Members', value: activeMembers, hint: totals.total_owners != null ? `${totals.total_owners} owners` : null, icon: UserPlus, color: 'text-green-600 bg-green-50' },
    ];

    return (
        <div className="space-y-5 sm:space-y-6">
            <div className="flex items-center justify-between gap-2">
                <p className="text-xs font-medium text-gray-500">Platform snapshot</p>
                {typeof onRefresh === 'function' ? (
                    <Button type="button" variant="outline" size="sm" className="h-8 shrink-0" onClick={onRefresh}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {kpis.map(kpi => (
                    <Card key={kpi.label} className="border-none shadow-sm hover:shadow-md transition-shadow min-w-0">
                        <CardContent className="p-3.5 sm:p-5">
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                    <p className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider leading-tight">{kpi.label}</p>
                                    <p className="text-xl sm:text-3xl font-semibold tabular-nums text-gray-900 mt-1 truncate">{kpi.value}</p>
                                    {kpi.hint ? (
                                        <p className="mt-0.5 text-[10px] sm:text-xs font-medium text-gray-400 truncate">{kpi.hint}</p>
                                    ) : null}
                                </div>
                                <div className={`w-9 h-9 sm:w-12 sm:h-12 shrink-0 rounded-xl flex items-center justify-center ${kpi.color}`}>
            ...
            <Card className="border-none shadow-sm">
                <CardHeader className="pb-3">
                    ...
                    <CardDescription>Subscriptions across all businesses</CardDescription>
                </CardHeader>
                <CardContent>
                    {planDist.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {planDist.map(plan => {
                                const total = Number(totalBusinesses) || 1;
                                ...
                                return (
                                    <div key={plan.plan_tier} className="space-y-1.5 min-w-0">
                                        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between text-sm">
                                            <span className="font-semibold text-gray-700">
                                                {planInfo?.name || plan.plan_tier}
                                            </span>
                                            <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-gray-500">
                                                <span className="tabular-nums">{count} businesses ({pctLabel}%)</span>
                                                {Number(plan.active_trials) > 0 && (
                                                    <span className="text-amber-600 font-medium">
                                                        {plan.active_trials} trials
                                                    </span>
                                                )}
                                                {Number(plan.expired_trials) > 0 && (
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            {recentBusinesses.map((biz) => (
                                <div key={biz.id} className="flex items-start justify-between gap-3 p-2.5 rounded-lg bg-gray-50 text-sm">
                                    <div className="min-w-0">
                                        <p className="font-semibold text-gray-900 truncate">{biz.business_name}</p>
                                        <p className="text-xs text-gray-500 capitalize truncate">{biz.category?.replace(/-/g, ' ') || 'General'}</p>
                                    </div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <Button variant="outline" size="sm" className="h-10 sm:h-9 shrink-0" onClick={fetchBusinesses}>
                    <RefreshCcw className="w-4 h-4 mr-1" /> Refresh
                </Button>
            </div>

            <p className="text-xs text-gray-500 tabular-nums">{total} total businesses</p>
            ...
            {loading ? (
                ...
                <div className="space-y-2">
                    {businesses.map(biz => {
                        ...
                        const isTrialing = isBusinessTrialing(biz);
                        return (
                                    ...
                                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                                        <div className="flex-1 min-w-0 space-y-1.5">
                                            <div className="flex flex-wrap items-center gap-1.5">
                                                <h4 className="font-semibold text-sm text-gray-900 truncate max-w-full">{biz.business_name}</h4>
                                                ...
                                                {isExpired && (
                                                    <span className="px-2 py-0.5 text-[10px] font-semibold rounded-full bg-red-50 text-red-700">
                                                        Expired
                                                    </span>
                                                )}
                                            </div>
                                            <div className="flex flex-col gap-0.5 text-xs text-gray-500 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-0.5">
                                                <span className="truncate font-mono text-[11px]">{biz.domain}</span>
                                                <span className="hidden sm:inline text-gray-300" aria-hidden>·</span>
                                                <span className="truncate">{biz.owner_name || biz.owner_email}</span>
                                                <span className="hidden sm:inline text-gray-300" aria-hidden>·</span>
                                                <span className="tabular-nums">{biz.active_users} users · {biz.product_count} products</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-wrap items-center gap-2 sm:justify-end sm:shrink-0">
                                            {editingPlan === biz.id ? (
                                                <div className="flex w-full sm:w-auto items-center gap-1">
                                                    ...
                                                    <Button size="sm" variant="ghost" onClick={() => setEditingPlan(null)} aria-label="Cancel plan edit">
                                                        <X className="w-3 h-3" />
                                                    </Button>
                                                </div>
                                            ) : (
                                                <>
                                                    <Button size="sm" variant="outline" className="text-xs h-8 flex-1 sm:flex-none" onClick={() => handleViewDetails(biz.id)}>
                                                        <Eye className="w-3 h-3 mr-1" /> Details
                                                    </Button>
                                                    <Button size="sm" variant="outline" className="text-xs h-8 flex-1 sm:flex-none" onClick={() => setEditingPlan(biz.id)}>
                                                        <Edit2 className="w-3 h-3 mr-1" /> Plan
                                                    </Button>
                                                    {(isExpired || isTrialing) && (
                                                        <Button size="sm" variant="outline" className="text-xs h-8 w-full sm:w-auto text-amber-700 border-amber-200" onClick={() => handleExtendTrial(biz.id)}>
            ...
            {total > 20 && (
                <div className="flex items-center justify-between gap-2 pt-2">
                    ...
                    <span className="text-xs text-gray-500 tabular-nums">Page {page} of {Math.ceil(total / 20)}</span>
    ...
    return (
        ...
        >
            <Card
                role="dialog"
                aria-modal="true"
                aria-labelledby="platform-admin-biz-title"
                className="flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border-0 sm:max-h-[90dvh] sm:rounded-xl"
                onClick={e => e.stopPropagation()}
            >
                <CardHeader className="shrink-0 border-b px-4 py-3 sm:px-6 sm:py-4">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <CardTitle id="platform-admin-biz-title" className="text-base sm:text-lg truncate">{business.business_name}</CardTitle>
                            <CardDescription className="truncate text-xs sm:text-sm">{business.domain} · {business.category} · {business.country}</CardDescription>
                        </div>
                ...
                <CardContent className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain p-4 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
                    {/* Business Info */}
                    ...
                    {/* Team Members */}
                    <div>
                        <h4 className="font-semibold text-sm mb-2">Team Members ({members?.length || 0})</h4>
                        <div className="space-y-2">
                            {(members || []).map(member => (
                                <div key={member.user_id} className="flex flex-col gap-2 p-2.5 bg-gray-50 rounded-lg sm:flex-row sm:items-center sm:justify-between">
                                    <div className="flex items-center gap-3 min-w-0">
                                        <div className="w-8 h-8 shrink-0 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-xs font-semibold">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                                        ...
                                        <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded-full ${member.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                            {member.status}
                                        </span>
                                        {member.role !== 'owner' && member.status === 'active' && (
                                            <Button
                                                ...
                                                aria-label={`Deactivate ${member.name || member.email}`}
                                            >
...
export default function PlatformAdminPanel() {
    ...
    const activeTabMeta = useMemo(
        () => ADMIN_TABS.find((t) => t.key === activeTab) || ADMIN_TABS[0],
        [activeTab]
    );

    return (
        <div className={cn(HUB_MOBILE_ROOT, 'space-y-5 sm:space-y-6 pb-[calc(5.5rem+env(safe-area-inset-bottom))] lg:pb-0')}>
            <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 bg-gradient-to-br from-amber-500 to-orange-600 text-white rounded-xl flex items-center justify-center">
                        <Shield className="w-4 h-4 sm:w-5 sm:h-5" />
                    </div>
                    <div className="min-w-0">
                        <h2 className="text-lg sm:text-xl font-semibold text-gray-900 leading-tight">Platform Administration</h2>
                        <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5 leading-snug">
                            Businesses, users, subscriptions, roles &amp; access
                        </p>
                    </div>
                </div>
            </div>

            {/* Mobile app hub tiles */}
            <div className="lg:hidden">
                {mobileMenuOpen ? (
                    ...
                    <button
                        ...
                        className="flex w-full items-center gap-2.5 rounded-2xl border border-neutral-200 bg-white px-3.5 py-3 text-left shadow-sm transition-colors active:bg-neutral-50"
                    >
                        <ChevronLeft className="h-4 w-4 shrink-0 text-neutral-500" aria-hidden />
                        <span className="text-[11px] font-semibold uppercase tracking-wider text-neutral-400">
                            All sections
                        </span>
                        <span className="ml-auto truncate text-sm font-semibold tracking-tight text-neutral-900">
                            {activeTabMeta?.shortLabel || activeTabMeta?.label || 'Admin'}
                        </span>
                    </button>
                )}
            </div>
