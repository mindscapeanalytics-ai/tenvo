      ...
      <div className="hidden lg:block overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
            <tr>
              <th className="px-3 py-2.5 whitespace-nowrap">A/C</th>
              ...
              {products.map((p) => (
                <th
                  key={p.id}
                  className="px-2 py-2 text-center align-bottom min-w-[7.5rem]"
                  title={`${p.name} · Del / Rec empties`}
                >
                  <span className="block truncate text-[11px] font-semibold uppercase tracking-wide text-gray-600">
                    {shortWaterHisabProductLabel(p, 10)}
                  </span>
                  <span className="mt-0.5 flex justify-center gap-3 font-normal normal-case text-[10px] text-gray-400">
                    <span>Del</span>
                    <span>Rec</span>
                  </span>
                </th>
              ))}
              <th className="px-2 py-2.5 text-center whitespace-nowrap">Cash</th>
              <th className="px-2 py-2.5 text-center whitespace-nowrap">Disc</th>
              <th className="px-3 py-2.5 whitespace-nowrap">Notes</th>
              <th className="px-2 py-2.5 text-center whitespace-nowrap">Bill</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {rows.map((row) => (
              <tr key={row.customerId} className="hover:bg-sky-50/40">
                  ...
                  <td key={p.id} className="px-1 py-1.5 text-center">
                    <div className="inline-flex items-center gap-1">
                      <Input
                        ...
                        onChange={(e) => onQty(row.customerId, p.id, e.target.value)}
                        className="h-8 w-[3.5rem] tabular-nums text-center"
                        disabled={readOnly}
                        title="Delivered"
                      />
                      <Input
                ...
                <td className="px-1.5 py-1.5 text-center">
                  <Input
                    ...
                    className="mx-auto h-8 w-[4rem] tabular-nums text-center"
                    disabled={readOnly}
                  />
                </td>
                <td className="px-2 py-1.5">
