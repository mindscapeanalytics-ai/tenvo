-- Migration: Add Missing Tables for Multi-Domain Inventory System
-- Created: 2026-01-19
...
CREATE POLICY "Users can view invoice_items for their businesses"
...
CREATE POLICY "Users can insert invoice_items for their businesses"
...
CREATE POLICY "Users can view purchase_items for their businesses"
    ON purchase_items FOR SELECT
    USING (
        purchase_id IN (
            SELECT id FROM purchases WHERE business_id IN (
                SELECT id FROM businesses WHERE user_id = auth.uid()
                UNION
                SELECT business_id FROM business_users WHERE user_id = auth.uid()
            )
...
CREATE POLICY "Users can insert purchase_items for their businesses"
...
CREATE POLICY "Users can manage quotations for their businesses"
...
CREATE POLICY "Users can manage quotation_items for their businesses"
...
CREATE POLICY "Users can manage sales_orders for their businesses"
...
CREATE POLICY "Users can manage sales_order_items for their businesses"
...
-- Create RLS policies for delivery_challans
CREATE POLICY "Users can manage delivery_challans for their businesses"
...
-- Create RLS policies for delivery_challan_items
CREATE POLICY "Users can manage delivery_challan_items for their businesses"
    ON delivery_challan_items FOR ALL
    USING (
        business_id IN (
            SELECT id FROM businesses WHERE user_id = auth.uid()
            UNION
