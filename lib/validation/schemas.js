...
import { z } from 'zod';
import { normalizePurchaseStatus } from '@/lib/constants/purchaseStatus';
import { normalizePaymentType } from '@/lib/utils/paymentTypes';
...
/** Optional UUID from forms — empty string becomes null (not "Invalid UUID format"). */
export const optionalUuidSchema = z.preprocess(
    (val) => (val === '' || val === undefined ? null : val),
    uuidSchema.nullable()
);

/** Coerce money fields from form strings (e.g. "1000000", "Rs 50000") to numbers */
const coerceMoney = (fallback = 0) =>
  z.preprocess(
    (v) => {
...
/** Merchant uploads: https URLs or inline data URLs (self-hosted fallback). */
export const productImageUrlSchema = z
    .union([
        z.string().url(),
        z.string().regex(/^data:image\/[\w.+-]+;base64,[\w+/=]+$/),
        z.literal(''),
    ])
    .optional()
    .nullable();
...
export const invoiceSchema = z.preprocess((raw) => {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return raw;
    const data = { ...raw };
    // Accept either alias; keep both so create/update writers stay compatible.
    const tax = data.total_tax ?? data.tax_total;
    ...
    // Omitted or null — DocumentSequenceService assigns the number at create time.
    invoice_number: z.preprocess(
        (val) => (val === null || val === undefined || val === '' ? undefined : val),
        z.string().min(1, 'Invoice number is required').optional()
    ),
    ...
    status: z.enum(['draft', 'sent', 'viewed', 'approved', 'payment_pending', 'partially_paid', 'paid', 'overdue', 'voided', 'amended', 'cancelled']).default('draft'),
...
export const removeStockSchema = z.object({
    ...
    state: z.string().optional().default('sellable'),
    fifo_sellable_locations: z.boolean().optional().default(false),
    skip_accounting: z.boolean().optional().default(false),
    skip_low_stock_notify: z.boolean().optional().default(false),
});

export const removeVariantStockSchema = z.object({
    business_id: uuidSchema,
    product_id: uuidSchema,
    variant_id: uuidSchema,
    quantity: z.number().positive('Quantity must be positive'),
    reference_type: z.string().max(50).default('storefront_order'),
    reference_id: uuidSchema.optional().nullable(),
    notes: z.string().max(500).optional().nullable(),
    domain_data: z.record(z.unknown()).optional().nullable(),
});
...
export const paymentSchema = z.object({
    ...
    payment_type: z.preprocess(
        (val) => normalizePaymentType(val),
        z.enum(['receipt', 'payment'], {
            errorMap: () => ({ message: "Payment type must be 'receipt' or 'payment'" })
        })
    ),
    payment_mode: z.enum(['cash', 'cheque', 'bank_transfer', 'bank', 'card', 'online', 'other']).optional().nullable(),
...
export const customerSchema = z.object({
    ...
    ntn: z.string().optional().nullable().or(z.literal('')),
    cnic: z.string().optional().nullable().or(z.literal('')),
    srn: z.string().optional().nullable().or(z.literal('')),
    ...
    market_location: z.string().optional().nullable().or(z.literal('')),

    credit_limit: coerceMoney(0).pipe(z.number().min(0)).optional(),
    outstanding_balance: coerceMoney(0).optional(),
    opening_balance: coerceMoney(0).optional(),

    filer_status: z.enum(['filer', 'non-filer', 'active', 'inactive', 'none']).optional().default('none'),
    type: z.enum(['individual', 'company', 'government', 'other']).optional().default('individual'),
...
export const vendorSchema = z.object({
    ...
    credit_limit: coerceMoney(0).pipe(z.number().min(0)),
    outstanding_balance: coerceMoney(0),
    opening_balance: coerceMoney(0),
...
export const purchaseSchema = z.object({
    ...
    status: z.preprocess(
        (val) => normalizePurchaseStatus(val) ?? 'draft',
        z.enum(['draft', 'ordered', 'received', 'cancelled'])
    ).default('draft'),
    ...
    if (data.status === 'received' && !data.warehouse_id) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'Warehouse is required when receiving stock',
            path: ['warehouse_id'],
        });
    }
});
...
export const expenseSchema = z.object({
    businessId: uuidSchema,
    /** Optional in Easy mode — server resolves from category account_code. */
    ...
    receiptUrl: z.preprocess(
        (val) => (val === '' || val === undefined ? null : val),
        z.string().url("Invalid receipt URL").optional().nullable()
    ),
}).refine(
    (data) => Boolean(data.accountId) || Boolean(String(data.category || '').trim()),
    {
        message: 'Select what the money was for (category), or pick an expense account',
        path: ['category'],
    }
);
