...
const POOL = {
  ...
  tyres: [
    cardUrl('https://images.unsplash.com/photo-1619642751034-765df43d7749'),
  ...
  furniture: [
    ...
    cardUrl('https://images.unsplash.com/photo-1615874959471-fe2bbce3e3e5'),
...
function pickFromPool(urls, seedStr) {
  ...
  // Prefer live URLs so a dead curated entry never gets picked over a working one.
  const live = urls.filter((u) => !isDeadImageUrl(u));
  const pool = live.length ? live : urls;
  const idx = stableStringHash(seedStr) % pool.length;
  return pool[idx];
}
