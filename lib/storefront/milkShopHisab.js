...
/**
 * Column header for day sheet (58mm). Matches formatMilkHisabDayLine widths:
 * "DD Mlk Egg Brd"
 * @param {Array<{ abbrev?: string }>} columns
 */
export function formatMilkHisabDayHeaderLine(columns = []) {
  const cells = (columns || []).map((col) =>
    String(col.abbrev || 'Itm')
      .trim()
      .padEnd(3, ' ')
      .slice(0, 3)
  );
  return `DD ${cells.join(' ')}`;
}

/**
 * One compact text line for a day (58mm). Y/N sits centered under each 3-char abbrev:
 * "01  Y   N   Y"
...
export function formatMilkHisabDayLine(day, columns = []) {
  ...
  const cells = (columns || []).map((col) => {
    ...
    // 3-char cell matching abbrev width — mark centered
    return ` ${mark} `;
  });
  return `${n} ${cells.join(' ')}`;
}
