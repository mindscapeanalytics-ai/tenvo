...
/** @param {{ has_variants?: boolean, domain_data?: object } | null | undefined} product */
export function productHasDomainVariantMatrix(product) {
...
export function catalogProductNeedsVariantPage(product) {
  ...
  return productHasDomainVariantMatrix(product);
}
...
export async function queryStorefrontVariantRequirement(client, productId, businessId) {
  await ensureStorefrontVariantsMaterialized(client, productId, businessId);
...
export async function ensureStorefrontVariantsMaterialized(client, productId, businessId) {
  ...
  for (const combo of combos) {
    ...
    const res = await client.query(
      ...
      ON CONFLICT (business_id, variant_sku) WHERE COALESCE(is_deleted, false) = false DO NOTHING
      RETURNING id`,
      [
        businessId,
        productId,
