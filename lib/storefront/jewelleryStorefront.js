...
export const JEWELLERY_DEFAULT_HERO_TILES = [
  { id: 'gold', label: 'Gold', desc: 'Rings & sets', hrefSuffix: '?category=gold', imageKey: 'gold' },
  { id: 'diamonds', label: 'Diamonds', desc: 'Fine jewellery', hrefSuffix: '?category=diamonds', imageKey: 'diamonds' },
  { id: 'bridal', label: 'Bridal', desc: 'Wedding sets', hrefSuffix: '?category=bridal', imageKey: 'bridal' },
  { id: 'gifts', label: 'Gifts', desc: 'Occasion picks', hrefSuffix: '?sort=featured', imageKey: 'gifts' },
];

/** Default hero category tiles (beauty / salon). */
export const BEAUTY_DEFAULT_HERO_TILES = [
  { id: 'polish', label: 'Polish', desc: 'Gel & lacquer', hrefSuffix: '?category=polish', imageKey: 'polish' },
  { id: 'press-ons', label: 'Press-Ons', desc: 'Instant mani', hrefSuffix: '?category=press-on', imageKey: 'press-ons' },
  { id: 'kits', label: 'Mani Kits', desc: 'Complete systems', hrefSuffix: '?category=kits', imageKey: 'kits' },
  { id: 'care', label: 'Care', desc: 'Serums & oils', hrefSuffix: '?category=care', imageKey: 'care' },
];
...
/**
 * Resolve hero category tiles — owner settings override, else mode defaults.
 * @param {string} base `/store/{domain}`
 * @param {object} [settings]
 * @param {string} [businessCategory]
 * @returns {Array<{ label: string; desc: string; href: string; image?: string; imageKey?: string }>}
 */
export function getJewelleryHeroTiles(base, settings = {}, businessCategory) {
  const config = getJewelleryStorefrontConfig(settings, null, businessCategory);
  const mode = getStoreMode(businessCategory);
  const defaults = mode === 'beauty' ? BEAUTY_DEFAULT_HERO_TILES : JEWELLERY_DEFAULT_HERO_TILES;
  const source = config.heroTiles?.length ? config.heroTiles : defaults;
  const products = `${base}/products`;

  return source.slice(0, 4).map((tile) => {
    const href = tile.href
      || (tile.hrefSuffix ? `${products}${tile.hrefSuffix}` : null)
      || (tile.slug ? `${products}?category=${encodeURIComponent(tile.slug)}` : products);
    return {
      id: tile.id || tile.label,
      label: tile.label || 'Shop',
      desc: tile.desc || tile.description || '',
      href: String(href).startsWith('/store/') || String(href).startsWith('http')
        ? href
        : `${base}${String(href).startsWith('/') ? href : `/${href}`}`,
      image: tile.image || null,
      imageKey: tile.imageKey || tile.id || String(tile.label || '').toLowerCase(),
    };
  });
}
