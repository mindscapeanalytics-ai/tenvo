...
export function getElectronicsHeroSlides(base, settings = {}, ctx = {}) {
  ...
  const featured = (ctx.products || []).filter(
    (p) => p.is_featured && getEffectiveProductImageUrl(p, ctx.businessCategory || 'electronics-goods')
  );
  const withImages = (ctx.products || []).filter((p) =>
    getEffectiveProductImageUrl(p, ctx.businessCategory || 'electronics-goods')
  );
  ...
  return buildTenantHeroSlides({
    ...
    featuredProducts: featured.length ? featured : withImages.slice(0, 4),
  });
}
...
export function resolveElectronicsCategoryTiles(settings, storeBase, ctx = {}) {
  const config = getElectronicsConfig(settings, ctx.businessDomain);
  const productsUrl = `${storeBase}/products`;
  const canon = ctx.businessCategory || 'electronics-goods';
  const mapHref = (item) => ({
    ...item,
    href:
      item.href ||
  ...
  const ensureImages = (tiles) =>
    enrichCategoryNavImages(tiles, ctx.products, canon).map((tile) => {
      const img = typeof tile.image === 'string' ? tile.image.trim() : '';
      if (img) return tile;
      return {
        ...tile,
        image: resolveElectronicsCategoryFallbackImage(tile, canon),
      };
    });

  const fromOwner = config.categoryTiles?.map(mapHref);
  if (fromOwner?.length) {
    return ensureImages(fromOwner);
  }
