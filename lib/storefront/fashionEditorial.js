 ...
 * @param {string | null | undefined} [businessDomain] Store slug (unused for toggles; kept for call-site compat)
 * @param {string | null | undefined} [category] Business category — textile-wholesale gets B2B defaults
 * @returns {FashionEditorialConfig}
 */
export function getFashionEditorialConfig(settings = {}, businessDomain, category) {
  const raw = settings?.storefront?.fashion || {};
  const str = (value) => (typeof value === 'string' ? value.trim() : '');
  const isWholesale = resolveDomainKey(category) === 'textile-wholesale';
  const bool = (value, defaultTrue) => {
  ...
  return {
    ...
    showTopCollections: bool(raw.showTopCollections, true),
    showTopPicks: bool(raw.showTopPicks, true),
    showEditorialSpotlight: bool(raw.showEditorialSpotlight, true),
    showUnstitched: bool(raw.showUnstitched, true),
    showReadyToWear: bool(raw.showReadyToWear, !isWholesale),
    showAccessories: bool(raw.showAccessories, !isWholesale),
    ...
    showPromoBanners: bool(raw.showPromoBanners, true),
    showSeoBlock: bool(raw.showSeoBlock, true),
    showHomeEdit: bool(raw.showHomeEdit, true),
    showSaleMosaic: bool(raw.showSaleMosaic, true),
    ...
    unstitchedTitle: str(raw.unstitchedTitle) || (isWholesale ? 'FABRIC & SUIT LOTS' : ''),
    readyToWearTitle: str(raw.readyToWearTitle),
    accessoriesTitle: str(raw.accessoriesTitle),
    offersTitle: str(raw.offersTitle) || (isWholesale ? 'TRADE OFFERS' : ''),
    newArrivalsTitle: str(raw.newArrivalsTitle) || (isWholesale ? 'NEW LOTS' : ''),
    searchPlaceholder: str(raw.searchPlaceholder) || (isWholesale ? 'Search lawn, cotton, khaddar, thaan…' : ''),
    featuredRailTitle: str(raw.featuredRailTitle) || (isWholesale ? 'Wholesale picks' : ''),
    featuredRailSubtitle:
      str(raw.featuredRailSubtitle) ||
      (isWholesale ? 'Thaan, suit packs, and seasonal fabric lots' : ''),
 ...
 * @param {string | null | undefined} [category] Domain key — textile-wholesale gets B2B fabric defaults
 * @returns {{ fashion: Record<string, boolean | string> }}
 */
export function buildDefaultFashionEditorialStorefrontSeed(category) {
  const canonical = resolveDomainKey(category);
  const isWholesale = canonical === 'textile-wholesale';
  return {
    fashion: {
      animations: true,
      showHeroRating: true,
      ...
      showUnstitched: true,
      // Fabric traders do not sell pret / accessories rows by default.
      showReadyToWear: !isWholesale,
      showAccessories: !isWholesale,
      showOffers: true,
      ...
      ...(isWholesale
        ? {
            unstitchedTitle: 'FABRIC & SUIT LOTS',
            featuredRailTitle: 'Wholesale picks',
            featuredRailSubtitle: 'Thaan, suit packs, and seasonal fabric lots',
            searchPlaceholder: 'Search lawn, cotton, khaddar, thaan…',
            offersTitle: 'TRADE OFFERS',
            newArrivalsTitle: 'NEW LOTS',
          }
        : {}),
    },
  };
...
/** B2B fabric-trader trust copy for textile-wholesale (not D2C boutique returns). */
export const TEXTILE_WHOLESALE_TRUST_PILLARS = [
  { id: 'quality', label: 'Trade-grade fabrics', desc: 'Lawn, cotton, khaddar, and bridal lots for retailers' },
  { id: 'bulk', label: 'Bulk & MOQ ready', desc: 'Thaan and suit packs sized for shop restock' },
  { id: 'delivery', label: 'Nationwide dispatch', desc: 'Careful packing for rolls and suit lots' },
  { id: 'trade', label: 'Wholesale terms', desc: 'Credit, COD, and broker-friendly ordering' },
];
...
/** Mill / house brands for textile-wholesale demo when catalog brands are empty. */
export const TEXTILE_WHOLESALE_DEMO_BRANDS = [
  { id: 'gul-ahmed-textile', name: 'Gul Ahmed Textile', slug: 'gul-ahmed-textile' },
  { id: 'nishat-mills', name: 'Nishat Mills', slug: 'nishat-mills' },
  { id: 'sapphire-textile', name: 'Sapphire Textile', slug: 'sapphire-textile' },
  { id: 'alkaram-textile', name: 'Al-Karam Textile', slug: 'al-karam-textile' },
  { id: 'crescent', name: 'Crescent Textile', slug: 'crescent-textile' },
  { id: 'masood', name: 'Masood Textile', slug: 'masood-textile' },
];

export const FASHION_DEMO_QUICK_SEARCH_TERMS = {
  boutique: ['Lawn', 'Unstitched', 'Ready to wear', 'Embroidered', 'Formal', 'Accessories'],
  textile: ['Lawn', 'Cotton', 'Khaddar', 'Thaan', 'Bridal', 'Wholesale'],
...
export function resolveFashionSearchPlaceholder(settings = {}, category, businessDomain) {
  const config = getFashionEditorialConfig(settings, businessDomain, category);
  if (config.searchPlaceholder) return config.searchPlaceholder;
  const variant = getLuxuryFashionVariant(category) || 'boutique';
  if (variant === 'textile') return 'Search lawn, cotton, khaddar, thaan…';
 ...
 * @param {string | null | undefined} [category]
 */
export function resolveFashionTrustPillars(settings = {}, businessDomain, category) {
  const config = getFashionEditorialConfig(settings, businessDomain, category);
  const canonical = resolveDomainKey(category);
  const defaults =
    canonical === 'textile-wholesale' ? TEXTILE_WHOLESALE_TRUST_PILLARS : FASHION_DEFAULT_TRUST_PILLARS;
  const raw = config.trustPillars || defaults;
  return (raw || [])
 ...
 * @param {string | null | undefined} [category]
 */
export function resolveFashionBrands(settings = {}, products = [], businessDomain, categories = [], category) {
  const config = getFashionEditorialConfig(settings, businessDomain, category);
  ...
  if (!isDemoStoreDomain(businessDomain)) return [];
  return resolveDomainKey(category) === 'textile-wholesale'
    ? TEXTILE_WHOLESALE_DEMO_BRANDS
    : FASHION_DEMO_BRANDS;
}
...
export function resolveFashionQuickSearchTerms(
  ...
  if (isDemoStoreDomain(businessDomain)) return demoTerms;
