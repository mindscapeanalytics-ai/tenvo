 ...
 *   footwearElevatedHero?: boolean;
 *   electronicsElevatedHero?: boolean;
...
export function buildStoreHomeCatalogPlan(flags = {}) {
  const popularityVertical =
    ...
    flags.footwearElevatedHero ||
    flags.electronicsElevatedHero ||
  ...
  if (flags.tyreElevatedHero) popularityLimit = Math.max(popularityLimit, 80);
  if (flags.footwearElevatedHero) popularityLimit = Math.max(popularityLimit, 80);
  if (flags.electronicsElevatedHero) popularityLimit = Math.max(popularityLimit, 80);
...
export function mapStoreHomeCatalogRails(bundle, plan, flags) {
  if (!bundle?.success) {
    ...
    return {
      ...
      tyreCatalogResult: empty,
      footwearCatalogResult: empty,
      electronicsCatalogResult: empty,
  ...
  return {
    ...
    tyreCatalogResult: flags.tyreElevatedHero ? asProductSlice(popularity, 80) : EMPTY_PRODUCTS,
    footwearCatalogResult: flags.footwearElevatedHero ? asProductSlice(popularity, 80) : EMPTY_PRODUCTS,
    electronicsCatalogResult: flags.electronicsElevatedHero ? asProductSlice(popularity, 80) : EMPTY_PRODUCTS,
