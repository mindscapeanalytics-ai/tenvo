...
export function resolveEligibleStorefrontPaymentMethods(context) {
  ...
  const allowPrepaid = settings?.allow_prepaid !== false;
  ...
  for (const method of methods) {
    ...
    if (provider === 'stripe' && !stripeReady) continue;
    if (provider === 'crypto' && !cryptoReady) continue;
    if (provider === 'cod') {
      if (!allowCod) continue;
      eligible.push(method);
      continue;
    }
    if (!allowPrepaid) continue;
    eligible.push(method);
  }
...
/**
 * Pick a payment method for checkout.
 * - Requested provider in eligible set → that provider
 * - Empty/missing request → COD if eligible, else first eligible, else null
 * - Explicit request not in eligible set → null (caller must fail closed; do not remap to COD)
...
export function coerceStorefrontPaymentMethod(requested, eligible) {
  ...
  return list.find((m) => m.provider === 'cod')?.provider || list[0]?.provider || null;
}
