...
function buildAccessoryCircles(base, categories, products, businessCategory) {
  ...
  if (circles.length < 1 && products.length) {
    const seenIds = new Set(circles.map((c) => c.id));
    for (const product of products.slice(0, 4)) {
      ...
      circles.push({
        id,
        label: String(product.category_name || product.name || 'Shop')
          .split(/\s+/)
          .slice(0, 2)
          .join(' ')
          .toUpperCase(),
...
export function buildFashionHomeSections({
  ...
  if (unstitchedTiles.length < 2 && categories.length) {
    const seenIds = new Set(unstitchedTiles.map((t) => t.id));
    const topCats = [...categories]
      .sort((a, b) => (Number(b.product_count) || 0) - (Number(a.product_count) || 0))
      .slice(0, 4);
    for (const cat of topCats) {
      if (seenIds.has(cat.slug)) continue;
      const catProducts = products.filter((p) => productInCategory(p, cat));
      if (!catProducts.length) continue;
      unstitchedTiles.push({
        id: cat.slug,
        label: String(cat.name || 'Shop').toUpperCase(),
        href: categoryHref(base, cat),
        image:
          getEffectiveProductImageUrl(catProducts[0], businessCategory) ||
          cat.image_url ||
          getFashionRuleImage(cat.slug, 700, 'unstitched'),
        productCount: Number(cat.product_count) || catProducts.length,
      });
      if (unstitchedTiles.length >= 2) break;
    }
  }
  ...
  if (readyToWearCircles.length < 2 && categories.length) {
    const seenIds = new Set(readyToWearCircles.map((c) => c.id));
    const topCats = [...categories]
      .sort((a, b) => (Number(b.product_count) || 0) - (Number(a.product_count) || 0))
      .slice(0, 6);
    for (const cat of topCats) {
      ...
      readyToWearCircles.push({
        ...
        image:
          getEffectiveProductImageUrl(catProducts[0], businessCategory) ||
          cat.image_url ||
          getFashionRuleImage(cat.slug, 500, 'rtw'),
      });
      if (readyToWearCircles.length >= 2) break;
    }
  }

  if (readyToWearCircles.length < 2 && rtwProducts.length) {
    const seenIds = new Set(readyToWearCircles.map((c) => c.id));
    for (const product of rtwProducts.slice(0, 6)) {
      const id = `rtw-${product.id}`;
      if (seenIds.has(id)) continue;
      seenIds.add(id);
      const slug = norm(product.category_name || product.category).replace(/\s+/g, '-');
      readyToWearCircles.push({
        id,
        label: String(product.category_name || 'Ready to wear').toUpperCase(),
        href: product.slug
          ? `${base}/products/${product.slug}`
          : slug
            ? `${base}/products?category=${encodeURIComponent(slug)}`
            : `${base}/products?search=${encodeURIComponent('ready to wear')}`,
        image: getEffectiveProductImageUrl(product, businessCategory),
      });
      if (readyToWearCircles.length >= 2) break;
    }
  ...
  return {
    ...
    readyToWear: {
      ...
      show: flags.rtw && readyToWearCircles.length >= 2,
      viewAllHref: `${base}/products?search=${encodeURIComponent('ready to wear')}`,
    },
    accessories: {
      title: 'ACCESSORIES',
      circles: accessoryCircles,
      show: flags.accessories && accessoryCircles.length >= 1,
    ...
    newArrivals: {
      ...
      show: flags.newArrivals && (newArrivals.length >= 1 || products.length >= 2),
