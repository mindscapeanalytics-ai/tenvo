...
import {
  ...
  parseSizeColorKey,
  productHasDomainVariantMatrix,
  productHasSelectableVariants,
  catalogProductNeedsVariantPage,
...
describe('storefrontProductVariants', () => {
  ...
  it('detects domain_data size/color matrix as needing variant page', () => {
    const product = {
      has_variants: false,
      domain_data: {
        size_color_matrix: { 'M-Black': 3, 'L-Black': 2, 'M-White': 1 },
      },
    };
    expect(productHasDomainVariantMatrix(product)).toBe(true);
    expect(catalogProductNeedsVariantPage(product)).toBe(true);
  });

  it('ignores single-combo matrix for catalog quick add', () => {
    const product = {
      has_variants: false,
      domain_data: { size_color_matrix: { 'M-Black': 5 } },
    };
    expect(productHasDomainVariantMatrix(product)).toBe(false);
    expect(catalogProductNeedsVariantPage(product)).toBe(false);
  });
});
