...
describe('jewelleryStorefront', () => {
  ...
  it('resolves hero tiles from inventory catalog', () => {
    const tiles = getJewelleryHeroTiles(
      '/store/demo-jewellery',
      {},
      'gems-jewellery',
      {
        categories: [{ id: 'c1', slug: 'gold', name: 'Gold', product_count: 2 }],
        products: [
          {
            id: 'p1',
            name: 'Gold Ring',
            category_slug: 'gold',
            category_id: 'c1',
            image_url: 'https://cdn.example/gold-ring.jpg',
          },
        ],
      }
    );
    expect(tiles[0].label).toBe('Gold');
    expect(tiles[0].href).toContain('category=gold');
    expect(tiles[0].image).toBe('https://cdn.example/gold-ring.jpg');
  });
