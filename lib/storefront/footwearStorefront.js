...
export function getFootwearHeroSlides(base, settings = {}, ctx = {}) {
  ...
  const featured = (ctx.products || []).filter(
    (p) => p.is_featured && getEffectiveProductImageUrl(p, ctx.businessCategory || 'leather-footwear')
  );
  return buildTenantHeroSlides({
    ...
    featuredProducts: featured.length
      ? featured
      : (ctx.products || [])
          .filter((p) => getEffectiveProductImageUrl(p, ctx.businessCategory || 'leather-footwear'))
          .slice(0, 4),
  });
}
...
export function resolveFootwearLifestyleSpotlight(settings, products = [], businessDomain) {
  ...
  const featured =
    (products || []).find(
      (p) => p.is_featured && getEffectiveProductImageUrl(p, 'leather-footwear')
    ) ||
    (products || []).find((p) => getEffectiveProductImageUrl(p, 'leather-footwear'));
  return {
    ...
    image:
      getEffectiveProductImageUrl(featured, 'leather-footwear') || FOOTWEAR_HERO_IMAGES.lifestyle1,
