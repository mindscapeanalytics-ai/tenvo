...
import { resolveEditorialSpotlightFallback } from './storefrontImagePlaceholders.js';

/**
 * @param {Array<{ id?: string; image?: string } & Record<string, unknown>>} spotlights
 * @param {string} canonical
 */
function enrichSpotlights(spotlights, canonical) {
  if (!Array.isArray(spotlights) || !spotlights.length) return spotlights;
  return spotlights.map((spotlight, index) => ({
    ...spotlight,
    image: spotlight.image || resolveEditorialSpotlightFallback(spotlight.id, canonical, index),
  }));
}
...
export function getDomainLanding(category, businessDomain, settings = {}, business = {}) {
      ...
      spotlights: [
        {
          ...
          tone: 'dark',
          image: 'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?w=1400&q=82&auto=format&fit=crop',
        },
      ],
    },
    'luxury-fashion': {
      ...
      spotlights: [
        {
          ...
          href: `${base}/products?sort=featured`,
          tone: 'dark',
          image: 'https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=1400&q=82&auto=format&fit=crop',
        },
      ],
    },
    supermarket: {
  ...
  preset = {
    ...preset,
    spotlights: enrichSpotlights(preset.spotlights, canonical),
  };
