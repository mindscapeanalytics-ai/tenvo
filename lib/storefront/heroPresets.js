...
import { applyOwnerHeroSlideOverrides } from './heroSlides.js';
import {
 ...
 */
export function getDefaultHeroSlidesTemplate(category, businessDomain, settings = {}) {
  const preset = buildHeroPreset(category, businessDomain, settings, {});
...
export function getHeroPreset(category, businessDomain, settings = {}, business = {}) {
  ...
  const preset = buildHeroPreset(category, businessDomain, settings, business);
  return applyOwnerHeroSlideOverrides(preset, settings, { coverImage: customCover });
}
