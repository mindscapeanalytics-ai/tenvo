  ...
  return slides
    ...
    .map((s) => {
      ...
      return {
        ...
        // Strip dead CDN URLs so they fall through to template/cover defaults
        image: rawImage && !isDeadImageUrl(rawImage) ? rawImage : '',
 ...
 * then an optional vertical key. Never walks other verticals (that leaked
 * cars into furniture / fitness when global slides were empty).
 * @param {object} [storeSettings] business_settings.settings
 * @param {{ verticalKey?: string }} [opts]
 */
export function resolveStoredHeroSlides(storeSettings = {}, opts = {}) {
  const sf =
  ...
  const preferred = typeof opts.verticalKey === 'string' ? opts.verticalKey.trim() : '';
  if (preferred && sf[preferred] && typeof sf[preferred] === 'object') {
    const legacy = sanitizeHeroSlides(sf[preferred]?.heroSlides);
...
export function inferHeroVerticalKey(presetType) {
  const map = {
    ...
    'tiles-elevated': 'tiles',
    'tyre-elevated': 'tyre',
    'footwear-elevated': 'footwear',
    'electronics-elevated': 'electronics',
...
export function nestAdminSettingsForHeroPreview(flat = {}) {
  ...
  if (flat.tiles) storefront.tiles = flat.tiles;
