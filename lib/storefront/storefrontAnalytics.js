...
export async function upsertStorefrontDailyStats(client, businessId, deltas = {}) {
  ...
  // Use SAVEPOINT to handle missing columns without aborting the transaction
  ...
  try {
    ...
    if (err?.code === '42P01') {
      // Table doesn't exist - release savepoint and return gracefully
      try {
        await client.query('RELEASE SAVEPOINT before_analytics_upsert');
      } catch {
        /* ignore */
    ...
    if (err?.code === '42703') {
      // Missing column - rollback to savepoint and retry without visitors column
