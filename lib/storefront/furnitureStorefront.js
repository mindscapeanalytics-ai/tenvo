...
import { resolveStoredHeroSlides } from '@/lib/storefront/heroSlides';
import {
...
/** Demo-only lifestyle hero loop (living-room atmosphere). Live tenants: no forced default. */
export const FURNITURE_DEMO_HERO_VIDEO_URL =
  'https://videos.pexels.com/video-files/5495899/5495899-uhd_2560_1440_25fps.mp4';

/**
 * Accept absolute http(s) video URLs only.
 * @param {unknown} raw
 * @returns {string}
 */
export function normalizeFurnitureVideoUrl(raw) {
...
export function getFurnitureConfig(settings = {}, businessDomain) {
  ...
  const heroVideoUrl =
    normalizeFurnitureVideoUrl(raw.heroVideoUrl) ||
    (isDemo ? FURNITURE_DEMO_HERO_VIDEO_URL : '');
  ...
  return {
    ...
    roomEssentialsTitle: typeof raw.roomEssentialsTitle === 'string' ? raw.roomEssentialsTitle.trim() : '',
    roomEssentialsSubtitle: typeof raw.roomEssentialsSubtitle === 'string' ? raw.roomEssentialsSubtitle.trim() : '',
    showroomEyebrow: typeof raw.showroomEyebrow === 'string' ? raw.showroomEyebrow.trim() : '',
    showroomTitle: typeof raw.showroomTitle === 'string' ? raw.showroomTitle.trim() : '',
    showroomSubtitle: typeof raw.showroomSubtitle === 'string' ? raw.showroomSubtitle.trim() : '',
    showroomBannerImage: typeof raw.showroomBannerImage === 'string' ? raw.showroomBannerImage.trim() : '',
    heroVideoUrl,
...
export const FURNITURE_DEMO_CATEGORY_ICONS = [
  ...
  { id: 'sale', label: 'Sale', slug: '', hrefSuffix: '?onSale=true', image: 'https://comfy.sg/cdn/shop/files/comfy-sleepperfect-hybrid-mattress.webp?width=200' },
];
...
export function getFurnitureHeroSlides(base, settings = {}, ctx = {}) {
  ...
  const videoUrl = config.heroVideoUrl;
  if (!videoUrl || !slides.length) return slides;
  return slides.map((s, i) => (i === 0 ? { ...s, videoUrl: s.videoUrl || videoUrl } : s));
}
...
export const FURNITURE_DEMO_PROMO_BANNERS = [
  ...
  },
  {
    ...
    image: 'https://comfy.sg/cdn/shop/collections/dark-blue-3-seater-full-grain-leather-sofa.jpg?width=900',
    href: '?onSale=true',
    tone: 'cream',
  },
];
...
/** Brand motivation blocks for Room essentials (big image + story). */
export const FURNITURE_DEMO_BRAND_STORIES = [
  {
    id: 'crafted',
    eyebrow: 'Designed for living',
    title: 'Furniture that feels like home',
    subtitle:
      'Thoughtful proportions, lasting materials, and finishes chosen for everyday comfort. Build a room that welcomes you back.',
    image: 'https://comfy.sg/cdn/shop/files/comfy-sofa-singapore.webp?width=1600',
    ctaLabel: 'Explore living',
    href: '?category=living-room',
  },
  {
    id: 'rest',
    eyebrow: 'Rest well',
    title: 'Bedrooms made for better nights',
    subtitle:
      'From storage beds to supportive mattresses, create a calm retreat with pieces scaled for modern homes.',
    image: 'https://comfy.sg/cdn/shop/collections/grey-queen-size-bed-frame-bed-room.jpg?width=1600',
    ctaLabel: 'Shop bedroom',
    href: '?category=bedroom-furniture',
  },
];

const FURNITURE_BRAND_STORIES_MAX = 2;
...
/**
 * Homepage rails with disjoint product ids (featured wins over deals).
 * @param {object[]} products
 */
export function partitionFurnitureHomepageRails(products = []) {
  const { topPicks, deals, newArrivals } = partitionFurnitureProducts(products);
  const featuredIds = new Set(topPicks.map((p) => String(p.id)));
  const dealsExclusive = deals.filter((p) => !featuredIds.has(String(p.id)));
  return {
    topPicks,
    deals: dealsExclusive,
    newArrivals,
  };
}

const FURNITURE_PROMO_MOSAIC_MAX = 2;

/**
 * Single lifestyle spotlight (first editorial banner).
 * @param {object} [settings]
 * @param {object[]} [products]
 * @param {string} [businessDomain]
 * @param {string} [businessCategory]
 */
export function resolveFurnitureLifestyleSpotlight(settings, products, businessDomain, businessCategory) {
  const banners = resolveFurnitureEditorialBanners(settings, products, businessDomain, businessCategory);
  return banners[0] || null;
}

/**
 * Collection mosaic (max 2 promo tiles).
 * @param {object} [settings]
 * @param {object[]} [products]
 * @param {string} [businessDomain]
 * @param {string} [businessCategory]
 */
export function resolveFurniturePromoMosaic(settings, products, businessDomain, businessCategory) {
  return resolveFurniturePromoBanners(settings, products, businessDomain, businessCategory).slice(
    0,
    FURNITURE_PROMO_MOSAIC_MAX
  );
}
 ...
 * @param {unknown} raw
 */
function sanitizeFurnitureBrandStory(raw, index = 0) {
  ...
  return {
    ...
    title: title || 'Crafted for your home',
    ...
    href: hrefRaw.startsWith('?') || hrefRaw.startsWith('/') ? hrefRaw : hrefRaw ? `?search=${encodeURIComponent(hrefRaw)}` : '',
  };
}

/**
 * Big-image brand motivation blocks for Room essentials.
 * Owner `brandStories` win; demo gets curated defaults; live tenants fall back to editorial leftovers / catalog.
 *
 * @param {object} [settings]
 * @param {object[]} [products]
 * @param {string} [businessDomain]
 * @param {string} [businessCategory]
 * @param {{ storeName?: string }} [ctx]
 */
export function resolveFurnitureBrandStories(
  settings = {},
  products = [],
  businessDomain,
  businessCategory,
  ...
  if (config.brandStories) {
    return config.brandStories
      .map((row, i) => sanitizeFurnitureBrandStory(row, i))
      .filter(Boolean)
      .slice(0, FURNITURE_BRAND_STORIES_MAX);
  }

  if (isDemoStoreDomain(businessDomain)) {
    return FURNITURE_DEMO_BRAND_STORIES.slice(0, FURNITURE_BRAND_STORIES_MAX);
  }

  const storeName = formatFurnitureStoreName(ctx.storeName);
  const editorial = resolveFurnitureEditorialBanners(settings, products, businessDomain, businessCategory);
  // Skip first editorial (used by lifestyle spotlight) to avoid duplicate messaging.
  const fromEditorial = editorial.slice(1, 1 + FURNITURE_BRAND_STORIES_MAX).map((b, i) =>
    sanitizeFurnitureBrandStory(
      {
        id: b.id || `editorial-${i}`,
        eyebrow: b.eyebrow,
        title: b.title,
  ...
  const imaged = (products || []).filter((p) => p.image_url).slice(0, FURNITURE_BRAND_STORIES_MAX);
  return imaged
    .map((p, i) =>
      sanitizeFurnitureBrandStory(
        {
          ...
          eyebrow: storeName,
          title: p.name,
          subtitle: (p.description && String(p.description).slice(0, 120)) || 'A standout piece from our collection.',
          image: p.image_url,
...
/**
 * Full-bleed showroom banner: never a plain color strip.
 * Prefer owner banner → branding hero slides → cover → featured catalog → curated showroom default.
 *
 * @param {object} [settings]
 * @param {{
 ...
 * }} [ctx]
 */
export function resolveFurnitureShowroomBanner(settings = {}, ctx = {}) {
  const config = getFurnitureConfig(settings, ctx.businessDomain);
  const storeName = formatFurnitureStoreName(ctx.storeName);
  const globalSlides = resolveStoredHeroSlides(settings);
  const heroImage =
    globalSlides.find((s) => typeof s.image === 'string' && s.image.startsWith('http'))?.image || '';
  const featuredImage =
    (ctx.products || []).find(
      (p) => p.is_featured && typeof p.image_url === 'string' && p.image_url.startsWith('http')
    )?.image_url ||
    (ctx.products || []).find((p) => typeof p.image_url === 'string' && p.image_url.startsWith('http'))
      ?.image_url ||
    '';
  const cover =
    typeof ctx.coverImage === 'string' && ctx.coverImage.startsWith('http') ? ctx.coverImage : '';
  const demoShowroom = FURNITURE_DEMO_HERO_SLIDES[2]?.image || FURNITURE_MARKETING_HERO_IMAGE;

  const image =
    config.showroomBannerImage ||
    heroImage ||
    cover ||
    featuredImage ||
    (isDemoStoreDomain(ctx.businessDomain) ? demoShowroom : '') ||
    demoShowroom;
  ...
  return {
    image,
    videoUrl: config.showroomVideoUrl || '',
    eyebrow: config.showroomEyebrow || 'Experience in person',
    title: config.showroomTitle || `Visit a ${storeName} showroom near you`,
    subtitle:
      config.showroomSubtitle ||
      'See fabrics, test comfort, and get expert guidance. Walk-ins welcome during store hours.',
    ctaLabel: config.showroomLabel || 'Showroom locations',
  };
}
