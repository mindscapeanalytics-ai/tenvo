...
export async function applyStorefrontCryptoFromIPN({ orderId, paymentId, status, amount }) {
  ...
  try {
    ...
    try {
      await commitStorefrontOrderInventoryIfPending(client, parsed.businessId, order.id);
    } catch (invErr) {
      const failMeta = {
        ...meta,
        crypto: cryptoMeta,
        inventory_commit_error: String(invErr?.message || invErr),
        inventory_commit_failed_at: new Date().toISOString(),
      };
      await client.query(
        `UPDATE storefront_orders SET
          metadata = $1::jsonb,
          ...
          updated_at = NOW()
         WHERE id = $2`,
        [JSON.stringify(failMeta), order.id]
      );
      console.error(
        '[applyStorefrontCryptoFromIPN] inventory commit failed:',
