...
export function resolveSupermarketCategoryIcons(settings, storeBase, ctx = {}) {
  ...
  return mapCatalogCategoryIcons(
    isMilkShopStore(ctx.businessCategory) ? MILK_SHOP_POPULAR_CATEGORIES : SUPERMARKET_POPULAR_CATEGORIES,
    productsUrl
  );
}
...
export function resolveSupermarketSidebarDepartments(settings, storeBase, ctx = {}) {
  ...
  const productsUrl = `${storeBase}/products`;
  const milkDefault =
    isMilkShopStore(ctx.businessCategory) || isMilkShopStore(ctx.businessDomain)
      ? MILK_SHOP_SIDEBAR_DEPARTMENTS
      : SUPERMARKET_SIDEBAR_DEPARTMENTS;
  const source = Array.isArray(raw) && raw.length ? raw : milkDefault;
 ...
 */
export function resolveSupermarketHomeRails(settings = {}, businessCategory) {
  const raw = settings?.storefront?.supermarket?.homeRails;
  const milkDefault = isMilkShopStore(businessCategory) ? MILK_SHOP_HOME_RAILS : SUPERMARKET_HOME_RAILS;
  const source = Array.isArray(raw) && raw.length ? raw : milkDefault;
...
export function resolveSupermarketQuickSearchTerms(settings = {}, products = [], categories = [], businessDomain, businessCategory) {
  ...
  if (isMilkShopStore(businessCategory)) {
    return MILK_SHOP_QUICK_SEARCH.slice(0, 8);
  }
