...
/**
 * Gym-vertical Unsplash fallback when Webflow/Synergize CDNs fail under Next Image.
 * @param {string} [seed]
 */
export function resolveFitnessImageFallback(seed = 'gym-fitness') {
  return getFallbackProductImageUrl({ name: String(seed || 'gym'), id: String(seed || 'gym') }, 'gym-fitness');
}
...
export const FITNESS_DEMO_CATEGORY_ICONS = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1583454110551-21f2fa2afe61?w=200&q=80&auto=format&fit=crop',
  },
  {
    id: 'bcaa',
    label: 'BCAA',
...
export function resolveFitnessCategoryIcons(settings, storeBase, ctx = {}) {
  ...
  if (config.categoryIcons?.length) {
    return enrichFitnessCategoryNavImages(
      config.categoryIcons.map((c) => ({
        ...c,
...
export function isFitnessServiceProduct(p) {
  ...
  const name = String(p?.name || '').toLowerCase();
  return /\b(personal training|pt session|group class|class pack|training session|nutrition consult|consultation)\b/i.test(
    name
  );
}

/**
 * Booking CTA target for memberships / training (tenant meeting URL, else contact).
 * @param {string} storeBase
 * @param {{ meetingUrl?: string | null; subject?: string; packageName?: string }} [opts]
 */
export function resolveFitnessBookHref(storeBase, opts = {}) {
  if (opts.meetingUrl) return opts.meetingUrl;
  const contact = `${storeBase}/contact`;
  const params = new URLSearchParams();
  params.set('subject', opts.subject || 'appointment');
  if (opts.packageName) params.set('package', String(opts.packageName));
