/**
 * Shared tenant-aware helpers for elevated public storefronts.
 * Prefer business settings, live categories, and catalog over static demo defaults.
 */
import { isDemoStoreDomain } from '@/lib/storefront/storeContactSanitize';
import {
  getEffectiveProductImageUrl,
  getFallbackProductImageUrl,
} from '@/lib/storefront/productImageFallback';
import {
  mergeHeroSlidesWithDefaults,
  resolveStoredHeroSlides,
  sanitizeHeroSlides,
} from '@/lib/storefront/heroSlides';
import { isDeadImageUrl } from '@/lib/storefront/deadImageHosts';

/**
 * @param {string | null | undefined} name
 * @param {string} [fallback]
 */
export function formatElevatedStoreName(name, fallback = 'Store') {
  const raw = String(name || '').trim();
  return raw.replace(/\s+Demo$/i, '').trim() || fallback;
}

/**
 * @param {Array<{ id?: string; name?: string; slug?: string; image_url?: string }>} categories
 * @param {string} storeBase
 * @param {{ max?: number; includeDeals?: boolean }} [opts]
 */
export function buildCategoryNavItems(categories, storeBase, opts = {}) {
  const { max = 12, includeDeals = true } = opts;
  const productsUrl = `${storeBase}/products`;
  const items = (categories || [])
    .filter((c) => c?.name && c?.slug)
    .slice(0, max)
    .map((cat) => ({
      id: String(cat.slug || cat.id),
      label: cat.name,
      slug: cat.slug,
      image: cat.image_url || '',
      href: `${productsUrl}?category=${encodeURIComponent(cat.slug)}`,
    }));

  if (includeDeals) {
    const hasDeals = items.some(
      (item) => item.id === 'deals' || String(item.slug || '').toLowerCase() === 'deals'
    );
    if (!hasDeals) {
      items.push({
        id: 'deals',
        label: 'Deals',
        slug: '',
        image: '',
        href: `${productsUrl}?onSale=true`,
        hrefSuffix: '?onSale=true',
      });
    }
  }
  return items;
}

/**
 * @param {Array<{ name?: string; slug?: string; id?: string }>} categories
 * @param {Array<{ id: string; label: string; slug: string }>} fallbackTabs
 */
export function buildCuratedTabsFromCategories(categories, fallbackTabs) {
  const rows = (categories || []).filter((c) => c?.name && c?.slug);
  if (rows.length >= 2) {
    return rows.slice(0, 6).map((c) => ({
      id: String(c.slug),
      label: c.name,
      slug: c.slug,
    }));
  }
  return fallbackTabs;
}

/**
 * @param {object[]} products
 * @param {object[]} categories
 * @param {string[] | null | undefined} configuredTerms
 * @param {number} [max]
 */
export function buildQuickSearchTerms(products, categories, configuredTerms, max = 6) {
  if (Array.isArray(configuredTerms) && configuredTerms.length) {
    return configuredTerms.slice(0, max);
  }
  const categoryList = Array.isArray(categories) ? categories : [];
  const productList = Array.isArray(products) ? products : [];
  const fromCategories = categoryList.slice(0, 4).map((c) => c?.name).filter(Boolean);
  const fromFeatured = productList
    .filter((p) => p.is_featured && p.name)
    .slice(0, 3)
    .map((p) => String(p.name).split(/\s+/).slice(0, 2).join(' '));
  const fromBrands = productList
    .map((p) => String(p?.brand || '').trim())
    .filter(Boolean)
    .slice(0, 4);
  const merged = [...new Set([...fromCategories, ...fromBrands, ...fromFeatured])].filter(Boolean).slice(0, max);
  return merged;
}

/**
 * @param {object[]} products
 * @param {string} slug
 */
export function filterProductsByCategorySlug(products, slug) {
  const normalized = String(slug || '').toLowerCase().trim();
  if (!normalized) return products;
  const compact = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
  const needleCompact = compact(normalized);
  return (products || []).filter((p) => {
    const catSlug = String(p.category_slug || '').toLowerCase();
    const catName = String(p.category_name || p.category || '').toLowerCase();
    if (catSlug) {
      if (catSlug === normalized) return true;
      if (catSlug.includes(normalized) || normalized.includes(catSlug)) return true;
    }
    const nameAsSlug = catName.replace(/\s+/g, '-');
    const nameSpaced = normalized.replace(/-/g, ' ');
    if (catName === nameSpaced || nameAsSlug === normalized) return true;
    if (needleCompact && (compact(catName) === needleCompact || compact(catSlug) === needleCompact)) {
      return true;
    }
    if (normalized.includes('main') && catName.includes('main')) return true;
    if (normalized.includes('appetizer') && catName.includes('appetizer')) return true;
    if (normalized.includes('dessert') && catName.includes('dessert')) return true;
    if (normalized.includes('beverage') && catName.includes('beverage')) return true;
    return catName.includes(normalized);
  });
}

/**
 * @param {object[]} products
 * @param {object[] | null | undefined} settingsBanners
 * @param {object[]} demoBanners
 * @param {{ isDemo?: boolean; businessCategory?: string }} ctx
 */
export function buildPromoBannersFromCatalog(products, settingsBanners, demoBanners, ctx = {}) {
  if (Array.isArray(settingsBanners) && settingsBanners.length) return settingsBanners;
  if (ctx.isDemo && demoBanners?.length) return demoBanners;

  const pool = (products || []).filter((p) => p.image_url || p.images?.length);
  const featured = pool.filter((p) => p.is_featured);
  const source = (featured.length ? featured : pool).slice(0, 4);
  if (!source.length) return [];

  return source.map((p, i) => ({
    id: String(p.id || `promo-${i}`),
    title: p.name,
    subtitle: p.category_name || p.category || 'Available now',
    image: getEffectiveProductImageUrl(p, ctx.businessCategory),
    href: p.slug ? `?search=${encodeURIComponent(String(p.name).split(/\s+/)[0])}` : '?onSale=true',
    tone: i % 2 === 0 ? 'purple' : 'cream',
  }));
}

/**
 * Resolve relative store CTA paths against `/store/{domain}`.
 * @param {string | null | undefined} href
 * @param {string} base
 */
function resolveHeroCtaHref(href, base) {
  const storeBase = String(base || '').replace(/\/products$/, '');
  const raw = String(href || '').trim();
  if (!raw) return `${storeBase}/products`;
  if (raw.startsWith('http') || raw.startsWith('/store/')) return raw;
  if (raw.startsWith('/')) return `${storeBase}${raw}`;
  return `${storeBase}/products`;
}

/**
 * Build catalog/demo fallback slides when no template defaults exist.
 * @param {object} params
 */
function buildCatalogHeroFallback({
  base,
  storeName,
  businessDescription,
  coverImage,
  featuredProducts = [],
}) {
  const slides = [];
  const subtitle =
    String(businessDescription || '').trim() ||
    `Browse our full menu with live prices and availability.`;

  slides.push({
    eyebrow: storeName,
    title: subtitle.split('.')[0]?.trim() || `Order from ${storeName}`,
    subtitle,
    image: coverImage || featuredProducts[0]?.image_url || '',
    ctaLabel: 'View menu',
    ctaHref: `${base}/products`,
  });

  for (const p of featuredProducts.slice(0, 3)) {
    if (!p?.image_url) continue;
    slides.push({
      eyebrow: p.category_name || p.category || storeName,
      title: p.name,
      subtitle: p.description?.slice(0, 120) || 'Order now while stock lasts.',
      image: p.image_url,
      ctaLabel: 'Order now',
      ctaHref: `${base}/products${p.slug ? `/${p.slug}` : ''}`,
    });
  }

  return slides;
}

/**
 * Build elevated-store hero slides with owner overrides.
 * Prefers global `settings.storefront.heroSlides`, then legacy vertical slides,
 * then demo/catalog defaults. Empty placeholder slots never wipe templates.
 *
 * @param {object} params
 * @param {object} [params.settings] Full business_settings.settings
 * @param {unknown} [params.settingsSlides] Legacy vertical heroSlides array
 * @param {string} base `/store/{domain}`
 * @param {string} [storeName]
 * @param {string} [businessDescription]
 * @param {string | null} [coverImage]
 * @param {object[]} [demoSlides]
 * @param {boolean} [isDemo]
 * @param {object[]} [featuredProducts]
 */
export function buildTenantHeroSlides({
  settings,
  settingsSlides,
  base,
  storeName,
  businessDescription,
  coverImage,
  demoSlides,
  isDemo,
  featuredProducts = [],
}) {
  const demoDefaults =
    isDemo && Array.isArray(demoSlides) && demoSlides.length
      ? demoSlides.map((s) => ({
          ...s,
          eyebrow: s.eyebrow?.replace(/\{storeName\}/g, storeName) || s.eyebrow,
          title: s.title?.replace(/\{storeName\}/g, storeName) || s.title,
        }))
      : [];

  const catalogDefaults = buildCatalogHeroFallback({
    base,
    storeName,
    businessDescription,
    coverImage,
    featuredProducts,
  });

  const defaultSlides = demoDefaults.length ? demoDefaults : catalogDefaults;

  const globalSlides = settings ? resolveStoredHeroSlides(settings) : [];
  const verticalSlides = sanitizeHeroSlides(settingsSlides);
  const verticalHasContent = verticalSlides.some((s) => s.image || s.title || s.subtitle);
  const globalHasContent = globalSlides.some((s) => s.image || s.title || s.subtitle);

  // Demo stores: prefer vertical-specific owner slides, else curated demo defaults.
  // Skip global `storefront.heroSlides` on demos — gallery/seed leftovers often
  // leak wrong-vertical imagery (e.g. car photos into furniture heroes).
  const ownerSlides = globalHasContent
    ? globalSlides
    : verticalHasContent
      ? verticalSlides
      : [];

  const merged = ownerSlides.length
    ? mergeHeroSlidesWithDefaults(ownerSlides, defaultSlides, { coverImage })
    : defaultSlides.map((s, i) => (i === 0 && coverImage ? { ...s, image: coverImage } : s));

  return merged.map((s) => ({
    ...s,
    ctaHref: resolveHeroCtaHref(s.ctaHref, base),
  }));
}

function productHasComparePrice(p) {
  const compare = p?.compare_price ?? p?.compare_at_price;
  return compare != null && Number(compare) > Number(p?.price);
}

function isDealsNavItem(item) {
  const id = String(item?.id || '').toLowerCase();
  const slug = String(item?.slug || '').toLowerCase();
  const label = String(item?.label || '').toLowerCase();
  return id === 'deals' || slug === 'deals' || label === 'deals' || label === 'sale';
}

/**
 * Enrich category nav rows with a representative product image when missing.
 * Matches category_slug, category_name, or legacy products.category text.
 * Falls back to vertical Unsplash placeholders so tiles never render empty.
 * @param {object[]} items
 * @param {object[]} products
 * @param {string | null | undefined} businessCategory
 */
export function enrichCategoryNavImages(items, products, businessCategory) {
  const pool = products || [];
  return items.map((item) => {
    const existing = typeof item.image === 'string' ? item.image.trim() : '';
    if (existing && !existing.startsWith('data:') && !isDeadImageUrl(existing)) {
      // Keep owner/DB image when present and not dead
      return item;
    }

    let match = null;
    if (isDealsNavItem(item)) {
      match =
        pool.find((p) => productHasComparePrice(p) && getEffectiveProductImageUrl(p, businessCategory)) ||
        pool.find((p) => getEffectiveProductImageUrl(p, businessCategory));
    } else {
      const label = String(item.label || '').toLowerCase();
      const slug = String(item.slug || '').toLowerCase();
      match = pool.find((p) => {
        if (p.category_slug && slug && String(p.category_slug).toLowerCase() === slug) return true;
        if (p.category_name && label && String(p.category_name).toLowerCase() === label) return true;
        // Seed / legacy rows often only set products.category (display name)
        if (p.category && label && String(p.category).toLowerCase() === label) return true;
        if (p.category && slug) {
          const catSlug = String(p.category)
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/^-|-$/g, '');
          if (catSlug === slug) return true;
        }
        return false;
      });
    }

    const fromProduct = match ? getEffectiveProductImageUrl(match, businessCategory) : '';
    if (fromProduct) return { ...item, image: fromProduct };

    const fallback = getFallbackProductImageUrl(
      { name: item.label || item.id || 'product', id: item.id || item.slug || 'category' },
      businessCategory,
      item.label
    );
    return { ...item, image: fallback || '' };
  });
}

export { isDemoStoreDomain };
