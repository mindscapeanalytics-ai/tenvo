...
import { enrichRestaurantCategoriesWithSeedImages, enrichRestaurantCuisineNavImages, buildRestaurantDemoCuisineIcons, buildRestaurantDemoSpotlightCards, buildRestaurantDemoPromoTiles, resolveRestaurantCategoryVisual, isPurchasableRestaurantProduct } from '@/lib/dataLab/restaurantSeedHelpers';
...
const RESTAURANT_DEMO_HERO_SLIDES = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=1600&q=82&auto=format&fit=crop',
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=1600&q=82&auto=format&fit=crop',
    ctaLabel: 'Explore menu',
    ctaHref: '/products',
  },
];

/** Marketing / homepage gallery hero — same asset as `/store/demo-restaurant` slide 0 */
...
export function resolveRestaurantCuisineIcons(settings = {}, storeBase, ctx = {}) {
  ...
  if (isDemoStoreDomain(ctx.businessDomain)) {
    return buildRestaurantDemoCuisineIcons(storeBase, ctx.products);
  }
 ...
 * @param {string} storeBase
 */
export function resolveRestaurantSpotlightCards(
  settings = {},
  categories = [],
  businessDomain,
  storeBase,
  ...
  if (isDemoStoreDomain(businessDomain)) {
    return buildRestaurantDemoSpotlightCards(storeBase, products);
  }
  ...
  const fromDb = (categories || [])
    ...
    .map((c) => ({
      ...
      image:
        resolveRestaurantCategoryVisual({
          slug: c.slug,
          label: c.name,
          products,
          categoryImageUrl: c.image_url || '',
        }) || null,
  ...
  return buildRestaurantDemoSpotlightCards(storeBase, products).slice(0, 4);
}
 ...
 * @param {string | null | undefined} [businessDomain]
 */
export function resolveRestaurantUpperPromoTiles(settings = {}, storeBase, products = [], businessDomain) {
  const config = getRestaurantConfig(settings, businessDomain);
  const productsUrl = `${storeBase}/products`;

  if (isDemoStoreDomain(businessDomain)) {
    return buildRestaurantDemoPromoTiles(storeBase, products);
  }
...
export function partitionRestaurantProducts(products = []) {
  ...
  const inStock = purchasable.filter((p) => p.stock == null || Number(p.stock) > 0);
  const pool = inStock.length ? inStock : purchasable;
