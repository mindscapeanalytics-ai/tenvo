/**
 * Storefront display / sellable stock.
 *
 * Variants and warehouse rows are different dimensions:
 * - Variant products (clothing size/color) sell via product_variants + removeVariantStock.
 * - Non-variant products sell via FIFO product_stock_locations + removeStock.
 *
 * Rules:
 * 1. When variants exist → max(headline, variant sum). Do not mix warehouse sum
 *    (location totals can be bootstrap mirrors and would overstate size-level stock).
 * 2. Else when sellable location rows exist (locationQty != null, including 0) → location sum.
 * 3. Otherwise → headline products.stock.
 *
 * Hub resolveInventoryEffectiveStock prefers locations first for warehouse ops;
 * storefront must stay variant-first for checkout parity.
 */
...
export function resolveStorefrontDisplayStock({ stock, variants = [], locationQty = null } = {}) {
  ...
  // Variant SKUs sell from variant stock — warehouse sum must not inflate card/cart availability.
  if (vars.length > 0) {
