...
export function resolveStoreContact({ business, settings }) {
  ...
  const email = pickString(contactSettings.email, business?.email);
  const phone = pickString(contactSettings.phone, business?.phone);
  const whatsapp = pickString(contactSettings.whatsapp, contactSettings.whatsApp, phone);
  ...
  const postalCode = pickString(contactSettings.postalCode, contactSettings.postal_code, business?.postal_code);
  ...
  return {
    ...
    hasAnyContact: Boolean(email || phone || fullAddress || website),
  };
}
