// Migrated to server actions - see lib/actions/standard/quotation.js
import {
    getQuotationsAction,
...
export const quotationAPI = {
    async getAll(businessId, options = {}) {
        try {
            ...
            const [quotations, salesOrders, challans] = await Promise.all([
                getQuotationsAction(businessId, pageOpts),
                getSalesOrdersAction(businessId, pageOpts),
                getChallansAction(businessId, pageOpts),
            ]);
    ...
    async getQuotationDetail(quotationId, businessId) {
        const result = await getQuotationDetailAction(quotationId, businessId);
        if (!result.success) throw new Error(result.error);
        return result.quotation;
    },
    ...
    async getSalesOrderDetail(orderId, businessId) {
        const result = await getSalesOrderDetailAction(orderId, businessId);
        if (!result.success) throw new Error(result.error);
        return result.salesOrder;
    },
    ...
    async getChallanDetail(challanId, businessId) {
        const result = await getChallanDetailAction(challanId, businessId);
        if (!result.success) throw new Error(result.error);
        return result.challan;
    }
};
