 ...
 *
 * NOTE: This is a process-local in-memory store. In a serverless / multi-instance
 * deployment (Vercel, AWS Lambda) each instance has its own counter, so limits are
 * per-instance, not global. For production-grade rate limiting use an external store
 * (Redis / Upstash) via a dedicated middleware layer. This implementation is suitable
 * for single-server or development use only.
 */
const RATE_LIMITS = {
    free:         60,
    basic:       120,
