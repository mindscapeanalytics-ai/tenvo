...
export function withApiAuth(handler) {
    return async (request, routeParams) => {
        try {
            ...
            //    IMPORTANT: request.json() consumes the body stream — it can only be called once.
            //    For mutation methods we parse the body here and pass the parsed object to the
            //    handler via context so handlers never need to call request.json() themselves.
            let businessId = null;
            let parsedBody = null;
            ...
            if (method === 'GET' || method === 'HEAD') {
                ...
                // Parse body once; store it so the handler can use it without re-reading the stream
                try {
                    ...
                    // Body is empty or not JSON — fall back to query params
                    businessId =
                        searchParams.get('business_id') ||
                        searchParams.get('businessId');
                }
            ...
            try {
                if (!platformAdmin) {
                    ...
                    if (memberRes.rows.length === 0 || memberRes.rows[0].status !== 'active') {
                        // Failsafe: business owner may not have a business_users row yet
            ...
            //    `parsedBody` is provided so handlers can use it directly instead of
            //    calling request.json() again (which would throw — stream already consumed).
            return await handler(request, {
                session,
                businessId,
                role,
                planTier,
                parsedBody,          // pre-parsed body for mutation handlers
                routeParams,
            });
