...
export const businessAPI = {
    ...
    async getByUserId(userId) {
        const result = await getJoinedBusinessesAction(userId || null);
        if (!result.success) throw new Error(result.error);
    ...
    /**
     * RBAC: Update user role (and optional module access overrides)
     * @param {string} userId
    ...
    async updateUserRole(userId, businessId, role, modules) {
        const payload = modules === undefined ? { role } : { role, modules };
        const result = await updateUserRoleAction(userId, businessId, payload);
        if (!result.success) throw new Error(result.error);
        return result.membership;
    },

    async addMember(businessId, email, role) {
        const result = await addBusinessMemberAction({ businessId, email, role });
        if (!result.success) throw new Error(result.error);
        return result.membership;
    },

    async removeMember(businessId, userId) {
        const result = await removeBusinessMemberAction({ businessId, userId });
        if (!result.success) throw new Error(result.error);
        return true;
    },

    async updatePlan(businessId, planTier) {
        const result = await updateBusinessPlanAction({ businessId, planTier });
        if (!result.success) throw new Error(result.error);
        return result.business;
    },
    /**
    ...
    async getRole(businessId, userId) {
        const result = await getUserBusinessRoleAction(businessId, userId || null);
