...
import { isUsableProductImageUrl } from '@/lib/utils/productImages';
...
/**
 * Supermarket showcase — DB products only, enriched from seed metadata.
...
export function resolveSupermarketShowcaseProducts(dbProducts, businessDomain, businessCategory) {
  ...
  if (!shouldUseSupermarketSeedCatalog(businessDomain, businessCategory)) {
    return list.filter(isPurchasableStorefrontProduct);
  }
  return enrichSupermarketProductsWithSeedImages(list.filter(isPurchasableStorefrontProduct));
}
...
export function enrichSupermarketProductsWithSeedImages(products) {
  ...
  return products.map((product) => {
    ...
    const merchant = typeof product.image_url === 'string' ? product.image_url.trim() : '';
    const keepMerchant =
      merchant && isUsableProductImageUrl(merchant) && !isDeadImageUrl(merchant);
    return {
      ...product,
      image_url: keepMerchant ? merchant : seed.image_url,
