...
export function enrichRestaurantProductsFromSeed(products = []) {
  return (products || []).filter(Boolean).map((p) => {
    ...
    const rawImageUrl = normalizeStorefrontRemoteImageUrl(
      p.image_url || seed?.image_url || ''
    );
    const imageUrl = rawImageUrl && !isDeadImageUrl(rawImageUrl) ? rawImageUrl : '';
...
export function resolveRestaurantCategoryProductImage({ slug, label, products = [] }) {
  ...
  const matches = pool.filter(
    (p) => productMatchesCategory(p, slug, label) && p.image_url && !isDeadImageUrl(p.image_url)
  );
...
export function resolveRestaurantCategoryVisual({ slug, label, products = [], categoryImageUrl = '' }) {
  ...
  if (fromProduct && !isDeadImageUrl(fromProduct)) return fromProduct;
  if (categoryImageUrl && !isDeadImageUrl(categoryImageUrl)) return categoryImageUrl;
  const seed = seedCategoryBySlug.get(String(slug || '').toLowerCase());
  const seedUrl = seed?.image_url || '';
  return seedUrl && !isDeadImageUrl(seedUrl) ? seedUrl : '';
}
...
export function enrichRestaurantCategoriesWithSeedImages(categories) {
  return (categories || []).map((cat) => {
    if (cat?.image_url && !isDeadImageUrl(cat.image_url)) return cat;
    const seed = seedCategoryBySlug.get(String(cat?.slug || '').toLowerCase());
    const seedUrl = seed?.image_url || '';
    if (!seedUrl || isDeadImageUrl(seedUrl)) return { ...cat, image_url: '' };
    return { ...cat, image_url: seedUrl };
  });
}
