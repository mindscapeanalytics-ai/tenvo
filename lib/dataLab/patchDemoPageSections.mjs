 ...
 */
import { mergeDemoPageSectionsIntoSettings, stripDemoAutoPageSections } from './demoPageSectionsSeed.js';
...
export async function patchDemoPageSections(pool, { domain, key, country, businessName, dryRun = false }) {
  ...
  try {
    ...
    const strippedSettings = stripDemoAutoPageSections(prevSettings);
    const hadDemoAuto = strippedSettings !== prevSettings;

    if (!hadDemoAuto && getActivePageSections(prevSettings.pageSections).length > 0) {
    ...
    const nextSettings = mergeDemoPageSectionsIntoSettings(strippedSettings, {
      domainKey: canonical,
      domainHandle,
      storefrontProfile,
      productImages,
    ...
    if (
      !hadDemoAuto &&
      JSON.stringify(prevSettings.pageSections || []) === JSON.stringify(nextSettings.pageSections || [])
    ) {
