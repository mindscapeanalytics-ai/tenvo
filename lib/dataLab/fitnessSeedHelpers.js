...
export function mapFitnessSeedRowToStorefrontProduct(row) {
  ...
  return {
    id: row.sku || slugifyFitnessCategory(name),
    // Seed rows are catalog previews, not real DB products. They must NOT carry a
    // product slug — a slug makes href builders link to /products/{slug}, which
    // 404s because the row does not exist in the database. Keeping slug null routes
    // these previews to the products search page instead.
    slug: null,
    sku: row.sku,
    name,
    price: row.price,
...
export function resolveFitnessShowcaseProducts(dbProducts, businessDomain) {
  ...
  return enrichFitnessProductsWithSeedImages(
    list.filter((p) => isStorefrontProductUuid(p?.id) && !p.catalog_preview),
    businessDomain
  );
}
