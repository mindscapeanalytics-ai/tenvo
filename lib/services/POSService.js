...
export const POSService = {
    ...
    async closeSession(data, userId, txClient = null) {
        ...
        try {
            ...
            const { cashMovements } = parsePosSessionNotes(sesRes.rows[0].notes);
            const movementNet = sumPosCashMovements(cashMovements);
            const expectedCash = openingCash + Number(stats.total_cash || 0) + movementNet;
    ...
    async createTransaction(data, userId, txClient = null) {
        ...
        try {
            // Idempotent offline sync: return existing row before BEGIN (no stock/GL redo).
            if (clientRef) {
                const existing = await client.query(
            ...
            const { businessId, customerId, items, payments, discountAmount: headerDiscount = 0 } = data;
            let sessionId = data.sessionId;

            // 1. Validate Session (offline sync may remap onto a currently open session)
            ...
            `, [businessId, sessionId, txNumber, customerId || null, subtotal, totalTax, totalDiscount, grandTotal, clientRef]);
            ...
            // Race: two sync workers inserted the same client_ref — return winner, no second stock move.
            if (clientRef && (error?.code === '23505' || /client_ref/i.test(String(error?.message || '')))) {
    ...
    async refundTransaction(data, userId, txClient = null) {
        ...
        try {
            ...
            if (originalTx.is_voided || originalTx.payment_status === 'voided') {
