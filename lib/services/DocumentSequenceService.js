...
export const DocumentSequenceService = {
    ...
    async resetSequence(businessId, documentType, startNumber, context, txClient = null) {
        ...
        try {
            ...
            // Check for existing documents with higher numbers (BIGINT — avoid INTEGER overflow)
            const checkQuery = `
                SELECT COUNT(*) as count,
                       MAX(
                         CASE
                           WHEN NULLIF(REGEXP_REPLACE(${mapping.column}, '[^0-9]', '', 'g'), '') ~ '^[0-9]{1,18}$'
                           THEN CAST(NULLIF(REGEXP_REPLACE(${mapping.column}, '[^0-9]', '', 'g'), '') AS BIGINT)
                           ELSE NULL
                         END
                       ) as max_num
                FROM ${mapping.table}
    ...
    async getSequenceStatus(businessId, documentType) {
        ...
        try {
            ...
            // Get current max number from documents (BIGINT — avoid INTEGER overflow)
            const query = `
                SELECT COALESCE(
                    MAX(
                      CASE
