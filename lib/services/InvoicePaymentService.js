...
export const InvoicePaymentService = {
    ...
    async recordPayment(params, txClient = null) {
        ...
        try {
            ...
            // Update customer outstanding balance if invoice had customer
        ...
        try {
            ...
            const res = await client.query(`
                SELECT 
                    ...
                    (SELECT COUNT(*)::int FROM invoice_payments ipc WHERE ipc.invoice_id = i.id) AS payment_count,
                    (SELECT MAX(ipm.payment_date) FROM invoice_payments ipm WHERE ipm.invoice_id = i.id) AS last_payment_date
                FROM invoices i
                LEFT JOIN LATERAL (
