...
export const MarketingAgentService = {
    ...
    async refreshSegment(businessId, segmentId) {
        ...
        try {
            ...
            let query = `SELECT id FROM customers WHERE business_id = $1 AND customers.is_deleted = false`;
    ...
    async queueCampaignMessages(businessId, campaignId) {
        ...
        try {
            ...
            const campaignRes = await client.query('SELECT segment_id, scheduled_at FROM campaigns WHERE id = $1', [campaignId]);
            const segmentId = campaignRes.rows[0].segment_id;
            const scheduledAt = campaignRes.rows[0].scheduled_at;
