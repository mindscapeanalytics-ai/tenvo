...
function isMissingRelationError(error) {
    ...
    return (
        code === '42P01' ||
        code === 'P2021' ||
        /relation ["']?business_custom_domains["']? does not exist/i.test(message) ||
        (/does not exist/i.test(message) && /business_custom_domains/i.test(message))
    );
}
...
export const StorefrontSyncService = {
            ...
            for (const product of productsResult.rows) {
                ...
                if (product.stock === null || product.stock > 0) {
                    ...
                    await client.query(
                        ...
                        [stockStatus, product.id, businessId]
                    );
                    synced++;
                }
            }
     ...
     */
    async updateStockAvailability(productId, newStock, businessId, tx = null) {
        if (!businessId) {
        ...
        try {
            ...
            await client.query(
                ...
                [newStock, stockStatus, productId, businessId]
            );
            ...
            const [productsResult, totalResult] = await Promise.all([
                client.query(
                    ...
                    LEFT JOIN product_variants pv ON p.id = pv.product_id 
                        AND pv.is_deleted = false AND pv.is_active = true AND pv.business_id = $1::uuid
                    WHERE ${whereClause}
                    GROUP BY p.id
     ...
     */
    async initializeStorefront(businessId, domain, tx = prismaBase) {
        try {
            log.info(`Initializing storefront for business ${businessId} with domain ${domain}`);

            const domainName = domain || `store-${businessId}`;
            ...
            return {
                success: true,
                domain: existingDomain?.domain || domainName,
            };
        } catch (error) {
            ...
            if (isMissingRelationError(error)) {
                return {
                    success: false,
                    error:
                        'business_custom_domains table is missing. Run `bun run db:migrate` (migration 20260721_ensure_business_custom_domains).',
                };
            }
            return { success: false, error: error.message };
        }
