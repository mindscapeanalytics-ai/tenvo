...
const FUNCTIONAL_TYPE_TO_CODE = {
    ...
    'input_tax':            ACCOUNT_CODES.INPUT_TAX_CREDIT,          // 2103
...
export const AccountingService = {
    ...
    async createJournalEntry(params, txClient = null) {
        ...
        try {
            ...
            const businessId = params.businessId || params.business_id;
            if (!businessId) {
                throw new Error('Business ID is required');
            }
            const { date = new Date(), description, referenceType, referenceId, referenceNumber, userId } = params;
            let { entries } = params;
        ...
        try {
            ...
            switch (type) {
                ...
                case 'purchase': {
                    ...
                    let purchaseNet = Number(data.netAmount);
                    if (
                        !Number.isFinite(purchaseNet) ||
                        Math.abs(purchaseNet + purchaseTax - purchaseTotal) > 0.01
                    ) {
                        purchaseNet = Math.round((purchaseTotal - purchaseTax) * 100) / 100;
                    }
                    const accountKeys = purchaseTax > 0
                        ? ['inventory', 'ap', 'input_tax']
                        : ['inventory', 'ap'];
                    const accounts = await this.getGLAccountsByTypes(businessId, accountKeys, client);
                    entries.push({ accountId: accounts.inventory.id, debit: purchaseNet, credit: 0 });
                    if (purchaseTax > 0) {
                        entries.push({ accountId: accounts.input_tax.id, debit: purchaseTax, credit: 0 });
                    }
    ...
    async getAccountBalance(businessId, accountId, asOfDate = null) {
        ...
        try {
            let query = `
                SELECT COALESCE(SUM(e.debit - e.credit), 0) as balance
                FROM gl_entries e
            ...
            if (asOfDate) {
                query += ` AND e.transaction_date <= $${params.length + 1}::date`;
    ...
    async closeFiscalPeriod(periodId, context, txClient = null) {
        ...
        try {
            ...
            // 3. Validate no open/draft transactions in this period (journal_entries.status = draft)
