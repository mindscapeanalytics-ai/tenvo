 ...
 */
export async function canMakeApiCall(businessId, planTier = 'free') {
  const { checkApiRateLimit } = await import('@/lib/cache/rateLimit');
  return checkApiRateLimit(businessId, planTier);
}
