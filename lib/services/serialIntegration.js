...
export async function handleSerialReturn(data) {
  ...
  // Get customer from original
  const originalData = await db.product_serials.findUnique({
    where: { id: serialId },
    select: { customer_id: true, invoice_id: true }
  });

  // Allocate replacement to same customer
  const replacement = await db.product_serials.update({
    where: { id: replacementSerialId },
    data: {
      status: 'sold',
      customer_id: originalData.customer_id,
      invoice_id: originalData.invoice_id,
      sale_date: new Date()
    }
  });
