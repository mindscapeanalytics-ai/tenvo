...
export const ExpenseService = {
    ...
    async createExpense(data, userId, txClient = null) {
        ...
        try {
            ...
            `, [
                ...
                eData.amount, eData.taxAmount || 0, eData.vendorId || null, eData.paymentMethod,
    ...
    async deleteExpense(businessId, expenseId, userId, txClient = null) {
        ...
        try {
            ...
            // 1. Soft Delete
            await client.query(`UPDATE expenses SET is_deleted = true, deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND business_id = $2`, [expenseId, businessId]);

            // 2. Reverse Balance
            if (expense.payment_method === 'credit' && expense.vendor_id) {
                await client.query(`UPDATE vendors SET outstanding_balance = COALESCE(outstanding_balance, 0) - $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`, [expense.amount, expense.vendor_id, businessId]);
            }

            // 3. Reverse GL via proper journal reversal (preserves audit trail)
            ...
            for (const row of journalRes.rows) {
                if (row.journal_id) {
                    await AccountingService.reverseJournalEntry(
                        row.journal_id,
                        { businessId, userId, reason: `Void expense ${expense.expense_number}` },
                        client
                    );
                }
            }

            // 4. Soft-delete linked payments
            await client.query(`
                UPDATE payments SET is_deleted = true, status = 'voided', deleted_at = NOW(), updated_at = NOW()
                WHERE reference_type = 'expense' AND reference_id = $1 AND business_id = $2
