...
import {
  ...
  lookupCategoryIdFromMap,
} from '../utils/productCategoryLink.js';
...
export async function provisionRegistrationSeed({
  ...
  return prismaBase.$transaction(async (tx) => {
    ...
    if (items.length > 0) {
      const categoryMap = await buildProductCategoryIdMap(tx, businessId);
      const linkedItems = items.map((item) => {
        if (item.category_id) return item;
        const category_id = lookupCategoryIdFromMap(categoryMap, item.category);
        return category_id ? { ...item, category_id } : item;
      });

      const results = await ProductService.seedProducts(businessId, linkedItems, tx);
