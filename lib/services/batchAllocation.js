 ...
 * 
 * @example
 * const validation = await validateBatchForSale(batchId, 10);
 * if (!validation.valid) {
...
export async function validateBatchForSale(
  ...
  const batch = await db.product_batches.findUnique({
    where: { id: batchId },
    select: {
      id: true,
      batch_number: true,
...
export async function createBatchStockMovement(data) {
  ...
  // Validate batch
  const validation = await validateBatchForSale(batchId, quantity);
  if (!validation.valid) {
