...
export const RestaurantService = {
    ...
    async createOrder(data, userId, txClient = null) {
        ...
        try {
            ...
            // Normalize item fields, support both {name, price} and {itemName, unitPrice}
            const items = data.items.map(item => ({
                ...item,
                itemName: item.itemName || item.name || item.item_name || 'Item',
                unitPrice: parseFloat(item.unitPrice ?? item.unit_price ?? item.price ?? 0),
                specialInstructions: item.specialInstructions || item.notes || item.special_instructions || null,
            }));
            ...
            for (const item of items) {
                ...
                subtotal += (item.unitPrice + modPrice) * (item.quantity || 1);
            }
            ...
            const totalAmount = Math.round((subtotal + taxAmount - discountAmount + deliveryFee) * 100) / 100;

            // Build enriched notes: include customer/delivery info for takeaway + delivery
            let notesValue = data.notes || null;
            if (data.customerName || data.deliveryAddress) {
                const meta = {};
                if (data.customerName) meta.customer_name = data.customerName;
                if (data.customerPhone) meta.customer_phone = data.customerPhone;
                if (data.deliveryAddress) meta.delivery_address = data.deliveryAddress;
                if (data.deliveryFee) meta.delivery_fee = data.deliveryFee;
                notesValue = JSON.stringify(meta) + (data.notes ? ' | ' + data.notes : '');
            }
            ...
            `, [
                ...
                data.customerId, userId, subtotal, taxAmount, discountAmount, totalAmount, notesValue
            ]);
            ...
            for (const item of items) {
                ...
                kitchenItems.push({
                    item_name: item.itemName, name: item.itemName,
                    quantity: item.quantity || 1, qty: item.quantity || 1,
                    mods: item.modifiers || [], special: item.specialInstructions || null
                });
                ...
                if (item.productId) {
                    try {
                        ...
                        }, null); // intentionally independent connection, failure must not poison parent txn
                    } catch (reserveError) {
            ...
            // Items & KDS (optional for quick-pay / skip-kitchen flow)
            if (!data.skipKitchen) {
                await client.query(`
        ...
        try {
            ...
            await client.query(`
                UPDATE kitchen_orders SET
                    status = $1::varchar,
                    started_at = CASE WHEN $1::varchar = 'preparing' THEN NOW() ELSE started_at END,
                    completed_at = CASE WHEN $1::varchar IN ('ready', 'completed') THEN NOW() ELSE completed_at END
