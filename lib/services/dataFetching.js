 ...
 * Uses proper React import — the original had a bare `React.useState` reference
 * without importing React, which would crash at runtime.
 * @param {string} url - URL to fetch
 * @param {object} options - Options including businessId and ttl
 * @returns {object} Data, loading, error, and refetch function
 */
export function useFetch(url, options = {}) {
  // Lazy import to keep this file importable from server-only contexts
  // (the class itself is used server-side; only this hook is client-only).
