...
export function storefrontOrderJournalDescription(order) {
...
/**
 * Create journal entries for a paid storefront order (cash/bank sale, not orphan AR receipt).
 * @param {number|string} orderId - storefront_orders.id (Int PK)
 * @param {string} businessId
 * @param {{ txClient?: object, paymentMode?: string, userId?: string|null }} [options]
 */
export async function reconcileOrderPayment(orderId, businessId, options = {}) {
  const { txClient = null, paymentMode = 'cash', userId = 'system' } = options;
  const ownClient = !txClient;
  const client = txClient || (await pool.connect());
  const orderIdNum = typeof orderId === 'string' ? parseInt(orderId, 10) : Number(orderId);
  const shouldManageTx = ownClient;
  ...
  try {
    ...
    // Idempotent via metadata pointer or description token
    const existingMetaId = order.metadata?.reconciliation?.journal_entry_id;
    if (existingMetaId) {
      if (shouldManageTx) await client.query('ROLLBACK');
      return {
        success: false,
        message: 'Order already reconciled',
        journal_entry_id: existingMetaId,
      };
    }
    ...
    const useCash = mode === 'cash' || mode === 'cod' || mode === 'cash_on_delivery';
    const gl = await AccountingService.getGLAccountsByTypes(
      businessId,
      [useCash ? 'cash' : 'bank', 'revenue', 'tax_payable'],
      client
    );
    const fundAccount = useCash ? gl.cash : gl.bank;
    ...
    // Prefer explicit line breakdown; fall back to net = total - tax when subtotal missing
    let revenueCredit = subtotal;
    if (!(revenueCredit > 0) && totalAmount > 0) {
      revenueCredit = Math.round((totalAmount - taxAmount - shippingAmount + discountAmount) * 100) / 100;
    }

    const entries = [
      { accountId: fundAccount.id, debit: totalAmount, credit: 0, description: 'Payment received' },
      { accountId: gl.revenue.id, debit: 0, credit: Math.max(0, revenueCredit), description: 'Sales revenue' },
    ];
    if (taxAmount > 0) {
      entries.push({
    ...
    await client.query(
      ...
      [journalId, totalAmount, taxAmount, shippingAmount, discountAmount, orderIdNum, businessId]
    );
