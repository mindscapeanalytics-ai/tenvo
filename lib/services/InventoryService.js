...
export const InventoryService = {
    ...
    async removeStock(params, userId, txClient = null) {
        ...
        try {
            ...
            if (serialNumbers.length > 0) {
                ...
                for (const serial of serialRes.rows) {
                    ...
                    await client.query(
                        ...
                        [invoiceDate, warrantyEndDate, customerId, referenceType, referenceId, serial.id, businessId]
                    );
                }
            }
            ...
            // 5. Sync truth to products cache
            let newStock;
            if (fifoSellableLocations) {
                const locSnapshot = fifoLocationSnapshot
                    || await this.resolveSellableLocationSnapshot(client, businessId, productId, state);
                if (locSnapshot.hasRows) {
                    // Always sync after location FIFO — including when drained to 0 —
                    // so stale headline cannot keep showing phantom stock.
                    newStock = await this.syncProductStock(productId, businessId, client);
                } else {
                    const headlineRes = await client.query(
                        'SELECT stock FROM products WHERE id = $1 AND business_id = $2',
                        [productId, businessId]
                    );
                    newStock = Number(headlineRes.rows[0]?.stock || 0);
                }
            } else {
                newStock = await this.syncProductStock(productId, businessId, client);
            }
    ...
    async transferStock(params, userId, txClient = null) {
        ...
        try {
            ...
            await this.syncProductStock(productId, businessId, client);
