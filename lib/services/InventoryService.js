    ...
    async removeStock(params, userId, txClient = null) {
        ...
        try {
            ...
            if (warehouseId) {
                ...
                const locationQty = await this.resolveSellableLocationQty(client, businessId, productId, state);
                const available = resolveStorefrontDisplayStock({
            ...
            if (warehouseId) {
                ...
                const locationQtyBefore = await this.resolveSellableLocationQty(client, businessId, productId, state);
                if (locationQtyBefore > 0) {
                    await this.decrementFifoSellableLocations(client, businessId, productId, finalQuantity, state);
                } else {
                    await client.query(
                        `UPDATE products
            ...
            if (fifoSellableLocations) {
                const locationQtyAfter = await this.resolveSellableLocationQty(client, businessId, productId, state);
                if (locationQtyAfter > 0) {
