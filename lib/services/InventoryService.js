import pool, { readPool } from '@/lib/db';
import {
    addStockSchema,
    removeStockSchema,
    removeVariantStockSchema,
    transferStockSchema,
    reserveStockSchema,
    releaseStockSchema,
    adjustStockSchema
...
export const InventoryService = {
     ...
     * NOTE: The DB trigger `trg_sync_product_stock` now handles automatic sync
     * on product_stock_locations INSERT/UPDATE/DELETE. This method is retained
     * for edge cases (variant-only products, explicit re-sync, pre-trigger compat).
     */
    async syncProductStock(productId, businessId, txClient = null) {
        const client = await this.getClient(txClient);
        const locRes = await client.query(
            `SELECT
                COALESCE(SUM(CASE WHEN COALESCE(state, 'sellable') = 'sellable' THEN quantity ELSE 0 END), 0)::float AS sellable,
                COUNT(*)::int AS location_count
             FROM product_stock_locations
             WHERE product_id = $1 AND business_id = $2`,
            [productId, businessId]
        );
        const locationCount = Number(locRes.rows[0]?.location_count || 0);
        let totalStock = parseFloat(locRes.rows[0]?.sellable ?? 0);

        // No warehouse rows: keep variant-sum truth for size/color parents (do not zero them).
        if (locationCount === 0) {
            const meta = await client.query(
                `SELECT COALESCE(has_variants, false) AS has_variants
                 FROM products WHERE id = $1 AND business_id = $2`,
                [productId, businessId]
            );
            if (meta.rows[0]?.has_variants) {
                const vRes = await client.query(
                    `SELECT COALESCE(SUM(stock), 0)::float AS variant_stock
                     FROM product_variants
                     WHERE product_id = $1 AND business_id = $2
                       AND COALESCE(is_deleted, false) = false`,
                    [productId, businessId]
                );
                totalStock = parseFloat(vRes.rows[0]?.variant_stock ?? 0);
            }
        }
    ...
    /**
     * Read the current headline stock after a trigger-synced location write.
     * Cheaper than syncProductStock — no SUM, just reads what the trigger set.
     */
    async readCurrentStock(productId, businessId, client) {
        const res = await client.query(
            'SELECT stock::float AS stock FROM products WHERE id = $1 AND business_id = $2',
            [productId, businessId]
        );
     ...
     * Injects RLS session variable for tenant isolation safety net.
     */
    ...
    /**
     * Set tenant context for RLS policies on a raw pg client.
     * Call inside BEGIN transaction block for SET LOCAL scoping.
     */
    async setTenantContext(client, businessId) {
        if (businessId) {
            await client.query(`SET LOCAL app.business_id = '${String(businessId).replace(/'/g, "''")}'`);
        }
    },

    /**
     * Read-only client from read replica pool (falls back to write pool).
     * Use for reports, dashboards, valuations, and any non-mutation query.
     */
    async getReadClient() {
    ...
    /**
     * FIFO decrement across all sellable location rows (storefront + hub parity).
     * @returns {Promise<number>} remaining quantity not allocated (0 = fully allocated)
     */
    async decrementFifoSellableLocations(client, businessId, productId, quantity, state = 'sellable') {
    ...
    /**
     * Sellable warehouse snapshot: qty + whether any sellable location rows exist.
     * hasRows distinguishes "no warehouse tracking" from "tracked but empty".
     */
    async resolveSellableLocationSnapshot(client, businessId, productId, state = 'sellable') {
        try {
            const res = await client.query(
            ...
            const qty = parseFloat(res.rows[0]?.location_qty ?? 0);
            const hasRows = Number(res.rows[0]?.location_count ?? 0) > 0;
            return { qty, hasRows };
        } catch (err) {
            if (err.code === '42P01') return { qty: 0, hasRows: false };
            throw err;
        }
    },

    async resolveSellableLocationQty(client, businessId, productId, state = 'sellable') {
        const snapshot = await this.resolveSellableLocationSnapshot(client, businessId, productId, state);
        return snapshot.qty;
    },
    ...
    async addStock(params, userId, txClient = null) {
        ...
        try {
            ...
            await this.setTenantContext(client, businessId);
            ...
            if (batchNumber) {
                const bRes = await client.query(`
                    ...
                    ON CONFLICT (business_id, product_id, batch_number) WHERE COALESCE(is_deleted, false) = false
            ...
            // 5. Read stock — trigger already synced from location write above
            const newStock = await this.readCurrentStock(productId, businessId, client);
            ...
            if (shouldManageTransaction) invalidateStorefrontCatalog(businessId);
    ...
    async removeStock(params, userId, txClient = null) {
        ...
        try {
            ...
            const {
                ...
                fifo_sellable_locations: fifoSellableLocations = false,
                skip_accounting: skipAccounting = false,
                skip_low_stock_notify: skipLowStockNotify = false,
            } = params;
            ...
            const pRes = await client.query(
                `SELECT stock, cost_price, unit, unit_conversions, name, reorder_point, min_stock, min_stock_level
                 FROM products
            ...
            let fifoLocationSnapshot = null;

            // Availability Check
            if (warehouseId) {
                const locRes = await client.query(
                    'SELECT quantity FROM product_stock_locations WHERE warehouse_id = $1 AND product_id = $2 AND business_id = $3 AND COALESCE(state, \'sellable\') = $4 FOR UPDATE',
                    [warehouseId, productId, businessId, state]
                );
                if (Number(locRes.rows[0]?.quantity || 0) < finalQuantity) throw new Error('Insufficient warehouse stock');
            } else if (fifoSellableLocations) {
                fifoLocationSnapshot = await this.resolveSellableLocationSnapshot(client, businessId, productId, state);
                // When warehouse rows exist, sellable = location sum only (not max with stale headline).
                const available = fifoLocationSnapshot.hasRows
                    ? fifoLocationSnapshot.qty
                    : Number(product.stock || 0);
                if (available < finalQuantity) throw new Error('Insufficient stock');
            ...
            if (warehouseId) {
                ...
                const locSnapshot = fifoLocationSnapshot
                    || await this.resolveSellableLocationSnapshot(client, businessId, productId, state);
                if (locSnapshot.hasRows) {
                    const remaining = await this.decrementFifoSellableLocations(
                        client, businessId, productId, finalQuantity, state
                    );
                    if (remaining > 0) {
                        throw new Error(
                            `Insufficient warehouse stock. Short by ${remaining} after FIFO allocation.`
                        );
                    }
                } else {
                    await client.query(
            ...
            if (serialNumbers.length > 0) {
                ...
                // Validate all serials are available first (fail fast)
                ...
                // Split: bulk update for non-warranty serials, per-row for warranty serials
                const noWarrantyIds = [];
                const warrantySerials = [];
                for (const serial of serialRes.rows) {
                    if (serial.warranty_period_months) {
                        warrantySerials.push(serial);
                    } else {
                        noWarrantyIds.push(serial.id);
                    }
                }

                // Batch UPDATE for non-warranty serials (O(1) instead of O(n))
                if (noWarrantyIds.length > 0) {
                    await client.query(
                        `UPDATE product_serials
                         SET status = 'sold',
                             ...
                             invoice_id = CASE WHEN $3 = 'invoice' THEN $4 ELSE invoice_id END,
                             updated_at = NOW()
                         WHERE id = ANY($5::uuid[]) AND business_id = $6`,
                        [invoiceDate, customerId, referenceType, referenceId, noWarrantyIds, businessId]
                    );
                }

                // Per-row UPDATE for warranty serials (needs individual date calc)
                for (const serial of warrantySerials) {
                    ...
                    await client.query(
                        ...
                        [invoiceDate, endDate, customerId, referenceType, referenceId, serial.id, businessId]
                    );
                }
            }

            // Accounting (COGS)
            if (!skipAccounting && costOfGoodsSold > 0) {
            ...
            // 5. Read stock — trigger already synced from location writes above.
            // For FIFO paths the trigger fired on each location UPDATE; for non-FIFO
            // the warehouse UPDATE triggered it. Just read the result.
            const newStock = await this.readCurrentStock(productId, businessId, client);

            if (referenceType === 'sale' || referenceType === 'storefront_order') {
                const qtyInt = Math.min(2147483647, Math.floor(finalQuantity));
                await client.query(
                    `UPDATE products SET sales_count = COALESCE(sales_count, 0) + $1
                     WHERE id = $2 AND business_id = $3`,
                    [qtyInt, productId, businessId]
                );
            }

            // Check for low stock and create notification if needed
            const minStock = Number(product.reorder_point || product.min_stock_level || product.min_stock || 10);
            if (!skipLowStockNotify && newStock <= minStock) {
                try {
    ...
    /**
     * Remove variant-level stock (clothing/footwear storefront + hub parity).
     */
    async removeVariantStock(params, userId, txClient = null) {
        const client = await this.getClient(txClient);
        const shouldManageTransaction = !txClient;

        try {
            removeVariantStockSchema.parse(params);
            ...
            const {
                ...
                quantity: qty,
                reference_type: referenceType = 'storefront_order',
                reference_id: referenceId = null,
                notes = null,
                domain_data: domainData = {},
            } = params;

            const finalQuantity = Number(qty);
            const vRes = await client.query(
            ...
            if (vRes.rows.length === 0) throw new Error('Variant not found');
            const variant = vRes.rows[0];
            if (Number(variant.stock || 0) < finalQuantity) throw new Error('Insufficient stock');

            await client.query(
                `UPDATE product_variants
                 SET stock = stock - $1, updated_at = NOW()
                 WHERE id = $2 AND business_id = $3`,
                [finalQuantity, variantId, businessId]
            );

            const newStock = await VariantService.syncParentStockFromVariants(productId, businessId, client);
            ...
            const moveRes = await client.query(
                `INSERT INTO stock_movements (
                    business_id, product_id, warehouse_id, variant_id,
                    movement_type, transaction_type, quantity_change, unit_cost,
                    reference_type, reference_id, notes, domain_data
                )
                VALUES ($1, $2, $3, $4, 'out', $5, $6, $7, $5, $8, $9, $10)
                RETURNING id`,
                [
                    businessId,
                    productId,
            ...
            return { success: true, movementId: moveRes.rows[0].id, newStock };
        } catch (error) {
            if (shouldManageTransaction) await client.query('ROLLBACK');
            throw error;
        } finally {
    ...
    async transferStock(params, userId, txClient = null) {
        ...
        try {
            ...
            const transferRow = trfRes.rows[0];
            const transferId = transferRow?.id || null;
            const transferNotes = notes || `Transfer ${transferNumber}`;

            await client.query(
                `INSERT INTO stock_movements (
                    business_id, product_id, warehouse_id, batch_id,
                    movement_type, transaction_type, quantity_change, notes,
                    reference_type, reference_id
                )
                VALUES ($1, $2, $3, $4, 'out', 'transfer', $5, $6, 'stock_transfer', $7)`,
                [businessId, productId, fromWarehouseId, batchId || null, -quantity, transferNotes, transferId]
            );
            ...
            await client.query(
                ...
                VALUES ($1, $2, $3, $4, 'in', 'transfer', $5, $6, 'stock_transfer', $7)`,
                [businessId, productId, toWarehouseId, batchId || null, quantity, transferNotes, transferId]
            );

            // Stock sync handled by DB trigger on location writes above

            if (shouldManageTransaction) await client.query('COMMIT');
            if (shouldManageTransaction) invalidateStorefrontCatalog(businessId);
            return { success: true, transfer: transferRow };
        } catch (error) {
            if (shouldManageTransaction) await client.query('ROLLBACK');
            throw error;
        } finally {
    ...
    async getStockValuation(businessId, warehouseId = null, method = 'standard') {
        ...
        try {
            ...
            const orderDir = valuationMethod === 'lifo' ? 'DESC' : 'ASC';
            const purchasesRes = await client.query(
                `SELECT product_id, quantity_change, unit_cost
                 ...
                 ORDER BY product_id, created_at ${orderDir}`,
                [businessId, productsRes.rows.map((p) => p.id)]
            );

            /** @type {Map<string, Array<{ quantity_change: unknown, unit_cost: unknown }>>} */
            const purchasesByProduct = new Map();
            for (const row of purchasesRes.rows) {
                const pid = row.product_id;
                if (!purchasesByProduct.has(pid)) purchasesByProduct.set(pid, []);
                purchasesByProduct.get(pid).push(row);
            }
        ...
        try {
            ...
            if (batchId) {
                ...
                await client.query(
                    ...
                    [requestedQty, batchId, businessId]
                );
            }
    ...
    async releaseStock(params, txClient = null) {
        ...
        try {
            ...
            if (reservationId) {
                ...
                await client.query(
                    `UPDATE inventory_reservations SET status = 'completed', updated_at = NOW() WHERE id = $1 AND business_id = $2`,
            ...
            if (effectiveBatchId) {
                await client.query(
                    ...
                    [releaseQty, effectiveBatchId, businessId]
                );
            }
        ...
        try {
            ...
            if (reservation.batch_id) {
                await client.query(`
                    ...
                    SET reserved_quantity = GREATEST(0, COALESCE(reserved_quantity, 0) - $1)
                    WHERE id = $2 AND business_id = $3
                `, [reservation.quantity, reservation.batch_id, businessId]);
            }
            ...
            const [locations, batches, serials] = await Promise.all([
                ...
                client.query(
                    `SELECT * FROM product_serials
                     WHERE product_id = $1 AND business_id = $2
                       AND COALESCE(is_deleted, false) = false
                       AND LOWER(COALESCE(status, '')) IN ('available', 'in_stock')
                     LIMIT 100`,
                    [productId, businessId]
                )
            ]);
