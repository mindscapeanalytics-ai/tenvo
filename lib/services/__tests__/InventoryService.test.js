...
describe('InventoryService regressions', () => {
  ...
  it('removeStock with fifo_sellable_locations decrements locations and skips accounting when requested', async () => {
    ...
    txClient.query.mockImplementation((query) => {
      ...
      if (query.includes('COALESCE(SUM(quantity), 0)::float AS location_qty')) {
        return Promise.resolve({ rows: [{ location_qty: 12 }] });
      }
      ...
      if (query.includes('SELECT stock FROM products WHERE id = $1')) {
        return Promise.resolve({ rows: [{ stock: 9 }] });
      }
