...
export const ProductService = {
    ...
    async getProducts(businessId, options = {}, tx = db) {
        // detailLevel: list (shell/Overview) | grid (Inventory tab) | full (edit/export + serials).
        // includeSerials: true forces full-depth serials (export/edit). Saves use `_serialsDeferred`
        // so empty client arrays cannot wipe serial rows.
        const {
            limit,
            offset,
            search,
            ...
            detailLevel: detailLevelOpt,
            unbounded = false,
        } = options;
        const detailLevel = includeSerials
            ? 'full'
            : (detailLevelOpt === 'grid' || detailLevelOpt === 'full' || detailLevelOpt === 'list'
                ? detailLevelOpt
                : 'list');
        // Safety: omit limit+offset used to load entire catalogs (hub freeze). Opt into
        // unbounded: true for intentional full reads (rare admin/export helpers).
        const DEFAULT_PAGE = 500;
        const effectiveLimit = unbounded
            ? undefined
            : (limit !== undefined ? limit : DEFAULT_PAGE);
        const effectiveOffset = unbounded
            ? undefined
            : (offset !== undefined ? offset : 0);
        const usePagination = effectiveLimit !== undefined && effectiveOffset !== undefined;
        const wantsBatches = detailLevel === 'grid' || detailLevel === 'full';
        const wantsFullVariants = detailLevel === 'grid' || detailLevel === 'full';
        const loadSerials = detailLevel === 'full' || includeSerials === true;
        ...
        // list: locations + minimal variant stocks (accurate display_stock without fat joins).
        // grid/full: batches + full variant fields; full adds serials.
        const include = {
        ...
        const [products, total] = await Promise.all([
            tx.products.findMany({
                ...
                include,
                orderBy: { name: 'asc' },
                ...(usePagination ? { skip: parseInt(effectiveOffset, 10), take: parseInt(effectiveLimit, 10) } : {})
            }),
        ...
        const mappedProducts = products.map(p => {
            const mapped = {
                ...p,
                batches: wantsBatches ? mergeProductBatchesFromRow(p) : [],
                serial_numbers: loadSerials ? (p.product_serials || []) : [],
                stock_locations: p.product_stock_locations,
                variants: p.product_variants,
                _detailLevel: detailLevel,
                // Inventory saves must not treat empty serials/batches as "clear all" when deferred.
                ...(loadSerials ? {} : { _serialsDeferred: true }),
                ...(wantsBatches ? {} : { _batchesDeferred: true }),
                ...(wantsFullVariants ? {} : { _variantsDeferred: true }),
            };
        ...
        const pageOffset = usePagination ? parseInt(effectiveOffset, 10) : 0;
        const pageLimit = usePagination ? parseInt(effectiveLimit, 10) : 0;

        return { 
            products: mappedProducts, 
            total: actualTotal, 
            hasMore: usePagination ? (pageOffset + pageLimit < actualTotal) : false 
        };
    },
    ...
    async updateProduct(id, businessId, updates, tx = db) {
        ...
        if (data.images !== undefined || data.image_url !== undefined) {
            ...
            data.images = pack.images;
            data.image_url = pack.image_url;
        }
