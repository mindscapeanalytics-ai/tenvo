...
export const ProductService = {
    ...
    async getProducts(businessId, options = {}, tx = db) {
        // Default omit serials on list loads (payload/join cost). Saves use `_serialsDeferred`
        // so empty client arrays cannot wipe serial rows. Pass includeSerials: true for export/edit.
        const { limit, offset, search, category, isActive, includeSerials = false } = options;
        ...
        // Serial rows are not used for display stock; omit on list loads to cut payload/join cost.
        // `_serialsDeferred` below protects save paths from treating the empty array as "clear all".
        const include = {
            product_batches: {
