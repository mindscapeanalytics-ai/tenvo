...
import {
    assertAnalystSqlSafe,
    capAnalystSqlRows,
    executeAnalystReadQuery,
    assertValidBusinessUuid,
} from '@/lib/services/ai/analystSqlGuard';
...
async function queryGemini(prompt, isJson = false, jsonSchema = null) {
    ...
    if (!apiKey) {
        if (hasOpenAI) {
            ...
            return await runOpenAIFallback(prompt, isJson);
        }
 ...
 */
async function runOpenAIFallback(prompt, isJson = false) {
    if (isJson) {
...
export const BusinessAnalyst = {
     ...
     * @param {{ businessName?: string | null; category?: string | null } | null} [context] - Registered business (prompt context)
     */
    async ask(businessId, question, context = null) {
        try {
            ...
            const businessLabel =
                context && (context.businessName || context.category)
                    ? `Registered business: "${context.businessName || 'Unknown'}". Industry / hub category: ${context.category || 'general'}. Frame the analysis for this tenant only. Never query or infer data for any other business_id.\n`
                    : '';
            ...
            SCHEMA CONTEXT:
            - products (id, name, sku, stock, price, cost_price, category, business_id)
            - invoices (id, total_amount, status, date, business_id)
            - invoice_items (invoice_id, product_id, quantity, unit_price)
            - inventory_ledger (product_id, quantity_change, transaction_type, created_at, business_id)
            - customers (id, name, city, business_id)
            - vendors (id, name, business_id)
            - gl_accounts (id, code, name, type, sub_type, business_id)
            - journal_entries (id, journal_number, transaction_date, description, status, business_id)
            - gl_entries (journal_id, account_id, debit, credit)
            
            RULES:
            - ALWAYS filter every tenant table by business_id = '${businessId}' (exact UUID).
            - Use joins where necessary.
            - Use aggregation (SUM, AVG, COUNT) for reporting.
            - Keep the query efficient; add LIMIT 500 when returning many rows.
            - Do NOT use DELETE, UPDATE, INSERT, or DDL.
            `;
            ...
            // 2. Execute SQL (read-only, tenant-scoped)
            const generatedSql = capAnalystSqlRows(String(sqlQuery.sql).trim());
            assertAnalystSqlSafe(generatedSql, businessId);

            const client = await pool.connect();
            let data;
            try {
                data = await executeAnalystReadQuery(client, generatedSql);
            } finally {
                client.release();
            }
    ...
    async semanticSearch(businessId, query, limit = 5) {
        try {
            ...
            const { rows: results } = await pool.query(`
                ...
                LIMIT $3
            `, [JSON.stringify(embedding), businessId, safeLimit]);
