...
/**
 * Enrich an invoice header for hub list/upsert (customer name, balance, slim items).
 * @param {import('pg').PoolClient} client
 * @param {string} businessId
 * @param {object} invoice
 * @param {object[]} [items]
 */
async function hydrateInvoiceForList(client, businessId, invoice, items = []) {
    if (!invoice?.id) return invoice;

    let customerName = null;
    if (invoice.customer_id) {
        const cRes = await client.query(
            `SELECT name FROM customers WHERE id = $1 AND business_id = $2 LIMIT 1`,
            [invoice.customer_id, businessId]
        );
        customerName = cRes.rows[0]?.name || null;
    }

    let balance = Number(invoice.grand_total) || 0;
    try {
        const balRes = await client.query(
            `
            SELECT GREATEST(
                COALESCE($2::numeric, 0) - COALESCE(SUM(ip.amount), 0),
                0
            ) AS balance
            FROM invoice_payments ip
            WHERE ip.invoice_id = $1::uuid
              AND ip.business_id = $3::uuid
              AND COALESCE(ip.is_deleted, false) = false
            `,
            [invoice.id, Number(invoice.grand_total) || 0, businessId]
        );
        // When there are no payment rows, SUM is null → COALESCE yields grand_total via GREATEST path above.
        // With zero rows the SELECT still returns one row (aggregate without GROUP BY).
        balance = Number(balRes.rows[0]?.balance ?? balance);
    } catch {
        // Soft-delete column or payments table may differ in older envs
        try {
            const fallback = await client.query(
                `SELECT COALESCE(calculate_invoice_balance($1), $2, 0) AS balance`,
                [invoice.id, Number(invoice.grand_total) || 0]
            );
            balance = Number(fallback.rows[0]?.balance ?? balance);
        } catch {
            /* keep grand_total */
        }
    }

    const taxHeader = Number(invoice.tax_total ?? invoice.total_tax ?? 0);
    const paymentStatus = String(invoice.payment_status || '').toLowerCase();
    const isPaid = paymentStatus === 'paid' || (Number(invoice.grand_total) > 0 && balance <= 0.009);
    ...
    return {
        ...
        tax_total: taxHeader,
        total_tax: taxHeader,
        payment_status: isPaid ? 'paid' : (invoice.payment_status || 'unpaid'),
...
export const InvoiceService = {
    ...
    async prevalidateStock(businessId, items, txClient = null) {
        ...
        try {
            ...
            const serviceItems = (items || []).filter((item) => !item.product_id);
            ...
            if (productItems.length > 0) {
                // Aggregate requested qty per product (one batched stock query).
                const requestedByProduct = new Map();
                for (const item of productItems) {
                    const id = String(item.product_id);
                    const qty = Number(item.quantity) || 0;
                    const prev = requestedByProduct.get(id) || { qty: 0, name: item.name };
                    prev.qty += qty;
                    if (!prev.name && item.name) prev.name = item.name;
                    requestedByProduct.set(id, prev);
                }
                ...
                const stockById = new Map(
                    (stockRes.rows || []).map((row) => [String(row.id), row])
                );

                for (const [productId, req] of requestedByProduct.entries()) {
                    const stockData = stockById.get(productId);
                    const totalStock = Number(stockData?.total_stock || 0);
                    const reserved = Number(stockData?.reserved || 0);
                    const available = totalStock - reserved;
                    const shortfall = Math.max(0, req.qty - available);
                    const isValid = available >= req.qty;

                    if (!isValid) {
                        insufficientStockItems.push({
                            name: stockData?.product_name || req.name || 'Unknown Product',
                            sku: stockData?.sku,
                            requested: req.qty,
                            available,
                            shortfall
                    ...
                    validationResults.push({
                        productId,
                        name: stockData?.product_name || req.name,
                        sku: stockData?.sku,
                        requested: req.qty,
                        available,
                        reserved,
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (!Number.isFinite(taxAmount) || taxAmount < 0) {
                    taxAmount = Math.round(((lineBase - discount) * taxPercent) / 100 * 100) / 100;
                }
            ...
            const headerTaxRaw = header.tax_total ?? header.total_tax;
            const headerTaxNum = Number(headerTaxRaw);
            const finalSubtotal = Number(header.subtotal) || calculatedSubtotal;
            // Prefer explicit header tax (including 0 when tax is disabled); else sum derived line tax.
            const finalTax = (headerTaxRaw !== undefined && headerTaxRaw !== null && headerTaxRaw !== '' && Number.isFinite(headerTaxNum))
                ? headerTaxNum
                : (calculatedTax || 0);
            ...
            // Skip check if payment_status is 'paid' (Cash Sales) or explicit skip (route hisab collection bills)
            if (header.payment_status !== 'paid' && !data.skip_credit_check) {
            ...
            const res = await client.query(`
                INSERT INTO invoices (
                    ...
                    subtotal, tax_total, total_tax, discount_total, grand_total, payment_method, 
            ...
            `, [
                ...
                finalSubtotal,
                finalTax, finalTax, finalDiscount, header.grand_total || 0,
            ...
            if (items?.length > 0) {
                for (const item of items) {
                    ...
                    `, [
                        ...
                        item.quantity, item.unit_price, item.tax_percent || 0,
                        Number.isFinite(Number(item.tax_amount)) ? Number(item.tax_amount) : 0,
                    ...
                    if (item.product_id && !data.skip_inventory) {
                        await InventoryService.removeStock({
                            ...
                            // Sale GL posts once at invoice header; skip per-line notify noise in the TX.
                            skip_low_stock_notify: true,
                        }, userId, client);
                    }
                }
            }
            ...
            // Re-read header after paid-at-create may have updated payment_status/status
            ...
            try {
                ...
                if (fresh.rows[0]) listInvoice = fresh.rows[0];
            } catch {
                /* keep original */
            }

            return hydrateInvoiceForList(client, businessId, listInvoice, items);
        } catch (error) {
    ...
    async updateInvoice(invoiceId, data, userId, txClient = null) {
        ...
        try {
            ...
            const updateTax = Number(header.total_tax ?? header.tax_total) || 0;
            const invQuery = `
                UPDATE invoices SET
                    customer_id = $1, date = $2, due_date = $3, status = $4,
                    subtotal = $5, tax_total = $6, total_tax = $7, discount_total = $8, grand_total = $9,
                    payment_method = $10, payment_status = $11, notes = $12, terms = $13,
                    tax_details = $14, updated_at = NOW()
                WHERE id = $15 AND business_id = $16
                RETURNING *
            `;
            const invValues = [
                header.customer_id, header.date, header.due_date, header.status,
                header.subtotal, updateTax, updateTax, header.discount_total,
            ...
            return hydrateInvoiceForList(client, businessId, invoice, items);
        } catch (error) {
            if (shouldManageTransaction) await client.query('ROLLBACK');
            throw error;
        } finally {
    ...
    async convertFromPOSTransaction(transactionId, context, txClient = null) {
        ...
        try {
            ...
            // 3. Verify transaction is a completed, non-voided sale
            if (transaction.is_voided) {
                throw new Error('Voided POS transactions cannot be converted to invoices');
            }
            const posPaymentStatus = String(transaction.payment_status || '').toLowerCase();
            if (posPaymentStatus && !['completed', 'paid', 'success'].includes(posPaymentStatus)) {
                throw new Error('Only completed POS transactions can be converted to invoices');
            }

            // 4. Get transaction items (join products for name — POS items have no product_name column)
            const itemsRes = await client.query(`
                SELECT
                    pti.*,
                    p.name AS product_name
                FROM pos_transaction_items pti
                LEFT JOIN products p ON p.id = pti.product_id AND p.business_id = pti.business_id
                WHERE pti.transaction_id = $1 AND pti.business_id = $2
            `, [transactionId, businessId]);

            const paymentRes = await client.query(`
                SELECT method FROM pos_payments
                WHERE transaction_id = $1 AND business_id = $2
                ORDER BY created_at ASC
                LIMIT 1
            `, [transactionId, businessId]);
            const paymentMethod = paymentRes.rows[0]?.method || 'cash';

            // 5. Create invoice from POS transaction (map Prisma POS columns → invoice fields)
            const invoiceData = {
                business_id: businessId,
                ...
                tax_total: transaction.tax_amount ?? 0,
                ...
                items: itemsRes.rows.map(item => {
                    ...
                    const taxPercent = lineBase > 0
                        ? Math.round((taxAmount / lineBase) * 10000) / 100
                        : 0;
                    return {
                        product_id: item.product_id,
                        name: item.product_name || 'POS Item',
                        description: null,
