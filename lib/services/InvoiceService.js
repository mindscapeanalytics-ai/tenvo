    ...
    async prevalidateStock(businessId, items, txClient = null) {
        ...
        try {
            ...
            for (const item of items) {
                ...
                // Get current stock with reservation consideration
                // Uses COALESCE to fallback to products.stock when inventory_stock table is empty
                    ...
                    SELECT 
                        ...
                        COALESCE(
                            (SELECT SUM(quantity) 
                             ...
                             WHERE product_id = $1 
                               AND status = 'active'
                               AND (inventory_reservations.is_deleted = false OR inventory_reservations.is_deleted IS NULL)),
                            0
                        ) as reserved
                    FROM products p
            ...
            const query = `
                SELECT i.*, 
                       ...
                       (SELECT COALESCE(SUM(amount), 0) FROM invoice_payments 
                        WHERE invoice_id = i.id AND (invoice_payments.is_deleted = false OR invoice_payments.is_deleted IS NULL)) as total_paid,
    ...
    async updateInvoice(invoiceId, data, userId, txClient = null) {
        ...
        try {
            ...
            const invValues = [
                ...
                JSON.stringify(header.domain_data || {}),
