...
export async function reserveStock(
  ...
  // Calculate available stock (total - already reserved, non-expired only)
  const now = new Date();
...
export async function completeReservation(
  ...
  const existing = await db.inventory_reservations.findFirst({
    where: { id: reservationId, business_id: businessId },
  });
  if (!existing) {
    throw new Error('Reservation not found for this business');
  }
...
export async function getExpiryReport(businessId, daysThreshold = 7) {
  ...
  return {
    ...
    reservations: expiring.map((res) => ({
      id: res.id,
      sku: res.products?.sku ?? null,
      name: res.products?.name ?? null,
      quantity: res.quantity,
      customer: res.customers?.name ?? null,
