    ...
    async createPayment(data, userId, txClient = null) {
        ...
        try {
            ...
            if (pData.reference_id) {
                const table = pData.reference_type === 'invoice' ? 'invoices' : 'purchases';
                const totalRes = await client.query(`SELECT total_amount, grand_total FROM ${table} WHERE id = $1`, [pData.reference_id]);
                ...
                await client.query(`UPDATE ${table} SET status = $1, updated_at = NOW() WHERE id = $2`, [status, pData.reference_id]);
            }
    ...
    async deletePayment(businessId, paymentId, userId, txClient = null) {
        ...
        try {
            ...
            if (payment.payment_type === 'receipt' && payment.customer_id) {
                await client.query(`UPDATE customers SET outstanding_balance = outstanding_balance + $1 WHERE id = $2`, [payment.amount, payment.customer_id]);
            } else if (payment.payment_type === 'payment' && payment.vendor_id) {
                await client.query(`UPDATE vendors SET outstanding_balance = outstanding_balance + $1 WHERE id = $2`, [payment.amount, payment.vendor_id]);
            }

            // 2. Reverse GL
            await client.query(`DELETE FROM gl_entries WHERE reference_type = 'payment' AND reference_id = $1`, [paymentId]);
