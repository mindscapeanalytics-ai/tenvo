...
export const PaymentService = {
    ...
    /**
     * Soft-delete invoice_payments rows mirrored from a legacy `payments` row.
     * Matches both legacy-payment:{id} and legacy-payment:{id}:{allocationId} refs.
     */
    async softDeleteMirroredInvoicePayments(client, businessId, paymentId, userId = null) {
        const legacyRef = `legacy-payment:${paymentId}`;
        await client.query(
            `
            UPDATE invoice_payments
            SET is_deleted = true,
                deleted_at = NOW(),
                deleted_by = COALESCE($3, deleted_by)
            WHERE business_id = $1
              AND (is_deleted = false OR is_deleted IS NULL)
              AND (
                reference_number = $2
                OR reference_number LIKE ($2 || ':%')
              )
            `,
            [businessId, legacyRef, userId]
        );
    },
    ...
    async createPayment(data, userId, txClient = null) {
        ...
        try {
            ...
            const paymentType = normalizePaymentType(pData.payment_type);
            if (!paymentType) {
                throw new Error('Invalid payment type');
            }
            pData.payment_type = paymentType;
    ...
    async deletePayment(businessId, paymentId, userId, txClient = null) {
        ...
        try {
            ...
            if (payment.payment_type === 'receipt' && payment.customer_id) {
                await client.query(`UPDATE customers SET outstanding_balance = COALESCE(outstanding_balance, 0) + $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`, [payment.amount, payment.customer_id, businessId]);
            } else if (payment.payment_type === 'payment' && payment.vendor_id) {
                await client.query(`UPDATE vendors SET outstanding_balance = COALESCE(outstanding_balance, 0) + $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`, [payment.amount, payment.vendor_id, businessId]);
            }
    ...
    async voidPayment(paymentId, reason, context, txClient = null) {
        ...
        try {
            ...
            if (payment.payment_type === 'receipt' && payment.customer_id) {
                await client.query(`
                    UPDATE customers
                    SET outstanding_balance = COALESCE(outstanding_balance, 0) + $1, updated_at = NOW()
                    WHERE id = $2 AND business_id = $3
    ...
    async getOutstandingBalance(entityType, entityId, businessId) {
        ...
        try {
            ...
            if (entityType === 'customer') {
                ...
                // A/R receipts truth is invoice_payments (same as InvoicePaymentService /
                // calculate_invoice_balance). Legacy payment_allocations alone undercounts
                // Modal receipts that never touched the payments table.
                const paidRes = await client.query(`
                    ...
                    WHERE i.customer_id = $1
                      ...
                      AND (ip.business_id = $2 OR ip.business_id IS NULL)
                `, [entityId, businessId]);
                ...
                const purchasedRes = await client.query(`
                    ...
                    WHERE vendor_id = $1 AND business_id = $2
                      AND status != 'voided'
                      AND (purchases.is_deleted = false OR purchases.is_deleted IS NULL)
                `, [entityId, businessId]);
    ...
    async allocatePayment(paymentId, allocations, context, txClient = null) {
        ...
        try {
            ...
            for (const alloc of allocations) {
                ...
                if (alloc.invoice_id) {
                    await InvoicePaymentService.syncFromLegacyPaymentAllocation(
                        {
                            businessId,
     ...
     * Customer A/R uses invoice_payments; vendor A/P uses payment_allocations on purchases.
     *
     * @param {string} businessId - Business ID
     * @param {Object} txClient - Optional transaction client
    ...
    async reconcileOutstandingBalances(businessId, txClient = null) {
        ...
        try {
            ...
            const customersRes = await client.query(`
                ...
                WHERE business_id = $1 AND (customers.is_deleted = false OR customers.is_deleted IS NULL)
            `, [businessId]);
            ...
            const vendorsRes = await client.query(`
                ...
                WHERE business_id = $1 AND (vendors.is_deleted = false OR vendors.is_deleted IS NULL)
            `, [businessId]);
