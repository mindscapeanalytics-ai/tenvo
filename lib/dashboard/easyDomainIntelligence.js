...
/** High + critical perishability (milk-shop uses critical). */
function isElevatedPerishability(perishability) {
  const p = String(perishability || '').toLowerCase();
...
const VERTICAL_PLAYBOOKS = {
  ...
  'milk-shop': {
    stockFocus:
      'Fresh milk and chill packs turn daily. Watch near expiry and morning supply gaps.',
    salesFocus:
      'Counter kg sales plus route deliveries. Peak winter and summer heat drive demand.',
    accountsFocus:
      'Weekly and monthly house credit. Collect with JazzCash, cash, or invoice receipt.',
    actionTab: 'route-hisab',
  },
  'water-delivery': {
    stockFocus:
      '19L refill, empty float, and sealed packs. Peak summer heat and Ramadan home demand — raise plant supply and deposits.',
    salesFocus:
      'Rider routes by city, area, and town code. Home & flat daily plus corporate weekday cadence across Karachi, Lahore, Hyderabad.',
    accountsFocus:
      'Weekly and monthly house/office credit plus daily cash recovery. Collect with JazzCash, Easypaisa, cash, or bank transfer.',
    actionTab: 'route-hisab',
  },
};
...
export function getDomainTabGuidance(profile, tab) {
  ...
  const map = {
    ...
    operations:
      profile.category === 'milk-shop'
        ? 'Counter POS for walk-ins; Daily Route for house credit. Chill FEFO stays on Stock.'
        : profile.category === 'water-delivery'
          ? 'Counter POS for walk-in packs; Daily Route for DEL/REC/BAL and cash recovery. Deposits stay on Stock.'
          : profile.membershipVertical
            ? 'Member enrollments, renewals, and storefront orders. Open the Memberships tab for expiring plans.'
...
export function buildVerticalInsightCards(profile, metrics = {}) {
  ...
  if (profile.category === 'milk-shop') {
    cards.push({
      title: 'Daily Route today',
      text: 'Log house deliveries on the daily sheet, then generate weekly or monthly bills for credit collect.',
      tone: 'sky',
      actionTab: 'route-hisab',
    });
    cards.push({
      title: 'Counter vs route',
      text: 'Walk-in kg sales stay on POS. Doorstep houses stay on Daily Route so credit and chill stock stay clear.',
      tone: 'cyan',
      actionTab: 'pos',
    });
  }

  if (profile.category === 'water-delivery') {
    cards.push({
      title: 'Daily Route today',
      text: 'Log DEL and empty REC by town code, take cash recovery, then bill weekly or monthly credit accounts.',
      tone: 'sky',
      actionTab: 'route-hisab',
    });
    cards.push({
      title: 'Bottles & deposits',
      text: 'BAL = previous + DEL − REC. Keep 19L empty float and refundable deposits separate from refill sale.',
      tone: 'cyan',
      actionTab: 'inventory',
    });
    cards.push({
      title: 'Counter vs rider',
      text: 'Walk-in PET and dispensers stay on POS. Home & flat plus corporate routes stay on Daily Route.',
      tone: 'emerald',
      actionTab: 'pos',
    });
  }
  ...
  if (isPeak) {
    cards.push({
      ...
      text: `${currentMonth} is a peak month for ${profile.name}. Add ${isElevatedPerishability(profile.perishability) ? '25%' : '15–20%'} safety stock on top movers and staff replenishment.`,
      tone: 'amber',
      actionTab: 'inventory',
    });
  ...
  if (isElevatedPerishability(profile.perishability) && coverageDays > 0 && coverageDays < 14) {
    cards.push({
...
export function getDomainKpiLabels(profile) {
  if (profile.category === 'milk-shop') {
    return {
      unitsSold: 'Kg & packs sold',
      inventoryLabel: 'Chill stock value',
      stockTabTitle: 'Fresh & chill stock',
      ordersLabel: 'Counter + online',
    };
  }

  if (profile.category === 'water-delivery') {
    return {
      unitsSold: 'Bottles & cases sold',
      inventoryLabel: 'Bottle & pack stock',
      stockTabTitle: 'Refill, PET & deposits',
      ordersLabel: 'Route + counter + online',
    };
  }
  ...
  const ordersLabel = profile.serviceMode ? 'Jobs / orders' : 'All Orders';
...
export function buildDomainStockSignals(profile, ctx = {}) {
  ...
  if (isElevatedPerishability(profile.perishability)) {
    rows.push({
      label: 'Cover est.',
