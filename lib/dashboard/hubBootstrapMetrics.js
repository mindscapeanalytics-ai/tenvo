...
export function buildDashboardMetricsFromSnapshot(snapshot) {
    ...
    return {
        revenue: {
            ...
            storefront: Number(revenue.storefront ?? channels.storefront ?? 0),
            pos: Number(revenue.pos ?? channels.pos ?? 0),
            invoices: Number(revenue.invoices ?? channels.invoice ?? 0),
        },
        channels: {
            storefront: Number(channels.storefront ?? revenue.storefront ?? 0),
            pos: Number(channels.pos ?? revenue.pos ?? 0),
            invoice: Number(channels.invoice ?? revenue.invoices ?? 0),
        },
        orders: {
