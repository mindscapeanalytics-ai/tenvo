...
const CATEGORY_MODE = {
  ...
  'milk-shop': 'online_store',
  'water-delivery': 'online_store',
};
...
const MODE_GUIDANCE = {
  ...
  milk_ops:
    'Counter sales, route house credit, and chill stock. Open Route Hisab for the daily sheet and week or month bills.',
  water_ops:
    'Rider sheet by city, area, and town code; 19L DEL/REC/BAL deposits; corporate vs home credit. Open Daily Route for delivery days and week or month bills.',
  parts_desk:
 ...
 */
function buildRegionalComplianceHint(countryIso, mode, category) {
  ...
  if (iso === 'PK' && key === 'water-delivery') {
    return 'Pakistan water supply: keep PSQCA plant license current; track 19L deposits separately from GST sales; JazzCash/Easypaisa for monthly recoveries.';
  }
  if (iso === 'PK' && key === 'milk-shop') {
    return 'Pakistan milk shop: chill FEFO on fresh lines; JazzCash/Easypaisa for house credit collect.';
  }
...
export function resolveOperationsProfile(category, domainKnowledge, business) {
  ...
  return {
    ...
    regionalHint: buildRegionalComplianceHint(countryIso, mode, key),
    ...
    showWaterRoute: key === 'water-delivery',
    showMilkRoute: key === 'milk-shop',
    showStorefront:
      retailMode ||
      mode === 'online_store' ||
      mode === 'pharmacy_desk' ||
...
export function buildOperationsKpiTiles(profile, snapshot, formatters) {
  ...
  if (profile.showWaterRoute) {
    tiles.push({
      id: 'water_daily_route',
      label: 'Daily Route',
      value: 'Open sheet',
      hint: 'DEL / REC / BAL + cash recovery',
      actionTab: 'route-hisab',
      tone: 'text-sky-700',
    });
    tiles.push({
      id: 'route_accounts',
      label: profile.customerLabel || 'House / Account',
      value: Number(snapshot.activeBuyers || 0),
      hint: 'Active house & corporate accounts',
      actionTab: 'customers',
    });
    tiles.push({
      id: 'cash_recovery',
      ...
      hint: 'Cash + credit recoveries this period',
      actionTab: 'route-hisab',
    });
    // Rider shift reconciliation tile
    const waterOps = /** @type {Record<string, unknown>} */ (snapshot.waterOps || {});
    const riderShortage = Number(waterOps.riderCashShortage || 0);
    if (riderShortage > 0 || Number(waterOps.activeRiders || 0) > 0) {
      tiles.push({
        id: 'rider_shift_status',
        label: 'Rider Shifts',
        value: riderShortage > 0 ? formatCurrency(riderShortage) + ' short' : (Number(waterOps.activeRiders || 0) + ' riders'),
        hint: riderShortage > 0 ? 'Cash shortage detected — reconcile shifts' : 'All riders reconciled',
        actionTab: 'route-hisab',
        tone: riderShortage > 0 ? 'text-rose-700' : 'text-emerald-700',
      });
    }
    // Bottle float tile
    const bottleFloat = Number(waterOps.bottlesWithCustomers || 0);
    const idleBottles = Number(waterOps.idleBottleCustomers || 0);
    if (bottleFloat > 0 || idleBottles > 0) {
      tiles.push({
        id: 'bottle_float',
        label: 'Bottle Float',
        value: bottleFloat,
        hint: idleBottles > 0 ? `${idleBottles} idle risk (14+ days)` : 'Deposits with customers',
        actionTab: 'route-hisab',
        tone: idleBottles > 0 ? 'text-amber-700' : undefined,
      });
    }
  }

  if (profile.showMilkRoute) {
    tiles.push({
      id: 'milk_daily_route',
      label: 'Daily Route',
      value: 'Open sheet',
      hint: 'House deliveries + week / month bills',
      actionTab: 'route-hisab',
      tone: 'text-sky-700',
    });
  }
