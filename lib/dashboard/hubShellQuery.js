...
export async function fetchHubShellQuery({ businessId, from, to }) {
    ...
    if (!result?.success) {
        if (prevShellCache?.kpis) {
            const stalePayload = buildHubShellCachePayload(
                { ...prevShellCache, errors: { ...(prevShellCache.errors || {}), bootstrap: result?.error || 'Bootstrap failed' } },
                businessId,
                prevShellCache
            );
            stalePayload._queryBusinessId = businessId;
