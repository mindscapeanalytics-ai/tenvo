...
export function expandScanCandidates(code) {
  ...
  // Only expand digit/UPC variants for mostly-numeric retail codes.
  // Alphanumeric article codes (ART-101) must not also match SKU "101".
  const mostlyNumeric = /^[\d\s\-]+$/.test(base);
  if (!mostlyNumeric) {
