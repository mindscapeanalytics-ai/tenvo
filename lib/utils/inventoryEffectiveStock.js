/**
 * Hub/grid effective on-hand quantity — single source of truth for ProductService.resolveDisplayStock.
 *
 * Precedence:
 * 1. Variant-tracked products (size/color rows present) → batch/variant/headline rules.
 *    Skip location sum so hub matches storefront sellable + removeVariantStock
 *    (location rows may be registration bootstrap mirrors).
 * 2. Else sellable location rows → location sum only.
 * 3. Else batches / headline.
 */
import {
...
export function resolveInventoryEffectiveStock(product) {
  ...
  const locs = Array.isArray(product.stock_locations)
    ...
    : Array.isArray(product.product_stock_locations)
      ...
      : [];
  // Sellable-only — mirrors ProductService.resolveDisplayStock / analytics low-stock SQL.
  const sellableLocs = locs.filter(
    (row) => String(row?.state || 'sellable').toLowerCase() === 'sellable'
  );
  ...
  // Size/color parents: do not let warehouse mirrors override variant sellable truth.
  if (vars.length === 0 && sellableLocs.length > 0) {
    return sellableLocs.reduce((sum, row) => sum + toFiniteNumber(row?.quantity, 0), 0);
  }
