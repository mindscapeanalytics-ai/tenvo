...
import { searchBrands } from '../domainData/pakistaniBrands.js';
import {
  getBrandsForMarket,
  getMarketFeatures,
  resolveMarketCountryIso,
} from '../regionalMarket/index.js';
 ...
 */
export function hasPakistaniFeatures(domainKey, countryIso = 'PK') {
  return resolveMarketCountryIso(countryIso) === 'PK';
}
 ...
 */
export function getPakistaniFeatures(domainKey, countryIso = 'PK') {
  const row = domainKnowledge[domainKey] || domainKnowledge['retail-shop'];
  return getMarketFeatures(countryIso, domainKey, row?.pakistaniFeatures || {});
}
 ...
 */
export function getPaymentGateways(domainKey, countryIso = 'PK') {
  const features = getPakistaniFeatures(domainKey, countryIso);
 ...
 */
export function getTaxCompliance(domainKey, countryIso = 'PK') {
  const features = getPakistaniFeatures(domainKey, countryIso);
 ...
 */
export function getBrandsForDomain(domainKey, category = null, countryIso = 'PK') {
  const features = getPakistaniFeatures(domainKey, countryIso);
  if (features?.popularBrands?.length) {
    return features.popularBrands;
  }
  return getBrandsForMarket(countryIso, domainKey, category);
}
