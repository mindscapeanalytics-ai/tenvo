...
export async function getUnresolvedConflicts() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      resolve(rows.filter((c) => c && !c.resolved));
    };
    request.onerror = () => reject(request.error);
  });
}
...
/**
 * Clear all queued data (operations + conflicts). Prefer this over deleteDatabase
 * so we do not block on open connections (see initDB onversionchange).
 * @returns {Promise<void>}
 */
export async function clearAllData() {
  const db = await initDB();
  await new Promise((resolve, reject) => {
    const tx = db.transaction([OPERATIONS_STORE, CONFLICTS_STORE], 'readwrite');
    tx.oncomplete = () => {
      db.close();
      resolve();
    };
    ...
    tx.onabort = () => {
      db.close();
      reject(tx.error || new Error('clearAllData transaction aborted'));
    };
