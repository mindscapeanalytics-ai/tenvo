...
export function getPosUiConfig(category, business = {}) {
    ...
    return {
        ...
        taxStrategy: pack.taxStrategy || 'VAT',
        defaultTaxRate,
...
export function computePosOrderTotals(items, opts = {}) {
    ...
    let taxAmount;
    let taxBreakdown = [];
    if (Array.isArray(opts.taxComponents)) {
        const cartTax = typeof opts.precomputedTax === 'object' && opts.precomputedTax
            ? opts.precomputedTax
            : computePosCartTax(lines, opts.taxComponents);
        taxAmount = cartTax.taxAmount;
        taxBreakdown = cartTax.breakdown || [];
    } else {
        taxAmount = Math.round(
            lines.reduce((sum, i) => {
                const line = Number(i.unitPrice || 0) * Number(i.quantity || 0);
                if (i.taxExempt || i.tax_exempt) return sum;
    ...
    return { subtotal, taxAmount, taxBreakdown, discountAmount, orderDiscount, lineDiscounts, total };
}
...
export function getPosAutoPrintEnabled(tenantDefault = true) {
    if (typeof window === 'undefined') return tenantDefault !== false;
    try {
        const stored = window.localStorage.getItem(POS_AUTO_PRINT_KEY);
        if (stored === null) return tenantDefault !== false;
        return stored !== 'false';
    } catch {
        return tenantDefault !== false;
    }
}
