...
describe('storefrontProductRef', () => {
  ...
  it('resolves SKU within tenant', async () => {
    ...
    const client = {
      query: async (sql, params) => {
        ...
        if (sql.includes('LOWER(TRIM(COALESCE(sku')) {
  ...
  it('resolves barcode within tenant', async () => {
    const client = {
      query: async (sql, params) => {
        if (sql.includes('LOWER(TRIM(COALESCE(sku')) return { rows: [] };
        if (sql.includes('LOWER(TRIM(barcode))')) {
          return { rows: [{ id: '22222222-2222-4333-8444-555555555555' }] };
        }
        return { rows: [] };
      },
    };
    const id = await resolveStorefrontProductId(client, '5901234123457', 'biz-uuid');
    expect(id).toBe('22222222-2222-4333-8444-555555555555');
  });
});
