...
import {
  ensureMinimumDemoCatalogItems,
  resolveDemoCatalogMinProducts,
} from '../dataLab/demoCatalogMinimum.js';
...
export function buildRegistrationDomainProfile({ domainKey, countryIso }) {
  ...
  return {
    ...
    automation: {
      ...
      manufacturingEnabled: Boolean(knowledge.manufacturingEnabled),
...
export function buildRegistrationSeedPayload({ businessId, domainKey, countryIso, domainPackageKey = null }) {
  ...
  if (shouldSeedRichCatalogOnRegistration(canonicalKey, iso, { domainPackageKey })) {
    ...
    const presetCategories = resolveRegistrationCategories(canonicalKey, profile.categories);
    const itemCategories = [
      ...new Set(richItems.map((item) => String(item.category || '').trim()).filter(Boolean)),
    ];
    // Prefer preset order; always union product categories so hub filters match SKUs.
    const categories = [
      ...presetCategories,
      ...itemCategories.filter(
        (name) => !presetCategories.some((c) => String(c).toLowerCase() === name.toLowerCase())
      ),
    ];
    return {
      items: richItems,
 ...
 * @param {{ businessId: string, domainKey: string, countryIso: string, fullSeed?: boolean }} params
 */
export function buildDemoCatalogPayload({ businessId, domainKey, countryIso, fullSeed = true }) {
  const iso = String(countryIso || 'PK').trim().toUpperCase();
  const standards = getRegionalStandards(iso);
  const canonicalKey = resolveDomainKey(domainKey);
  const knowledge = getDomainKnowledge(canonicalKey, { countryIso: iso });
  ...
  const minProducts = resolveDemoCatalogMinProducts({ key: canonicalKey, fullSeed });
  richItems = ensureMinimumDemoCatalogItems(richItems, minProducts, {
    businessId,
    domainKey: canonicalKey,
    countryIso: iso,
  });
...
/** @param {import('./regionalHelpers').RegionalStandards} regional @param {{ currency?: string; currencySymbol?: string }} [overrides] */
export function buildRegistrationFinancialsSnapshot(regional, overrides = {}) {
  const currency =
    (typeof overrides.currency === 'string' && overrides.currency.trim()) ||
    regional.currency;
  const currencySymbol =
    (typeof overrides.currencySymbol === 'string' && overrides.currencySymbol.trim()) ||
