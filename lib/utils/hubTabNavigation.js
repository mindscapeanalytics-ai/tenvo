...
const HOT_TAB_PREFETCHERS = {
  ...
  'route-hisab': () => import('@/components/milk/MilkRouteHisab'),
};
 ...
 */
export function prefetchHotHubTabsIdle(opts = {}) {
  if (typeof window === 'undefined') return;
  const run = () => {
    let index = 0;
    const scheduleNext = () => {
      if (index >= IDLE_PREFETCH_ORDER.length) return;
      const tab = IDLE_PREFETCH_ORDER[index];
      index += 1;
      prefetchHubTabChunk(tab);
      if (index < IDLE_PREFETCH_ORDER.length) {
