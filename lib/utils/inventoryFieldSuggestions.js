 ...
 * Hub callers must pass `countryIso` from the business regional pack (do not rely on the PK default).
 */
...
/** Quick-add name hints for neighborhood milk shops (PK). */
export const MILK_SHOP_NAME_SUGGESTIONS = [
  'Fresh Cow Milk',
  'Fresh Buffalo Milk',
  'Homemade Dahi',
  'Sweet Lassi',
...
export function getInventoryFieldSuggestions(accessorKey, ctx = {}) {
  ...
  if (scalarKey === 'name') {
    const fromCatalog = collectFromProducts(products, (p) => p.name);
    if (resolveDomainKey(category) === 'milk-shop') {
      return uniqueStrings([...MILK_SHOP_NAME_SUGGESTIONS, ...fromCatalog]);
    }
    return fromCatalog;
  }
  ...
  if (canonical === 'milktype' && resolveDomainKey(category) === 'milk-shop') {
    return uniqueStrings([...configOptions, ...fromRows, 'Cow', 'Buffalo', 'Goat', 'Mixed', 'Packaged']);
  }

  if (canonical === 'chilled' && resolveDomainKey(category) === 'milk-shop') {
    return uniqueStrings([...configOptions, ...fromRows, 'Yes', 'No']);
  }
