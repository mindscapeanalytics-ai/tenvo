...
/**
 * @param {File} file
 * @param {'hero' | 'product' | 'logo'} purpose
 */
...
export async function uploadOptimizedImage(file, businessId, purpose = 'product') {
  ...
  const maxBytes = purpose === 'hero' ? 8 * 1024 * 1024 : 5 * 1024 * 1024;
  if (file.size > maxBytes) {
    throw new Error(`File too large. Max ${purpose === 'hero' ? '8' : '5'} MB before optimization.`);
  }
