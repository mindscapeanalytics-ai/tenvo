...
import {
  generateStorefrontOrderNumber,
  isStorefrontOrderNumberConflict,
} from '@/lib/utils/storefrontOrderNumber';

export { isStorefrontOrderNumberConflict };

/**
 * Generate unique order number for a storefront checkout.
 * Atomic per business + day — safe under concurrent customer checkouts.
 * @param {import('pg').PoolClient} client — must be inside BEGIN…COMMIT
 * @param {string} businessId
 */
export async function generateOrderNumber(client, businessId) {
  return generateStorefrontOrderNumber(client, businessId);
}
