...
/**
 * Parse vendor/business payment terms into day count (e.g. "Net 30", "30", "Net30 days").
 * @param {string|number|null|undefined} terms
 * @param {number} [fallback=30]
 * @returns {number}
 */
export function parsePaymentTermsDays(terms, fallback = 30) {
  const fb = Number.isFinite(Number(fallback)) && Number(fallback) > 0 ? Number(fallback) : 30;
  if (terms == null || terms === '') return fb;
  if (typeof terms === 'number' && Number.isFinite(terms) && terms > 0) {
    return Math.min(Math.floor(terms), 3650);
  }
  const match = String(terms).match(/(\d{1,4})/);
  if (!match) return fb;
  const days = Number(match[1]);
  if (!Number.isFinite(days) || days <= 0) return fb;
