...
export async function openCashDrawer(opts = {}) {
  ...
  try {
    ...
    // Exact-size PDF first (same path as POS receipts — avoids Letter/A4 HTML @page bugs).
    try {
      const [{ default: jsPDF }, { printJsPdfDocument }] = await Promise.all([
        import('jspdf'),
        import('@/lib/print/thermalReceipt'),
      ]);
      const doc = new jsPDF({
        unit: 'mm',
        format: [pageW, pageH],
      ...
      const line = (text, { size = 8, bold = false, align = 'center' } = {}) => {
        ...
        doc.text(String(text), align === 'center' ? pageW / 2 : margin, y, {
          align,
      ...
      line(businessName, { size: 10, bold: true });
      doc.setDrawColor(160);
      doc.line(margin, y, pageW - margin, y);
      y += 3;
      line(String(label).toUpperCase(), { size: 9, bold: true });
      if (amountLine) line(amountLine, { size: 12, bold: true });
      if (reason) line(reason, { size: 7 });
      line(when, { size: 7 });
      y += 1;
      doc.line(margin, y, pageW - margin, y);
      y += 3;
      line(`${paperSize} receipt · Actual size`, { size: 7 });

      const ok = await printJsPdfDocument(doc, {
        delayMs: Math.max(300, Number(opts.delayMs) || 400),
        pageW,
        pageH,
        title: `Drawer ${pageW}x${pageH}mm`,
      });
      if (ok) return true;
    } catch {
      /* HTML fallback below */
    ...
    if (!iframe) {
      ...
      iframe.style.cssText =
        'position:fixed;width:58mm;height:42mm;top:0;left:-9999px;border:0;opacity:0;pointer-events:none;z-index:-9999;';
