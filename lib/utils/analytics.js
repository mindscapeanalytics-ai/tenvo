...
/** Canonical unpaid / in-flight invoice statuses (matches lib/validation/schemas.js). */
...
/** SQL fragment for pending invoice counts in raw queries. */
export const PENDING_INVOICE_STATUS_SQL_IN = `(${PENDING_INVOICE_STATUSES.map((s) => `'${s}'`).join(', ')})`;
...
/** Unpaid sales documents awaiting collection (hub reminders + dashboard KPIs). */
/** @param {Record<string, unknown>} inv */
export function isPendingInvoice(inv) {
