...
const ROW_SPECIFIC_DOMAIN_KEYS = new Set([
  ...
  'imei',
  'serialnumber',
  // Perishables: do not copy best-before onto the next new row.
  'expirydate',
  'batchnumber',
]);
...
export function buildNewInventoryRow(category, businessId, previousRow = null, options = {}) {
  ...
  const domainDefaults = countryIso
    ? getDomainDefaults(category, previousRow, { countryIso })
    : getDomainDefaults(category, previousRow);
