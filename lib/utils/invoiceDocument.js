...
export function normalizeInvoiceForDocument(invoice, items = [], business = {}) {
  ...
  const taxTotal = Number(invoice.tax_total ?? invoice.total_tax ?? 0);
  ...
  const resolvedType =
    ...
    invoice.invoiceType ||
    taxDetails.invoice_type ||
    taxDetails.invoiceType ||
    'retail';
...
export function normalizeBuilderInvoiceForDocument(invoice, totals = {}, business = {}) {
  ...
  const pack = getBusinessRegionalPack({ ...business, settings: settingsParsed });
  const standards = getRegionalStandards(pack.countryIso);
  const taxDetails = parseTaxDetails(invoice.tax_details || invoice.taxDetails || totals.taxDetails);
  const resolvedType =
    invoice.invoiceType ||
    invoice.invoice_type ||
    taxDetails.invoice_type ||
  ...
  return {
    ...
    invoiceType: resolvedType,
    paymentMethod: invoice.payment?.method || invoice.paymentMethod || invoice.payment_method || null,
    notes: invoice.notes || '',
    currency: business?.currency || pack.currency || standards.currency || 'PKR',
    standards,
    category: business?.category || invoice.category || null,
    ...
    customer: {
      ...
      ntn: invoice.customer?.ntn || invoice.customer?.taxId || null,
    ...
    totals: {
      ...
      taxDetails: totals.taxDetails || taxDetails,
    },
    business: {
      name: business.business_name || business.name || 'Business',
