...
export function getBusinessRegionalPack(business) {
  ...
  const columnCurrency =
    business?.currency && String(business.currency).trim()
      ? String(business.currency).trim().toUpperCase()
      : '';
  const financialsCurrency =
    financials.currency && String(financials.currency).trim()
      ? String(financials.currency).trim().toUpperCase()
      : '';
  // Prefer settings.financials (owner/registration pack), then businesses.currency column, then registry.
  // Registration writes both in sync; this order keeps pack edits authoritative without ignoring legacy column-only tenants.
  const currency = financialsCurrency || columnCurrency || registry.currency;
  const currencySymbol =
    (financials.currencySymbol && String(financials.currencySymbol).trim()) ||
 ...
 * Thin wrapper over regional pack so hub/POS/PDF stay aligned.
 * @param {Record<string, unknown> | null | undefined} [business]
