 ...
 *
 * Format: ORD-YYYYMMDD-XXXX (daily sequence per business).
 */
...
export async function generateStorefrontOrderNumber(client, businessId) {
  ...
  const result = await client.query(
    ...
    WHERE business_id = $1::uuid
      AND order_number LIKE ($3 || '%')`,
    [businessId, datePrefix, prefix]
  );
...
export function isStorefrontOrderNumberConflict(error) {
  ...
  const pgError = /** @type {{ code?: string; constraint?: string; message?: string }} */ (error);
  if (pgError.code !== '23505') return false;
  const target = `${pgError.constraint || ''} ${pgError.message || ''}`.toLowerCase();
