...
const EXCEL_MODE_KEY = 'tenvo_pending_excel_mode';

/** @type {string | null} */
let pendingInventoryFocus = null;

/** @type {boolean} */
let pendingExcelMode = false;
...
/**
 * Queue opening Excel / Bulk Entry after navigating to the inventory tab.
 * InventoryManager is lazy-mounted; a bare CustomEvent often fires before it exists.
 */
export function setPendingExcelMode() {
  pendingExcelMode = true;
  if (typeof window !== 'undefined') {
    try {
      sessionStorage.setItem(EXCEL_MODE_KEY, '1');
    } catch {
      // ignore quota / private mode
    }
  }
 ...
 * @returns {boolean}
 */
export function consumePendingExcelMode() {
  let open = pendingExcelMode;
  pendingExcelMode = false;

  if (!open && typeof window !== 'undefined') {
    try {
      open = sessionStorage.getItem(EXCEL_MODE_KEY) === '1';
      if (open) sessionStorage.removeItem(EXCEL_MODE_KEY);
    } catch {
      // ignore
    }
  }
