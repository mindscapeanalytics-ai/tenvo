...
/** Incomplete NTN drafts should not block save. */
const preprocessOptionalNtn = (v) => {
  const cleaned = preprocessOptionalTaxId(v);
  if (!cleaned) return '';
  return NTN_REGEX.test(cleaned) ? cleaned : '';
};

/** Incomplete CNIC drafts should not block save. */
const preprocessOptionalCnic = (v) => {
  const cleaned = preprocessOptionalTaxId(v);
  if (!cleaned) return '';
  const digits = cleaned.replace(/\D/g, '');
  if (digits.length < 13) return '';
  return CNIC_REGEX.test(cleaned) ? cleaned : '';
};
...
// Customer validation schema (client form — name required; contact/tax optional like vendors)
export const customerSchema = z.object({
  ...
  pincode: z.preprocess(
    ...
    z.union([
      z.literal(''),
      z.string().regex(/^\d{4,10}$/, 'Invalid postal code'),
    ])
  ),
  market_location: z.string().optional().or(z.literal('')).or(z.null()),
  ntn: z.preprocess(
    preprocessOptionalNtn,
    z.union([z.literal(''), z.string().regex(NTN_REGEX, 'Invalid NTN format (e.g. 1234567-8)')])
  ),
  cnic: z.preprocess(
    preprocessOptionalCnic,
    z.union([
      z.literal(''),
      z.string().regex(CNIC_REGEX, 'Invalid CNIC format (e.g. 42201-1234567-1)'),
    ])
  ),
  srn: z.preprocess(
