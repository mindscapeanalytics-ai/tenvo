 ...
 * Platform-level users (owner email or admin role) bypass all checks.
 * 
 ...
 * @param {object} user - Optional BetterAuth user object for platform-level check
 * @returns {Promise<boolean>} - True if access is allowed
 * @throws {Error} - Throws "Unauthorized" if access is denied
 */
export async function verifyBusinessAccess(userId, businessId, requiredRoles = [], existingClient = null, user = null) {
    if (!userId || !businessId) {
        throw new Error('Unauthorized: Missing credentials');
    }

    // Platform-level users (owner email or BetterAuth admin) bypass all business access checks
    if (user && isPlatformLevel(user)) {
        return true;
    }
