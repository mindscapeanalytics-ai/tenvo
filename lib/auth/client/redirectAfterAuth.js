...
function resolveOnboardingRegisterPath() {
  ...
  try {
    ...
    const savedRaw = localStorage.getItem('tenvo_registration_data');
    if (!savedRaw) return '/register';

    const saved = JSON.parse(savedRaw);
    const businessName = String(saved?.businessName || '').trim();
    const handle = String(saved?.handle || '').trim();
    const category = String(saved?.category || '').trim();

    if (savedStep === '3' && businessName && handle && category) {
      return '/register?step=3';
    }
    if (savedStep === '2' && businessName && handle) {
...
export function resolveDefaultBusinessPath(businesses) {
...
export function resolvePostAuthDestination(businesses, returnPath) {
...
/**
 * Hard navigation so the session cookie is always present on the next document
 * request. Soft router.replace can bounce unauthenticated users back to /login
 * before the cookie is visible to the RSC layout.
 * @param {string} destination
 * @param {import('next/navigation').AppRouterInstance} [router]
 */
export function navigateAfterAuth(destination, router) {
  const path = typeof destination === 'string' && destination.startsWith('/') ? destination : '/multi-business';
...
async function fetchJoinedBusinessesWithRetry(userId) {
  let lastError = null;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await businessAPI.getByUserId(userId);
    } catch (error) {
      lastError = error;
      if (attempt < 2) {
...
export async function redirectAfterAuth(router, user) {
  ...
  try {
    const businesses = await fetchJoinedBusinessesWithRetry(user.id);
    ...
    if (!businesses?.length) {
      ...
      navigateAfterAuth(destination, router);
      return;
    }
    ...
    navigateAfterAuth(resolveOnboardingRegisterPath(), router);
  }
}
