...
function resolveBetterAuthSecret() {
    ...
    // `next build` collects route data with NODE_ENV=production; host secrets may be
    // runtime-only. Do not fail compile — still require a real secret when serving.
    ...
    if (isProduction) {
        if (!secret || secret.length < 32) {
            if (isNextBuildPhase) {
                return "build-time-placeholder-better-auth-secret-do-not-use";
            }
            throw new Error(
