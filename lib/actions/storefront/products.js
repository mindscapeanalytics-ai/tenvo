...
import { assertPublicStorefrontCatalogAccess } from '@/lib/storefront/assertPublicStorefrontCatalogAccess';
import {
...
import { enrichProductWithNormalizedImages } from '@/lib/utils/productImages';
...
function buildProductListWhere(businessId, filters, { compat = false } = {}) {
  ...
  if (category) {
    ...
    // Match slug, name, slugified name, product.category string, or category UUID.
    whereConditions.push(`(
      lower(coalesce(c.slug, '')) = lower($${paramIndex})
      OR lower(coalesce(c.name, '')) = lower($${paramIndex})
      OR lower(regexp_replace(trim(coalesce(c.slug, '')), '[^a-zA-Z0-9]+', '-', 'g'))
         = lower(regexp_replace(trim($${paramIndex}), '[^a-zA-Z0-9]+', '-', 'g'))
      OR lower(regexp_replace(trim(coalesce(c.name, '')), '[^a-zA-Z0-9]+', '-', 'g'))
         = lower(regexp_replace(trim($${paramIndex}), '[^a-zA-Z0-9]+', '-', 'g'))
      OR lower(regexp_replace(trim(coalesce(p.category, '')), '[^a-zA-Z0-9]+', '-', 'g'))
         = lower(regexp_replace(trim($${paramIndex}), '[^a-zA-Z0-9]+', '-', 'g'))
      OR lower(trim(coalesce(p.category, ''))) = lower($${paramIndex})
      OR (
        $${paramIndex} ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        AND c.id::text = $${paramIndex}
      )
    )`);
  ...
  if (search) {
    ...
    if (mode === 'partnumber') {
      ...
      whereConditions.push(`(
        coalesce(p.domain_data->>'oemnumber', '') ILIKE $${paramIndex}
        OR coalesce(p.domain_data->>'interchangecode', '') ILIKE $${paramIndex}
        OR p.sku ILIKE $${paramIndex}
        OR p.name ILIKE $${paramIndex}
      )`);
      params.push(`%${term}%`);
  ...
  if (equipmentType) {
    whereConditions.push(`lower(coalesce(p.domain_data->>'equipmenttype', '')) = lower($${paramIndex})`);
    params.push(String(equipmentType).trim());
    paramIndex++;
  }

  if (vesselType) {
    whereConditions.push(`lower(coalesce(p.domain_data->>'vesseltype', '')) = lower($${paramIndex})`);
    params.push(String(vesselType).trim());
    paramIndex++;
  }

  if (systemCondition) {
    whereConditions.push(`lower(coalesce(p.domain_data->>'systemcondition', '')) = lower($${paramIndex})`);
    params.push(String(systemCondition).trim());
    paramIndex++;
  }

  if (manufacturer) {
    whereConditions.push(`(
      lower(coalesce(p.domain_data->>'manufacturer', '')) = lower($${paramIndex})
      OR lower(coalesce(p.brand, '')) = lower($${paramIndex})
    )`);
  ...
  if (gender) {
    ...
    whereConditions.push(`(
      lower(coalesce(p.domain_data->>'gender', '')) = $${paramIndex}
      OR lower(coalesce(p.category, '')) ILIKE $${paramIndex + 1}
      OR lower(coalesce(c.name, '')) ILIKE $${paramIndex + 1}
    )`);
    params.push(genderVal, `%${genderVal}%`);
    paramIndex += 2;
  }
  ...
  if (style) {
    ...
    whereConditions.push(`(
      lower(coalesce(p.domain_data->>'style', '')) = lower($${paramIndex})
      OR lower(coalesce(p.category, '')) ILIKE $${paramIndex + 1}
      OR lower(coalesce(p.name, '')) ILIKE $${paramIndex + 1}
    )`);
    params.push(styleVal, `%${styleVal}%`);
    paramIndex += 2;
  }
  ...
  try {
    ...
    const fallbackQuery = `
      SELECT 
        ...
        COALESCE(p.domain_data, '{}'::jsonb) as domain_data,
        c.id as category_id, c.name as category_name, c.slug as category_slug,
            ...
            try {
              ...
              const minimalFallback = `
                SELECT 
                  ...
                  NULL as brand, NULL as category,
                  0 as sales_count,
                  CASE WHEN p.stock IS NULL THEN 'in_stock'
                       WHEN p.stock <= 0   THEN 'out_of_stock'
                       WHEN p.stock <= 5   THEN 'low_stock'
                ...
                LIMIT $${limIdx} OFFSET $${limIdx + 1}
              `;
              result = await client.query(minimalFallback, listParams);
            }
          } else {
            throw e2;
          }
    ...
    const products = result.rows.map((row) => {
      ...
      const imagePack = enrichProductWithNormalizedImages({
        name: row.name,
...
export async function getProducts(businessId, filters = {}) {
  ...
  const disabled = await assertPublicStorefrontCatalogAccess(businessId);
...
async function fetchStorefrontProductBySlugUncached(businessId, slug) {
  ...
  try {
    ...
    try {
      ...
      if (colErr.code === '42703') {
        ...
        const q2 = `SELECT p.id, p.name, p.sku, p.description, p.price, p.stock, p.image_url, p.is_active,
                      p.category_id, p.created_at, p.updated_at, p.domain_data, p.brand, p.category,
                      NULL as slug, NULL as compare_price, false as is_featured, false as is_new,
                      0 as sales_count, NULL as stock_status,
                      COALESCE(p.images, '[]'::jsonb) as images,
                      COALESCE(p.has_variants, false) as has_variants,
                      NULL as rating, 0 as review_count, true as enable_reviews,
    ...
    let extraSpecs = {};
    const rawExtraSpecs = domainData.specifications || domainData.specs;
    if (Array.isArray(rawExtraSpecs)) {
      rawExtraSpecs.forEach(spec => {
        if (spec && typeof spec === 'object') {
          const key = spec.key || spec.attribute_name;
          const value = spec.value || spec.attribute_value;
          if (key) extraSpecs[key] = value;
        }
      });
    } else if (rawExtraSpecs && typeof rawExtraSpecs === 'object') {
      extraSpecs = { ...rawExtraSpecs };
    }
    ...
    const enrichedProduct = enrichProductWithNormalizedImages(
    ...
    return actionSuccess(serializeDecimalsDeep({
      ...
      related_products: relatedResult.rows.map(p => {
        ...
        const enriched = enrichProductWithNormalizedImages(p);
        return {
          ...enriched,
