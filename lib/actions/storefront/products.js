  ...
  try {
    ...
    if (product.has_variants) {
      try {
        ...
        let variantsResult;
        try {
          variantsResult = await client.query(variantsSql, [product.id, businessId]);
        } catch (vErr) {
          if (vErr.code === '42703' && String(vErr.message).includes('is_default')) {
            variantsResult = await client.query(variantsSqlNoDefaultCol, [product.id, businessId]);
          } else {
            throw vErr;
          }
        }
 ...
 * Check product stock
 */
export async function checkProductStock(productId, variantId, quantity, businessId) {
  if (!businessId) {
  ...
  try {
    ...
    if (variantId) {
      ...
      params = [variantId, productId, businessId];
    } else {
