...
import {
  ...
  getPlatformPaymentCapabilities,
  isPlatformCryptoConfigured,
  isPlatformStripeConfigured,
} from '@/lib/storefront/storefrontPaymentEligibility';
import Stripe from 'stripe';
import { reconcileOrderPayment } from '@/lib/services/paymentReconciliation';
import { onStorefrontOrderPaid, onStorefrontOrderRefunded } from '@/lib/memberships/membershipOrderHooks';
import { commitStorefrontOrderInventoryIfPending } from '@/lib/storefront/storefrontOrderInventory';

export async function getPlatformPaymentCapabilitiesForHub() {
  return getPlatformPaymentCapabilities();
}
...
/**
 * Get available payment methods for customers (COD default; Stripe/Crypto only when owner + platform ready).
 */
export async function getAvailablePaymentMethods(businessId) {
  ...
  try {
    const bizCheck = await client.query(
      `SELECT id FROM businesses WHERE id = $1::uuid AND COALESCE(is_active, true) = true`,
      [businessId]
    );
    ...
    let context;
    try {
      context = await loadStorefrontPaymentContext(client, businessId);
    } catch (tableErr) {
      if (tableErr.code === '42P01' || tableErr.message?.includes('business_payment_methods')) {
        return actionSuccess({ methods: [{ ...STOREFRONT_COD_METHOD }] });
      }
      throw tableErr;
    }

    const methods = resolveEligibleStorefrontPaymentMethods(context);
    return actionSuccess({ methods });
  } catch (error) {
...
/**
 * Enable cryptocurrency checkout for this store (requires platform NOWPayments keys).
 */
export async function addCryptoPaymentMethod(businessId, settings = {}) {
  const authCheck = await requireStorefrontHubAccess(businessId);
  if (!authCheck.ok) return authCheck.response;

  if (!isPlatformCryptoConfigured()) {
    return actionFailure(
      'NOT_CONFIGURED',
      'Cryptocurrency payments are not available until NOWPayments is configured for the platform.'
    );
  }
  ...
  try {
    await client.query(
      ...
      ) VALUES ($1::uuid, 'crypto', $2, true, true, $3, 0, 0, NOW())
      ON CONFLICT (business_id, provider) DO UPDATE SET
        is_active = true,
        display_name = COALESCE(EXCLUDED.display_name, business_payment_methods.display_name),
        description = COALESCE(EXCLUDED.description, business_payment_methods.description),
        updated_at = NOW()`,
      [
        businessId,
        settings.displayName || 'Cryptocurrency',
        settings.description || 'BTC, ETH, USDT and 100+ coins via NOWPayments',
      ]
    );

    return actionSuccess({ message: 'Cryptocurrency payments enabled for your storefront' });
  } catch (error) {
    console.error('[addCryptoPaymentMethod] Error:', error);
...
export async function updateOrderPaymentStatus(orderId, businessId, paymentStatus, notes = '') {
  ...
  try {
    ...
    await client.query(
      `UPDATE storefront_orders SET metadata = $1 WHERE id = $2 AND business_id = $3::uuid`,
    ...
    if (paymentStatus === 'paid') {
      try {
        await client.query(
          `INSERT INTO payments (
           ...
           ) VALUES ($1, 'receipt', 'storefront_order', $2, 'cod', CURRENT_DATE, $3, 'active', $4, NOW(), NOW())`,
          [
            businessId,
        ...
        // Accounting: cash-sale JE (not orphan AR receipt)
        await client.query('SAVEPOINT acc_storefront_cod');
        try {
          await reconcileOrderPayment(orderId, businessId, {
            txClient: client,
            paymentMode: 'cash',
          });
          ...
          console.error('[updateOrderPaymentStatus] GL reconcile failed:', accErr.message);
          throw accErr;
        }
      ...
      await onStorefrontOrderPaid(client, businessId, orderId);
      try {
        await commitStorefrontOrderInventoryIfPending(client, businessId, orderId);
      } catch (invErr) {
        console.error('[updateOrderPaymentStatus] inventory commit failed:', invErr?.message || invErr);
        throw invErr;
      }
    }

    if (paymentStatus === 'refunded') {
      await onStorefrontOrderRefunded(client, businessId, orderId);
    }
  ...
  try {
    ...
    await client.query(
      `INSERT INTO payments (
       ...
       ) VALUES ($1, 'receipt', 'storefront_order', $2, $3, $4, $5, 'active', $6, NOW(), NOW())`,
      [
        businessId,
        parseFloat(amount),
        paymentMode || 'cash',
    ...
    // Update order payment_status first so GL reconcile can run
    ...
    await client.query(
      `UPDATE storefront_orders
        ...
        WHERE id = $3 AND business_id = $4::uuid`,
      [newPaymentStatus, JSON.stringify(meta), orderId, businessId]
    );
    ...
    if (newPaymentStatus === 'paid') {
      ...
      try {
        ...
        console.error('[recordManualPayment] GL reconcile failed:', accErr.message);
        throw accErr;
      }
      await onStorefrontOrderPaid(client, businessId, orderId);
      try {
        ...
        console.error('[recordManualPayment] inventory commit failed:', invErr?.message || invErr);
        throw invErr;
      }
    }
