  ...
  try {
    ...
    try {
      ordersResult = await client.query(
        `SELECT 
          o.id, o.business_id, o.order_number, 
      ...
      try {
        ...
        if (payCheck.rows.length > 0) {
          ...
          if (
            ...
            try {
              await reconcileOrderPayment(parseInt(orderId, 10), businessId, {
                txClient: client,
                paymentMode: 'cod',
              });
            } catch (glErr) {
              console.error('[updateOrderStatus] GL reconcile failed:', glErr.message);
              throw glErr;
            }
    ...
    if (newStatus === 'cancelled' || newStatus === 'refunded') {
      ...
      if (shouldRestock) {
        ...
        for (const item of itemsResult.rows) {
          ...
          // Never fall back to headline-only SQL — that desyncs locations/variants.
