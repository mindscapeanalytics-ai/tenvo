    ...
    for (const item of orderData.items) {
      ...
      await client.query(
        `INSERT INTO storefront_order_items (
          order_id, business_id, product_id, product_name, product_sku, quantity,
          unit_price, total_price, tax_amount, variant_id, metadata
    ...
    if (orderData.payment?.status === 'paid' && orderData.payment?.transactionId) {
      try {
        await client.query(
          ...
          ) VALUES ($1, 'in', 'storefront_order', $2, $3, $4, $5, CURRENT_DATE, $6, $7, 'active', $8, NOW(), NOW())`,
          [
            businessId,
            order.id,
            customerId,
        ...
        await AccountingService.recordBusinessTransaction('payment', {
          ...
          description: `Storefront Payment Receipt: ${orderNumber}`,
          userId: customerId
        }, client);
