    ...
    if (result.rows.length === 0) {
      ...
      try {
        result = await client.query(
          ...
          JOIN businesses b ON cd.business_id = b.id::uuid
          LEFT JOIN business_settings bs ON b.id::uuid = bs.business_id
          LEFT JOIN subscription_plans p ON bs.plan_id::uuid = p.id
      ...
      result.rows.map(async (order) => {
        const itemsResult = await client.query(
          `SELECT 
            ...
            quantity, unit_price, total_price, tax_amount, metadata
