...
import { getMarinePartsConfig } from '@/lib/storefront/marineParts';
...
import { getTilesConfig } from '@/lib/storefront/tilesStorefront';
import { getTyreConfig } from '@/lib/storefront/tyreStorefront';
import { getFootwearConfig } from '@/lib/storefront/footwearStorefront';
...
import { getJewelleryAdminFormSettings } from '@/lib/storefront/jewelleryStorefront';
import {
  sanitizeHeroSlides,
...
import { normalizeStorefrontBrandingForForm } from '@/lib/storefront/storefrontBrandMark';
import { getStoreConnectionConfig } from '@/lib/storefront/storeConnectionActions';
import { normalizeAboutStorefrontConfig } from '@/lib/storefront/aboutStorefront';
import { normalizeStoreTopBarForForm } from '@/lib/storefront/storeTopBar';
...
export async function getStorefrontSettings(businessId) {
  ...
  try {
    ...
    const marineConfig = getMarinePartsConfig(storeSettings);
    ...
    const tilesConfig = getTilesConfig(storeSettings);
    const tyreConfig = getTyreConfig(storeSettings, storeDomain);
    const footwearConfig = getFootwearConfig(storeSettings, storeDomain);
    const electronicsConfig = getElectronicsConfig(storeSettings);
    ...
    const jewelleryConfig = getJewelleryAdminFormSettings(storeSettings, storeDomain, row?.category);
    const bookingConfig = getBookingConfig(storeSettings);
    const brandingConfig = normalizeStorefrontBrandingForForm(storeSettings.storefront?.branding);
    const connectionConfig = getStoreConnectionConfig(storeSettings);
    const aboutConfig = normalizeAboutStorefrontConfig(storeSettings.storefront?.about, {
      category: row?.category,
    });
    const topBarConfig = normalizeStoreTopBarForForm({
      ...(storeSettings.storefront || {}),
      showTopBarPhone: storeSettings.storefront?.showTopBarPhone ?? storeSettings.showTopBarPhone,
      showTopBar: storeSettings.storefront?.showTopBar ?? storeSettings.showTopBar,
      showTopBarCity: storeSettings.storefront?.showTopBarCity ?? storeSettings.showTopBarCity,
    });
    ...
    return actionSuccess({
      ...
      about: aboutConfig,
      showTopBar: topBarConfig.showTopBar,
      showTopBarPhone: topBarConfig.showTopBarPhone,
      showTopBarCity: topBarConfig.showTopBarCity,
...
export async function updateBusinessSettings(businessId, settings) {
  ...
  try {
    ...
    const { heroSlides: _marineHero, ...marineRaw } = marine || {};
    ...
    const { heroSlides: _tilesHero, ...tilesRaw } = tiles || {};
    const { heroSlides: _tyreHero, ...tyreRaw } = tyre || {};
    const { heroSlides: _footwearHero, ...footwearRaw } = footwear || {};
    const { heroSlides: _electronicsHero, ...electronicsRaw } = electronics || {};
    ...
    const { heroSlides: _jewelleryLegacyHero, jewelleryEditTiles: _jewelleryEditTiles, ...jewelleryRaw } = jewellery || {};
    ...
    if (marketplaceRaw && typeof marketplaceRaw === 'object') {
      const prev = currentStoreSettings.storefront?.marketplace || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        marketplace: {
          ...prev,
          ...
          banners: Array.isArray(marketplaceRaw.banners) ? marketplaceRaw.banners : prev.banners,
          ...
          showMarketingBanners: marketplaceRaw.showMarketingBanners !== false,
          showTrustStrip: marketplaceRaw.showTrustStrip !== false,
        },
      };
    }
    ...
    if (marineRaw && typeof marineRaw === 'object') {
      const prev = currentStoreSettings.storefront?.marine || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        marine: {
          ...prev,
          ...marineRaw,
          slides: Array.isArray(marineRaw.slides) ? marineRaw.slides : prev.slides,
          trustStats: Array.isArray(marineRaw.trustStats) ? marineRaw.trustStats : prev.trustStats,
          insights: Array.isArray(marineRaw.insights) ? marineRaw.insights : prev.insights,
          quickLinks: Array.isArray(marineRaw.quickLinks) ? marineRaw.quickLinks : prev.quickLinks,
          brandChips: Array.isArray(marineRaw.brandChips) ? marineRaw.brandChips : prev.brandChips,
          sectorCards: Array.isArray(marineRaw.sectorCards) ? marineRaw.sectorCards : prev.sectorCards,
        },
      };
    }
    ...
    if (tilesRaw && typeof tilesRaw === 'object') {
      const prev = currentStoreSettings.storefront?.tiles || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        tiles: {
          ...prev,
          ...tilesRaw,
          roomCollections: Array.isArray(tilesRaw.roomCollections) ? tilesRaw.roomCollections : prev.roomCollections,
          categoryIcons: Array.isArray(tilesRaw.categoryIcons) ? tilesRaw.categoryIcons : prev.categoryIcons,
          promoBanners: Array.isArray(tilesRaw.promoBanners) ? tilesRaw.promoBanners : prev.promoBanners,
          editorialBanners: Array.isArray(tilesRaw.editorialBanners) ? tilesRaw.editorialBanners : prev.editorialBanners,
          trustPillars: Array.isArray(tilesRaw.trustPillars) ? tilesRaw.trustPillars : prev.trustPillars,
          curatedTabs: Array.isArray(tilesRaw.curatedTabs) ? tilesRaw.curatedTabs : prev.curatedTabs,
          quickSearchTerms: Array.isArray(tilesRaw.quickSearchTerms) ? tilesRaw.quickSearchTerms : prev.quickSearchTerms,
          testimonials: Array.isArray(tilesRaw.testimonials) ? tilesRaw.testimonials : prev.testimonials,
        },
      };
    }

    if (tyreRaw && typeof tyreRaw === 'object') {
      const prev = currentStoreSettings.storefront?.tyre || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        tyre: {
          ...prev,
          ...tyreRaw,
          vehicleTiles: Array.isArray(tyreRaw.vehicleTiles) ? tyreRaw.vehicleTiles : prev.vehicleTiles,
          brands: Array.isArray(tyreRaw.brands) ? tyreRaw.brands : prev.brands,
          trustPillars: Array.isArray(tyreRaw.trustPillars) ? tyreRaw.trustPillars : prev.trustPillars,
          services: Array.isArray(tyreRaw.services) ? tyreRaw.services : prev.services,
          promoBanners: Array.isArray(tyreRaw.promoBanners) ? tyreRaw.promoBanners : prev.promoBanners,
          editorialBanners: Array.isArray(tyreRaw.editorialBanners) ? tyreRaw.editorialBanners : prev.editorialBanners,
          brandStories: Array.isArray(tyreRaw.brandStories) ? tyreRaw.brandStories : prev.brandStories,
          exploreSegments: Array.isArray(tyreRaw.exploreSegments) ? tyreRaw.exploreSegments : prev.exploreSegments,
          quickSearchTerms: Array.isArray(tyreRaw.quickSearchTerms) ? tyreRaw.quickSearchTerms : prev.quickSearchTerms,
          testimonials: Array.isArray(tyreRaw.testimonials) ? tyreRaw.testimonials : prev.testimonials,
        },
      };
    }

    if (footwearRaw && typeof footwearRaw === 'object') {
      const prev = currentStoreSettings.storefront?.footwear || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        footwear: {
          ...prev,
          ...footwearRaw,
          genderTiles: Array.isArray(footwearRaw.genderTiles) ? footwearRaw.genderTiles : prev.genderTiles,
          brands: Array.isArray(footwearRaw.brands) ? footwearRaw.brands : prev.brands,
          trustPillars: Array.isArray(footwearRaw.trustPillars) ? footwearRaw.trustPillars : prev.trustPillars,
          conditionTiles: Array.isArray(footwearRaw.conditionTiles) ? footwearRaw.conditionTiles : prev.conditionTiles,
          quickSearchTerms: Array.isArray(footwearRaw.quickSearchTerms) ? footwearRaw.quickSearchTerms : prev.quickSearchTerms,
          testimonials: Array.isArray(footwearRaw.testimonials) ? footwearRaw.testimonials : prev.testimonials,
        },
      };
    }

    if (electronicsRaw && typeof electronicsRaw === 'object') {
      const prev = currentStoreSettings.storefront?.electronics || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        electronics: {
          ...prev,
          ...electronicsRaw,
          categoryTiles: Array.isArray(electronicsRaw.categoryTiles) ? electronicsRaw.categoryTiles : prev.categoryTiles,
          brandWall: Array.isArray(electronicsRaw.brandWall) ? electronicsRaw.brandWall : prev.brandWall,
          trustPillars: Array.isArray(electronicsRaw.trustPillars) ? electronicsRaw.trustPillars : prev.trustPillars,
          quickSearchTerms: Array.isArray(electronicsRaw.quickSearchTerms) ? electronicsRaw.quickSearchTerms : prev.quickSearchTerms,
        },
      };
    }
    ...
    if (fashionRaw && typeof fashionRaw === 'object') {
      ...
      newStoreSettings.storefront = {
        ...
        fashion: {
          ...
          trustPillars: Array.isArray(fashionRaw.trustPillars) ? fashionRaw.trustPillars : prev.trustPillars,
          brands: Array.isArray(fashionRaw.brands) ? fashionRaw.brands : prev.brands,
          quickSearchTerms: Array.isArray(fashionRaw.quickSearchTerms)
            ? fashionRaw.quickSearchTerms
            : prev.quickSearchTerms,
          homeEdit:
    ...
    if (jewelleryRaw && typeof jewelleryRaw === 'object') {
      const prev = currentStoreSettings.storefront?.jewellery || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        jewellery: {
          ...prev,
          ...jewelleryRaw,
          trustPillars: Array.isArray(jewelleryRaw.trustPillars) ? jewelleryRaw.trustPillars : prev.trustPillars,
          brands: Array.isArray(jewelleryRaw.brands) ? jewelleryRaw.brands : prev.brands,
          quickSearchTerms: Array.isArray(jewelleryRaw.quickSearchTerms)
            ? jewelleryRaw.quickSearchTerms
            : prev.quickSearchTerms,
          heroTiles: Array.isArray(jewelleryRaw.heroTiles) ? jewelleryRaw.heroTiles : prev.heroTiles,
          heroTrustPills: Array.isArray(jewelleryRaw.heroTrustPills)
            ? jewelleryRaw.heroTrustPills
            : prev.heroTrustPills,
          jewelleryEdit:
            jewelleryRaw.jewelleryEdit && typeof jewelleryRaw.jewelleryEdit === 'object'
              ? jewelleryRaw.jewelleryEdit
              : prev.jewelleryEdit,
        },
      };
    }
    ...
    if (branding !== undefined) {
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        branding: normalizeStorefrontBrandingForForm(branding),
      };
    }

    if (connection !== undefined && typeof connection === 'object') {
      const prev = currentStoreSettings.storefront?.connection || {};
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        connection: {
          ...prev,
          ...
          showQuote: connection.showQuote !== false,
          showCall: connection.showCall !== false,
          showMail: connection.showMail !== false,
          preferWhatsApp: connection.preferWhatsApp === true,
          quoteLabel: String(connection.quoteLabel || prev.quoteLabel || '').trim(),
          callLabel: String(connection.callLabel || prev.callLabel || '').trim(),
          mailLabel: String(connection.mailLabel || prev.mailLabel || '').trim(),
          whatsappLabel: String(connection.whatsappLabel || prev.whatsappLabel || '').trim(),
        },
      };
    }
    ...
    if (about !== undefined) {
      newStoreSettings.storefront = {
        ...(newStoreSettings.storefront || currentStoreSettings.storefront || {}),
        about: normalizeAboutStorefrontConfig(about),
      };
    }

    if (
      showTopBar !== undefined ||
      showTopBarPhone !== undefined ||
      showTopBarCity !== undefined
    ) {
      const prevSf = newStoreSettings.storefront || currentStoreSettings.storefront || {};
      const topBarFlags = normalizeStoreTopBarForForm({
        showTopBar: showTopBar !== undefined ? showTopBar : prevSf.showTopBar,
        showTopBarPhone: showTopBarPhone !== undefined ? showTopBarPhone : prevSf.showTopBarPhone,
        showTopBarCity: showTopBarCity !== undefined ? showTopBarCity : prevSf.showTopBarCity,
      });
      newStoreSettings.storefront = {
        ...prevSf,
        ...topBarFlags,
      };
      // Prefer nested storefront flags; drop legacy flat duplicates if present.
      delete newStoreSettings.showTopBar;
      delete newStoreSettings.showTopBarPhone;
      delete newStoreSettings.showTopBarCity;
    }
