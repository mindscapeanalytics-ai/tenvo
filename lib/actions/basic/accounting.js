...
export async function updateGLAccountAction(id, data) {
    try {
        ...
        const client = await pool.connect();
        try {
            await assertEntityBelongsToBusiness(client, 'gl_accounts', id, data.business_id);
            
            // Cannot modify system accounts code or type (only name/description)
            const accRes = await client.query('SELECT is_system, code FROM gl_accounts WHERE id = $1', [id]);
            ...
            const query = `UPDATE gl_accounts SET ${updates.join(', ')} WHERE id = $1 RETURNING *`;
...
export async function deleteGLAccountAction(businessId, id) {
    try {
        ...
        const client = await pool.connect();
        try {
            await assertEntityBelongsToBusiness(client, 'gl_accounts', id, businessId);
            
            const accRes = await client.query('SELECT is_system FROM gl_accounts WHERE id = $1', [id]);
            ...
            const entriesRes = await client.query('SELECT 1 FROM gl_entries WHERE account_id = $1 LIMIT 1', [id]);
            ...
            await client.query('DELETE FROM gl_accounts WHERE id = $1', [id]);
            return { success: true };
        } finally {
            client.release();
        }
