...
export async function createGLAccountAction(data) {
    try {
        await checkAuth(data.business_id, null, 'finance.manage_accounts');
        const validation = validateWithSchema(glAccountSchema, data);
        if (!validation.success) return validation;

        const client = await pool.connect();
        try {
            const { business_id, code, name, type, sub_type, description, is_active } = data;
            
            // Check duplicate
            const existing = await client.query('SELECT id FROM gl_accounts WHERE business_id = $1 AND code = $2', [business_id, code]);
            ...
            const res = await client.query(`
                ...
                RETURNING *
            `, [business_id, code, name, type, sub_type, description, is_active ?? true]);
...
export async function updateGLAccountAction(id, data) {
    try {
        ...
        const client = await pool.connect();
        try {
            await assertEntityBelongsToBusiness(client, 'gl_account', id, data.business_id);
            ...
            if (acc.is_system) {
                ...
                const existing = await client.query('SELECT id FROM gl_accounts WHERE business_id = $1 AND code = $2 AND id != $3', [data.business_id, code, id]);
                if (existing.rows.length > 0) return { success: false, error: 'Account code already exists' };
            }

            const updates = [];
            const values = [id, data.business_id];
            let idx = 3;
