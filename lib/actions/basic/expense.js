...
async function generateExpenseNumber(client, businessId) {
    return generateScopedDocumentNumber(client, {
        businessId,
        table: 'expenses',
...
export async function createExpenseAction(data) {
    ...
    try {
        const session = await checkAuth(validData.businessId, client, 'finance.manage_expenses');
        await client.query('BEGIN');

        const expenseNumber = await generateExpenseNumber(client, validData.businessId);
        ...
        const result = await client.query(`
            ...
            RETURNING *
        `, [
            validData.businessId,
            expenseNumber,
            validData.accountId,
            validData.category || null,
            validData.amount,
            validData.taxAmount || 0,
            validData.vendorId || null,
            validData.paymentMethod,
            validData.date,
            validData.description || null,
            validData.receiptUrl || null,
            'recorded'
        ]);
        ...
        switch (validData.paymentMethod) {
            case 'bank':
        ...
        const accountRes = await client.query(
            ...
            [validData.accountId, validData.businessId]
        );
        ...
        await createGLEntryAction({
            ...
            date: validData.date,
            description: `Expense: ${validData.description || validData.category || expenseNumber}`,
        ...
        if (validData.paymentMethod === 'credit' && validData.vendorId) {
            await client.query(
                `UPDATE vendors SET outstanding_balance = outstanding_balance + $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`,
                [totalAmount, data.vendorId, validData.businessId]
            );
        }

        // ── Create payment record for immediate payments (cash/bank) ──
        // This ensures the Payments ledger is complete for all outflows
        if (validData.paymentMethod !== 'credit') {
            const paymentMode = validData.paymentMethod === 'bank' ? 'bank' : 'cash';
            await client.query(`
            ...
            `, [
                ...
                paymentMode,
                validData.date,
                `Expense: ${expenseNumber} — ${validData.description || validData.category || ''}`
            ]);
        }
        ...
        auditWrite({
            ...
            entityType: 'expense',
            entityId: expense.id,
            description: `Created expense ${expenseNumber} — ${validData.description || validData.category || 'N/A'}`,
            metadata: { expenseNumber, amount: validData.amount, paymentMethod: validData.paymentMethod, category: validData.category },
        });
...
export async function deleteExpenseAction(businessId, expenseId) {
    const client = await pool.connect();
    try {
        await checkAuth(businessId, client, 'finance.manage_expenses');
        await client.query('BEGIN');
        ...
        if (expense.payment_method === 'credit' && expense.vendor_id) {
            await client.query(
                `UPDATE vendors SET outstanding_balance = outstanding_balance - $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`,
                [expense.amount, expense.vendor_id, businessId]
            );
        }
