...
export async function createInvoiceAction(params) {
    try {
        ...
        await checkAuth(validated.business_id, 'sales.create_invoice');
        ...
        try {
            ...
            auditWrite({
                ...
                metadata: { invoiceNumber: validated.invoice_number, grandTotal: validated.grand_total, customerId: validated.customer_id },
            });
        ...
        if (e?.code === '23505') {
            return actionFailure('DUPLICATE_INVOICE_NUMBER', 'Invoice number already exists for this business');
        }
        return actionFailure('CREATE_INVOICE_FAILED', getErrorMessage(e));
    }
}
...
export async function getInvoicesAction(businessId) {
    try {
        ...
        try {
            ...
            `, [businessId]);
            return actionSuccess({ invoices: res.rows });
        } finally {
            client.release();
        }
    } catch (e) {
        return actionFailure('GET_INVOICES_FAILED', getErrorMessage(e));
    }
}

export async function deleteInvoiceAction(businessId, invoiceId) {
    try {
        await checkAuth(businessId, 'sales.delete_invoice');
        const client = await pool.connect();
        ...
        try {
            ...
            await client.query(
                `UPDATE invoices SET is_deleted = true, deleted_at = NOW(), status = 'voided', updated_at = NOW() WHERE id = $1 AND business_id = $2`,
                [invoiceId, businessId]
            );
            await client.query('COMMIT');
            ...
            auditWrite({
                ...
                metadata: { invoiceNumber: invoice.invoice_number, grandTotal: invoice.grand_total },
            });
        ...
        return actionFailure('DELETE_INVOICE_FAILED', getErrorMessage(e));
    }
}
export async function updateInvoiceAction(params) {
    try {
        ...
        return actionFailure('UPDATE_INVOICE_FAILED', getErrorMessage(e));
    }
}
