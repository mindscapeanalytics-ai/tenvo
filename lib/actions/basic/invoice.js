...
import { getTenantCache, setTenantCache, purgeTenantCacheKey } from '@/lib/cache/tenantCache';
...
export async function createInvoiceAction(params) {
    try {
        ...
        // Purge invoice list cache for instant consistency
        void purgeTenantCacheKey(invoiceData.business_id, 'invoice-list');
 ...
 * }} [options]
 * Defaults: take 500 newest, headers only (no line items).
 * Pass includeItems: true when credit-note / detail UIs need items.
 * Pass limit: null for unbounded (export).
 */
export async function getInvoicesAction(businessId, options = {}) {
    try {
        ...
        const cacheSuffix = `${offset}:${limit || 500}:${customerId || ''}:${includeItems ? '1' : '0'}`;
        const cached = await getTenantCache(businessId, 'invoice-list', cacheSuffix);
        ...
        // Enrich open balances in one aggregated payment sum (avoid per-id calculate_invoice_balance).
        ...
        // Cache list-level reads without date filters for instant repeat loads
        if (!dateFrom && !dateTo && !includeItems) {
            void setTenantCache(businessId, 'invoice-list', response, cacheSuffix);
        }
...
/**
 * Load one invoice with line items for standard A4 PDF / thermal print.
 * List payloads omit items for performance — print must hydrate lines first.
 */
export async function getInvoiceForPrintAction(businessId, invoiceId) {
    try {
        await checkAuth(businessId, 'sales.view');
        ...
        console.error('getInvoiceForPrintAction', e);
        return await actionFailure(e?.code || 'INVOICE_PRINT_LOAD_FAILED', await getErrorMessage(e));
    }
}
