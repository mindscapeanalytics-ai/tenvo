...
export async function createCreditNoteAction(data) {
    ...
    try {
        ...
        const cnNumber = await generateScopedDocumentNumber(client, {
...
export async function applyCreditNoteAction(data) {
    const client = await pool.connect();
    try {
        await checkAuth(data.businessId, client, 'finance.credit_notes');
        await client.query('BEGIN');
        ...
        const totalPaid = await client.query(`
            ...
            FROM payment_allocations WHERE invoice_id = $1 AND business_id = $2
        `, [data.targetInvoiceId, data.businessId]);
        ...
        await client.query(
            `UPDATE invoices SET payment_status = $1, updated_at = NOW() WHERE id = $2 AND business_id = $3`,
            [newStatus, data.targetInvoiceId, data.businessId]
        );
