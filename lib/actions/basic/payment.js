...
import { normalizePaymentType, paymentTypeFilterValues } from '@/lib/utils/paymentTypes';
...
export async function getPaymentsAction(businessId, filters = {}) {
    try {
        ...
        if (filters.payment_type) {
            const typeValues = paymentTypeFilterValues(filters.payment_type);
            where.payment_type = typeValues.length > 1 ? { in: typeValues } : typeValues[0];
        }
        ...
        const mappedPayments = payments.map(p => {
            ...
            return {
                ...rest,
                payment_type: normalizePaymentType(rest.payment_type) || rest.payment_type,
...
export async function createPaymentAction(paymentData) {
    try {
        // Validate allocations array structure before handing off to PaymentService
        const { allocations } = paymentData;
        if (allocations !== undefined) {
            if (!Array.isArray(allocations)) {
                return await actionFailure('INVALID_ALLOCATIONS', 'allocations must be an array');
            }
            for (const alloc of allocations) {
                if (!alloc || typeof alloc !== 'object') {
                    return await actionFailure('INVALID_ALLOCATIONS', 'Each allocation must be an object');
                }
                if (!alloc.invoice_id && !alloc.purchase_id) {
                    return await actionFailure('INVALID_ALLOCATIONS', 'Each allocation must reference an invoice_id or purchase_id');
                }
                const amt = Number(alloc.amount);
                if (isNaN(amt) || amt <= 0) {
                    return await actionFailure('INVALID_ALLOCATIONS', 'Each allocation amount must be a positive number');
                }
            }
        }

        const session = await ensureAccess(paymentData.business_id, 'payments.create');
        const normalizedType = normalizePaymentType(paymentData.payment_type);
        if (!normalizedType) {
            return await actionFailure('INVALID_PAYMENT_TYPE', 'Payment type must be receipt or payment');
        }
