...
async function ensureAccess(businessId, permission, client = null) {
    return withGuard(businessId, { permission, client });
}
...
export async function getPaymentsAction(businessId, filters = {}) {
    try {
        ...
        try {
            ...
            return actionSuccess({ payments: result.rows });
        } finally {
            client.release();
        }
    } catch (error) {
        console.error('Get payments error:', error);
        return actionFailure('GET_PAYMENTS_FAILED', getErrorMessage(error));
    }
}
...
export async function createPaymentAction(paymentData) {
    ...
    try {
        await ensureAccess(paymentData.business_id, 'payments.create', client);

        await client.query('BEGIN');

        const normalizedTransactionId = (paymentData.transaction_id || '').trim();
        if (normalizedTransactionId) {
            const duplicate = await client.query(
                `SELECT * FROM payments WHERE business_id = $1 AND transaction_id = $2 LIMIT 1`,
                [paymentData.business_id, normalizedTransactionId]
            );
            if (duplicate.rows.length > 0) {
                await client.query('ROLLBACK');
                return actionFailure('DUPLICATE_TRANSACTION', 'A payment with this transaction reference already exists', {
                    payment: duplicate.rows[0]
                });
            }
        }
        ...
        // Supports both legacy single-reference and new multi-allocation pattern
        const allocations = paymentData.allocations || [];

        if (allocations.length > 0) {
            // Multi-allocation: distribute payment across multiple invoices/purchases
            ...
            for (const alloc of allocations) {
                ...
                allocatedTotal += parseFloat(alloc.amount);
            }
            // Validate total allocation doesn't exceed payment
            if (allocatedTotal > parseFloat(payment.amount) + 0.01) {
                throw new Error(`Total allocation (${allocatedTotal}) exceeds payment amount (${payment.amount})`);
            }
        } else if (paymentData.reference_id) {
            // Legacy single-reference: auto-create allocation
            const invoiceId = paymentData.reference_type === 'invoice' ? paymentData.reference_id : null;
            const purchaseId = paymentData.reference_type === 'purchase' ? paymentData.reference_id : null;
        ...
        if (paymentData.payment_type === 'receipt' && paymentData.customer_id) {
            ...
            `, [paymentData.amount, paymentData.customer_id, paymentData.business_id]);
        }
        ...
        if (paymentData.payment_type === 'payment' && paymentData.vendor_id) {
            await client.query(`
                ...
                SET outstanding_balance = outstanding_balance - $1,
                    updated_at = NOW()
                WHERE id = $2 AND business_id = $3
            `, [paymentData.amount, paymentData.vendor_id, paymentData.business_id]);
        }
        ...
        if (paymentData.reference_type === 'invoice' && paymentData.reference_id) {
            ...
            const invoiceResult = await client.query(
                'SELECT total_amount FROM invoices WHERE id = $1 AND business_id = $2',
                [paymentData.reference_id, paymentData.business_id]
            );
            ...
            if (invoiceResult.rows.length > 0) {
                ...
                const paymentsResult = await client.query(`
                    ...
                    FROM payments
                    WHERE reference_type = 'invoice' AND reference_id = $1 AND business_id = $2
                `, [paymentData.reference_id, paymentData.business_id]);
                ...
                await client.query(
                    'UPDATE invoices SET status = $1, updated_at = NOW() WHERE id = $2 AND business_id = $3',
                    [newStatus, paymentData.reference_id, paymentData.business_id]
                );
            }
        }
        ...
        if (paymentData.reference_type === 'purchase' && paymentData.reference_id) {
            ...
            const purchaseResult = await client.query(
                'SELECT total_amount FROM purchases WHERE id = $1 AND business_id = $2',
                [paymentData.reference_id, paymentData.business_id]
            );
            ...
            if (purchaseResult.rows.length > 0) {
                ...
                const paymentsResult = await client.query(`
                    SELECT COALESCE(SUM(amount), 0) as total_paid
                    FROM payments
                    WHERE reference_type = 'purchase' AND reference_id = $1 AND business_id = $2
                `, [paymentData.reference_id, paymentData.business_id]);
        ...
        return actionFailure('CREATE_PAYMENT_FAILED', getErrorMessage(error));
    } finally {
        client.release();
    }
}
...
export async function deletePaymentAction(businessId, paymentId) {
    ...
    try {
        await ensureAccess(businessId, 'finance.manage_payments', client);

        await client.query('BEGIN');

        // Get payment details (scoped to business)
        const paymentResult = await client.query(
        ...
        if (paymentResult.rows.length === 0) {
            await client.query('ROLLBACK');
            return actionFailure('PAYMENT_NOT_FOUND', 'Payment not found');
        }
        ...
        // Delete GL entries (scoped to business via payment reference)
        await client.query(
            'DELETE FROM gl_entries WHERE reference_type = $1 AND reference_id = $2 AND business_id = $3',
            ['payment', paymentId, businessId]
        );

        // Delete payment allocations
        await client.query(
            'DELETE FROM payment_allocations WHERE payment_id = $1 AND business_id = $2',
            [paymentId, businessId]
        );
        ...
        return actionSuccess({ message: 'Payment deleted and balances reversed' });

    } catch (error) {
        await client.query('ROLLBACK');
        console.error('Delete payment error:', error);
        return actionFailure('DELETE_PAYMENT_FAILED', getErrorMessage(error));
    } finally {
        client.release();
    }
}
