...
export async function deletePaymentAction(paymentId) {
    ...
    try {
        ...
        const payment = paymentResult.rows[0];
        await ensureAccess(payment.business_id, 'finance.manage_payments', client);
        ...
        if (payment.payment_type === 'receipt' && payment.customer_id) {
            await client.query(`
                ...
                SET outstanding_balance = outstanding_balance + $1,
                    updated_at = NOW()
                WHERE id = $2 AND business_id = $3
            `, [payment.amount, payment.customer_id, payment.business_id]);
        }
        ...
        if (payment.payment_type === 'payment' && payment.vendor_id) {
            await client.query(`
                ...
                SET outstanding_balance = outstanding_balance + $1,
                    updated_at = NOW()
                WHERE id = $2 AND business_id = $3
            `, [payment.amount, payment.vendor_id, payment.business_id]);
        }
