...
export async function createBusiness(data) {
    ...
    const registrationSnapshot = buildRegistrationSettingsSnapshot(country, regional, {
        domainVertical: normalizedCategory,
    });
    ...
    try {
        ...
        try {
            seedSummary = await provisionRegistrationSeed({
                ...
                domainKey: normalizedCategory,
                countryIso: regional.countryCode,
            });
            if (seedSummary.productCount > 0 || seedSummary.categoryCount > 0) {
                await markRegistrationInventorySeeded(result.id, seedSummary);
            }
        } catch (seedErr) {
            console.error('[createBusiness] starter inventory seed failed:', seedErr);
        }
        ...
        return await actionSuccess({
            ...
            countryIso: regional.countryCode,
            seeded: seedSummary.productCount > 0 || seedSummary.categoryCount > 0,
            seedSummary,
        });
    } catch (error) {
...
export async function seedRegistrationInventoryAction({ businessId, domainKey, countryIso, itemNames = null, force = false }) {
    try {
        ...
        if (!force) {
            ...
            if (seedState.alreadySeeded) {
                return await actionSuccess({
                    ...
                    categoryCount: 0,
                    skipped: true,
                    message: 'Starter inventory is already loaded for this business',
                });
            }
        }

        const vertical = domainKey || owned.category || 'retail-shop';
        ...
        if (!result.productCount && !result.categoryCount) {
            return await actionSuccess({ count: 0, message: 'No template items for this vertical' });
        }
        ...
        return await actionSuccess({
            ...
            message: `Seeded ${result.productCount} products and ${result.categoryCount} categories`,
        });
    } catch (error) {
...
export async function getBusinessByDomainAndUser(domain, userId) {
    try {
        ...
        if (ownerCheck) {
            ...
            return await actionSuccess({
                business: enrichBusinessForClient({ ...ownerCheck, user_role: 'owner' }),
            });
        }
...
export async function updateOwnerBusinessPackagingAction({ businessId, mode = 'tier', featureOverrides = undefined }) {
    try {
        ...
        return await actionSuccess({ business: updated });
    } catch (error) {
