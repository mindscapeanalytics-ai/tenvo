...
import { getCurrencySymbol, isValidCurrency } from '@/lib/currency';
...
import { isPlatformOwner, TRIAL_CONFIG, getTrialExpiryDate, isPlatformLevel, isPartnerUser } from '@/lib/config/platform';
...
import { mergeBusinessSettingsAllowingClientPatch } from '@/lib/utils/mergeBusinessSettingsSafe';
...
import { invalidateStorefrontTenant } from '@/lib/storefront/invalidateStorefrontCatalog';
...
import { buildDefaultWaterDeliverySettingsSeed, isWaterDeliveryStore } from '@/lib/storefront/waterShopHisab';
...
import { buildDealTermsPayload } from '@/lib/utils/registrationCostCalculator';
...
export async function createBusiness(data) {
    ...
    if (!phone || !String(phone).trim()) {
        return await actionFailure('INVALID_PHONE_NUMBER', 'Business phone number is mandatory for registration.');
    }
    ...
    const currencyCode = String(resolvedCurrency || '').trim().toUpperCase();
    const resolvedCurrencySymbol = isValidCurrency(currencyCode)
        ? getCurrencySymbol(currencyCode)
        : regional.currencySymbol;
    const financialsSnapshot = buildRegistrationFinancialsSnapshot(regional, {
        currency: currencyCode || regional.currency,
    ...
    if (typeof affiliateCode === 'string' && affiliateCode.trim().length > 0) {
        const cleanCode = affiliateCode.trim();
        const affiliateRecord = await prismaBase.affiliates.findFirst({
            where: {
    ...
    try {
        const result = await prismaBase.$transaction(async (tx) => {
            ...
            const registrationStorefront = resolveRegistrationStorefrontDefaults({
                ...
                regional: {
                    ...
                    currency: currencyCode || resolvedCurrency,
            ...
            const seoKeywords =
                typeof registrationStorefront.seoKeywords === 'string' &&
                registrationStorefront.seoKeywords.trim()
                    ? registrationStorefront.seoKeywords.trim()
                    : null;
            const seoSettingsPatch = seoKeywords ? { seo: { keywords: seoKeywords } } : {};

            // Water-delivery: write explicit waterHisab defaults so 19L DEL+REC columns
            // are durable from day one rather than relying on runtime fallback reads.
            const waterHisabSettingsPatch = isWaterDeliveryStore(registrationCategory)
                ? buildDefaultWaterDeliverySettingsSeed()
                : {};

            const registrationSettingsPatch = applyRegistrationVerticalPackaging(
                domainPkgRegistration.settingsPatch || {},
                registrationCategory
            );

            const normalizedDealTerms = buildDealTermsPayload(dealTerms || {
                monthlyFee: effectivePlan.price_pkr || 0,
                currency: currencyCode || resolvedCurrency,
            });

            // 1. Insert Business
            // Only Prisma businesses columns in businessMedia (logo_url / cover_image_url).
            // SEO keywords live in settings.seo — never on businesses.create().
            const biz = await tx.businesses.create({
                data: {
                    ...
                    settings: {
                        ...
                        deal_terms: normalizedDealTerms,
                        ...seoSettingsPatch,
                        ...waterHisabSettingsPatch,
            ...
            if (affiliateId) {
                try {
                    ...
                    console.warn('[createBusiness] Immediate referral create notice:', refErr.message);
                }
            }
            ...
            // Explicit UUID id: live DBs may still have integer serial until
            // 20260711_business_settings_id_uuid is applied; after migrate, DB default also works.
            ...
            await tx.store_payment_settings.create({
                data: {
                    ...
                    allow_cod: true,
                    allow_prepaid: true,
                    default_currency: currencyCode || resolvedCurrency,
            ...
            // 6. Initialize storefront (INSIDE transaction for atomicity)
            ...
            if (!storefrontInit.success) {
                throw new Error(`Storefront initialization failed: ${storefrontInit.error || 'Unknown error'}`);
            }
        ...
        try {
            ...
            if (seedSummary.categoryCount > 0 || seedSummary.productCount > 0) {
                ...
                // Mirror seedRegistrationInventoryAction — bust L1/L2 so day-one catalog is not stale.
                await invalidateStorefrontTenant(result.id);
            }
        } catch (seedErr) {
            seedFailed = true;
        ...
        return await actionSuccess({
            ...
            seeded: !seedFailed && (seedSummary.categoryCount > 0 || seedSummary.productCount > 0),
            seedFailed,
            seedSummary,
        });
    } catch (error) {
        ...
        if (
            ...
            /business_custom_domains table is missing/i.test(msg) ||
            /relation ["']?business_custom_domains["']? does not exist/i.test(msg)
        ) {
            return await actionFailure(
                'DATABASE_SCHEMA_OUT_OF_DATE',
 ...
 * Hub/packaging keys stay on `businesses.settings`. Public storefront chrome keys also merge into
 * `business_settings.settings` (resolve SoT) and bust storefront cache.
...
export async function completeRegistrationSetupAction(businessId, patch = {}) {
    try {
        ...
        const PUBLIC_STOREFRONT_SETTING_KEYS = [
            'storefront',
            'brand',
            'contact',
            'announcement',
        ...
        const publicPatch = {};
        for (const key of PUBLIC_STOREFRONT_SETTING_KEYS) {
            if (Object.prototype.hasOwnProperty.call(incoming, key)) {
                publicPatch[key] = incoming[key];
            }
        }

        if (Object.keys(publicPatch).length > 0) {
            const bs = await prismaBase.business_settings.findFirst({
                where: { business_id: businessId },
                select: { id: true, settings: true },
            });
            if (bs) {
                const prevPublic =
                    bs.settings && typeof bs.settings === 'object' && !Array.isArray(bs.settings)
                        ? bs.settings
                        : {};
                await prismaBase.business_settings.update({
                    where: { id: bs.id },
                    data: { settings: { ...prevPublic, ...publicPatch } },
                });
...
export async function updateBusinessAction(businessId, updates) {
    try {
        ...
        if (settings !== undefined) {
            ...
            updateData.settings = mergeBusinessSettingsAllowingClientPatch(
                existing.settings,
                settings
            );
        }
 ...
 */
export async function updateOwnerBusinessPackagingAction() {
    return await actionFailure('UNAUTHORIZED_ACTION', 'Custom module packaging can only be managed by the platform administrator or through official plan upgrades.');
}

/**
 * Owner/admin: set `businesses.settings.domainKnowledge` industry overrides (merge patch).
 * Pass `domainKnowledge: null` to clear all customizations.
 *
 * @param {{ businessId: string, domainKnowledge: object | null }} params
 */
export async function updateDomainKnowledgeOverridesAction({ businessId, domainKnowledge }) {
    try {
        ...
        try {
            ...
            return await actionFailure(
                'VALIDATION_ERROR',
                /** @type {{ message?: string }} */ (validationError)?.message || 'Invalid industry overrides'
            );
        }
...
export async function getJoinedBusinessesAction(userId) {
    try {
        ...
        const businesses = businessUsers.map(bu => ({
            ...bu.businesses,
            user_role: bu.role,
            user_module_access: extractModuleAccess(bu.permissions),
            user_status: bu.status
        }));

        // If the caller is an affiliate partner, also surface all registered demo businesses
        if (session?.user && isPartnerUser(session.user)) {
            const joinedIds = new Set(businesses.map(b => b.id));
            const demoBusinesses = await prismaBase.businesses.findMany({
                where: {
                    OR: [
            ...
            for (const demo of demoBusinesses) {
                if (!joinedIds.has(demo.id)) {
                    businesses.push({
                        ...demo,
                        user_role: 'manager',
                        user_module_access: extractModuleAccess(null),
                        user_status: 'active',
...
export async function getUserBusinessRoleAction(businessId, userId) {
    try {
        ...
        if (!businessId || !effectiveUserId) {
            return await actionFailure('INVALID_INPUT', 'Business and user are required');
        }
        ...
        const owned = await prismaBase.businesses.findFirst({
            where: { id: businessId, user_id: effectiveUserId },
            select: { id: true },
        });
        if (owned) {
            return await actionSuccess({ role: 'owner' });
        }

        return await actionFailure('BUSINESS_ACCESS_DENIED', 'No role for this business');
    } catch (error) {
...
/**
 * Update a member's role and optional module access overrides.
 * Prefer: updateUserRoleAction(userId, businessId, { role, modules })
 * Legacy: updateUserRoleAction(userId, businessId, roleString)
 *
 * modules === undefined → role only (leave permissions unchanged)
 * modules === null → clear overrides (permissions: {})
 * modules object → normalize and persist via buildPermissionsPayload
 */
export async function updateUserRoleAction(userId, businessId, roleOrOptions) {
    try {
        const { role: actorRole } = await withGuard(businessId, {
        ...
        if (
            ...
            if (Object.prototype.hasOwnProperty.call(roleOrOptions, 'modules')) {
                modules = roleOrOptions.modules;
        ...
        if (modulesSpecified) {
            if (modules === null) {
                ...
                // Explicit { role, modules: undefined } → role only
            } else {
                const normalized = normalizeModuleAccess(modules);
                if (!normalized) {
                    return actionFailure(
                        'INVALID_MODULE_ACCESS',
                        'Select at least one valid access module'
                    );
                }
                // Only owners may grant settings module access
                if (normalized.settings === true && actorRole !== 'owner') {
                    normalized.settings = false;
                }
...
export async function deleteBusinessAction({ businessId }) {
    try {
        if (!businessId) {
            return await actionFailure('INVALID_BUSINESS_ID', 'Business ID is required');
        }

        const session = await getServerSession();
        if (!session?.user?.id) {
            return await actionFailure('UNAUTHORIZED', 'You must be logged in to delete a business.');
        }
        ...
        const business = await prismaBase.businesses.findFirst({
            where: { id: businessId },
            select: { id: true, domain: true, business_name: true },
        });

        if (!business) {
            return await actionFailure('NOT_FOUND', 'Business entity not found.');
        }
        ...
        if (!platformOwner) {
            ...
            if (membership?.role !== 'owner') {
                return await actionFailure('FORBIDDEN', 'Only the business owner can delete this business entity.');
            }
        }

        await prismaBase.businesses.delete({
            where: { id: businessId },
        });

        if (business.domain) {
            try {
                invalidateStorefrontTenant(business.domain);
            } catch (e) {
                console.warn('[deleteBusinessAction] tenant invalidation notice:', e);
            }
        }

        return await actionSuccess({
            deletedId: businessId,
            message: `Business "${business.business_name}" deleted successfully.`,
        });
    } catch (error) {
        console.error('[deleteBusinessAction] error:', error);
        return await actionFailure('DELETE_BUSINESS_FAILED', await getErrorMessage(error));
    }
}
