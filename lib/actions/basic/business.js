...
export async function getBusinessByUserId(userId) {
    try {
        ...
        const businessUser = await db.business_users.findFirst({
            where: {
...
export async function getBusinessByDomainAndUser(domain, userId) {
    try {
        ...
        const ownerCheck = await db.businesses.findFirst({
            where: {
                domain: domain,
                user_id: effectiveUserId
            }
        ...
        if (ownerCheck) {
            ...
            await db.business_users.upsert({
                where: {
                    business_id_user_id: {
...
export async function updateBusinessAction(businessId, updates) {
    try {
        ...
        const updatedBiz = await db.businesses.update({
            where: { id: businessId },
            data: updateData
        });
...
export async function getBusinessTeamAction(businessId) {
    try {
        ...
        const team = await db.business_users.findMany({
...
export async function getJoinedBusinessesAction(userId) {
    try {
        ...
        const businessUsers = await db.business_users.findMany({
            where: {
                user_id: effectiveUserId,
                status: 'active'
            },
...
export async function getUserBusinessRoleAction(businessId, userId) {
    try {
        const effectiveUserId = await resolveSessionUserId(userId);
        const bu = await db.business_users.findFirst({
            where: {
...
export async function updateUserRoleAction(userId, businessId, role) {
    try {
        ...
        const targetMembership = await db.business_users.findFirst({
            where: {
                user_id: userId,
                business_id: businessId
            }
        ...
        const updated = await db.business_users.update({
...
export async function addBusinessMemberAction({ businessId, email, role = 'salesperson' }) {
    try {
        ...
        const seatCount = await db.business_users.count({
            where: {
                business_id: businessId,
                status: 'active'
            }
        ...
        const membership = await db.business_users.upsert({
            where: {
                business_id_user_id: {
                    business_id: businessId,
...
export async function removeBusinessMemberAction({ businessId, userId }) {
    try {
        ...
        const membership = await db.business_users.findFirst({
            where: {
                business_id: businessId,
                user_id: userId
            }
        ...
        await db.business_users.update({
