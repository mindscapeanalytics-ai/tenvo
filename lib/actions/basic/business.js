...
const AVAILABLE_PLAN_TIERS = new Set(Object.keys(PLAN_TIERS));
...
export async function createBusiness(data) {
    ...
    const selectedPlan = PLAN_TIERS[normalizedPlanTier] || PLAN_TIERS.free;
...
export async function addBusinessMemberAction({ businessId, email, role = 'salesperson' }) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(businessId, ['owner', 'admin'], client);
