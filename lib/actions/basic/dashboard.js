...
export async function getDashboardKPIs(businessId, options = {}) {
    try {
        ...
        // Redis cache for instant hub shell bootstrap (30s TTL, auto-purged on mutations)
        const period = options.period || 'month';
        const cacheSuffix = `${period}:${options.dateFrom || ''}:${options.dateTo || ''}`;
        const cached = await getTenantCache(businessId, 'dashboard', cacheSuffix);
        const now = new Date();
        let dateFrom, dateTo;
            ...
            // Inventory uses one grouped join (not per-product correlated EXISTS/SUM).
            const result = await pool.query(`
                WITH sellable_location_qty AS (
                    SELECT
                        product_id,
                        COALESCE(SUM(COALESCE(quantity, 0)), 0) AS qty
                    FROM product_stock_locations
                    WHERE business_id = $1
                      AND LOWER(COALESCE(state, 'sellable')) = 'sellable'
                    GROUP BY product_id
                ),
                product_display_stock AS (
                    SELECT
                        p.id,
                        p.cost_price,
                        COALESCE(p.min_stock, p.reorder_point, 0) AS reorder_at,
                        CASE
                          WHEN loc.product_id IS NOT NULL THEN loc.qty
                          ELSE COALESCE(p.stock, 0)
                        END AS display_qty
                    FROM products p
                    LEFT JOIN sellable_location_qty loc ON loc.product_id = p.id
                ...
                SELECT
                    ...
                    (SELECT COALESCE(SUM(total_amount), 0) FROM period_storefront) as storefront_revenue,
                    (SELECT COALESCE(SUM(total_amount), 0) FROM period_pos) as pos_revenue,
                    (SELECT COALESCE(SUM(grand_total), 0) FROM period_invoices 
                     WHERE status NOT IN ('draft', 'voided')) as invoice_revenue,
                    (SELECT COALESCE(SUM(grand_total), 0) FROM period_invoices 
                    ...
                    -- One grouped join CTE — not per-product correlated EXISTS/SUM scans.
                    (SELECT COUNT(*) FROM product_display_stock) as active_products,
                    (SELECT COUNT(*) FROM product_display_stock
                     WHERE display_qty <= reorder_at) as low_stock_count,
                    (SELECT COALESCE(SUM(display_qty * COALESCE(cost_price, 0)), 0)
                     FROM product_display_stock) as inventory_value,
                    (SELECT COALESCE(SUM(display_qty), 0)
                     FROM product_display_stock) as total_stock_units,
            ...
            const cogsRes = await pool.query(SALES_COGS_PERIOD_SQL, [
                businessId,
                fromIso,
                toIso,
                'all',
            ...
            const storefrontRevenue = Number(kpi.storefront_revenue || 0);
            const posRevenue = Number(kpi.pos_revenue || 0);
            const invoiceRevenue = Number(kpi.invoice_revenue || 0);

            const response = await actionSuccess({
            ...
            // Cache dashboard KPIs (30s TTL — stale is acceptable for dashboards)
            void setTenantCache(businessId, 'dashboard', response, cacheSuffix);
...
export async function getSalesPerformanceAction(businessId, options = {}) {
    try {
        ...
        const [
            ...
            pool.query(SALES_TREND_UNIFIED_SQL, [
                businessId,
                curFrom,
                curTo,
                channel,
            ...
            pool.query(TOP_MOVING_PRODUCTS_UNIFIED_SQL, [
                businessId,
                topLimit,
                curFrom,
                curTo,
            ...
            pool.query(kpiSql, prevKpiParams),
            pool.query(SALES_COGS_PERIOD_SQL, [
                businessId,
                curFrom,
                curTo,
                channel,
            ...
            categoryScoped
                ? Promise.resolve({ rows: [{ units_total: 0 }] })
                : pool.query(SALES_UNITS_PERIOD_SQL, [businessId, curFrom, curTo, channel]),
            pool.query(RECENT_SALES_ACTIVITY_SQL, [
                businessId,
                8,
                curFrom,
                curTo,
            ...
            categoryScoped
                ...
                : pool.query(SALES_RETENTION_PERIOD_SQL, [
                      businessId,
                      curFrom,
                      curTo,
                      channel,
                  ]),
            pool.query(TOP_CUSTOMERS_UNIFIED_SQL, [
                businessId,
                5,
                curFrom,
                curTo,
            ...
            pool.query(SALES_PRODUCT_CATEGORIES_SQL, [businessId]),
        ]);
        ...
        const totalUnits = categoryScoped
            ? 0
            : parseInt(curUnitsRes.rows[0]?.units_total, 10) || 0;
