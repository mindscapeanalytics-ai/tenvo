    ...
    try {
        ...
        try {
            ...
            const result = await client.query(`
                ...
                SELECT
                    -- Revenue KPIs (UNIFIED: invoices + POS + storefront)
                    (
                    ...
                    -- Order count (UNIFIED: all three ledgers)
                    (
                    ...
                    (SELECT COALESCE(SUM(COALESCE(stock, 0)), 0) FROM products 
                     WHERE business_id = $1 AND is_deleted = false AND is_active = true) as total_stock_units,
            ...
            // Use unified order count (invoices + POS + storefront)
...
export async function getSalesPerformanceAction(businessId, options = {}) {
    try {
        ...
        try {
            ...
            const curFrom = toDateOnly(curStart);
            const curTo = toDateOnly(curEnd);
            const prevFrom = toDateOnly(prevStart);
            const prevTo = toDateOnly(prevEnd);

            const [trendRes, topRes, curKpiRes, prevKpiRes, curCogsRes, prevCogsRes, recentRes, retentionRes] = await Promise.all([
                ...
                client.query(SALES_KPI_PERIOD_SQL, [businessId, curFrom, curTo]),
                client.query(SALES_KPI_PERIOD_SQL, [businessId, prevFrom, prevTo]),
                client.query(SALES_COGS_PERIOD_SQL, [businessId, curFrom, curTo]),
                client.query(SALES_COGS_PERIOD_SQL, [businessId, prevFrom, prevTo]),
