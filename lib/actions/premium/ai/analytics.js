...
/** Six rolling months: invoice + paid storefront revenue/count; GL profit unchanged. Params: $1 business_id, $2 trend_anchor::date */
const SALES_TREND_WITH_STOREFRONT_SQL = `
                ...
                SELECT 
                    ...
                    COALESCE(s.sales, 0) + COALESCE(sf.sales, 0) as sales,
                    COALESCE(s.count, 0) + COALESCE(sf.count, 0) as count,
                    COALESCE(p.profit, 0) as profit
                FROM months m
                LEFT JOIN invoice_sales s ON s.month = m.month
...
const REVENUE_GROWTH_VS_PRIOR_SQL = `
                WITH b AS (
...
async function fetchSalesTrendRows(client, businessId, trendAnchor) {
    const result = await client.query(SALES_TREND_WITH_STOREFRONT_SQL, [businessId, trendAnchor]);
    return result.rows.map((row) => ({
        date: row.date,
        /** Invoice + paid storefront `total_amount` for the month (excl. cancelled orders) */
        revenue: parseFloat(row.sales),
        /** Net from GL: revenue/income accounts minus expense accounts */
        profit: parseFloat(row.profit),
        /** Invoices + paid storefront orders in the month */
...
export async function getDashboardMetricsAction(businessId) {
    ...
    try {
        ...
        try {
            ...
            // 5. Active Customers & Cash Flow (Current vs Last Month)
            ...
            const alertsRes = await client.query(`
                SELECT 
                    (SELECT COUNT(*) FROM products WHERE business_id = $1 AND stock <= COALESCE(min_stock_level, min_stock, 5) AND is_active = true) as low_stock_count,
...
export async function getDomainIndustryInsightsAction(businessId) {
    ...
    try {
        ...
        // 1. Fetch business category
        const bizResult = await client.query('SELECT category FROM businesses WHERE id = $1', [businessId]);
        if (bizResult.rows.length === 0) throw new Error('Business not found');
        const category = bizResult.rows[0].category;
        ...
        } else if (category === 'textile-mill' || category === 'garments') {
            insight += "Fashion seasonality is shifting. In Pakistan, the Winter/Wedding season prep should begin 2 months in advance.";
            suggestedAction = "Evaluate Raw Material (Yarn/Fabric) lock-in prices before the demand surge.";
