...
import { checkPlanFeature } from '@/lib/auth/planGuard';

async function checkAuth(businessId, requiredRoles = [], client = null, featureKey = null) {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session) throw new Error('Unauthorized');
    if (businessId) {
        await verifyBusinessAccess(session.user.id, businessId, requiredRoles, client);
        if (featureKey) await checkPlanFeature(businessId, featureKey, client);
    }
    return session;
}
...
export async function getSalesTrendAction(businessId) {
    ...
    try {
        await checkAuth(businessId, [], client, 'ai_analytics');
        try {
...
export async function getCategoryDistributionAction(businessId) {
    try {
        await checkAuth(businessId, [], null, 'ai_analytics');
        const client = await pool.connect();
        try {
            const result = await client.query(`
                SELECT 
...
export async function getDashboardMetricsAction(businessId) {
    ...
    try {
        ...
        try {
            ...
            // 5. Active Customers & Cash Flow (This Month)
            const extraMetricsRes = await client.query(`
                SELECT 
                    ...
                    (
                        -- Cash Flow: Inflow (Payments) - Outflow (Expenses)
                        -- Simplified: Sum of all credits to Cash - Sum of all debits to Cash? 
                        -- Easier: Revenue - Expense for this month from GL
                        SELECT 
                            COALESCE(SUM(CASE WHEN LOWER(a.type) IN ('revenue', 'income') THEN (e.credit - e.debit) ELSE 0 END), 0) -
                            COALESCE(SUM(CASE WHEN LOWER(a.type) = 'expense' THEN (e.debit - e.credit) ELSE 0 END), 0)
                        FROM gl_entries e
                        JOIN gl_accounts a ON e.account_id = a.id
                        WHERE a.business_id = $1
            ...
            const alertsRes = await client.query(`
                SELECT 
                    (SELECT COUNT(*) FROM products WHERE business_id = $1 AND stock <= min_stock_level AND is_active = true) as low_stock_count,
            ...
            return {
                ...
                data: {
                    ...
                    customers: {
                        active: parseInt(extraMetricsRes.rows[0].active_customers),
                        growth: 8.2 // Placeholder or calc real time if expensive
                    },
                    cashFlow: {
                        current: parseFloat(extraMetricsRes.rows[0].cash_flow_mtd),
                        growth: 18.7 // Placeholder
                    },
                    growth: {
                        value: `${growthPercent >= 0 ? '+' : ''}${growthPercent.toFixed(1)}%`,
                        trend: growthPercent >= 0 ? 'up' : 'down',
 ...
 * Returns expenses grouped by parent account or category for the current month.
 */
export async function getExpenseBreakdownAction(businessId) {
    try {
        await checkAuth(businessId, [], null, 'ai_analytics');
        const client = await pool.connect();
        try {
