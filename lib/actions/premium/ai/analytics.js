        ...
        try {
            ...
            const [revenueRes, ordersRes, productsRes, growthRes] = await Promise.all([
                ...
                client.query(`
                    ...
                    SELECT 
                        COALESCE((SELECT total_revenue FROM gl_revenue), 0) + COALESCE((SELECT total_revenue FROM pos_revenue), 0) + COALESCE((SELECT total_revenue FROM storefront_revenue), 0) as total_revenue
                ...
                client.query(`
                    ...
                    SELECT
                        COALESCE((SELECT active_orders FROM invoice_orders), 0) + COALESCE((SELECT active_orders FROM pos_orders), 0) + COALESCE((SELECT active_orders FROM storefront), 0) as active_orders,
                        COALESCE((SELECT pending_orders FROM invoice_orders), 0) + COALESCE((SELECT pending_orders FROM pos_orders), 0) + COALESCE((SELECT pending_orders FROM storefront), 0) as pending_orders,
                        COALESCE((SELECT paid_orders FROM invoice_orders), 0) + COALESCE((SELECT paid_orders FROM pos_orders), 0) + COALESCE((SELECT paid_orders FROM storefront), 0) as paid_orders
                `, [businessId]).catch(err => {
                    if (err.code === '42P01') {
                        return client.query(`
                            SELECT
                ...
                client.query(`
                    ...
                    combined_revenue AS (
                        SELECT 
                            COALESCE(mr.month, pr.month, sr.month) as month,
                            COALESCE(mr.revenue, 0) + COALESCE(pr.revenue, 0) + COALESCE(sr.revenue, 0) as revenue
                        FROM monthly_revenue mr
                        FULL OUTER JOIN pos_revenue pr ON mr.month = pr.month
