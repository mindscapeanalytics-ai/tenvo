...
async function checkAuth(businessId, permission = 'analytics.basic', client = null, featureKey = 'ai_analytics', options = {}) {
        ...
        try {
            const result = await client.query(`
                SELECT 
                    ...
                    COUNT(*) as count,
                    COALESCE(SUM(COALESCE(loc.sellable_qty, p.stock, 0) * COALESCE(p.price, 0)), 0) as value
                FROM products p
                LEFT JOIN (
...
export async function getKPIMetricsAction(businessId, filter = {}) {
    ...
    try {
        ...
        try {
            ...
            // 1. Inventory Asset Value (sellable location qty when present — matches hub display stock)
            const inventoryRes = await client.query(`
                SELECT COALESCE(SUM(COALESCE(loc.sellable_qty, p.stock, 0) * COALESCE(p.price, 0)), 0) as total_value
                FROM products p
                LEFT JOIN (
                    SELECT product_id, business_id, SUM(quantity) AS sellable_qty
                    FROM product_stock_locations
...
export async function getExpenseBreakdownAction(businessId, filter = {}) {
    try {
        const { skipAuth = false, ...rangeFilter } = filter || {};
        await checkAuth(businessId, 'analytics.basic', null, 'ai_analytics', { skipAuth });
        const client = await pool.connect();
        try {
