...
import {
    ...
    SALES_CATEGORY_REVENUE_UNIFIED_SQL,
    SALES_DAILY_REVENUE_UNIFIED_SQL,
...
import { isTrustedAuthBypassActive, runWithTrustedAuthBypass } from '@/lib/actions/_shared/trustedAuthBypass';
...
async function checkAuth(businessId, permission = 'analytics.basic', client = null, featureKey = 'ai_analytics') {
    ...
    const featureKeys = Array.isArray(featureKey) ? featureKey.filter(Boolean) : [featureKey].filter(Boolean);
    ...
    throw lastErr || new Error('This feature requires a plan upgrade.');
}

/** Reports tab unlocks on advanced_reports (Professional+); AI suite also on ai_analytics. */
const REPORTS_FEATURE_ANY = ['advanced_reports', 'ai_analytics'];
const FORECAST_FEATURE_ANY = ['advanced_reports', 'ai_forecasting', 'ai_analytics'];
 ...
 * @param {{ from: string; to: string; channel?: string; category?: string | null }} range
 */
async function fetchSalesTrendRows(client, businessId, range) {
    const channel = normalizeSalesChannel(range.channel);
    const category = normalizeSalesCategory(range.category);
    const result = await client.query(SALES_TREND_UNIFIED_SQL, [
        businessId,
        range.from,
        range.to,
        channel,
 ...
 */
async function fetchRevenueGrowthTotals(client, businessId, from, to, channel = 'all') {
    const ch = normalizeSalesChannel(channel);
    const growthRes = await client.query(REVENUE_GROWTH_VS_PRIOR_SQL, [businessId, from, to, ch]);
    const current = parseFloat(growthRes.rows[0]?.cur_total || 0);
    const last = parseFloat(growthRes.rows[0]?.prev_total || 0);
    const orderCount = parseInt(growthRes.rows[0]?.cur_orders || 0, 10) || 0;
    let growthPercent = 0;
    if (last > 0) growthPercent = ((current - last) / last) * 100;
    else if (current > 0) growthPercent = 100;
    const trend = growthPercent > 0 ? 'up' : growthPercent < 0 ? 'down' : 'neutral';
    return { current, last, growthPercent, orderCount, trend };
}
...
export async function getSalesTrendAction(businessId, filter = {}) {
    ...
    try {
        await checkAuth(businessId, 'analytics.basic', client, REPORTS_FEATURE_ANY);
        try {
            const range = resolveAnalyticsRange(filter);
            const data = await fetchSalesTrendRows(client, businessId, {
                from: range.from,
                to: range.to,
...
export async function getTopProductsAction(businessId, limit = 5, filter = {}) {
    ...
    try {
        ...
        try {
            ...
            const category = normalizeSalesCategory(filter.category);
            const result = await client.query(TOP_MOVING_PRODUCTS_SQL, [
                businessId,
                limit,
                from,
                to,
...
export async function getCategoryDistributionAction(businessId) {
    try {
        await checkAuth(businessId, 'analytics.basic', null, REPORTS_FEATURE_ANY);
        const client = await pool.connect();
        try {
            const result = await client.query(`
                SELECT 
                    ...
                    COUNT(*) as count,
                    COALESCE(SUM(COALESCE(loc.sellable_qty, p.stock, 0) * COALESCE(p.cost_price, 0)), 0) as value
                FROM products p
                LEFT JOIN (
                    SELECT product_id, business_id, SUM(quantity) AS sellable_qty
                    FROM product_stock_locations
                    WHERE business_id = $1
                      AND COALESCE(state, 'sellable') = 'sellable'
                    GROUP BY product_id, business_id
                ) loc ON loc.product_id = p.id AND loc.business_id = p.business_id
...
export async function getKPIMetricsAction(businessId, filter = {}) {
    ...
    try {
        ...
        try {
            ...
            // 1. Inventory Asset Value at cost (sellable location qty when present — matches hub KPIs)
            const inventoryRes = await client.query(`
                SELECT COALESCE(SUM(COALESCE(loc.sellable_qty, p.stock, 0) * COALESCE(p.cost_price, 0)), 0) as total_value
                FROM products p
                LEFT JOIN (
                    SELECT product_id, business_id, SUM(quantity) AS sellable_qty
                    FROM product_stock_locations
            ...
            const { current, last, growthPercent, orderCount, trend } = await fetchRevenueGrowthTotals(
                client,
                businessId,
                from,
                to,
...
export async function getDemandForecastAction(businessId, intelligence = {}, useAI = true, filter = {}) {
    const client = await pool.connect();
    try {
        await checkAuth(businessId, 'analytics.basic', client, FORECAST_FEATURE_ANY);
        try {
...
/**
 * Get Dashboard Metrics (legacy template shape).
 * Delegates to date-range getDashboardKPIs (same truth as hub shell / advanced snapshot).
 * Optional filter: { from, to } via resolveAnalyticsRange — never calendar-month-only SQL.
 * @param {string} businessId
 * @param {{ from?: unknown; to?: unknown }} [filter]
 */
export async function getDashboardMetricsAction(businessId, filter = {}) {
    try {
        const { skipAuth: _ignoredSkipAuth, ...rangeFilter } = filter || {};
        await checkAuth(businessId, 'analytics.basic', null, REPORTS_FEATURE_ANY);
        const { from, to } = resolveAnalyticsRange(rangeFilter);
        const fromDate = new Date(`${from}T00:00:00`);
        const toDate = new Date(`${to}T23:59:59.999`);

        const kpisRes = await runWithTrustedAuthBypass(businessId, () =>
        ...
        if (!kpisRes?.success || !kpisRes.data) {
            return { success: false, error: kpisRes?.error || 'Could not load dashboard KPIs' };
        }

        const d = kpisRes.data;
        const revenue = Number(d.revenue?.total || 0);
        const orderTotal = Number(d.orders?.total || 0);
        const invoiceCount = Number(d.orders?.invoices || 0);

        // Lightweight status split for legacy EnhancedDashboard cards (invoice channel).
        const client = await pool.connect();
        let pending = 0;
        let paid = 0;
        try {
            const statusRes = await client.query(
                `SELECT
                    COUNT(*) FILTER (
                      WHERE status IN ${PENDING_INVOICE_STATUS_SQL_IN}
            ...
            pending = Number(statusRes.rows[0]?.pending_orders || 0);
            paid = Number(statusRes.rows[0]?.paid_orders || 0);
        } finally {
            client.release();
        }
        ...
        return {
            ...
            data: {
                ...
                orders: {
                    total: orderTotal,
                    pending,
                    paid: paid || Math.max(0, invoiceCount - pending),
                },
                products: {
                    count: Number(d.inventory?.activeProducts || 0),
                    growth: 0,
                },
                customers: {
                    active: Number(d.entities?.activeCustomers || 0),
                    growth: 0,
                },
                cashFlow: {
                    current: Number(d.payments?.netCashFlow || 0),
                    growth: 0,
                },
                growth: {
                    value: '+0%',
                ...
                alerts: {
                    lowStock: Number(d.inventory?.lowStockCount || 0),
                    overdueInvoices: Number(d.receivables?.overdueCount || 0),
                },
                period: d.period || { from: fromDate.toISOString(), to: toDate.toISOString() },
            },
        };
    } catch (error) {
 ...
 *
 * @param {string} businessId
 * @param {{ from?: unknown; to?: unknown; channel?: unknown; category?: unknown }} [filter]
 */
export async function getAnalyticsBundleAction(businessId, filter = {}) {
    ...
    try {
        ...
        try {
            ...
            const channel = normalizeSalesChannel(filter.channel);
            const category = normalizeSalesCategory(filter.category);
            const perfRange = resolveSalesPerformanceRange(filter);

            const categoryAssetSql = `
                SELECT 
                    COALESCE(p.category, 'Uncategorized') as name,
                    COUNT(*) as count,
                    COALESCE(SUM(COALESCE(loc.sellable_qty, p.stock, 0) * COALESCE(p.cost_price, 0)), 0) as value
            ...
            const [
                ...
                fetchSalesTrendRows(client, businessId, {
                    from: range.from,
                    to: range.to,
                    channel,
                    category,
                }),
                client.query(TOP_MOVING_PRODUCTS_SQL, [
                    businessId,
                    5,
                    range.from,
                    range.to,
                ...
                client.query(categoryAssetSql, [businessId]),
                client.query(SALES_CATEGORY_REVENUE_UNIFIED_SQL, [
                    businessId,
                    range.from,
                    range.to,
                    channel,
                ...
                client.query(SALES_DAILY_REVENUE_UNIFIED_SQL, [
                    businessId,
                    range.from,
                    range.to,
                    channel,
                ...
                client.query(inventoryAssetSql, [businessId]),
                client.query(
                    `SELECT COUNT(*)::int AS c FROM products WHERE business_id = $1 AND is_active = true AND COALESCE(is_deleted, false) = false`,
                    [businessId]
                ),
                fetchRevenueGrowthTotals(client, businessId, range.from, range.to, channel),
                ...
                client.query(expenseSql, [businessId, range.from, range.to]),
                client.query(SALES_PRODUCT_CATEGORIES_SQL, [businessId]),
                client.query(SALES_KPI_PERIOD_SQL, [businessId, range.from, range.to, channel]),
                ...
                client.query(SALES_ORDER_STATUS_PERIOD_SQL, [businessId, range.from, range.to]),
                client.query(SALES_ORDER_STATUS_PERIOD_SQL, [
                    businessId,
                    perfRange.prevFrom,
                    perfRange.prevTo,
                ]),
            ...
            const categoryRevenueData = categoryRevenueRes.rows.map(mapCategoryRevenueRow);
            const dailyRevenueTrend = dailyRevenueRes.rows.map(mapDailyRevenueRow);

            const productCount = parseInt(productCountRes.rows[0]?.c ?? 0, 10) || 0;
            const { current, last, growthPercent, orderCount, trend } = growthTotals;

            const repeat = parseInt(retentionRes.rows[0]?.repeat_customers || 0, 10);
            const total = parseInt(retentionRes.rows[0]?.total_customers || 0, 10);
            const retentionRate = total > 0 ? (repeat / total) * 100 : 0;
            ...
            const productCategories = (categoriesRes.rows || []).map((r) => r.category).filter(Boolean);
            ...
            const orderStatus = {
                current: mapOrderStatusRow(curOrderStatusRes.rows[0] || {}),
                previous: mapOrderStatusRow(prevOrderStatusRes.rows[0] || {}),
            };

            return {
                success: true,
                data: {
                    range: { ...range, channel, category },
                    salesTrend,
                    topProducts,
                    categoryData,
                    categoryRevenueData,
...
export async function getDomainIndustryInsightsAction(businessId) {
    ...
    try {
        ...
        const { category: rawCategory, domainKnowledge } = marketCtx;
        const category = resolveDomainKey(String(rawCategory || 'retail-shop'));
        ...
        // Water / milk: vertical-specific peak copy beats generic peak season text.
        if (category === 'water-delivery') {
            const rates = intelligence.typical19LRefillRatePkr || {};
            const commonRate = rates.common || 150;
            if (isPeakSeason) {
                priority = 'high';
                insight += `Pakistan bottled-water peak (${currentMonth}): heat and wedding / Ramadan demand lift 19L refill and PET cases. `;
                insight += `Raise empty bottle float (BAL = prev + DEL − REC) and rider capacity by town code. `;
                suggestedAction = `Increase 19L refill and deposit float ~${intelligence.smartRestockBufferPctPeak || 25}% above daily pars; sequence villa routes early in monsoon weeks.`;
            } else {
                insight += 'Pakistan water supply: plant-to-rider home & flat plus corporate weekday cadence. ';
                insight += `Typical 19L refill around Rs ${commonRate}; keep PSQCA license and deposit ledger current. `;
                suggestedAction =
                    'Open Daily Route for DEL/REC/BAL, chase monthly credit with JazzCash/Easypaisa, and restock 19L empties before the next heat wave.';
            }
        } else if (category === 'milk-shop') {
            if (isPeakSeason) {
                priority = 'high';
                insight += `Peak dairy demand (${currentMonth}): raise morning supply and chill FEFO coverage. `;
                suggestedAction = 'Review Daily Route house kg pars and clear near-expiry chill packs before midday.';
            } else {
                insight += 'Neighborhood milk shop: counter kg sales plus doorstep Route Hisab credit. ';
                suggestedAction = 'Keep chill FEFO tight; collect weekly/monthly house credit via JazzCash, cash, or invoice receipt.';
            }
