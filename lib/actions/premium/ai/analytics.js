...
export async function getDashboardMetricsAction(businessId) {
    ...
    try {
        await checkAuth(businessId, 'analytics.basic', client, REPORTS_FEATURE_ANY);
        try {
            // Batch 1: Run revenue, orders, products, and growth queries in parallel
            const [revenueRes, ordersRes, productsRes, growthRes] = await Promise.all([
                // 1. Revenue — operational ledgers only (never GL + channel; POS/invoices already post to GL)
                client.query(`
                    WITH invoice_revenue AS (
                        ...
                        WHERE business_id = $1
                          ...
                          AND LOWER(COALESCE(status, '')) NOT IN ('draft', 'voided', 'cancelled')
                    ...
                    SELECT
                        COALESCE((SELECT total_revenue FROM invoice_revenue), 0)
                        + COALESCE((SELECT total_revenue FROM pos_revenue), 0)
                        + COALESCE((SELECT total_revenue FROM storefront_revenue), 0)
                        + COALESCE((SELECT total_revenue FROM restaurant_revenue), 0) as total_revenue
                `, [businessId]),
                // 2. Orders Count (UNIFIED: invoices + POS + storefront + restaurant)
                client.query(`
                    WITH invoice_orders AS (
                        SELECT
                            ...
                            COUNT(*) FILTER (WHERE status IN ${PENDING_INVOICE_STATUS_SQL_IN} AND COALESCE(is_deleted, false) = false) as pending_orders,
                            COUNT(*) FILTER (WHERE status = 'paid' AND COALESCE(is_deleted, false) = false) as paid_orders
                        FROM invoices
                        WHERE business_id = $1
                          AND date >= date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
                    ),
                    pos_orders AS (
                        SELECT
                            ...
                            COUNT(*) FILTER (WHERE is_voided = false AND LOWER(COALESCE(payment_status, '')) IN ('pending', 'processing')) as pending_orders,
                            COUNT(*) FILTER (WHERE is_voided = false AND LOWER(COALESCE(payment_status, '')) = 'completed') as paid_orders
                        FROM pos_transactions
                        WHERE business_id = $1
                          AND created_at >= date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
                    ),
                    ...
                    restaurant AS (
                        SELECT
                            COUNT(*) FILTER (WHERE LOWER(COALESCE(status, '')) IN ('completed', 'served')) as active_orders,
                            COUNT(*) FILTER (WHERE LOWER(COALESCE(status, '')) IN ('pending', 'preparing', 'ready')) as pending_orders,
                            COUNT(*) FILTER (WHERE LOWER(COALESCE(status, '')) IN ('completed', 'served')) as paid_orders
                        FROM restaurant_orders
                        WHERE business_id = $1
                          AND created_at >= date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
                    )
                    SELECT
                        COALESCE((SELECT active_orders FROM invoice_orders), 0)
                          ...
                          + COALESCE((SELECT active_orders FROM storefront), 0)
                          + COALESCE((SELECT active_orders FROM restaurant), 0) as active_orders,
                        COALESCE((SELECT pending_orders FROM invoice_orders), 0)
                          + COALESCE((SELECT pending_orders FROM pos_orders), 0)
                          + COALESCE((SELECT pending_orders FROM storefront), 0)
                          + COALESCE((SELECT pending_orders FROM restaurant), 0) as pending_orders,
                        COALESCE((SELECT paid_orders FROM invoice_orders), 0)
                          + COALESCE((SELECT paid_orders FROM pos_orders), 0)
                          + COALESCE((SELECT paid_orders FROM storefront), 0)
                          + COALESCE((SELECT paid_orders FROM restaurant), 0) as paid_orders
                ...
                client.query(`
                    SELECT 
                        COUNT(*) FILTER (WHERE true) as current_count,
                        COUNT(*) FILTER (WHERE created_at < date_trunc('month', CURRENT_DATE)) as last_month_count
                    FROM products
                    WHERE business_id = $1 AND is_active = true
                `, [businessId]),
                // 4. Growth — operational ledgers only (aligned with getDashboardKPIs)
                client.query(`
                    WITH invoice_revenue AS (
                        SELECT
                    ...
                    combined_revenue AS (
                        SELECT
                            COALESCE(ir.month, pr.month, sr.month, rr.month) as month,
                            COALESCE(ir.revenue, 0) + COALESCE(pr.revenue, 0)
                              + COALESCE(sr.revenue, 0) + COALESCE(rr.revenue, 0) as revenue
                        FROM invoice_revenue ir
                        FULL OUTER JOIN pos_revenue pr ON ir.month = pr.month
                        FULL OUTER JOIN storefront_revenue sr ON COALESCE(ir.month, pr.month) = sr.month
                        FULL OUTER JOIN restaurant_revenue rr
                          ON COALESCE(ir.month, pr.month, sr.month) = rr.month
                    )
                    SELECT
            ...
            // Batch 2: Run customer/cashflow and alerts queries in parallel
            const [extraMetricsRes, alertsRes] = await Promise.all([
                // 5. Active Customers + Cash Flow
                client.query(`
                ...
                // 6. Alerts — LEFT JOIN sellable location aggregates (avoids per-row correlated SUM).
                // Threshold order matches resolveSafetyStock / Easy countLowStockProducts (default 10).
                client.query(`
                    SELECT 
                        (
                            SELECT COUNT(*)
                            ...
                            WHERE p.business_id = $1
                              ...
                              AND COALESCE(p.is_deleted, false) = false
                              AND COALESCE(loc.sellable_qty, p.stock, 0)
                                  <= COALESCE(
                                       NULLIF(p.reorder_point, 0),
                                       NULLIF(p.min_stock_level, 0),
                                       NULLIF(p.min_stock, 0),
                                       10
                                     )
                        ) as low_stock_count,
                        (SELECT COUNT(*) FROM invoices WHERE business_id = $1 AND COALESCE(is_deleted, false) = false AND status NOT IN ('paid', 'cancelled', 'draft', 'voided') AND (
                            LOWER(status) LIKE '%overdue%'
                            OR LOWER(status) = 'overdue'
                            OR (due_date IS NOT NULL AND due_date < CURRENT_DATE AND status NOT IN ('paid', 'cancelled', 'draft', 'voided'))
                        )) as overdue_invoices
                `, [businessId]),
            ]);
