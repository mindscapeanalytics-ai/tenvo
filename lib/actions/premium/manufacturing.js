...
async function checkAuthAndPlan(businessId) {
    ...
    await checkPlanFeature(businessId, 'manufacturing');
    return session;
}
...
export async function getBOMsAction(businessId) {
    try {
        await checkAuthAndPlan(businessId);
...
export async function createBOMAction(bomData) {
    ...
    try {
        await checkAuthAndPlan(bomData.business_id);
