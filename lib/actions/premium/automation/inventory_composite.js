'use server';

import pool from '@/lib/db';
import { withGuard } from '@/lib/rbac/serverGuard';
import { InventoryService } from '@/lib/services/InventoryService';
import { ProductService } from '@/lib/services/ProductService';
import { assertEntityBelongsToBusiness } from '@/lib/actions/_shared/tenant';
import {
    filterMeaningfulBatches,
    filterMeaningfulSerials,
    isMeaningfulInventoryBatch,
    isMeaningfulInventorySerial,
} from '@/lib/utils/inventoryTrackingHelpers';
import { serializeDecimalsDeep } from '@/lib/utils/serializePrismaDecimals';
import { VariantService } from '@/lib/services/VariantService';
import { matrixToVariantRows, mergeVariantSources, totalVariantStock } from '@/lib/utils/variantSync';
import { invalidateStorefrontCatalog } from '@/lib/storefront/invalidateStorefrontCatalog';
import { loadProductStockSnapshot } from '@/lib/utils/inventoryEffectiveStock';
import { enrichProductDataWithCategoryId } from '@/lib/utils/productCategoryLink';
import { StorefrontSyncService } from '@/lib/services/StorefrontSyncService';
import {
    normalizeProductImageUrls,
    productImagesFromUrls,
} from '@/lib/utils/productImages';

async function checkAuthAndPlan(businessId, permission = 'inventory.create', client = null, feature = null) {
    const { session } = await withGuard(businessId, {
        permission,
        client,
        ...(feature ? { feature } : {}),
    });
    return session;
}

/**
 * Stock movements must run on the SAME pg client as the composite transaction.
 * Calling addStockAction/removeStockAction opens a new connection, under READ COMMITTED
 * the new product row from INSERT is invisible until COMMIT → "Product not found".
 */
async function invokeAddStockInTx(payload, client, userId) {
    await InventoryService.addStock(payload, userId, client);
}

async function invokeRemoveStockInTx(payload, client, userId) {
    await InventoryService.removeStock(payload, userId, client);
}

async function reconcileInlineStockEdit(client, {
    businessId,
    productId,
    targetStock,
    costPrice,
    userId,
    note = 'Manual Stock Adjustment',
}) {
    const { effectiveStock } = await loadProductStockSnapshot(client, productId, businessId);
    const newStock = Number(targetStock);
    if (!Number.isFinite(newStock) || newStock === effectiveStock) {
        return;
    }
    const delta = newStock - effectiveStock;
    if (delta > 0) {
        await invokeAddStockInTx({
            business_id: businessId,
            product_id: productId,
            warehouse_id: null,
            quantity: delta,
            cost_price: Number(costPrice) || 0,
            notes: note,
            reference_type: 'adjustment',
            reference_id: productId,
        }, client, userId);
    } else {
        await invokeRemoveStockInTx({
            business_id: businessId,
            product_id: productId,
            warehouse_id: null,
            quantity: Math.abs(delta),
            notes: note,
            reference_type: 'adjustment',
            reference_id: productId,
        }, client, userId);
    }

    const singleBatchRes = await client.query(
        `SELECT id FROM product_batches
         WHERE product_id = $1::uuid AND business_id = $2::uuid
           AND is_active = true AND (is_deleted = false OR is_deleted IS NULL)`,
        [productId, businessId]
    );
    if (singleBatchRes.rows.length === 1) {
        await client.query(
            `UPDATE product_batches SET quantity = $1, updated_at = NOW() WHERE id = $2`,
            [newStock, singleBatchRes.rows[0].id]
        );
    }
}

/**
 * Server-side orchestration for atomic product creation/update with inventory
 * This ensures consistency and avoids race conditions or duplicate batch errors.
 */
export async function upsertIntegratedProductAction(params) {
    const {
        productData,
        batches = [],
        serialNumbers = [],
        variants = [],
        initialStock = 0,
        isUpdate = false,
        productId = null
    } = params;

    const businessId = productData.business_id;

    const client = await pool.connect();
    try {
        const permission = isUpdate ? 'inventory.edit' : 'inventory.create';
        const batchesInput = filterMeaningfulBatches(Array.isArray(batches) ? batches : []);
        const serialsInput = filterMeaningfulSerials(Array.isArray(serialNumbers) ? serialNumbers : []);

        const session = await checkAuthAndPlan(businessId, permission, client);
        // Plan gates for tracking payloads (sidebar already gates tabs; writes must match)
        if (batchesInput.length > 0) {
            await checkAuthAndPlan(businessId, permission, client, 'batch_tracking');
        }
        if (serialsInput.length > 0) {
            await checkAuthAndPlan(businessId, permission, client, 'serial_tracking');
        }
        const userId = session?.user?.id != null ? String(session.user.id) : 'system';

        await client.query('BEGIN');

        let finalProductId = productId;

        const hasBatchesOrSerials =
            batchesInput.length > 0 || serialsInput.length > 0;

        const safeFields = [
            'business_id', 'name', 'sku', 'barcode', 'category', 'category_id', 'brand', 'unit',
            'description', 'price', 'cost_price', 'mrp', 'tax_percent', 'min_stock', 'max_stock', 'min_stock_level',
            'reorder_point', 'reorder_quantity', 'location', 'image_url', 'images',
            'is_featured', 'expiry_date', 'batch_number',
            'manufacturing_date',
            'domain_data', 'hsn_code', 'sac_code', 'is_active', 'unit_conversions',
            // CRITICAL: 'tracking_mode' and 'attributes' columns do NOT exist in Prisma schema
            // and have been removed to prevent SQL errors (42703: column does not exist).
            // Size/color variants live in `product_variants` and must be written
            // via VariantService / the VariantMatrixEditor dialog — never as raw products columns.
            // Include 'stock' for products without batch/serial tracking
            // (batch/serial products derive stock from their sub-records)
            ...(!hasBatchesOrSerials ? ['stock'] : []),
        ];

        const numericFieldMap = {
            price: 0,
            cost_price: 0,
            mrp: 0,
            tax_percent: 0,
            min_stock: 0,
            max_stock: 0,
            min_stock_level: 0,
            reorder_point: 0,
            reorder_quantity: 0,
            stock: 0,
        };

        const dateFields = new Set(['expiry_date', 'manufacturing_date']);

        const sanitizeProductData = (data) => {
            const sanitized = {};
            Object.keys(data).forEach(key => {
                if (safeFields.includes(key)) {
                    const value = data[key];

                    if (key === 'domain_data') {
                        let dd = value;
                        if (typeof dd === 'string') {
                            const t = (dd || '').trim();
                            if (!t || t === '[object Object]') {
                                dd = {};
                            } else {
                                try {
                                    dd = JSON.parse(t);
                                } catch {
                                    dd = {};
                                }
                            }
                        }
                        if (dd === null || dd === undefined || typeof dd !== 'object' || Array.isArray(dd)) {
                            dd = {};
                        }
                        sanitized[key] = dd;
                        return;
                    }

                    if (Object.prototype.hasOwnProperty.call(numericFieldMap, key)) {
                        if (value === '' || value === null || value === undefined) {
                            // Blank stock on update must not zero inventory (Busy non-stock cell commits).
                            if (key === 'stock' && isUpdate) return;
                            sanitized[key] = numericFieldMap[key];
                        } else {
                            const parsed = Number(value);
                            sanitized[key] = Number.isFinite(parsed) ? parsed : numericFieldMap[key];
                        }
                        return;
                    }

                    if (dateFields.has(key)) {
                        sanitized[key] = value === '' ? null : value;
                        return;
                    }

                    sanitized[key] = value === '' ? null : value;
                }
            });
            return sanitized;
        };

        const sanitizedData = sanitizeProductData(productData);

        // Persist gallery JSON + keep primary image_url in sync (max 3).
        // Only when the payload includes image fields so partial grid updates do not wipe media.
        // Busy/Excel stock-only rows may send empty image_url — never clear existing media then.
        const hasImagePayload =
            Object.prototype.hasOwnProperty.call(productData, 'images') ||
            Object.prototype.hasOwnProperty.call(productData, 'image_url');
        if (hasImagePayload) {
            const urls = normalizeProductImageUrls({
                images: productData.images,
                image_url: productData.image_url,
            });
            const intentionalGallery =
                Array.isArray(productData.images) ||
                Array.isArray(productData.productImageUrls);
            if (urls.length > 0) {
                const pack = productImagesFromUrls(
                    urls,
                    String(productData.name || sanitizedData.name || '')
                );
                sanitizedData.images = pack.images;
                sanitizedData.image_url = pack.image_url;
            } else if (intentionalGallery || !isUpdate) {
                const pack = productImagesFromUrls([], String(productData.name || sanitizedData.name || ''));
                sanitizedData.images = pack.images;
                sanitizedData.image_url = pack.image_url;
            } else {
                delete sanitizedData.image_url;
                delete sanitizedData.images;
            }
        }

        if (!sanitizedData.category_id && sanitizedData.category) {
            const enriched = await enrichProductDataWithCategoryId(client, businessId, sanitizedData);
            if (enriched.category_id) sanitizedData.category_id = enriched.category_id;
        }

        const variantRows = (() => {
            if (Array.isArray(variants) && variants.length > 0) return variants;
            const fromPayload = mergeVariantSources(
                null,
                productData.variants,
                {
                    sku: sanitizedData.sku,
                    price: sanitizedData.price,
                    cost_price: sanitizedData.cost_price,
                    mrp: sanitizedData.mrp,
                    min_stock: sanitizedData.min_stock,
                }
            );
            if (fromPayload.length > 0) return fromPayload;
            const matrix = sanitizedData.domain_data?.size_color_matrix;
            if (matrix && typeof matrix === 'object') {
                return matrixToVariantRows(matrix, {
                    sku: sanitizedData.sku,
                    price: sanitizedData.price,
                    cost_price: sanitizedData.cost_price,
                    mrp: sanitizedData.mrp,
                    min_stock: sanitizedData.min_stock,
                });
            }
            return [];
        })();

        const hasVariants = variantRows.length > 0;

        // 1. Upsert Product
        if (isUpdate && productId) {
            await assertEntityBelongsToBusiness(client, 'product', productId, businessId);
            // Update logic
            const fields = Object.keys(sanitizedData).filter(f => f !== 'id' && f !== 'business_id');
            if (fields.length > 0) {
                const values = fields.map(f => sanitizedData[f]);
                const setClause = fields.map((f, i) => `${f} = $${i + 3}`).join(', ');

                await client.query(`
                    UPDATE products 
                    SET ${setClause}, updated_at = NOW() 
                    WHERE id = $1 AND business_id = $2
                `, [productId, businessId, ...values]);
            } else {
                await client.query(
                    `UPDATE products SET updated_at = NOW() WHERE id = $1 AND business_id = $2`,
                    [productId, businessId]
                );
            }
        } else {
            // Create logic
            const fields = Object.keys(sanitizedData);
            const values = Object.values(sanitizedData);
            const placeholders = fields.map((_, i) => `$${i + 1}`).join(', ');

            const res = await client.query(`
                INSERT INTO products (${fields.join(', ')})
                VALUES (${placeholders})
                RETURNING id
            `, values);
            finalProductId = res.rows[0].id;
        }

        // 2. Handle Batches (Delta Reconciliation)
        if (batchesInput.length > 0) {
            // Fetch existing batches to compare
            const existingBatchesRes = await client.query(
                `SELECT id, quantity, batch_number FROM product_batches
                 WHERE product_id = $1 AND business_id = $2
                   AND COALESCE(is_deleted, false) = false`,
                [finalProductId, businessId]
            );
            const existingBatches = existingBatchesRes.rows;

            for (const batch of batchesInput) {
                if (!isMeaningfulInventoryBatch(batch) && !batch?.id) continue;
                const incomingQty = Number(batch.quantity) || 0;
                // Match by ID if exists (reliable), else fallback to batch_number+warehouse (if we had it), else treat as new
                const existing = existingBatches.find(eb =>
                    (batch.id && String(batch.id) === String(eb.id)) ||
                    (!batch.id && batch.batch_number === eb.batch_number) // fallback for new entries in UI that might match existing
                );

                if (existing) {
                    const currentQty = Number(existing.quantity);
                    const delta = incomingQty - currentQty;

                    if (delta > 0) {
                        // Stock In
                        await invokeAddStockInTx({
                            business_id: businessId,
                            product_id: finalProductId,
                            warehouse_id: batch.warehouse_id || batch.warehouseId || null,
                            quantity: delta,
                            cost_price: Number(batch.cost_price || batch.costPrice || productData.cost_price || 0),
                            batch_number: existing.batch_number,
                            notes: 'Batch Quantity Adjustment (Increase)',
                            reference_type: 'adjustment',
                            reference_id: finalProductId
                        }, client, userId);
                    } else if (delta < 0) {
                        // Stock Out
                        await invokeRemoveStockInTx({
                            business_id: businessId,
                            product_id: finalProductId,
                            warehouse_id: batch.warehouse_id || batch.warehouseId || null,
                            quantity: Math.abs(delta),
                            batch_id: existing.id,
                            notes: 'Batch Quantity Adjustment (Decrease)',
                            reference_type: 'adjustment',
                            reference_id: finalProductId
                        }, client, userId);
                    } else {
                        // No quantity change, but maybe metadata update?
                        await client.query(`
                            UPDATE product_batches 
                            SET expiry_date = $1, manufacturing_date = $2, cost_price = $3, updated_at = NOW()
                            WHERE id = $4 AND business_id = $5
                        `, [
                            batch.expiry_date || batch.expiryDate || null,
                            batch.manufacturing_date || batch.manufacturingDate || null,
                            batch.cost_price || batch.costPrice || 0,
                            existing.id,
                            businessId
                        ]);
                    }
                } else {
                    // New Batch -> Stock In
                    if (incomingQty > 0) {
                        await invokeAddStockInTx({
                            business_id: businessId,
                            product_id: finalProductId,
                            warehouse_id: batch.warehouse_id || batch.warehouseId || null,
                            quantity: incomingQty,
                            cost_price: Number(batch.cost_price || batch.costPrice || productData.cost_price || 0),
                            batch_number: batch.batch_number || batch.batchNumber,
                            manufacturing_date: batch.manufacturing_date || batch.manufacturingDate || null,
                            expiry_date: batch.expiry_date || batch.expiryDate || null,
                            notes: batch.notes || 'Opening Balance (Batch)',
                            reference_type: 'adjustment',
                            reference_id: finalProductId
                        }, client, userId);
                    }
                }
            }
        }
        // 3. Handle Serials (Delta Reconciliation)
        else if (serialsInput.length > 0) {
            // For serials, "Quantity" is always 1 per serial.
            // We check if the serial exists. If not, add it.
            // If it exists but is 'sold', maybe we don't reactivate it here unless explicitly requested?
            // For now, let's assume this form only sends "Available" serials or new ones.

            // 1. Get existing active serials
            const existingSerialsRes = await client.query(
                `SELECT serial_number FROM product_serials
                 WHERE product_id = $1 AND business_id = $2
                   AND COALESCE(is_deleted, false) = false
                   AND LOWER(COALESCE(status, '')) IN ('available', 'in_stock')`,
                [finalProductId, businessId]
            );
            const existingSet = new Set(existingSerialsRes.rows.map(r => r.serial_number));

            for (const serial of serialsInput) {
                const sn = serial.serial_number || serial.serialNumber;
                if (!existingSet.has(sn)) {
                    // New Serial -> Add Stock (Qty 1)
                    // Insert Serial record first (or let addStock handle it? addStock handles basics, but we might want explicit control)
                    // addStockAction currently has a hacky "serialNumbers" param but it's better to explicitly insert here if possible, 
                    // OR just rely on addStockAction to do it. 
                    // Let's use addStockAction as it handles Ledger + Product Stock update.

                    // Insert Serial record
                    await client.query(`
                        INSERT INTO product_serials (
                            business_id, product_id, serial_number, imei, status, warehouse_id
                        ) VALUES ($1, $2, $3, $4, 'available', $5)
                        ON CONFLICT (business_id, serial_number) WHERE COALESCE(is_deleted, false) = false DO NOTHING
                    `, [businessId, finalProductId, sn, serial.imei || null, serial.warehouse_id || null]);

                    // Record movement
                    await invokeAddStockInTx({
                        business_id: businessId,
                        product_id: finalProductId,
                        warehouse_id: serial.warehouse_id || null,
                        quantity: 1,
                        cost_price: Number(productData.cost_price || 0),
                        notes: `Stock Addition (Serial: ${sn})`,
                        reference_type: 'adjustment',
                        reference_id: finalProductId
                    }, client, userId);
                }
            }

            // Detect Deleted Serials? 
            // If a serial was in existingSet but NOT in incoming serialNumbers, it means it was removed from the list.
            // Should we mark it as 'removed'? 
            // For safety, let's strictly handle ADDITIONS for now to avoid accidental bulk deletions.
            // If user wants to delete a serial, they should use a specific "Delete Serial" action or we implement strictly mirroring the list.
            // PROCEEDING WITH ADDITION-ONLY FOR SAFETY as per "Split Brain" fix scope.
        }
        // 4. Handle Simple Stock (no batches/serials/variants)
        else if (!hasBatchesOrSerials && !hasVariants) {
            if (!isUpdate && initialStock > 0) {
                // New product with opening stock
                await invokeAddStockInTx({
                    business_id: businessId,
                    product_id: finalProductId,
                    warehouse_id: null,
                    quantity: initialStock,
                    cost_price: Number(productData.cost_price || 0),
                    notes: 'Opening Balance',
                    reference_type: 'adjustment',
                    reference_id: finalProductId
                }, client, userId);
            } else if (isUpdate && productData.stock !== undefined) {
                await reconcileInlineStockEdit(client, {
                    businessId,
                    productId: finalProductId,
                    targetStock: productData.stock,
                    costPrice: productData.cost_price,
                    userId,
                });
            }
        }
        // 4b. Handle product variants (size/color matrix)
        else if (hasVariants) {
            await VariantService.syncProductVariants(
                finalProductId,
                businessId,
                variantRows,
                {
                    baseSku: sanitizedData.sku,
                    basePrice: sanitizedData.price,
                    baseCostPrice: sanitizedData.cost_price,
                    baseMrp: sanitizedData.mrp,
                },
                client
            );
        }

        // 5. Headline stock when batch/serial mode omitted `stock` from UPDATE but the client sent an explicit
        //    quantity (BusyGrid / dashboard inline edit). Skip when this request already moved batch qty —
        //    otherwise a stale headline undoes the batch delta.
        const batchQtyMoved = batchesInput.some((batch) => {
            if (!isMeaningfulInventoryBatch(batch) && !batch?.id) return false;
            return Object.prototype.hasOwnProperty.call(batch, 'quantity');
        });
        if (
            isUpdate &&
            finalProductId &&
            productId &&
            hasBatchesOrSerials &&
            !batchQtyMoved &&
            productData &&
            Object.prototype.hasOwnProperty.call(productData, 'stock')
        ) {
            await reconcileInlineStockEdit(client, {
                businessId,
                productId: finalProductId,
                targetStock: productData.stock,
                costPrice: productData.cost_price,
                userId,
                note: 'Manual Stock Adjustment (headline)',
            });
        }

        await client.query('COMMIT');

        if (finalProductId) {
            if (!isUpdate) {
                await StorefrontSyncService.autoPublishProduct(finalProductId, businessId);
            } else if (Object.prototype.hasOwnProperty.call(sanitizedData, 'is_active')) {
                // Only touch is_active when the client explicitly sent it (avoid reactivating archived SKUs).
                await client.query(
                    `UPDATE products
                     SET is_active = $3,
                         stock_status = CASE
                           WHEN COALESCE(stock, 0) <= 0 THEN 'out_of_stock'
                           ELSE 'in_stock'
                         END,
                         updated_at = NOW()
                     WHERE id = $1::uuid AND business_id = $2::uuid`,
                    [finalProductId, businessId, Boolean(sanitizedData.is_active)]
                );
            } else {
                await client.query(
                    `UPDATE products
                     SET stock_status = CASE
                           WHEN COALESCE(stock, 0) <= 0 THEN 'out_of_stock'
                           ELSE 'in_stock'
                         END,
                         updated_at = NOW()
                     WHERE id = $1::uuid AND business_id = $2::uuid`,
                    [finalProductId, businessId]
                );
            }
        }

        invalidateStorefrontCatalog(businessId);

        // Return sanitized hub shape (display_stock + relations) — not raw SELECT *.
        const product = await ProductService.getProduct(finalProductId, businessId);
        const serialized = product ? serializeDecimalsDeep(product) : serializeDecimalsDeep({ id: finalProductId });
        return { success: true, product: serialized };

    } catch (error) {
        await client.query('ROLLBACK');
        console.error('Composite Upsert Error:', error);
        return { success: false, error: error.message };
    } finally {
        client.release();
    }
}

/**
 * Bulk composite upsert for Excel mode / imports (Busy/Zoho-style batch entry).
 * Each row runs through the same atomic upsert + InventoryService ledger path.
 */
export async function bulkUpsertIntegratedProductsAction(businessId, items, options = {}) {
    const { stopOnFirstError = false } = options;

    if (!businessId) {
        return { success: false, error: 'business_id is required' };
    }
    if (!Array.isArray(items) || items.length === 0) {
        return { success: false, error: 'No items to save' };
    }

    const results = { created: 0, updated: 0, failed: [], products: [] };

    for (let index = 0; index < items.length; index++) {
        const item = items[index];
        try {
            const res = await upsertIntegratedProductAction({
                ...item,
                productData: { ...item.productData, business_id: businessId },
            });
            if (res.success) {
                if (item.isUpdate) results.updated += 1;
                else results.created += 1;
                if (res.product?.id) {
                    results.products.push(res.product);
                }
            } else {
                results.failed.push({
                    index,
                    error: res.error || 'Upsert failed',
                    name: item.productData?.name,
                });
                if (stopOnFirstError) break;
            }
        } catch (error) {
            results.failed.push({
                index,
                error: error.message || 'Upsert failed',
                name: item.productData?.name,
            });
            if (stopOnFirstError) break;
        }
    }

    if (results.created + results.updated > 0) {
        invalidateStorefrontCatalog(businessId);
    }

    return {
        success: results.failed.length === 0,
        ...results,
        total: items.length,
    };
}
