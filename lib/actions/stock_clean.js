            ...
            if (serialNumbers && serialNumbers.length > 0) {
                for (const sn of serialNumbers) {
                    await client.query(`
                        ...
                        ON CONFLICT (business_id, serial_number) WHERE COALESCE(is_deleted, false) = false
                        DO UPDATE SET 
                            status = 'available',
                            updated_at = NOW()
                ...
                if (quantityChange < 0) {
                    ...
                    for (const s of serialNumbers) {
                        await client.query(`
                            ...
                            ON CONFLICT (business_id, serial_number) WHERE COALESCE(is_deleted, false) = false DO UPDATE SET status = 'in_stock', warehouse_id = EXCLUDED.warehouse_id, updated_at = NOW()
