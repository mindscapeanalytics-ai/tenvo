 ...
 * Ages on bill due date = purchase date + vendor/business payment terms (mirrors A/R due_date logic).
 * Paid amounts stay on payment_allocations (canonical vendor ledger — no purchase_payments table).
 */
        ...
        const result = await client.query(
            ...
            SELECT
                ...
                (
                    ...
                    + (
                        COALESCE(
                            NULLIF(SUBSTRING(COALESCE(v.payment_terms, '') FROM '([0-9]{1,4})'), '')::int,
                            NULLIF(b.default_payment_terms, 0),
                            30
                        )
                    ) * INTERVAL '1 day'
                )::date AS due_date,
