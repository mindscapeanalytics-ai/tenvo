...
import { removeStockAction } from '@/lib/actions/standard/inventory/stock';
...
export async function createPosTerminalAction(data) {
    ...
    try {
        await checkAuth(data.businessId, client, 'pos.access', 'pos');
...
export async function openPosSessionAction(data) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(data.businessId, client, 'pos.open_session', 'pos');
...
export async function closePosSessionAction(data) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(data.businessId, client, 'pos.close_session', 'pos');
...
export async function createPosTransactionAction(data) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(data.businessId, client, 'pos.process_sale', 'pos');
        await client.query('BEGIN');
        ...
        const paymentTotal = (data.payments || []).reduce((sum, payment) => {
            return sum + Number(payment.amount || 0);
        }, 0);
        if (paymentTotal + 0.01 < grandTotal) {
            throw new Error(
                `Insufficient payment total. Required ${grandTotal.toFixed(2)}, received ${paymentTotal.toFixed(2)}`
            );
        }
        ...
        for (const item of data.items) {
            ...
            await client.query(`
                INSERT INTO pos_transaction_items (
                    business_id, transaction_id, product_id, product_name, quantity,
                    unit_price, tax_amount, discount_amount, line_total
                ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
            `, [
                data.businessId, transaction.id, item.productId, item.productName,
                item.quantity || 1, item.unitPrice, lineTax,
                item.discountAmount || 0, lineTotal + lineTax - (item.discountAmount || 0)
            ]);

            // Deduct stock via standardized pipeline (warehouse + batch + serial + movement history)
            if (item.productId) {
        ...
        for (const payment of data.payments) {
            await client.query(`
                ...
                VALUES ($1, $2, $3, $4, $5)
            `, [data.businessId, transaction.id, payment.method, payment.amount, payment.reference || null]);
        }
        ...
        auditWrite({
            ...
            description: `POS sale ${txNumber} — total ${grandTotal}`,
            metadata: { txNumber, grandTotal, itemCount: data.items.length, sessionId: data.sessionId },
        });
