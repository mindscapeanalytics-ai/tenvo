...
import { parsePosSessionNotes, sumPosCashMovements } from '@/lib/utils/posCashDrawer';
...
export async function getPosTerminalsAction(businessId) {
    try {
        await checkAuth(businessId, null, 'pos.access', 'pos');
...
export async function openPosSessionAction(data) {
    try {
        ...
        const posSession = await POSService.openSession(data, userId);
...
export async function closePosSessionAction(data) {
    try {
        ...
        const userId = session?.user?.id || 'system';
        const result = await POSService.closeSession(data, userId);
...
export async function createPosTransactionAction(data) {
    try {
        ...
        const userId = session?.user?.id || 'system';
        const transaction = await POSService.createTransaction(data, userId);
...
export async function getPosSessionSummaryAction(businessId, sessionId) {
    try {
        ...
        const { cashMovements } = parsePosSessionNotes(session.notes);
        const movementNet = sumPosCashMovements(cashMovements);
        const expectedCash = openingCash + cashTotal + movementNet;
...
/**
 * Read GST/PST (or VAT) rates for the till — gated by pos.access so cashiers can load rates
 * without tax.view. Does not expose NTN/SRN beyond what is needed for till math.
 */
export async function getPosTaxConfigAction(businessId) {
    try {
        await checkAuth(businessId, null, 'pos.access', 'pos');
        ...
        return {
            success: true,
            config: {
                sales_tax_rate: Number(config?.sales_tax_rate ?? config?.gst_rate ?? 0),
                provincial_tax_rate: Number(config?.provincial_tax_rate ?? 0),
                gst_rate: Number(config?.gst_rate ?? config?.sales_tax_rate ?? 0),
            },
        };
    } catch (error) {
        console.error('Get POS tax config error:', error);
        return { success: false, error: error.message };
    }
}
