...
export async function submitApprovalAction(data) {
    ...
    try {
        const session = await checkAuth(data.businessId, client, 'approval_workflows');
...
export async function resolveApprovalAction(data) {
    ...
    try {
        const session = await checkAuth(data.businessId, client, 'approval_workflows');
        const _auditUserId = session.user.id;
        const _auditUserEmail = session.user.email;
        ...
        const { auditWrite } = await import('@/lib/actions/_shared/audit');
        auditWrite({
            businessId: data.businessId,
            action: data.action === 'approve' ? 'approve' : 'reject',
            entityType: 'approval',
            entityId: data.requestId,
            description: `${newStatus.charAt(0).toUpperCase() + newStatus.slice(1)} approval request ${data.requestId}`,
            metadata: { requestType: request.request_type, entityType: request.entity_type, entityId: request.entity_id, rejectionReason: data.rejectionReason || null },
            userId: _auditUserId,
...
export async function getPendingApprovalsAction(businessId) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(businessId, client, 'approval_workflows');
