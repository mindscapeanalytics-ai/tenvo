...
import { GL_EXCLUDE_DRAFT_JOURNAL_SQL } from '@/lib/utils/glReportSql';

/** Equity system codes replaced by computed retained earnings on the Balance Sheet (avoid double-count). */
const BS_DYNAMIC_RE_CODES = new Set([
    ACCOUNT_CODES.RETAINED_EARNINGS,
    ACCOUNT_CODES.CURRENT_YEAR_EARNINGS,
]);

/** Busy/Zoho-style statements omit dormant zero-balance accounts from line items. */
const ZERO_BALANCE_EPS = 0.005;

function hasMaterialBalance(value) {
    return Math.abs(Number(value) || 0) > ZERO_BALANCE_EPS;
}
...
export async function getTrialBalanceAction(businessId, asOfDate = new Date().toISOString()) {
    ...
    try {
        ...
        try {
            ...
            }).filter((a) => hasMaterialBalance(a.total_debit) || hasMaterialBalance(a.total_credit));
...
export async function getProfitLossAction(businessId, startDate, endDate) {
    ...
    try {
        ...
        try {
            ...
            // Let's separate them — omit dormant zero-balance lines (totals unchanged)
            const incomeAccounts = accounts
                .filter((a) => ['income', 'revenue'].includes(a.type.toLowerCase()))
                .map((a) => ({
                    ...a,
                    amount: a.balance, // Positive for revenue
                }))
                .filter((a) => hasMaterialBalance(a.amount));
            ...
            const cogsFinal = allExpenses
                .filter((a) => cogsCodeSet.has(a.code))
                .map((a) => ({
                    ...a,
                    amount: -a.balance,
                }))
            ...
            const otherExpenseAccounts = allExpenses
                .filter((a) => !cogsCodeSet.has(a.code))
                .map((a) => ({
                    ...a,
                    amount: -a.balance,
    ...
    try {
        ...
        try {
            ...
            const assets = accounts
                ...
                .filter((a) => hasMaterialBalance(a.balance));
            const liabilities = accounts
                .filter((a) => a.type.toLowerCase() === 'liability')
                .filter((a) => hasMaterialBalance(a.balance));
            // Exclude system RE / CYE equity accounts — their economic content is the computed retainedEarnings line
            const equity = accounts
                .filter(
                    (a) => a.type.toLowerCase() === 'equity' && !BS_DYNAMIC_RE_CODES.has(String(a.code))
                )
                .filter((a) => hasMaterialBalance(a.balance));
    ...
    try {
        ...
        try {
            ...
            const result = await client.query(`
                ...
                LEFT JOIN gl_entries e ON a.id = e.account_id
                    AND e.business_id = $1
                    AND ($2::timestamp IS NULL OR e.transaction_date >= $2::date)
                    AND ($3::timestamp IS NULL OR e.transaction_date <= $3::date)
                    ${GL_EXCLUDE_DRAFT_JOURNAL_SQL}
                WHERE a.business_id = $1
...
export async function reconcilePendingStorefrontGlAction(businessId) {
    try {
        await checkAuth(businessId, null, 'finance.manage_accounts', 'basic_accounting');
        ...
        try {
            ...
            const result = await client.query(
                `SELECT
                 ...
                 JOIN gl_accounts a ON a.id = e.account_id AND a.business_id = je.business_id
