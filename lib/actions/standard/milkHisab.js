...
export async function getMilkHisabDayAction({ businessId, category, deliveryDate }) {
  try {
    ...
    const milkProductId = hisabProducts.find((p) =>
      /milk|doodh/i.test(`${p.name} ${p.category || ''}`)
    )?.id;
    ...
    const rows = customers
      .map((c) => {
        ...
        if (!hasStop) {
          ...
          if (isMilkHisabWalkInCustomer(c.name)) return null;
        }
        ...
        for (const p of hisabProducts) {
          ...
          } else if (!stop && milkProductId && pid === String(milkProductId) && prefs.dailyMilkKg > 0) {
            qty = prefs.dailyMilkKg;
          }
          qtyByProduct[pid] = qty;
          dayAmount += qty * (Number(p.price) || 0);
        }
        return {
          customerId: cid,
          customerName: c.name,
    ...
    await prismaBase.$transaction(async (tx) => {
      for (const row of rows) {
        ...
        for (const [productId, rawQty] of Object.entries(qtyMap)) {
          ...
          if (!product) {
            console.warn('[milkHisab] skip line; product not found', pid);
            continue;
          }
          lineCreates.push({
            business_id: businessId,
        ...
        if (!lineCreates.length) {
          const existing = await tx.milk_delivery_stops.findFirst({
            where: {
              business_id: businessId,
          ...
          if (existing) {
            await tx.milk_delivery_lines.deleteMany({
              where: { stop_id: existing.id, business_id: businessId },
            });
            await tx.milk_delivery_stops.update({
        ...
        await tx.milk_delivery_lines.createMany({
          data: lineCreates.map((line) => ({ ...line, stop_id: stop.id })),
        });

        // Keep customer route book in sync when house/route are edited on the sheet.
        const nextHouse = String(row.houseNo || '').trim();
        const nextRoute = String(row.routeLabel || '').trim();
        if (
          nextHouse !== prefs.houseNo ||
          nextRoute !== prefs.routeLabel
        ) {
          const prevDd =
            customer.domain_data && typeof customer.domain_data === 'object' ? customer.domain_data : {};
          await tx.customers.update({
  ...
  try {
    ...
    const invoices = await prismaBase.invoices.findMany({
      where: {
        business_id: businessId,
        is_deleted: false,
        notes: { contains: MILK_HISAB_PERIOD_PREFIX },
      },
      select: {
        id: true,
        customer_id: true,
    ...
    const rows = [...byCustomer.values()]
      .map((r) => {
        const inv = resolveMilkHisabInvoiceForPeriod(invoices, r.customerId, periodKey);
        const hisabPaymentStatus = readMilkHisabPeriodPayment(r.domainData, periodKey);
        const billed = Boolean(inv);
        const paymentStatus = resolveMilkHisabRowPaymentStatus({
        ...
        return {
          ...
          hisabPaymentStatus,
          billed,
          billedPeriod: inv ? (String(inv.notes || '').match(/\[milk_hisab_period=([^\]]+)\]/)?.[1] || periodKey) : null,
        };
      })
    ...
    const kpis = buildMilkHisabPeriodKpis(rows);
  ...
  try {
    ...
    for (const row of targets) {
      try {
        ...
        const existing = resolveMilkHisabInvoiceForPeriod(existingInvoices, row.customerId, periodKey);
        if (existing) {
          skipped.push({
        ...
        if (String(row.paymentStatus || '').toLowerCase() === 'paid') {
          try {
            await InvoicePaymentService.recordPayment({
              ...
              paymentMethod: 'cash',
              notes: MILK_HISAB_COLLECTION_NOTE,
              userId: session.user.id,
            });
          } catch (payErr) {
            console.error('generateMilkHisabInvoicesAction apply hisab paid', row.customerId, payErr);
          }
        }
        ...
        created.push({
          ...
          paymentStatus:
            String(row.paymentStatus || '').toLowerCase() === 'paid' ? 'paid' : 'unpaid',
        });
      } catch (err) {
        console.error('generateMilkHisabInvoicesAction customer', row.customerId, err);
        failed.push({
          customerId: row.customerId,
 ...
 */
export async function getMilkHisabCustomerDayBreakdownAction({
  businessId,
  category,
  customerId,
  period,
  ...
  try {
    ...
    const breakdown = buildMilkHisabDayBreakdownGrid({
      stops,
      columns,
      startIso,
      endIso,
    ...
    const inv = resolveMilkHisabInvoiceForPeriod(invoices, customerId, periodKey);
    ...
    console.error('getMilkHisabCustomerDayBreakdownAction', e);
    return await actionFailure(e?.code || 'MILK_HISAB_DAY_SHEET_FAILED', await getErrorMessage(e));
  }
}
...
export async function prepareMilkHisabReminderAction({
  ...
  try {
    ...
    const hisabPaid = readMilkHisabPeriodPayment(customer.domain_data, periodKey) === 'paid';
    if (hisabPaid && !invoiceId) {
      return await actionFailure('MILK_HISAB_ALREADY_PAID', 'Already paid. No reminder needed.');
    }
      ...
      try {
        ...
        const sendRes = await sendCampaignOutreachEmail({
          ...
          react: React.createElement(CampaignOutreachEmail, {
            businessName: business?.business_name || 'Milk shop',
...
export async function sendMilkHisabBulkRemindersAction({
  ...
  try {
    ...
    const targets = (summary.rows || []).filter((r) => isMilkHisabBillRemindable(r));
 ...
 */
export async function setMilkHisabBillPaymentStatusAction({
  businessId,
  category,
  invoiceId = null,
  customerId = null,
  ...
  try {
    ...
    if (period) {
      try {
        periodKey = parseMilkHisabBillingPeriod(period).period;
      } catch {
    ...
    if (resolvedInvoiceId) {
      ...
      invoiceNumber = invoice.invoice_number;
      if (!periodKey) {
        periodKey = String(invoice.notes || '').match(/\[milk_hisab_period=([^\]]+)\]/)?.[1] || null;
      }
    }
    ...
    const nextDomain = patchMilkHisabPeriodPayment(customer.domain_data, periodKey, next);
    ...
    const routePayments = (payments || []).filter((p) =>
      String(p.notes || '').includes(MILK_HISAB_COLLECTION_NOTE)
    );
    ...
    if (!routePayments.length) {
      ...
      if (status === 'paid' || status === 'partial') {
        return await actionFailure(
          'MILK_HISAB_PAYMENT_OPEN_INVOICES',
    ...
    console.error('setMilkHisabBillPaymentStatusAction', e);
    return await actionFailure(e?.code || 'MILK_HISAB_PAYMENT_STATUS_FAILED', await getErrorMessage(e));
  }
}
