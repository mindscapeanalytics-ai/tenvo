...
/**
 * Compact Bills toggle: mark a Route Hisab invoice paid/unpaid via invoice_payments.
 * Paid records a cash receipt; unpaid voids Route Hisab collection receipts only.
 */
export async function setMilkHisabBillPaymentStatusAction({
  ...
  try {
    ...
    if (!invoiceId) {
      return await actionFailure('MILK_HISAB_NO_INVOICE', 'Generate the bill first, then mark paid');
    }
    ...
    if (next === 'paid') {
      ...
      const balance = Number(summary?.balance ?? invoice.grand_total) || 0;
      if (balance <= 0.009) {
        return await actionSuccess({
          invoiceId,
          paymentStatus: 'paid',
    ...
    try {
      const refreshed = await refreshInvoicePaymentStatusFromBalance(businessId, invoiceId, client);
