...
import { reserveStockAction, releaseStockAction, removeStockAction, addStockAction } from '@/lib/actions/standard/inventory/stock';
...
export async function createQuotationAction(quotationData) {
    ...
    try {
        ...
        await verifyBusinessAccess(session.user.id, quotationData.business_id, [], client);
...
export async function getQuotationDetailAction(quotationId) {
    ...
    try {
        ...
        try {
            ...
            await verifyBusinessAccess(session.user.id, headerResult.rows[0].business_id, [], client);
...
export async function createSalesOrderAction(orderData) {
    ...
    try {
        ...
        await verifyBusinessAccess(session.user.id, orderData.business_id, [], client);
...
export async function createChallanAction(challanData) {
    ...
    try {
        ...
        await verifyBusinessAccess(session.user.id, challanData.business_id, [], client);
