...
import { getTenantCache, setTenantCache } from '@/lib/cache/tenantCache';
...
export async function getProductsAction(businessId, options = {}) {
    try {
        // Ignore any client-supplied skipAuth — only ALS trusted bypass skips membership.
        const { skipAuth: _ignoredSkipAuth, ...productOptions } = options || {};
        await checkAuth(businessId, null, 'inventory.view');

        // Redis cache for instant repeated loads (60s TTL, auto-purged on mutations)
        const cacheSuffix = `${productOptions.offset || 0}:${productOptions.limit || 500}:${productOptions.search || ''}:${productOptions.category || ''}:${productOptions.detailLevel || 'list'}`;
        const cached = await getTenantCache(businessId, 'products', cacheSuffix);
        if (cached) return cached;

        const result = await ProductService.getProducts(businessId, productOptions);
        const response = { success: true, ...result };

        // Cache only list-level reads (not full/export) and only when not searching
        if (!productOptions.search && (productOptions.detailLevel || 'list') === 'list') {
            void setTenantCache(businessId, 'products', response, cacheSuffix);
        }
...
export async function createProductAction(productData) {
    try {
        ...
        const lean = leanProductPayloadForCreate(productData);
        const hasLedgerPayload =
            Number(lean.stock) > 0 ||
            (Array.isArray(lean.batches) && lean.batches.length > 0) ||
            (Array.isArray(lean.serial_numbers) && lean.serial_numbers.length > 0) ||
            (Array.isArray(lean.serialNumbers) && lean.serialNumbers.length > 0) ||
            (Array.isArray(lean.variants) && lean.variants.length > 0);

        // Opening stock / batches / serials / variants must hit InventoryService via composite.
        if (hasLedgerPayload) {
            const res = await upsertIntegratedProductAction({
                productData: { ...lean, business_id: productData.business_id },
                batches: lean.batches || [],
                serialNumbers: lean.serialNumbers || lean.serial_numbers || [],
                variants: lean.variants || [],
                initialStock: Number(lean.stock) || 0,
                isUpdate: false,
                productId: null,
            });
            if (!res.success) {
            ...
            return { success: true, product: res.product, data: res.product };
        }

        const result = await ProductService.createProduct(lean);
...
export async function updateProductAction(id, businessId, updates) {
    try {
        ...
        await assertEntityBelongsToBusiness(null, 'product', id, businessId);

        const stockProvided = Object.prototype.hasOwnProperty.call(updates, 'stock');
        const stockRaw = stockProvided ? updates.stock : undefined;
        const lean = leanProductPayloadForUpdate(updates);

        // Stock qty belongs on the InventoryService ledger — do not write headline-only stock.
        if (stockProvided) {
            delete lean.stock;
        }

        let product = await ProductService.updateProduct(id, businessId, lean);
        ...
        if (
            ...
            const res = await upsertIntegratedProductAction({
                productData: {
                    ...
                    cost_price: lean.cost_price ?? product.cost_price,
                },
                batches: [],
                serialNumbers: [],
                isUpdate: true,
            ...
            if (!res.success) {
                return { success: false, error: res.error || 'Failed to reconcile stock' };
            }
            product = res.product || (await ProductService.getProduct(id, businessId));
        }
...
export async function bulkImportProductsAction(businessId, file, options = {}) {
    try {
        ...
        if (Array.isArray(file)) {
            ...
            const columnMapping = options.columnMapping || detectColumnMapping(sheetHeaders, { category: options.category });
        ...
        return {
            ...
            errors: errors.filter((e) => e.errors?.length > 0).slice(0, 20),
