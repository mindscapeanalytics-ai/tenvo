        ...
        const products = await db.products.findMany({
            ...
            include: {
                ...
                product_stock_locations: {
                    ...
                    select: { warehouse_id: true, quantity: true, reserved_quantity: true, state: true }
                }
            },
...
export async function bulkImportProductsAction(businessId, file, options = {}) {
    try {
        ...
        const toInsert = transformImportedData(validatedRows, businessId, options.domainData || {}, parseResult.sheets);
        ...
        await db.$transaction(async (tx) => {
            for (const product of toInsert) {
                ...
                if (existingId && !options.skipExisting) {
                    await ProductService.updateProduct(existingId, businessId, {
                        name: product.name,
                        cost_price: product.cost,
                        price: product.price,
                        stock: product.stock,
                        min_stock: product.minStock,
                        domain_data: product.domain_data
                    }, tx);
                    updated++;
                } else if (!existingId) {
                    await ProductService.createProduct({
                        ...
                        cost_price: product.cost, // map cleaned data to expected keys
                        price: product.price,
                        min_stock: product.minStock,
                    }, tx);
                    imported++;
