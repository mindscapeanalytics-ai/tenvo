 ...
 * Note: alertId-only — no businessId available; auth is best-effort via the
 * service layer which scopes by business. Callers should pass businessId when possible.
 */
export async function dismissLowStockAlertAction(alertId, businessId = null) {
    try {
        if (businessId) await checkAuth(businessId, 'inventory.adjust_stock');
