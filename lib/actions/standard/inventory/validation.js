...
async function checkAuth(businessId) {
    const { session } = await withGuard(businessId, { permission: 'inventory.view' });
    return session;
}
...
export async function checkBarcodeExistsAction(barcode, businessId, excludeProductId = null) {
    try {
        await checkAuth(businessId);
        const candidates = expandScanCandidates(barcode).map((c) => c.toLowerCase());
        ...
        try {
            ...
            `;
            const params = [businessId, candidates];
 ...
 */
export async function checkStockAvailabilityAction(businessId, productId, quantity, warehouseId = null) {
    try {
        await checkAuth(businessId);
        const client = await pool.connect();
        ...
        try {
            if (warehouseId) {
                ...
                const query = `
                    ...
                    WHERE psl.product_id = $1 AND psl.warehouse_id = $2 AND p.business_id = $3
                    GROUP BY psl.quantity
                `;
                const result = await client.query(query, [productId, warehouseId, businessId]);
                ...
                `;
                const result = await client.query(query, [productId, businessId]);
