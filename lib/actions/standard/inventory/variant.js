...
export async function getProductVariantsAction(productId) {
    try {
        ...
        try {
            const result = await client.query(
                'SELECT * FROM product_variants WHERE product_id = $1 AND is_active = true AND is_deleted = false ORDER BY size, color',
                [productId]
            );
...
export async function getVariantMatrixAction(productId) {
    try {
        ...
        try {
            const result = await client.query(
                'SELECT * FROM product_variants WHERE product_id = $1 AND is_active = true AND is_deleted = false',
                [productId]
            );
