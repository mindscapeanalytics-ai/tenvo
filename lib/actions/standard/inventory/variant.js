...
export async function createSingleVariantAction(variantData) {
    try {
        await checkAuth(variantData.business_id, 'inventory.create');
        const variant = await VariantService.createSingleVariant(variantData);
        return { success: true, variant };
    } catch (error) {
        return { success: false, error: error.message };
    }
...
export async function updateVariantAction(variantId, businessId, updates) {
    try {
        await checkAuth(businessId, 'inventory.edit');
        const variant = await VariantService.updateVariant(variantId, businessId, updates);
        return { success: true, variant };
    } catch (error) {
        return { success: false, error: error.message };
    }
