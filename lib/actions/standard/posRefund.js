...
export async function getPosTransactionLookupAction(businessId, reference) {
    ...
    try {
        ...
        if (transaction.is_voided || transaction.payment_status === 'voided') {
            return { success: false, error: 'This sale was voided and cannot be refunded' };
        }
    ...
    try {
        ...
        const res = await client.query(
            ...
            SELECT
                ...
                t.is_voided AS "isVoided",
                c.name AS "customerName"
            FROM pos_transactions t
            LEFT JOIN customers c ON c.id = t.customer_id
            WHERE t.business_id = $1
              AND COALESCE(t.is_voided, false) = false
              AND COALESCE(t.payment_status, '') <> 'voided'
        ...
        // Canonical camelCase + snake_case aliases for hub void/refund UIs.
        const transactions = res.rows.map((row) => {
            ...
            return {
                ...
                is_voided: Boolean(row.isVoided),
