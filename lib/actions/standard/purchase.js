    ...
    try {
        ...
        try {
            const columnResult = await client.query(`
                ...
                LIMIT 1
            `);
            const hasIsDeleted = columnResult.rows.length > 0;
            ...
            const result = await client.query(`
                ...
                ${hasIsDeleted ? 'AND (p.is_deleted = false OR p.is_deleted IS NULL)' : ''}
        ...
        return actionFailure('GET_PURCHASES_FAILED', getErrorMessage(error));
    }
}
...
export async function getPurchaseByIdAction(businessId, purchaseId) {
    try {
        ...
        return actionFailure('GET_PURCHASE_BY_ID_FAILED', getErrorMessage(error));
    }
}
...
export async function createPurchaseAction(purchaseData) {
    ...
    try {
        ...
        return actionFailure('CREATE_PURCHASE_FAILED', getErrorMessage(error));
    } finally {
        client.release();
    }
}
...
export async function createAutoReorderPOAction(params) {
    ...
    try {
        ...
        return { success: true, purchaseId, purchase_number: purchase_number, purchaseNumber: purchase_number };
