...
async function checkAuth(businessId, client = null, featureKey = null) {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session) throw new Error('Unauthorized');
    if (businessId) await verifyBusinessAccess(session.user.id, businessId, [], client);
    if (businessId && featureKey) await checkPlanFeature(businessId, featureKey, client);
    return session;
}
...
export async function upsertTableAction(data) {
    ...
    try {
        await checkAuth(data.businessId, client, 'restaurant_kds');
...
export async function getTablesAction(businessId) {
    const client = await pool.connect();
    try {
        await checkAuth(businessId, client, 'restaurant_kds');
        const result = await client.query(`
            SELECT t.*,
...
export async function createRestaurantOrderAction(data) {
    const client = await pool.connect();
    try {
        const session = await checkAuth(data.businessId, client, 'restaurant_kds');
...
export async function updateOrderStatusAction(data) {
    ...
    try {
        ...
        // When order is completed, deduct stock for each item
        if (data.status === 'completed') {
            const itemsRes = await client.query(
                `SELECT product_id, item_name, quantity FROM restaurant_order_items WHERE order_id = $1`,
                [data.orderId]
            );
            for (const item of itemsRes.rows) {
                if (item.product_id) {
                    try {
                        ...
                        console.error(`Stock deduction failed for ${item.item_name}:`, stockErr.message);
                        // Non-blocking: order still completes even if stock deduction fails
                    }
                }
            }
            ...
            auditWrite({
                ...
                description: `Restaurant order completed — stock deducted for ${itemsRes.rows.length} items`,
                metadata: { orderId: data.orderId, itemCount: itemsRes.rows.length },
            });
        }
