...
import { mapOrderLinesToPosItems } from '@/lib/utils/posTax';
...
export async function settleRestaurantOrderAction(data) {
    ...
    try {
        ...
        if (data.sessionId) {
            ...
            const posItems = mapOrderLinesToPosItems(itemsRes.rows, {
            ...
            if (posItems.length > 0) {
                try {
                    ...
                    console.error('[settleRestaurantOrder] POS ledger sync failed (payment continues):', posErr);
                }
            }
        }
        ...
        await client.query(
            ...
            ) VALUES ($1, 'receipt', 'restaurant_order', $2, $3, $4, $5, CURRENT_DATE, $6, 'active', $7, NOW(), NOW())`,
            [
                data.businessId,
                order.id,
        ...
        // 3b. GL: post cash sale unless POS already posted (POSService.createTransaction posts pos_sale)
        if (!posTransactionId) {
            const { AccountingService } = await import('@/lib/services/AccountingService');
            const payMethod = String(data.paymentMethod || 'cash').toLowerCase();
            const isCash = payMethod === 'cash';
            const totalAmount = Number(data.amount || order.total_amount || 0);
            ...
            const existingJe = await client.query(
                `SELECT id FROM journal_entries
                 ...
                 LIMIT 1`,
                [data.businessId, `%[ref:restaurant_order:${order.id}]%`]
            );
            if (existingJe.rows.length === 0 && totalAmount > 0) {
                await AccountingService.recordBusinessTransaction(
                    'pos_sale',
                    {
                        ...
                        cardAmount: isCash ? 0 : totalAmount,
                        description: `[ref:restaurant_order:${order.id}] Restaurant Order #${order.order_number}`,
