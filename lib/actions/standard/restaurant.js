...
export async function upsertTableAction(data) {
    ...
    try {
        await checkAuth(data.businessId, client, 'restaurant.manage_tables', 'restaurant_kds');
...
export async function createRestaurantOrderAction(data) {
    ...
    try {
        ...
        for (const item of data.items) {
            ...
            `, [
                data.businessId, order.id, item.productId || null, item.itemName,
...
export async function updateOrderStatusAction(data) {
    const client = await pool.connect();
    try {
        await checkAuth(data.businessId, client, 'restaurant.create_order', 'restaurant_kds');
        await client.query('BEGIN');
...
export async function updateKitchenOrderAction(data) {
    const client = await pool.connect();
    try {
        await checkAuth(data.businessId, client, 'restaurant.view_kds', 'restaurant_kds');
        await client.query('BEGIN');
