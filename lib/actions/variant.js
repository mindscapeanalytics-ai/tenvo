...
export async function updateVariantStockAction(variantId, businessId, quantityChange) {
    try {
        ...
        try {
            ...
            if (result.rows.length === 0) return { success: false, error: 'Variant not found' };
            return { success: true, variant: result.rows[0] };
        } finally {
            client.release();
        }
