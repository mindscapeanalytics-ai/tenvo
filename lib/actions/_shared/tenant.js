...
export async function assertEntityBelongsToBusiness(client, entityType, id, businessId, messageLabel = null) {
  ...
  if (client && typeof client.query === 'function') {
    const softDeleteClause =
      table === 'products' ? ' AND COALESCE(is_deleted, false) = false' : '';
    const result = await client.query(
      `SELECT 1 FROM ${table} WHERE id = $1 AND business_id = $2${softDeleteClause} LIMIT 1`,
      [id, businessId]
    );
  ...
  if (!client) {
    const record = await withBusinessContext(businessId, () =>
      prismaClient[prismaModel].findFirst({
        where: {
          id,
          ...(table === 'products' ? { is_deleted: false } : {}),
        },
        select: { id: true },
      })
    );
