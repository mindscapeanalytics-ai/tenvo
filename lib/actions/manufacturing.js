...
import { removeStockAction, addStockAction } from '@/lib/actions/stock';
 ...
 */
async function consumeMaterials(client, businessId, bomId, productionOrderId, quantityToProduce, warehouseId) {
    // 1. Get BOM materials with current cost price
    const materialsRes = await client.query(`
        SELECT bm.material_id, bm.quantity, bm.unit, p.cost_price
        FROM bom_materials bm
    ...
    for (const mat of materials) {
        ...
        const matCost = Number(mat.cost_price || 0) * consumedQty;
        totalCost += matCost;
...
export async function createProductionOrderAction(orderData) {
    ...
    try {
        ...
        if (orderData.status === 'completed') {
            ...
            const totalMaterialCost = await consumeMaterials(client, orderData.business_id, orderData.bom_id, productionOrder.id, orderData.quantity_to_produce, warehouseId);
    ...
    try {
        ...
        try {
            ...
            if (status === 'completed') {
                ...
                // 1. Consume Materials (get total cost)
                const totalMaterialCost = await consumeMaterials(client, businessId, productionOrder.bom_id, productionOrder.id, productionOrder.quantity_to_produce, warehouseId);
...
export async function deleteBOMAction(bomId) {
    try {
        ...
        try {
            ...
            const checkRes = await client.query('SELECT count(*) FROM production_orders WHERE bom_id = $1 AND status != $2', [bomId, 'completed']);
            ...
            // Delete header
            await client.query('DELETE FROM boms WHERE id = $1', [bomId]);
            await client.query('COMMIT');
