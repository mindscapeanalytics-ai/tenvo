...
import { runWithTrustedAuthBypass } from '@/lib/actions/_shared/trustedAuthBypass';
import { withGuard } from '@/lib/rbac/serverGuard';
import { resolveAnalyticsRange } from '@/lib/utils/analyticsRange';
import { getHubDashboardPeriodExtras } from '@/lib/actions/dashboard/hubDashboardPeriodExtras';
import {
 ...
 * Nested helpers run inside runWithTrustedAuthBypass so membership SQL runs once
 * (client-supplied skipAuth is never honored on the nested actions).
 *
...
export async function getHubShellBootstrapAction(businessId, filter = {}) {
    ...
    try {
        ...
        const [
            ...
            invoicesSettled,
            productsSettled,
        ] = await runWithTrustedAuthBypass(businessId, () =>
            Promise.allSettled([
        ...
        const [
            ...
            Promise.allSettled([
                ...
                getHubDashboardPeriodExtras(businessId, { from, to }),
            ])
        );
        ...
        const periodExtrasRes = settledValue(periodExtrasSettled, { success: false });
        if (periodExtrasSettled.status === 'rejected' || !periodExtrasRes?.success) {
            errors.periodExtras =
                periodExtrasSettled.status === 'rejected'
                    ? String(periodExtrasSettled.reason?.message || 'Period comparison failed')
                    : periodExtrasRes?.error || 'Period comparison unavailable';
        }
        ...
        const customersRes = settledValue(customersSettled, { success: false });
        if (customersSettled.status === 'rejected' || !customersRes?.success) {
            errors.customers =
                customersSettled.status === 'rejected'
                    ? String(customersSettled.reason?.message || 'Customers failed')
                    : customersRes?.error || 'Customers unavailable';
        }
        ...
        const hasMoreCustomers = customersRes?.success
            ? Boolean(customersRes.hasMore)
            : false;
        ...
        return await actionSuccess({
            ...
            glSummary: gl || null,
            comparison: periodExtrasRes?.success ? periodExtrasRes.comparison || null : null,
            orderStatus: periodExtrasRes?.success ? periodExtrasRes.orderStatus || null : null,
            ...
            meta: {
                ...
                customerLimit: HUB_SHELL_CUSTOMER_LIMIT,
                activityLimit: HUB_SHELL_ACTIVITY_LIMIT,
                invoiceTruncated: hasMoreInvoices,
                customerTruncated: hasMoreCustomers,
            },
