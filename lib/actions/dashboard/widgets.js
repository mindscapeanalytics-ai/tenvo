...
/**
 * Get tax calculations for a business (schema-aligned: invoices tax columns + tax_payments).
 */
export async function getTaxCalculations(businessId, currency = 'PKR') {
    ...
    try {
        // Current month sales tax from invoices (canonical tax_total / total_tax).
        const salesResult = await client.query(`
            SELECT 
                COALESCE(SUM(COALESCE(subtotal, 0)), 0) as total_sales,
                COALESCE(SUM(COALESCE(tax_total, total_tax, 0)), 0) as total_tax,
                COALESCE(SUM(COALESCE((tax_details->>'pst')::numeric, (tax_details->>'pst_amount')::numeric, 0)), 0) as pst_amount,
                COALESCE(SUM(COALESCE((tax_details->>'fst')::numeric, (tax_details->>'fst_amount')::numeric, 0)), 0) as fst_amount
            FROM invoices
            WHERE business_id = $1
              AND DATE_TRUNC('month', COALESCE(date::timestamptz, created_at)) = DATE_TRUNC('month', CURRENT_DATE)
              AND COALESCE(is_deleted, false) = false
              AND COALESCE(status, '') NOT IN ('cancelled', 'voided', 'draft')
        `, [businessId]);
        
        // Prefer tax_configurations when present; fall back to regional defaults.
        let taxConfig = { pst_rate: 17, fst_rate: 1 };
        try {
            const taxConfigResult = await client.query(`
                SELECT
                    COALESCE(default_sales_tax_rate, 17) as pst_rate,
                    COALESCE(withholding_tax_percent, 1) as fst_rate
                FROM tax_configurations
                WHERE business_id = $1
                LIMIT 1
            `, [businessId]);
            if (taxConfigResult.rows[0]) {
                taxConfig = taxConfigResult.rows[0];
            }
        } catch (cfgErr) {
            // Missing/legacy tax_configurations shape — keep defaults.
            console.warn('[Dashboard] tax_configurations read skipped:', cfgErr.message);
        }
        ...
        try {
            const paymentsResult = await client.query(`
                ...
                WHERE business_id = $1
                  ...
                  AND COALESCE(is_deleted, false) = false
            `, [businessId]);
            taxPaid = parseFloat(paymentsResult.rows[0]?.tax_paid || 0);
        } catch (payErr) {
            // Table missing on older DBs until migration applied.
            if (payErr.code !== '42P01') throw payErr;
            taxPaid = 0;
        }
