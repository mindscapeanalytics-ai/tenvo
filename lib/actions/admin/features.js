...
export async function listFeatureFlags() {
    ...
    try {
        const result = await client.query(`
            ...
            LEFT JOIN feature_flag_overrides fo ON f.id = fo.feature_flag_id 
        ...
        return actionFailure('LIST_FEATURE_FLAGS_FAILED', error.message);
    } finally {
        client.release();
    }
}
...
export async function createFeatureFlagOverride({
    ...
    try {
        ...
        const result = await client.query(`
            ...
            (feature_flag_id, target_type, target_id, value, expires_at, reason, created_by)
            ...
            RETURNING *
        `, [feature_flag_id, target_type, target_id, JSON.stringify(value), expires_at, reason, user.id]);
...
export async function deleteFeatureFlagOverride(overrideId) {
    ...
    try {
        await client.query(
            'DELETE FROM feature_flag_overrides WHERE id = $1',
            [overrideId]
        );
...
export async function checkFeatureForBusiness(featureKey, businessId) {
    ...
    try {
        ...
        const overrideResult = await client.query(`
            ...
            WHERE feature_flag_id = (SELECT id FROM feature_flags WHERE key = $1)
        ...
        if (overrideResult.rows.length > 0) {
            return actionSuccess({ enabled: overrideResult.rows[0].value });
        }
        
        // Return default value
        const flagResult = await client.query(
            'SELECT default_value, rollout_percentage FROM feature_flags WHERE key = $1',
            [featureKey]
        );
