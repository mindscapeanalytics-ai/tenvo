...
export async function getPendingRegistrations() {
  try {
    ...
    const enrichedRequests = requests.map(r => ({
      ...r,
      deal_terms: r.businesses?.settings?.deal_terms || null,
    }));
...
export async function approveRegistration({ businessId, notes = '', payment = null }) {
  try {
    ...
    // Pending tenants were gated from public resolve — bust cache so approved store paints immediately.
    try {
      await invalidateStorefrontTenant(result.id);
    } catch (invalidateErr) {
      console.warn('[approveRegistration] storefront invalidate skipped:', invalidateErr?.message);
    }
...
export async function rejectRegistration({ businessId, reason = '' }) {
  try {
    ...
    // Rejected tenants must not remain publicly reachable from stale Redis/Next tags.
    try {
      await invalidateStorefrontTenant(result.id);
    } catch (invalidateErr) {
      console.warn('[rejectRegistration] storefront invalidate skipped:', invalidateErr?.message);
    }
...
export async function requestMoreInfo({ businessId, message }) {
  try {
    ...
    try {
      await invalidateStorefrontTenant(business.id);
    } catch (invalidateErr) {
      console.warn('[requestMoreInfo] storefront invalidate skipped:', invalidateErr?.message);
    }
