...
export async function getPlatformAffiliates() {
  try {
    ...
    // Fetch affiliates with referrals (no businesses join — enriched below)
    ...
    // Collect all unique business_ids across all referrals
    const businessIds = [...new Set(
      affiliates.flatMap(a => a.referrals.map(r => r.business_id)).filter(Boolean)
    )];

    // Fetch business names in one batch query
    let businessMap = {};
    ...
    // Merge business info onto each referral
    const enriched = affiliates.map(aff => ({
      ...aff,
      referrals: aff.referrals.map(ref => ({
        ...ref,
        businesses: businessMap[ref.business_id] || null,
      }))
    }));

    return { success: true, data: serializeDecimalsDeep(enriched) };
  } catch (error) {
