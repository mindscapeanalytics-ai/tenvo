...
import { createUserInvitationForBusiness } from '@/lib/invitations/userInvitationService';
...
/**
 * Create user invitation (platform admin — any business).
 */
export async function createUserInvitation({
    ...
    try {
        const invitation = await createUserInvitationForBusiness({
            email,
            businessId,
            role,
...
/**
 * Accept invitation — caller must be signed in; userId and email must match invitation.
 */
export async function acceptInvitation(token, userId) {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user?.id) {
        return actionFailure('UNAUTHENTICATED', 'You must be signed in to accept an invitation');
    }
    if (session.user.id !== userId) {
        return actionFailure('FORBIDDEN', 'Invitation can only be accepted for your signed-in account');
    }
    ...
    try {
        ...
        if (inviteEmail && userEmail && inviteEmail !== userEmail) {
            ...
            return actionFailure(
                'EMAIL_MISMATCH',
                'This invitation was sent to a different email address. Sign in with the invited email or register using that address.'
            );
        }

        // Enforce seat limit before adding user — check current active member count
        ...
        const { canAddUser } = await import('@/lib/services/planLimits');
        const seatCheck = await canAddUser(invitation.business_id);
        if (!seatCheck.allowed && seatCount >= seatCheck.limit) {
            await client.query('ROLLBACK');
            return actionFailure(
                'SEAT_LIMIT_REACHED',
                `This business has reached its team member limit (${seatCheck.limit}). The workspace owner must upgrade their plan before new members can join.`
            );
        }
...
/**
 * Log user activity — scoped to the signed-in user and a business they can access.
 */
export async function logUserActivity({
    ...
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user?.id) {
        return actionFailure('UNAUTHENTICATED', 'Not authenticated');
    }
    if (session.user.id !== userId) {
        return actionFailure('FORBIDDEN', 'Cannot log activity for another user');
    }
    if (!businessId) {
    ...
    try {
        ...
        return actionFailure(error.code || 'FORBIDDEN', error.message);
    }
