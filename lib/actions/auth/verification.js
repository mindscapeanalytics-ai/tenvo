...
export async function sendVerificationEmail(userId, email, name = '') {
  try {
    ...
    // For now, we'll log it and return the URL for testing
    console.log('[sendVerificationEmail] Verification URL:', verificationUrl);

    // Example email sending (uncomment when you have email service configured):
    // await sendEmail({
    //   to: email,
    //   subject: 'Verify your email - Tenvo',
    //   html: generateVerificationEmailTemplate({ name, verificationUrl })
    // });

    return { 
      success: true, 
      message: 'Verification email sent',
      verificationUrl // Remove in production, for testing only
    };
  } catch (error) {
