 ...
 * Layout (32 chars usable ≈ 54mm @ 8.5px courier):
 * NAME----------- DEL REC   AMT
 */
export function formatWaterDailyLineHeader() {
  return `${pad('PRODUCT', 12)}${pad('DEL', 4, 'right')}${pad('REC', 4, 'right')}${pad('AMOUNT', 8, 'right')}`;
}

export function formatWaterDailyProductLine(line) {
  const name = pad(line.name || 'Item', 12);
  const del = pad(String(line.del ?? 0), 4, 'right');
  const rec = pad(String(line.rec ?? 0), 4, 'right');
  const amt = pad(
...
export async function createWaterDailySalePdf(model) {
  ...
  const render = (pageH) => {
    ...
    write(d.deliveryDate, { size: 7, bold: true });
    rule();
    write(d.customerName, { size: 7.5, bold: true });
    const locBits = [
      d.accountNo ? `A/C ${d.accountNo}` : '',
      d.townCode ? `Town ${d.townCode}` : '',
    ]
      .filter(Boolean)
      .join(' · ');
    if (locBits) write(locBits, { size: 6.5 });
    const houseBits = [
      d.houseNo ? `House ${d.houseNo}` : '',
      d.floorFlat || '',
      d.routeLabel || '',
    ]
      .filter(Boolean)
      .join(' · ');
    if (houseBits) write(houseBits, { size: 6.5 });
    rule();
    write(formatWaterDailyLineHeader(), { size: 6.5, bold: true });
    for (const line of d.lines || []) {
      write(formatWaterDailyProductLine(line), { size: 6.5 });
      if (Number(line.rate) > 0) {
        write(`  @ ${money(line.rate, d.currencyCode, d.numberLocale)} / ${line.unit || 'pcs'}`, {
          size: 5.5,
        });
      }
    }
    rule();
    pair('Del bottles', String(d.delTotal));
    pair('Rec empties', String(d.recTotal));
    pair('Prev BAL', String(d.prevBottle));
    pair('BAL bottles', String(d.bottleBalance), 7);
    if (d.specialDiscount > 0) pair('Sp. discount', money(d.specialDiscount, d.currencyCode, d.numberLocale));
    pair('Cash recovery', money(d.cashCollected, d.currencyCode, d.numberLocale));
    y += 0.4;
    doc.setDrawColor(30);
    doc.line(margin, y, pageW - margin, y);
    y += 3.2;
...
export function buildWaterDailySaleHtml(model) {
  ...
  const gridLines = [
    ...
    ...(d.lines || []).map((line) => esc(formatWaterDailyProductLine(line))),
  ].join('\n');
  const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body {
    ...
    font-size: 8.5px; line-height: 1.28; color: #111;
  }
  .c { text-align: center; }
  .b { font-weight: 700; }
  .t { font-size: 10.5px; margin-bottom: 0.6mm; }
  .s { font-size: 7.5px; color: #444; }
  hr { border: none; border-top: 1px dashed #999; margin: 1.6mm 0; }
  .grid { font-size: 7.5px; white-space: pre; letter-spacing: 0; line-height: 1.35; text-align: center; }
  .grid .hdr { font-weight: 700; border-bottom: 1px solid #333; padding-bottom: 0.5mm; margin-bottom: 0.5mm; }
  .row { display: flex; justify-content: space-between; gap: 1.5mm; font-size: 8px; padding: 0.45mm 0; }
  .tot { font-weight: 800; font-size: 10px; border-top: 1px solid #333; padding-top: 1.2mm; margin-top: 1mm; }
  .foot { margin-top: 2mm; text-align: center; font-size: 8px; }
  `;
  const pair = (l, r) =>
    `<div class="row"><span>${esc(l)}</span><span>${esc(r)}</span></div>`;
  ...
  <hr/>
  <div class="c b">DAILY SALE SUMMARY</div>
  <div class="c s b">${esc(d.deliveryDate)}</div>
  <hr/>
  <div class="c b">${esc(d.customerName)}</div>
  <div class="c s">${esc(
    [d.accountNo ? `A/C ${d.accountNo}` : '', d.townCode ? `Town ${d.townCode}` : '']
      .filter(Boolean)
      .join(' · ')
  )}</div>
  <div class="c s">${esc(
    [d.houseNo ? `House ${d.houseNo}` : '', d.floorFlat, d.routeLabel].filter(Boolean).join(' · ')
  )}</div>
  <hr/>
  ...
  ${pair('Rec empties', String(d.recTotal))}
  ${pair('Prev BAL', String(d.prevBottle))}
  ${pair('BAL bottles', String(d.bottleBalance))}
  ${d.specialDiscount > 0 ? pair('Sp. discount', money(d.specialDiscount, d.currencyCode, d.numberLocale)) : ''}
  ${pair('Cash recovery', money(d.cashCollected, d.currencyCode, d.numberLocale))}
  <div class="row tot"><span>TOTAL</span><span>${esc(money(d.grandTotal, d.currencyCode, d.numberLocale))}</span></div>
  <hr/>
  <div class="foot"><div class="b">${esc(d.thanks)}</div><div class="s">${esc(d.legend)}</div></div>
</body></html>`;
}
