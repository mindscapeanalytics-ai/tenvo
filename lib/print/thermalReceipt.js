...
export async function printThermalReceiptPdf(opts = {}, printOpts = {}) {
  ...
  try {
    // Prefer 100% scale in the print dialog (no shrink-to-fit onto A4).
  ...
  if (!iframe) {
    ...
    iframe.setAttribute('title', 'Thermal receipt PDF print');
  ...
  return await new Promise((resolve) => {
    ...
    const triggerPrint = () => {
      try {
        ...
        win.focus();
        // Backup if PDF openAction autoPrint did not fire
        win.print();
        finish(true);
      } catch {
        finish(false);
    ...
    // Hint printers / OS of custom size via title (some drivers read job name)
    iframe.setAttribute('title', `Receipt ${pageW}x${pageH}mm`);
    iframe.src = url;
