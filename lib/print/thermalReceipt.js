...
/** Off-screen iframe sized to paper so PDF/HTML viewers layout before print. */
function thermalPrintFrameStyle(pageWmm, pageHmm) {
  const w = Math.min(Math.max(Number(pageWmm) || 58, 40), 120);
  const h = Math.min(Math.max(Number(pageHmm) || 120, 50), 400);
  return (
    'position:fixed;' +
    `width:${w}mm;height:${h}mm;` +
    'top:0;left:-9999px;' +
    'border:0;opacity:0;pointer-events:none;z-index:-9999;'
  );
}
...
export function printThermalReceiptHtml(html, opts = {}) {
  ...
  if (!iframe) {
    ...
    iframe.style.cssText = thermalPrintFrameStyle(58, 120);
  ...
  try {
    const match = html.match(/@page\s*\{[^}]*size:\s*(\d+)mm\s+(\d+)mm/i);
    if (match) {
      iframe.style.cssText = thermalPrintFrameStyle(Number(match[1]), Number(match[2]));
    }
  } catch {
    /* ignore */
  }
...
export async function createThermalReceiptPdf(opts = {}) {
  ...
  try {
    doc.setProperties({
      title: `Thermal receipt ${pageW}x${pageH}mm`,
      subject: `${paperSize} receipt`,
      creator: 'Tenvo',
    });
  } catch {
    /* ignore */
...
export async function printJsPdfDocument(doc, printOpts = {}) {
  ...
  if (!iframe) {
    ...
    iframe.style.cssText = thermalPrintFrameStyle(printOpts.pageW || 58, printOpts.pageH || 120);
    document.body.appendChild(iframe);
  }
  ...
  if (pageW && pageH) {
    iframe.style.cssText = thermalPrintFrameStyle(pageW, pageH);
  }
