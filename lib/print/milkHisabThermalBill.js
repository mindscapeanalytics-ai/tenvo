...
export function buildMilkHisabDayBreakdownHtml(model) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  html { width: 58mm; max-width: 58mm; height: ${pageH}mm; }
  body {
    width: ${width}; max-width: ${width}; margin: 0 auto;
    padding: 2.5mm 2mm 4mm;
    font-family: ${fontStack};
    font-size: ${isUrdu ? '9px' : '8.5px'}; line-height: ${isUrdu ? '1.4' : '1.28'}; color: #111;
  ...
  .m { color: #666; }
  .sec {
    font-size: ${isUrdu ? '7.5px' : '7px'}; font-weight: 700; color: #555;
    margin: 1.2mm 0 0.8mm; text-align: center;
  }
  ...
  .row { display: flex; justify-content: space-between; align-items: baseline; gap: 1.5mm; font-size: ${isUrdu ? '8.5px' : '8px'}; padding: 0.55mm 0; direction: ltr; }
 ...
 */
export async function createMilkHisabDayBreakdownPdf(model) {
  ...
  const render = (pageH) => {
    ...
    for (const t of d.totals || []) {
      ...
      doc.setFontSize(7);
      doc.text(String(left), margin, y, { maxWidth: contentW * 0.58 });
      if (right) doc.text(String(right), pageW - margin, y, { align: 'right' });
      y += 3.1;
    }
    y += 0.5;
    doc.setDrawColor(40);
