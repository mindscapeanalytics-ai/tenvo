...
export function buildMilkHisabDayBreakdownPrintModel({
  ...
  const pack = getBusinessRegionalPack(business);
  const { kind, label } = resolvePeriodMeta(period, periodLabel);
  const documentLabel = kind === 'week' ? 'Weekly Day Sheet' : 'Monthly Day Sheet';
    ...
    .map((t) => {
      ...
      const amount =
        unitPrice > 0
          ...
          : null;
      return {
        label: shortMilkHisabProductLabel({ name: t.name, hisabShortLabel: t.shortLabel }, 12),
  ...
  return {
    ...
    houseNo: houseNo || '',
    invoiceNumber: invoiceNumber || (kind === 'week' ? 'WEEKLY-DRAFT' : 'MONTHLY-DRAFT'),
    paymentMethod: String(paymentStatus || 'unpaid').toLowerCase() === 'paid' ? 'CASH' : 'CREDIT',
    currencyCode: pack.currency,
    locale: pack.locale,
    headerLine,
    dayLines,
...
export function buildMilkHisabDayBreakdownHtml(model) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .c { text-align: center; }
  .b { font-weight: 700; }
  .t { font-size: 10.5px; margin-bottom: 0.8mm; }
  ...
  .row { display: flex; justify-content: space-between; gap: 1mm; font-size: 8px; padding: 0.4mm 0; }
  .row .l { color: #444; }
  ...
  const totalRows = (d.totals || [])
    .map((t) => {
      ...
      const right =
        t.amount != null ? money(t.amount, d.currencyCode, d.locale) : '';
...
<style>${styles}</style>
  ...
  <hr/>
  <div class="c b">${esc(String(d.documentLabel).toUpperCase())}</div>
  <div class="c s m">${esc(d.periodLabel)}</div>
  <div class="c s">${esc(d.customerName)}${d.houseNo ? ` · H ${esc(d.houseNo)}` : ''}</div>
  <div class="c s m">${esc(d.invoiceNumber)} · ${esc(d.paymentMethod)} · ${esc(String(d.activeDays || 0))} days</div>
  <hr/>
  <div class="grid">${dayBlock}</div>
  <hr/>
  ${totalRows}
  <div class="row tot"><span class="l">TOTAL</span><span class="r">${esc(money(d.grandTotal, d.currencyCode, d.locale))}</span></div>
  <hr/>
  <div class="foot">
    <div class="b">Shukriya · Thank you</div>
    <div class="s m">Y = delivered · N = not delivered</div>
  </div>
</body></html>`;
}
...
export async function downloadMilkHisabDayBreakdownPdf(model) {
  try {
    ...
    write(`${d.customerName}${d.houseNo ? ` · H ${d.houseNo}` : ''}`, {
      size: 7,
      align: 'center',
    });
    write(`${d.invoiceNumber} · ${d.paymentMethod} · ${d.activeDays || 0} days`, {
      size: 6.5,
      align: 'center',
    });
    rule();
    ...
    doc.text(money(d.grandTotal, d.currencyCode, d.locale), pageW - margin, y, {
      align: 'right',
    });
    y += 4;
    rule();
    write('Shukriya · Thank you', { size: 7, bold: true, align: 'center' });
    write('Y = delivered · N = not delivered', { size: 6, align: 'center' });
