...
export function buildMilkHisabDayBreakdownHtml(model) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sec {
    ...
    margin: 1.2mm 0 0.8mm; text-align: ${isUrdu ? 'right' : 'left'};
  }
...
export async function downloadMilkHisabDayBreakdownPdf(model) {
  ...
  try {
    ...
    write(d.periodTitle || d.periodLabel, { size: 7, bold: true, align: 'center' });
    if (d.periodRange) write(d.periodRange, { size: 6.5, align: 'center' });
    write(
