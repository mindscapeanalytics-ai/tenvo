...
const getCachedTenantAccess = cache(async (userId, businessId, platformAdmin) => {
    ...
    try {
        ...
        return { role, planTier, tenantSettings, limitOverrides };
    } finally {
        client.release();
    }
});
 ...
 * @returns {Promise<{ session: object, role: string, planTier: string, isPlatformAdmin?: boolean }>}
 */
export async function withGuard(businessId, options = {}) {
    ...
    if (!txClient) {
        ...
        const { role, planTier, tenantSettings, limitOverrides } = access;
        ...
        return { session, role, planTier, isPlatformAdmin: platformAdmin };
    }
