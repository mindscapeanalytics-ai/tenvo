...
import { isModuleAllowed, permissionToModule } from '@/lib/rbac/moduleAccess';
...
/**
 * Each permission key maps to an exact allow-list of roles (Zoho/Busy least privilege).
 * Owner always bypasses via hasPermission(); do not rely on hierarchy inheritance —
 * list every role that should have access (admin/manager/etc. already appear on most keys).
 */
export const PERMISSION_DEFINITIONS = {
    ...
    // -- Orders --
    'orders.view': ['salesperson', 'cashier', 'warehouse_manager', 'manager', 'admin', 'owner'],
    ...
    // -- Tax Compliance --
    'tax.view': ['accountant', 'manager', 'admin', 'owner'],
    ...
    'warehouses.manage': ['warehouse_manager', 'manager', 'admin', 'owner'],
    ...
    // -- Workflows & Approvals --
    'approvals.request': ['salesperson', 'cashier', 'accountant', 'warehouse_manager', 'manager', 'admin', 'owner'],
 ...
 * @param {Record<string, boolean> | null | undefined} [moduleAccess] - Optional module overrides; null/undefined = role-only
 * @returns {boolean}
 */
export function hasPermission(role, permission, moduleAccess) {
    ...
    if (!allowedRoles) {
        ...
        // Exact allow-list only (no "viewer listed ⇒ waiter/chef inherit" leakage)
        const normalizedAllowed = allowedRoles.map((r) => r.toLowerCase().trim());
        roleAllowed = normalizedAllowed.includes(effectiveRole);
    }
...
export const NAV_PERMISSION_MAP = {
    ...
    'route-hisab': { permission: 'sales.view', feature: null },
    ...
    // Unlock when basic_reports OR basic_accounting (Free has both; expense_tracking is Starter+)
    finance: { permission: 'finance.view_reports', feature: 'basic_reports', featuresAny: ['basic_reports', 'basic_accounting'] },
    ...
    inquiries: { permission: 'orders.view', feature: 'storefront_orders' },
    'view-storefront': { permission: 'dashboard.view', feature: null },
    ...
    batches: { permission: 'inventory.edit', feature: 'batch_tracking' },
    serials: { permission: 'inventory.edit', feature: 'serial_tracking' },
    ...
    reports: { permission: 'analytics.basic', feature: 'advanced_reports', featuresAny: ['advanced_reports', 'ai_analytics'] },
 ...
 * @returns {{ visible: boolean, locked: boolean, requiredPlan: string | null }}
 */
export function getNavItemAccess(navKey, role, planTier, businessSettings, platformOverrides, moduleAccess) {
    const mapping = NAV_PERMISSION_MAP[navKey];
    // Unknown keys stay hidden (exact allow-list) — never fail-open.
    if (!mapping) return { visible: false, locked: false, requiredPlan: null };

    // 1. Role-based visibility
    const hasRole = hasPermission(role, mapping.permission, moduleAccess);
    if (!hasRole) {
        return { visible: false, locked: false, requiredPlan: null };
    }

    // 2. Feature flag (subscription) lock — support featuresAny (OR) or single feature
    const featureKeys = Array.isArray(mapping.featuresAny) && mapping.featuresAny.length > 0
        ? mapping.featuresAny
        : (mapping.feature ? [mapping.feature] : []);

    if (featureKeys.length > 0) {
        const anyEnabled = featureKeys.some((fk) =>
            planHasFeatureWithPackaging(
                planTier || 'free',
                fk,
                businessSettings,
                platformOverrides
        ...
        if (!anyEnabled) {
            const primary = mapping.feature || featureKeys[0];
            return {
                visible: true,
                locked: true,
                requiredPlan: FEATURE_MIN_PLAN[primary] || 'starter',
            };
        }
    }
...
export const DOMAIN_DEFAULT_ROLES = {
    ...
    'wholesale-distribution': ['owner', 'admin', 'manager', 'warehouse_manager', 'accountant', 'salesperson'],
    'pharmacy': ['owner', 'admin', 'manager', 'warehouse_manager', 'cashier', 'salesperson'],
    'auto-parts': ['owner', 'admin', 'manager', 'warehouse_manager', 'cashier', 'salesperson'],
    'textile-wholesale': ['owner', 'admin', 'manager', 'warehouse_manager', 'accountant', 'salesperson'],
