...
export const NAV_PERMISSION_MAP = {
    ...
    journal: { permission: 'finance.view_gl', feature: 'journal_entries' },
    
    // POS Module Features - Available on all plans
    pos: { permission: 'pos.access', feature: null },
    refunds: { permission: 'pos.process_refund', feature: 'pos_refunds' },
    
    // Store Module Features - Available on all plans
    'store-settings': { permission: 'settings.view', feature: null },
    restaurant: { permission: 'restaurant.view_tables', feature: 'restaurant_pos' },
    kds: { permission: 'restaurant.view_kds', feature: 'kitchen_display_system' },
    
    // CRM Module Features - Growth+
    loyalty: { permission: 'crm.manage_loyalty', feature: 'loyalty_programs' },
    campaigns: { permission: 'crm.view_campaigns', feature: 'campaigns_email_sms' },
    crm: { permission: 'crm.view_segments', feature: 'customer_segmentation' },
    promotions: { permission: 'crm.manage_promotions', feature: 'promotions_discounts' },
    ...
    bom: { permission: 'manufacturing.view', feature: 'bill_of_materials' },
    
    // HR Module Features - Business+
    payroll: { permission: 'hr.view_employees', feature: 'payroll_processing' },
    ...
    forecasting: { permission: 'analytics.basic', feature: 'ai_demand_forecasting' },
    ...
    audit_trail: { permission: 'audit.view_logs', feature: 'audit_trail' },
    ...
    integrations: { permission: 'settings.view', feature: 'third_party_integrations' },
...
export function getNavItemAccess(navKey, role, planTier) {
    ...
    if (mapping.feature) {
        const featureEnabled = planHasFeature(planTier || 'free', mapping.feature);
        if (!featureEnabled) {
            return {
                visible: true,
                locked: true,
