 ...
 * @returns {{ visible: boolean, locked: boolean, requiredPlan: string | null }}
 */
export function getNavItemAccess(navKey, role, planTier, businessSettings) {
    ...
    if (mapping.feature) {
        const featureEnabled = planHasFeatureWithPackaging(planTier || 'free', mapping.feature, businessSettings);
        if (!featureEnabled) {
            return {
                visible: true,
                locked: true,
 ...
 */
export function canAccessTab(role, planTier, navKey, businessSettings) {
    const access = getNavItemAccess(navKey, role, planTier, businessSettings);
