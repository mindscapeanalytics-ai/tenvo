 ...
 * @param {object} params
 */
export function resolveInvoiceDocumentTitle({ category, invoiceType, standards, isPakistaniDomain, transactionMode }) {
  const domainLabel = getDomainConfig(category)?.label_overrides?.invoice;
  const type = String(invoiceType || 'retail').toLowerCase();

  const isVehicleVertical = ['vehicle-dealership', 'auto-marketplace', 'rent-a-car'].includes(String(category || '').toLowerCase());
  if (isVehicleVertical) {
    if (transactionMode === 'purchase') return 'VEHICLE TRADE-IN & PURCHASE RECEIPT';
    if (transactionMode === 'rental') return 'VEHICLE RENTAL & LEASE AGREEMENT';
    return 'VEHICLE SALES AGREEMENT & DELIVERY RECEIPT';
  }
...
export function normalizePdfLineItem(item) {
  ...
  let rawName = item.name || item.product_name || item.description || 'Item';
  const dd = item.domain_data || item.metadata?.domain_data || item.metadata || {};
  const itemCode = dd.itemcode || dd.item_code || dd.modelno || dd.partnumber;
  const finish = dd.finish || dd.shadecolor;
  const material = dd.material;
  const size = dd.size || dd.sizedimension;
  const pcsPerBox = Number(dd.pcsperbox || dd.pcs_per_box || 0);

  let name = rawName;
  if (itemCode && !rawName.includes(itemCode)) {
    name = `[${itemCode}] ${rawName}`;
  }

  const specParts = [];
  if (finish && !rawName.toLowerCase().includes(String(finish).toLowerCase())) specParts.push(`Finish: ${finish}`);
  if (material && !rawName.toLowerCase().includes(String(material).toLowerCase())) specParts.push(`Material: ${material}`);
  if (size && !rawName.toLowerCase().includes(String(size).toLowerCase())) specParts.push(`Size: ${size}`);

  const articleNo = item.article_no || dd.article_no || dd.articleno;
  const designNo = item.design_no || dd.design_no || dd.designno;
  const fabricType = item.fabric_type || dd.fabric_type || dd.fabrictype;
  const colorShade = item.color_shade || dd.color_shade || dd.colorshade;
  const thaanLen = Number(item.thaan_length || dd.thaan_length || dd.thaanlength || 0);

  if (articleNo) specParts.push(`Art: ${articleNo}`);
  if (designNo) specParts.push(`Des: ${designNo}`);
  if (fabricType) specParts.push(`Fabric: ${fabricType}`);
  if (colorShade) specParts.push(`Shade: ${colorShade}`);

  let unitStr = item.unit || item.metadata?.unit || 'pcs';
  const uLower = String(unitStr).toLowerCase().trim();
  if (uLower === 'thaan' && thaanLen > 0 && qty > 0) {
    unitStr = `Thaan (${qty * thaanLen}m)`;
  } else if (uLower === 'meter' || uLower === 'mtr' || uLower === 'm') {
    unitStr = 'mtr';
  } else if (pcsPerBox > 1 && qty >= pcsPerBox && ['pcs', 'piece', 'set', 'pair'].includes(uLower)) {
    const boxes = (qty / pcsPerBox).toFixed(1).replace(/\.0$/, '');
    unitStr = `${unitStr} (${boxes} Box${boxes === '1' ? '' : 'es'} @ ${pcsPerBox}/box)`;
  }

  let desc = item.description || '';
  if (specParts.length > 0) {
    desc = desc ? `${desc} | ${specParts.join(' • ')}` : specParts.join(' • ');
  }
