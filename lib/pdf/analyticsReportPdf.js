...
export function generateAnalyticsReportPDF(opts = {}) {
  ...
  const summaryRows = [
    ...
    ['Trailing revenue (6 mo chart)', formatPdfNumber(snapshot.trailingRevenue || 0, locale)],
    ['Trailing GL profit (6 mo)', formatPdfNumber(snapshot.trailingProfit || 0, locale)],
  ];
  ...
  if (trend.length) {
    ...
    doc.setFontSize(11);
    doc.setTextColor(...brand);
    doc.text('Monthly sales trend (last 6 months)', 14, y);
    y += 2;
    doc.autoTable({
      startY: y,
      head: [['Month', 'Revenue', 'Orders', 'GL profit']],
      body: trend.map((m) => [
        m.date || m.month || '—',
