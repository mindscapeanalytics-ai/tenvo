...
import { getMilkShopLeanFeatureStrip, getWaterDeliveryLeanFeatureStrip } from '@/lib/config/domainPackageFeatures';
...
export function getRegistrationVerticalFeatureOverrides(domainKey) {
  ...
  // Milk-shop: always strip F&B / gym / warehouse / HR clutter (any plan tier).
  // Package SKU may still add Pro diffs (batch, offline POS, etc.).
  if (canonical === 'milk-shop') {
    return pickAllowedFeatureOverrides(getMilkShopLeanFeatureStrip());
  }
  if (canonical === 'water-delivery') {
    return pickAllowedFeatureOverrides(getWaterDeliveryLeanFeatureStrip());
  }

  const knowledge = getDomainKnowledge(canonical);
  if (!knowledge) return {};

  // Other verticals: do not auto-inject plan bypasses (suite SKU / plan tier only).
  return {};
}
