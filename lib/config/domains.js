...
import { resolveDomainKey } from './domainKeyAliases.js';
...
export const BUSINESS_DOMAINS = {
    ...
    'leather-footwear': {
        ...
        default_categories: [
            ...
            'Sports Shoes', 'Casual', 'Sandals', 'Boots', 'Orthopedic', 'Leather Goods', 'Accessories',
        ],
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting'],
        recommended_modules: ['pos', 'batch_tracking', 'multi_warehouse'],
        label_overrides: {
    ...
    'marine-parts': {
        ...
        icon: 'Ship',
        description: 'Marine propulsion spares, thrusters, rudder propellers, seals, and lifecycle parts',
        tax_config: {
            default_tax_rate: 21,
            tax_label: 'VAT',
            withholding_enabled: false,
        ...
        default_units: ['pcs', 'set', 'kit', 'pair', 'box'],
        default_categories: [
            'New Systems', 'Used Systems', 'Spare Parts', 'Repair Service',
            'Rudder Propellers', 'Tunnel Thrusters', 'Azimuth Drives',
            'Seals & Sterntubes', 'Steering & Rudders', 'Hydraulic Cranes',
        ],
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting'],
        recommended_modules: [
            'serial_tracking', 'batch_tracking', 'multi_warehouse',
            'quotations', 'delivery_challans', 'expense_tracking',
        ],
        label_overrides: {
            product: 'Part / Unit',
            sku: 'Part Number / OEM',
    ...
    'milk-shop': {
        ...
        name_ur: 'ملک شاپ',
        icon: 'Milk',
        description: 'Neighborhood milk shop: fresh milk by kg, dahi, makkhan, lassi, packaged dairy',
        tax_config: {
            default_tax_rate: 0,
            tax_label: 'GST',
            withholding_enabled: false,
        ...
        default_units: ['kg', 'litre', 'pack', 'pcs', 'dozen'],
        default_categories: [
            'Fresh Milk',
            'Yogurt / Dahi',
            'Cream & Butter',
        ...
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting', 'pos'],
        recommended_modules: ['batch_tracking', 'expiry_tracking', 'expense_tracking'],
        label_overrides: {
            product: 'Dairy Item',
            invoice: 'Sales Bill',
        },
    ...
    'water-delivery': {
        key: 'water-delivery',
        name: 'Water Delivery',
        name_ur: 'پانی ڈیلیوری',
        icon: 'Droplets',
        description:
            'Pakistan bottled water supply: plant/rider routes by city and town code, 19L refill DEL/REC/BAL, PET packs, deposits for home and corporate',
        tax_config: {
            default_tax_rate: 0,
            tax_label: 'GST',
            withholding_enabled: false,
        ...
        default_units: ['bottle', 'case', 'pcs', 'dozen', 'litre'],
        default_categories: [
            '19L Dispenser',
            'Family Packs (1.5L / 5L)',
            'PET Small Bottles',
        ...
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting', 'pos'],
        recommended_modules: ['expense_tracking', 'credit_notes'],
        label_overrides: {
            product: 'Water SKU',
    ...
    'textile-wholesale': {
        ...
        name: 'Textile Wholesale (Jama Cloth)',
        name_ur: 'ٹیکسٹائل ہول سیل / جامہ کلاتھ',
        icon: 'Scroll',
        description:
            'Fabric and thaan wholesale for retailers and boutiques — lawn, cotton, khaddar, suit lots',
        tax_config: {
            default_tax_rate: 18,
            tax_label: 'GST',
            withholding_enabled: true,
        ...
        default_units: ['meter', 'gaz', 'suit', 'thaan', 'guth', 'kg'],
        default_categories: [
            'Lawn', 'Cotton', 'Wash & Wear', 'Chiffon', 'Silk', 'Khaddar',
            'Linen', 'Imported Fabric', 'Lunda Bazaar', 'Mens Unstitched', 'Bridal Collection',
        ],
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting'],
        recommended_modules: [
            'pos', 'batch_tracking', 'multi_warehouse', 'delivery_challans', 'quotations',
        ],
        label_overrides: {
            ...
            customer: 'Shop / Buyer',
            warehouse: 'Godown',
            invoice: 'Sales Invoice / Challan',
        },
        special_rules: {
            thaan_tracking: true,
            wholesaler_mode: true,
            further_tax_unregistered: true,
...
export function getDomainConfig(domainKey) {
    ...
    return BUSINESS_DOMAINS[key] || BUSINESS_DOMAINS[raw] || buildSyntheticBusinessDomain(key);
}
...
export function isPosRelevant(category, domainKnowledge = null) {
    ...
    if (getPosSet().has(key) || getPosSet().has(category)) return true;
    return !!(domainKnowledge?.retailMode || domainKnowledge?.posEnabled);
}

export function isHospitality(category) {
    const key = resolveDomainKey(category);
    return getHospSet().has(key) || getHospSet().has(category);
}

export function isMembershipRelevant(category) {
    const key = resolveDomainKey(category);
    return getMembershipSet().has(key) || getMembershipSet().has(category);
}
...
export function isCampaignRelevant(category, domainKnowledge = null) {
    const key = resolveDomainKey(category);
    if (getCampSet().has(key) || getCampSet().has(category)) return true;
...
export function isManufacturingRelevant(category, domainKnowledge = null) {
    const key = resolveDomainKey(category);
    if (getMfgSet().has(key) || getMfgSet().has(category)) return true;
