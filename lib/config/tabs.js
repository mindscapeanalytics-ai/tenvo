 ...
 * @returns {{ tab: string, statementReport: string | null, preferPayments?: boolean, preferTax?: boolean }}
 */
export function resolveFinanceHubNavigation(view) {
    ...
    if (key === 'vouchers' || key === 'voucher' || key === 'payments' || key === 'payment') {
        return { tab: 'overview', statementReport: null, preferPayments: true };
    }
    if (key === 'gst' || key === 'tax' || key === 'tax-gst') {
        return { tab: 'overview', statementReport: null, preferTax: true };
    }
    ...
    if (mapped && !FINANCE_STATEMENT_REPORT_BY_VIEW[mapped]) {
        // vouchers maps into FINANCE_VIEW_BY_TAB but is handled above
        if (mapped === 'vouchers') {
            return { tab: 'overview', statementReport: null, preferPayments: true };
        }
