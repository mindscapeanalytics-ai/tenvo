...
export function resolveStorefrontItemState(item, ctx) {
  ...
  if (item.alwaysShow && !item.isExternal) {
    return getNavItemAccess(item.key, effectiveRole, planTier, businessSettings, undefined, moduleAccess);
  }
  ...
  if (item.isExternal) {
    const access = getNavItemAccess(item.key === 'view-storefront' ? 'store-settings' : item.key, effectiveRole, planTier, businessSettings, undefined, moduleAccess);
