...
export const STOREFRONT_MOBILE_ITEMS = [
  { key: 'orders', label: 'Storefront Orders', sublabel: 'Online', icon: Package, alwaysShow: true },
  ...
  { key: 'sales', label: 'Sales', sublabel: 'Performance', icon: TrendingUp, alwaysShow: true },
  {
    key: 'view-storefront',
    label: 'Store',
    sublabel: 'Public',
...
export function resolveStorefrontItemState(item, ctx) {
  ...
  if (item.alwaysShow && !item.isExternal) {
    const navSettings = mergeMilkShopLeanNavSettings(businessSettings, category);
    return getNavItemAccess(item.key, effectiveRole, planTier, navSettings, undefined, moduleAccess);
  }
  ...
  if (item.isExternal) {
    const access = getNavItemAccess(item.key === 'view-storefront' ? 'store-settings' : item.key, effectiveRole, planTier, navSettings, undefined, moduleAccess);
