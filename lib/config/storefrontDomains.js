...
import { isTyreElevatedStore, TYRE_ACCENTS } from '../storefront/tyreStorefront.js';
import { isFootwearElevatedStore, FOOTWEAR_ACCENTS } from '../storefront/footwearStorefront.js';
import { isElectronicsElevatedStore, ELECTRONICS_ACCENTS } from '../storefront/electronicsStorefront.js';
import { isMilkShopStore, MILK_SHOP_ACCENTS } from '../storefront/milkShopStorefront.js';
import { isWaterShopStore, WATER_SHOP_ACCENTS } from '../storefront/waterShopStorefront.js';
...
export const STOREFRONT_DOMAIN_CONFIG = {
  ...
  'footwear-shop': {
    label: 'Footwear',
    heroTagline: 'The running edit',
    heroSubtitle: 'Local and imported sneakers, formal pairs, and kids shoes with size-first search.',
    ctaLabel: 'Shop shoes',
    accentColor: '#FFDA00',
    accentDark: '#E6C400',
    accentLight: '#FFF8CC',
    heroImage:
      'https://m.media-amazon.com/images/G/01/zappos/2026/Homepage/July/07.26.26/HP-ZIM-CARL-RADKE-DESKTOP-RETINA-2880X1240._FMwebp_QL85_.jpg',
    categoryImages: {
      default:
        'https://m.media-amazon.com/images/G/01/zappos/2026/Homepage/July/07.26.26/HP-VIS-NAV-ZIM-CARL-RADKE-NIKE-444X555-FRAME3._FMwebp_QL85_.jpg',
    },
    trustBadges: [
      { icon: 'shield', title: '100% authentic', subtitle: 'Local + imported brands' },
      { icon: 'refresh', title: 'Easy exchanges', subtitle: 'Size swaps on unused pairs' },
      { icon: 'star', title: 'Size-first search', subtitle: 'UK / EU sizes in stock' },
      { icon: 'truck', title: 'Fast delivery', subtitle: 'Nationwide shipping' },
    ],
    featuredSectionTitle: 'Trending styles',
    newArrivalsSectionTitle: 'Just in',
    bannerText: 'Find your size · Shop men, women, kids, and sports',
  },
  ...
  'hardware-parts': {
    ...
    categoryImages: {
      default: 'https://images.unsplash.com/photo-1581092160562-40aa08e78837?w=600&q=80&auto=format&fit=crop',
    },
    trustBadges: [
...
export const STOREFRONT_CATEGORY_TO_VERTICAL = {
  ...
  'leather-footwear': 'footwear-shop',
  ...
  'milk-shop': 'supermarket',
  milk: 'supermarket',
  milkshop: 'supermarket',
  'doodh-shop': 'supermarket',
  'dairy-shop': 'supermarket',
  'water-delivery': 'supermarket',
  water: 'supermarket',
  'water-shop': 'supermarket',
  'water-supply': 'supermarket',
  'bottled-water': 'supermarket',
  'mineral-water': 'supermarket',
  'aqua-delivery': 'supermarket',
  'water-rider': 'supermarket',
  // services.js
  travel: 'default',
  'auto-workshop': 'hardware-parts',
  'diagnostic-lab': 'pharmacy',
  'gym-fitness': 'retail-shop',
  'hotel-guesthouse': 'default',
  'event-management': 'default',
  'rent-a-car': 'vehicle-dealership',
  ...
  'livestock-cattle': 'supermarket',
  'poultry-farm': 'supermarket',
...
function inferVerticalFromCategorySlug(c) {
  ...
  if (/supermarket|grocery|fmcg|dairy-farm|milk-shop|doodh|produce|water-delivery|bottled-water|mineral-water/.test(c)) return 'supermarket';
...
export function getStoreAccentColor(settings, category) {
  ...
  if (isTyreElevatedStore(category)) return TYRE_ACCENTS.accent;
  if (isFootwearElevatedStore(category)) return FOOTWEAR_ACCENTS.accent;
  if (isElectronicsElevatedStore(category)) return ELECTRONICS_ACCENTS.accent;
  if (isMilkShopStore(category)) return MILK_SHOP_ACCENTS.accent;
  if (isWaterShopStore(category)) return WATER_SHOP_ACCENTS.accent;
...
export function getStoreAccentPalette(settings, category) {
  ...
  const tyre = isTyreElevatedStore(category) ? TYRE_ACCENTS : null;
  const footwear = isFootwearElevatedStore(category) ? FOOTWEAR_ACCENTS : null;
  const electronics = isElectronicsElevatedStore(category) ? ELECTRONICS_ACCENTS : null;
  const milkShop = isMilkShopStore(category) ? MILK_SHOP_ACCENTS : null;
  const waterShop = isWaterShopStore(category) ? WATER_SHOP_ACCENTS : null;
  const accent = getStoreAccentColor(settings, category);
  return {
    accent,
    accentDark:
      marketplace?.accentDark ||
      dealership?.accentDark ||
      tyre?.accentDark ||
      footwear?.accentDark ||
      electronics?.accentDark ||
      milkShop?.accentDark ||
      waterShop?.accentDark ||
      luxury?.accentDark ||
      cfg.accentDark ||
      accent,
    accentLight:
      marketplace?.accentLight ||
      dealership?.accentLight ||
      tyre?.accentLight ||
      footwear?.accentLight ||
      electronics?.accentLight ||
      milkShop?.accentLight ||
      waterShop?.accentLight ||
      luxury?.accentLight ||
      cfg.accentLight ||
      '#f8fafc',
  };
