...
export function getStoreAccentPalette(settings, category) {
  ...
  return {
    accent,
    accentDark: marketplace?.accentDark || dealership?.accentDark || tyre?.accentDark || electronics?.accentDark || luxury?.accentDark || cfg.accentDark || accent,
    accentLight: marketplace?.accentLight || dealership?.accentLight || tyre?.accentLight || electronics?.accentLight || luxury?.accentLight || cfg.accentLight || '#f8fafc',
  };
}
