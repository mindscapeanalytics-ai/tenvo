...
export function getStoreAccentPalette(settings, category) {
  ...
  return {
    accent,
    accentDark: marketplace?.accentDark || dealership?.accentDark || tyre?.accentDark || luxury?.accentDark || cfg.accentDark || accent,
    accentLight: marketplace?.accentLight || dealership?.accentLight || tyre?.accentLight || luxury?.accentLight || cfg.accentLight || '#f8fafc',
  };
}
