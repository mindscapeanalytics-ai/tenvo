...
/**
 * Milk shop (Pakistan doodh shop) — counter kg POS, FEFO, Route Hisab domain tab.
 * Base tier is Professional (not Business): strip warehouse/serials/CRM/F&B clutter.
 */
const MILK_DESIRED = {
  restaurant_pos: false,
  restaurant_kds: false,
...
/**
 * False-only lean strip for any milk-shop tenant (Starter → Enterprise).
 * Ensures CRM / F&B / HR / warehouse chrome stays off without requiring milk-commerce SKU.
 * @returns {Record<string, boolean>}
 */
export function getMilkShopLeanFeatureStrip() {
  /** @type {Record<string, boolean>} */
  const strip = {};
  for (const [key, value] of Object.entries(MILK_DESIRED)) {
    if (value === false) strip[key] = false;
  }
  return strip;
}

export const MILK_COMMERCE_FEATURE_OVERRIDES = Object.freeze(
  buildDomainPackageFeatureOverrides('professional', MILK_DESIRED)
);
...
export const MILK_COMMERCE_HIGHLIGHTS = pickEnabledPackageFeatures(MILK_DESIRED, [
  'pos',
  'offline_pos_mode',
  'barcode_scanning',
  'batch_tracking',
...
/**
 * Lean strip for water-delivery tenants (any plan).
 * @returns {Record<string, boolean>}
 */
export function getWaterDeliveryLeanFeatureStrip() {
  /** @type {Record<string, boolean>} */
  const strip = {};
  for (const [key, value] of Object.entries(WATER_DESIRED)) {
    if (value === false) strip[key] = false;
  }
  return strip;
}

export const WATER_COMMERCE_FEATURE_OVERRIDES = Object.freeze(
  buildDomainPackageFeatureOverrides('professional', WATER_DESIRED)
);

export const WATER_COMMERCE_HIGHLIGHTS = pickEnabledPackageFeatures(WATER_DESIRED, [
  'expense_tracking',
  'customers',
  'tax_compliance',
  'advanced_reports',
