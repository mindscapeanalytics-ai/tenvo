 ...
 */
/**
 * When `NEXT_PUBLIC_DEV_FULL_FEATURES=true`, per-user rollout is bypassed so any
 * flag that is on in env works for QA (owner / plan testing). Production: omit or false.
 */
function isDevFullFeaturesBypass() {
  return process.env.NEXT_PUBLIC_DEV_FULL_FEATURES === 'true';
}
