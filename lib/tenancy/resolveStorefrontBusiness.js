...
import { cacheStorefrontRead, StorefrontCacheBypassError } from '@/lib/storefront/storefrontCachedRead';
import {
  storefrontBusinessTag,
  STOREFRONT_BUSINESS_COMPACT_CACHE_KEY,
...
function isStorefrontApprovalBlocked(row) {
  const status = String(row?.approval_status ?? '')
    .trim()
    .toLowerCase();
  // Legacy rows with no approval column/value stay publicly reachable.
...
function isStorefrontDisabled(row, settings) {
  return (
    isStorefrontApprovalBlocked(row) ||
...
/** Missing table/column on legacy business_settings (42P01 / 42703). */
function shouldFallbackBusinessSettings(err) {
  return (
    err?.code === '42P01' ||
    err?.code === '42703' ||
    Boolean(err?.message?.includes('business_settings')) ||
    Boolean(err?.message?.includes('is_storefront_enabled'))
  );
}
...
async function queryBusinessByDomainSegment(normalizedDomain, client) {
  ...
  try {
    ...
    if (!shouldFallbackBusinessSettings(tableError)) {
      throw tableError;
    }
    result = await client.query(
      `SELECT
...
async function loadResolveStorefrontBusinessUncached(normalizedDomain) {
  ...
  if (redisCached?.id) {
    ...
    try {
      // PK lookup is indexed; avoid LOWER(domain) scan on every Redis hit.
      const domainRow = await queryBusinessRowById(redisCached.id, client);

      if (domainRow) {
        const rowDomain = normalizeStorefrontDomainSegment(domainRow.domain);
        let domainStillMatches = expandStorefrontDomainAliasKeys(rowDomain).includes(
          normalizedDomain
        );

        // Custom domains / secondary hostnames are not on businesses.domain — confirm
        // the requested segment still resolves to the same tenant.
        if (!domainStillMatches) {
          const segmentRow = await queryBusinessByDomainSegment(normalizedDomain, client);
          domainStillMatches = segmentRow?.id === redisCached.id;
        }
        ...
        if (domainStillMatches) {
          ...
          if (!isStorefrontDisabled(domainRow, settings)) {
            ...
            // Soft refresh cache — do not block TTFB on Redis writes.
            void setCachedStorefrontBusiness(normalizedDomain, resolved);
            return resolved;
          }
        }
      }
      // Stale or domain moved — purge and fall through to domain segment lookup.
...
export async function resolveStorefrontBusiness(domain) {
  ...
  return cacheStorefrontRead(
    async () => {
      const resolved = await loadResolveStorefrontBusinessUncached(normalizedDomain);
      if (!resolved) {
        throw new StorefrontCacheBypassError(null);
      }
      return resolved;
    },
    [STOREFRONT_BUSINESS_COMPACT_CACHE_KEY, normalizedDomain],
    {
...
export async function loadStorefrontBusinessShell(domain) {
  ...
  try {
    ...
    if (redisCached?.id) {
      const cachedRow = await queryBusinessRowById(redisCached.id, client);
      if (cachedRow) {
        const rowDomain = normalizeStorefrontDomainSegment(cachedRow.domain);
        let domainStillMatches = expandStorefrontDomainAliasKeys(rowDomain).includes(
          normalizedDomain
        );
        if (!domainStillMatches) {
        ...
        if (domainStillMatches) {
          const redisSettings = parseSettingsJson(cachedRow.store_settings);
          if (!isStorefrontDisabled(cachedRow, redisSettings)) {
            row = cachedRow;
          }
        } else {
          await purgeCachedStorefrontDomain(normalizedDomain);
