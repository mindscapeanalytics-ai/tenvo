...
export async function setCachedStorefrontBusiness(normalizedDomain, business) {
  ...
  // Exact request key + canonical domain only — do not fan out hyphen/underscore aliases
  // (avoids my-shop vs my_shop warm-path cross-tenant routing).
  const keys = new Set([normalizedDomain]);
  const canonical = business.domain ? String(business.domain).toLowerCase().trim() : '';
