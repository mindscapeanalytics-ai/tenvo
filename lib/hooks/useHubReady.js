 ...
 *
 * Optimistic localStorage is applied only after mount (useLayoutEffect) so the
 * first client render matches SSR HTML and avoids hydration mismatches.
 */
export function useHubReady() {
    ...
    const [clientCacheReady, setClientCacheReady] = useState(false);

    const domainFromPath = resolveBusinessDomainFromPath(pathname);

    useLayoutEffect(() => {
        setClientCacheReady(true);
    }, []);

    const cachedOptimisticShell = useMemo(() => {
        if (!clientCacheReady) return { business: null, role: null };
        return readOptimisticBusinessShell(domainFromPath);
    }, [domainFromPath, business?.id, role, clientCacheReady]);
