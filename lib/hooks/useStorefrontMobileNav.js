...
import { isHospitality, isPosRelevant, isMembershipRelevant } from '@/lib/config/domains';
...
export function useStorefrontMobileNav() {
  const { business, role, planTier: contextPlanTier, isLoading: businessLoading, isPlatformOwner, moduleAccess } = useBusiness();
  ...
  const ctx = useMemo(
    ...
    [category, domainKnowledge, posRelevant, hospitalityDomain, membershipRelevant, effectiveRole, planTier, business?.settings, moduleAccess]
  );
