...
export const validationHelpers = {
     ...
     */
    checkStockAvailability: async (productId, quantity, warehouseId = null, businessId = null) => {
        try {
            const { checkStockAvailabilityAction } = await import('@/lib/actions/standard/inventory/validation');
            if (!businessId) {
                return { success: false, error: 'Business context required for stock validation' };
            }
            const result = await checkStockAvailabilityAction(businessId, productId, quantity, warehouseId);
