...
function resolveNavVisibility(item, ctx) {
  ...
  return getNavItemAccess(item.key, effectiveRole, planTier, navSettings, platformFeatureOverrides, moduleAccess);
}
...
export function useHubMobileNav() {
  ...
  const ctx = useMemo(
    ...
    [category, domainKnowledge, posRelevant, hospitalityDomain, campaignRelevant, membershipRelevant, milkHisabRelevant, effectiveRole, planTier, business?.settings, business?.platformFeatureOverrides, moduleAccess]
  );
