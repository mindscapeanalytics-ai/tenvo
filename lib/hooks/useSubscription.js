...
export function useSubscription() {
  ...
  const [devInstantBilling, setDevInstantBilling] = useState(false);
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    try {
      ...
      setBillingMode(data.billingMode === 'manual' ? 'manual' : 'stripe');
      setStripeCheckoutAvailable(Boolean(data.stripeCheckoutAvailable));
      setDevInstantBilling(Boolean(data.devInstantBilling));
      setSubscription(data.subscription);
      // Invoices ship on the subscription payload — no separate /api/billing/invoices route
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      // Reuse subscription route (already lists Stripe customer invoices)
      const response = await fetch(
        `/api/billing/subscription?business_id=${encodeURIComponent(businessId)}`
      );
  ...
  const fetchPaymentMethods = useCallback(async () => {
    // No /api/billing/payment-methods route — cards are managed via Stripe Customer Portal
    setPaymentMethods([]);
    return [];
  }, []);

  const initiateCheckout = useCallback(
    async ({ planTier, domainPackageKey, currency = 'pkr', interval = 'monthly' }) => {
      if (!businessId) {
        toast.error('No active business selected');
        throw new Error('Missing business');
      }
      if (!planTier && !domainPackageKey) {
        toast.error('Select a plan or package');
        throw new Error('Missing billing SKU');
      }
  ...
  const createCryptoPayment = useCallback(
    async ({ amount, currency = 'usd', cryptoCurrency = 'btc', planTier = null }) => {
      if (!businessId) {
        toast.error('No active business selected');
        return;
      }
  ...
  const checkCryptoPaymentStatus = useCallback(async (paymentId) => {
    ...
    try {
      const response = await fetch(
        `/api/billing/crypto/status?paymentId=${encodeURIComponent(paymentId)}&business_id=${encodeURIComponent(businessId)}`
      );
