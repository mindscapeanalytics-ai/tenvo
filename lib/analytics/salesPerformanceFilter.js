 ...
 * channel: all | invoice | pos | storefront | restaurant
 */

import { resolveAnalyticsRange, toAnalyticsIsoDate } from '@/lib/utils/analyticsRange';

export const SALES_CHANNELS = Object.freeze(['all', 'invoice', 'pos', 'storefront', 'restaurant']);
...
/**
 * @param {unknown} raw
 * @returns {'all' | 'invoice' | 'pos' | 'storefront' | 'restaurant'}
 */
export function normalizeSalesChannel(raw) {
  ...
  if (v === 'restaurant' || v === 'resto' || v === 'kitchen' || v === 'dine-in') return 'restaurant';
  if (v === 'all' || v === '') return 'all';
  if (SALES_CHANNELS.includes(v)) {
    return /** @type {'all' | 'invoice' | 'pos' | 'storefront' | 'restaurant'} */ (v);
  }
  return 'all';
}
