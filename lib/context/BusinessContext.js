'use client';

import { createContext, useContext, useState, useEffect, useLayoutEffect, useMemo, useCallback } from 'react';
import { usePathname } from 'next/navigation';
import { authClient } from '@/lib/auth-client';
import { useAuth } from '@/lib/context/AuthContext';
import { getBusinessByUserId, getBusinessByDomainAndUser } from '@/lib/actions/basic/business';
import {
    getRegionalStandardsForBusiness,
    getBusinessRegionalPack,
} from '@/lib/utils/businessRegionalContext';
import { isPlatformOwner, TRIAL_CONFIG, getTrialDaysRemaining, isTrialActive } from '@/lib/config/platform';
import {
    readOptimisticBusinessShell,
    hasValidOptimisticShell,
    persistBusinessShell,
    clearBusinessShell,
    restoreCachedRoleForBusiness,
    findJoinedBusinessByDomain,
    persistJoinedBusinessesList,
    readJoinedBusinessesList,
    shellMatchesDomain,
} from '@/lib/utils/businessClientCache';

const BusinessContext = createContext(undefined);

/** Max wait before unblocking the hub shell with cached context (stale-while-revalidate). */
const BUSINESS_SYNC_TIMEOUT_MS = 10_000;

/** Routes that need tenant business resolution — skip on marketing/storefront for faster loads. */
function pathNeedsBusinessSync(pathname) {
  if (!pathname) return false;
  return (
    pathname.startsWith('/business/') ||
    pathname.startsWith('/admin') ||
    pathname.startsWith('/purchases') ||
    pathname === '/multi-business' ||
    pathname.startsWith('/pending-approval')
  );
}

/**
 * Apply cached shell into React state (client-only).
 * Must not seed useState from localStorage — that diverges SSR HTML from the
 * client first paint. Call from useLayoutEffect before browser paint instead.
 */
function applyOptimisticShell(setters, domainFromPath) {
    const shell = readOptimisticBusinessShell(domainFromPath);
    const { setBusiness, setRole, setModuleAccess, setRegionalStandards, setIsLoading } = setters;

    if (shell.business) {
        setBusiness((prev) => {
            if (
                prev?.id &&
                domainFromPath &&
                String(prev.domain || '').toLowerCase() !== String(domainFromPath).toLowerCase()
            ) {
                return shell.business;
            }
            return prev ?? shell.business;
        });
        setRegionalStandards((prev) => prev ?? getRegionalStandardsForBusiness(shell.business));
        setModuleAccess((prev) => prev ?? shell.business.user_module_access ?? null);
    }
    if (shell.role) {
        setRole((prev) => prev ?? shell.role);
    }
    if (hasValidOptimisticShell(shell.business, shell.role, domainFromPath)) {
        setIsLoading(false);
    }

    return shell;
}

/**
 * Business Context Provider
 * Syncs Better Auth User with their Business record using Server Actions
 */
export function BusinessProvider({ children }) {
    const pathname = usePathname();
    const { data: sessionData, isPending } = authClient.useSession();
    const { user: hydratedAuthUser, serverHydrated } = useAuth();
    const businessDomainFromPath = useMemo(() => {
        const pathParts = pathname.split('/');
        return pathParts[1] === 'business' ? pathParts[2] || null : null;
    }, [pathname]);

    const [business, setBusiness] = useState(null);
    const [role, setRole] = useState(null);
    const [moduleAccess, setModuleAccess] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isRevalidating, setIsRevalidating] = useState(false);
    const [regionalStandards, setRegionalStandards] = useState(null);

    // Prefer live Better Auth client session; fall back to hub server hydrate.
    const effectiveUser = sessionData?.user ?? hydratedAuthUser;
    // Stable primitive for the sync effect — session object identity can change on
    // refetches without the user actually changing (would re-trigger business sync).
    const effectiveUserId = effectiveUser?.id ?? null;
    // Hub layouts hydrate AuthContext after getServerSession — do not stall tenant sync.
    const sessionReady = !isPending || serverHydrated || Boolean(hydratedAuthUser?.id);

    // Apply cache before paint (mount + domain change) so chrome is instant without
    // SSR/client HTML drift from sync localStorage in useState.
    useLayoutEffect(() => {
        applyOptimisticShell(
            { setBusiness, setRole, setModuleAccess, setRegionalStandards, setIsLoading },
            businessDomainFromPath
        );
    }, [businessDomainFromPath]);

    // Sync business record with logged in user (background revalidate when cache exists).
    useEffect(() => {
        let isActive = true;
        let timeoutId;

        if (!pathNeedsBusinessSync(pathname)) {
            if (sessionReady) {
                queueMicrotask(() => {
                    setIsLoading(false);
                    setIsRevalidating(false);
                });
            }
            return () => {
                isActive = false;
            };
        }

        const syncBusiness = async () => {
            if (!sessionReady) {
                return;
            }

            const cachedShell = readOptimisticBusinessShell(businessDomainFromPath);
            let shellBusiness = cachedShell.business;
            let shellRole = cachedShell.role;

            const domainMismatch =
                businessDomainFromPath &&
                shellBusiness?.domain &&
                shellBusiness.domain !== businessDomainFromPath;

            // Joined-business list fallback: instant paint when navigating /business/{domain}
            // without awaiting getBusinessByDomainAndUser (multi-business picker, deep links).
            if (
                businessDomainFromPath &&
                (!shellBusiness || domainMismatch) &&
                !hasValidOptimisticShell(shellBusiness, shellRole, businessDomainFromPath)
            ) {
                const joined = findJoinedBusinessByDomain(businessDomainFromPath);
                if (joined?.id) {
                    shellBusiness = joined;
                    shellRole = joined.user_role || shellRole;
                }
            }

            const optimisticShell = hasValidOptimisticShell(
                shellBusiness,
                shellRole,
                businessDomainFromPath
            );

            if (isActive) {
                const shellMatchesUrl = shellMatchesDomain(shellBusiness, businessDomainFromPath);
                if (optimisticShell && shellMatchesUrl) {
                    setIsRevalidating(true);
                    setIsLoading(false);
                    setBusiness((prev) => {
                        if (
                            prev?.id &&
                            businessDomainFromPath &&
                            String(prev.domain || '').toLowerCase() !==
                                String(businessDomainFromPath).toLowerCase()
                        ) {
                            return shellBusiness;
                        }
                        return prev ?? shellBusiness;
                    });
                    setRegionalStandards(getRegionalStandardsForBusiness(shellBusiness));
                    if (shellRole) {
                        setRole(shellRole);
                    }
                    if (shellBusiness?.user_module_access !== undefined) {
                        setModuleAccess(shellBusiness.user_module_access ?? null);
                    }
                } else if (!shellBusiness || domainMismatch) {
                    if (business?.id) {
                        setIsRevalidating(true);
                        setIsLoading(false);
                    } else {
                        setIsLoading(true);
                        setIsRevalidating(false);
                    }
                }
            }

            timeoutId = setTimeout(() => {
                if (!isActive) return;
                if (process.env.NODE_ENV !== 'production') {
                    console.info('[BusinessContext] Using cached business data (fetch took >5s)');
                }
                setIsLoading(false);
                setIsRevalidating(false);
            }, BUSINESS_SYNC_TIMEOUT_MS);

            try {
                if (effectiveUserId) {
                    const domainFromUrl = businessDomainFromPath;

                    let result;

                    if (domainFromUrl) {
                        result = await getBusinessByDomainAndUser(domainFromUrl);
                        if (!result?.success) {
                            console.warn(
                                `[BusinessContext] Not authorized for domain '${domainFromUrl}' or it doesn't exist.`
                            );
                            if (!isActive) return;
                            setBusiness(null);
                            setRole(null);
                            setModuleAccess(null);
                            clearBusinessShell();
                            setIsLoading(false);
                            setIsRevalidating(false);
                            if (typeof window !== 'undefined') {
                                const target = `/multi-business?reason=domain_access&domain=${encodeURIComponent(domainFromUrl)}`;
                                if (window.location.pathname !== '/multi-business') {
                                    window.location.replace(target);
                                }
                            }
                            return;
                        }
                    } else {
                        result = await getBusinessByUserId();
                    }

                    if (!isActive) return;

                    if (result?.success && result.business) {
                        const biz = result.business;

                        setBusiness(biz);
                        setRegionalStandards(getRegionalStandardsForBusiness(biz));

                        const userRole = biz.user_role || 'salesperson';
                        setRole(userRole);
                        setModuleAccess(biz.user_module_access ?? null);
                        persistBusinessShell(biz, userRole);
                        persistJoinedBusinessesList(
                            readJoinedBusinessesList().map((row) =>
                                row?.id === biz.id ? { ...row, ...biz, user_role: userRole } : row
                            )
                        );
                    } else {
                        setBusiness(null);
                        setRole(null);
                        setModuleAccess(null);
                        clearBusinessShell();
                    }
                } else if (!isPending) {
                    // Client session settled with no user — clear tenant.
                    setBusiness(null);
                    setRole(null);
                    setModuleAccess(null);
                    clearBusinessShell();
                }
                // If still pending but server-hydrated without effectiveUser yet, keep waiting.
            } catch (error) {
                console.error('Business sync error:', error);
                if (!isActive) return;

                const cachedRole = restoreCachedRoleForBusiness(shellBusiness);
                if (shellBusiness?.id && cachedRole) {
                    setRole((prev) => prev ?? cachedRole);
                    setBusiness((prev) => prev ?? shellBusiness);
                    setRegionalStandards((prev) =>
                        prev ?? getRegionalStandardsForBusiness(shellBusiness)
                    );
                } else if (!effectiveUserId) {
                    setBusiness(null);
                    setRole(null);
                    setModuleAccess(null);
                    clearBusinessShell();
                }
            } finally {
                clearTimeout(timeoutId);
                if (isActive) {
                    setIsLoading(false);
                    setIsRevalidating(false);
                }
            }
        };

        syncBusiness();

        return () => {
            isActive = false;
            clearTimeout(timeoutId);
        };
    }, [
        effectiveUserId,
        sessionReady,
        isPending,
        businessDomainFromPath,
        pathname,
    ]);

    const getSetting = useCallback((path, defaultValue) => {
        if (!business?.settings) return defaultValue;

        const keys = path.split('.');
        let current = business.settings;

        for (const key of keys) {
            if (current && typeof current === 'object' && key in current) {
                current = current[key];
            } else {
                return defaultValue;
            }
        }

        return current;
    }, [business]);

    const regionalPack = useMemo(() => (
        business
            ? getBusinessRegionalPack(business)
            : regionalStandards
                ? {
                    currency: regionalStandards.currency,
                    currencySymbol: regionalStandards.currencySymbol,
                    taxLabel: regionalStandards.taxLabel,
                    taxIdLabel: regionalStandards.taxIdLabel,
                    taxEnabled: true,
                    defaultTaxRate: regionalStandards.defaultTaxRate,
                    locale: regionalStandards.locale,
                    countryIso: regionalStandards.countryCode,
                }
                : null
    ), [business, regionalStandards]);

    const currency = regionalPack?.currency || 'PKR';
    const currencySymbol = regionalPack?.currencySymbol || '₨';

    const isPlatformOwnerUser = effectiveUser ? isPlatformOwner(effectiveUser) : false;
    const isPlatformLevelUser = effectiveUser
        ? (isPlatformOwnerUser || effectiveUser.role === 'admin')
        : false;

    const updateBusiness = useCallback((data) => {
        setBusiness(prev => {
            const updated = { ...prev, ...data };
            try {
                localStorage.setItem('businessData', JSON.stringify(updated));
            } catch {
                // localStorage may be unavailable (private browsing quota exceeded)
            }
            return updated;
        });
    }, []);

    const clearBusiness = useCallback(() => {
        setBusiness(null);
        setRole(null);
        setModuleAccess(null);
        clearBusinessShell();
    }, []);

    const switchBusinessByDomain = useCallback(async (domain) => {
        if (!effectiveUserId || !domain) return { success: false, error: 'Missing business context' };

        const hadTenant = Boolean(business?.id);
        const joined = findJoinedBusinessByDomain(domain);
        if (joined?.id && joined.user_role) {
            const userRole = joined.user_role || 'salesperson';
            setBusiness(joined);
            setRegionalStandards(getRegionalStandardsForBusiness(joined));
            setRole(userRole);
            setModuleAccess(joined.user_module_access ?? null);
            persistBusinessShell(joined, userRole);
            setIsRevalidating(true);
            setIsLoading(false);
        } else if (hadTenant) {
            setIsRevalidating(true);
        } else {
            setIsLoading(true);
        }
        try {
            const result = await getBusinessByDomainAndUser(domain);
            if (result.success && result.business) {
                const biz = result.business;
                setBusiness(biz);
                setRegionalStandards(getRegionalStandardsForBusiness(biz));
                const userRole = biz.user_role || 'salesperson';
                setRole(userRole);
                setModuleAccess(biz.user_module_access ?? null);
                persistBusinessShell(biz, userRole);
                const existing = readJoinedBusinessesList();
                if (existing.length > 0) {
                    persistJoinedBusinessesList(
                        existing.map((row) =>
                            row?.id === biz.id ? { ...row, ...biz, user_role: userRole } : row
                        )
                    );
                }
                return { success: true, business: biz };
            }
            const fallbackMessage = result.code === 'BUSINESS_ACCESS_DENIED'
                ? 'You do not have access to this business'
                : 'Unable to switch business';
            return { success: false, error: result.error || fallbackMessage };
        } catch (error) {
            console.error('Switch business error:', error);
            return { success: false, error: error.message };
        } finally {
            setIsLoading(false);
            setIsRevalidating(false);
        }
    }, [business?.id, effectiveUserId]);

    const planTier = business?.plan_tier || 'free';
    const effectivePlanTier = isPlatformOwnerUser ? 'enterprise' : planTier;
    const effectiveRole = isPlatformOwnerUser ? 'owner' : role;
    // Platform owner uses role owner bypass; keep moduleAccess null (role-only / full access).
    const effectiveModuleAccess = isPlatformOwnerUser ? null : moduleAccess;

    const planExpiresAt = business?.plan_expires_at || null;
    const trialActive = planExpiresAt ? isTrialActive(planExpiresAt) : false;
    const trialDaysRemaining = planExpiresAt ? getTrialDaysRemaining(planExpiresAt) : 0;
    const isOnTrial = trialActive
      && !isPlatformOwnerUser
      && (planTier === TRIAL_CONFIG.trialPlanTier || planTier === 'starter')
      && business?.stripe_subscription_status !== 'manual_payment_active'
      && business?.stripe_subscription_status !== 'manual_dev';

    const value = useMemo(() => ({
        business,
        role: effectiveRole,
        planTier: effectivePlanTier,
        moduleAccess: effectiveModuleAccess,
        rawRole: role,
        rawPlanTier: planTier,
        isPlatformOwner: isPlatformOwnerUser,
        isPlatformAdmin: isPlatformLevelUser,
        isOnTrial,
        trialDaysRemaining,
        trialActive,
        planExpiresAt,
        updateBusiness,
        clearBusiness,
        getSetting,
        isLoading,
        isRevalidating,
        currency,
        currencySymbol,
        regionalStandards,
        regionalPack,
        switchBusinessByDomain
    }), [
        business,
        effectiveRole,
        effectivePlanTier,
        effectiveModuleAccess,
        role,
        planTier,
        isPlatformOwnerUser,
        isPlatformLevelUser,
        isOnTrial,
        trialDaysRemaining,
        trialActive,
        planExpiresAt,
        updateBusiness,
        clearBusiness,
        getSetting,
        isLoading,
        isRevalidating,
        currency,
        currencySymbol,
        regionalStandards,
        regionalPack,
        switchBusinessByDomain,
    ]);

    return (
        <BusinessContext.Provider value={value}>
            {children}
        </BusinessContext.Provider>
    );
}

export function useBusiness() {
    const context = useContext(BusinessContext);
    if (context === undefined) {
        throw new Error('useBusiness must be used within a BusinessProvider');
    }
    return context;
}
