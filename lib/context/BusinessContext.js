...
import {
    ...
    getBusinessDomainFromWindowPath,
...
/**
 * First-paint optimistic shell (Zoho/Busy-style): sync localStorage read so
 * chrome is not blank for one frame waiting on useEffect.
 */
function getInitialOptimisticState() {
    if (typeof window === 'undefined') {
        return { business: null, role: null, regionalStandards: null, isLoading: true };
    }
    const domain = getBusinessDomainFromWindowPath();
    const shell = readOptimisticBusinessShell(domain);
    const valid = hasValidOptimisticShell(shell.business, shell.role, domain);
    return {
        business: shell.business,
        role: shell.role,
        regionalStandards: shell.business
            ? getRegionalStandardsForBusiness(shell.business)
            : null,
        isLoading: !valid,
    };
}
...
export function BusinessProvider({ children }) {
    ...
    const [business, setBusiness] = useState(() => getInitialOptimisticState().business);
    const [role, setRole] = useState(() => getInitialOptimisticState().role);
    const [isLoading, setIsLoading] = useState(() => getInitialOptimisticState().isLoading);
    const [isRevalidating, setIsRevalidating] = useState(false);
    const [regionalStandards, setRegionalStandards] = useState(
        () => getInitialOptimisticState().regionalStandards
    );
    ...
    // Re-apply cache before paint when the URL domain changes (company switch / deep link).
    useLayoutEffect(() => {
                ...
                if (effectiveUserId) {
                    ...
                    if (domainFromUrl) {
                        ...
                        if (!result?.success) {
                            ...
                            // Second RTT only on domain miss — success path stays single-action.
