...
export function BusyModeProvider({ children }) {
    // SSR + first client paint share the same defaults; localStorage sync runs in layout effect only.
    const [isBusyMode, setIsBusyMode] = useState(false);
    const [appMode, setAppModeState] = useState('easy');
    const [dashboardStyle, setDashboardStyleState] = useState('retail_simple');
