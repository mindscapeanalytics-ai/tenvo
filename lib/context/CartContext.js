...
export function CartProvider({ children }) {
  ...
  const addItem = useCallback(async ({ productId, quantity, variantId, businessId }) => {
    if (!businessId) {
    ...
    try {
      const response = await fetch(`/api/storefront/products/${productId}/stock`, {
        ...
        body: JSON.stringify({ variantId, quantity, businessId }),
      });
      ...
      setCart((prev) => {
        const switchingStore =
          prev.businessId && prev.businessId !== businessId && prev.items.length > 0;
        const base = switchingStore
          ? { items: [], businessId: null, lastUpdated: null }
          : prev;
        ...
        let newItems;
        if (existingIndex >= 0) {
          const newQuantity = base.items[existingIndex].quantity + quantity;
          if (newQuantity > maxQuantity) throw new Error(`Maximum ${maxQuantity} items allowed`);
          newItems = base.items.map((item, i) =>
