...
import { HUB_SHELL_PRODUCT_PAGE_LIMIT } from '@/lib/dashboard/hubShellBootstrapConstants';
import { buildDashboardMetricsFromSnapshot } from '@/lib/dashboard/hubBootstrapMetrics';
import {
    clearHubShellCache,
    clearHubShellCacheKey,
    hubShellCacheKey,
    readHubShellCache,
    writeHubShellCache,
...
export function DataProvider({ children }) {
    ...
    /** 'bootstrap' = slim headers; 'full' = customers + quotations (still no bulk line items). */
    ...
    // Stable primitives for dependency arrays — YYYY-MM-DD aligns shell cache, bootstrap, and RQ keys.
    const businessId = business?.id;
    const dateFromISO = toAnalyticsIsoDate(dateRange.from) || dateRange.from.toISOString().slice(0, 10);
    const dateToISO = toAnalyticsIsoDate(dateRange.to) || dateRange.to.toISOString().slice(0, 10);

    const applyHubShellPayload = useCallback((payload, { markReady = true } = {}) => {
        if (!payload) return;
        ...
        setProductTotal(Number(payload.productTotal) || 0);
        setHasMoreProducts(Boolean(payload.hasMoreProducts));
        productOffsetRef.current = Array.isArray(payload.products) ? payload.products.length : 0;
    ...
    const fetchHubShell = useCallback(async ({ force = false } = {}) => {
        ...
        try {
            if (!force) {
                ...
                if (cached && !isStale()) {
                    applyHubShellPayload(cached, { markReady: true });
                }
            } else {
                // Invalidate only this range key — keep other presets warm for instant paint.
                clearHubShellCacheKey(cacheKey);
            }
            ...
            applyHubShellPayload(result, { markReady: true });
    ...
    }, [businessId, dateFromISO, dateToISO, applyHubShellPayload]);
    ...
    useEffect(() => {
        ...
        if (hadPriorKey) fetchHubShell({ force: true });
    }, [businessId, dateFromISO, dateToISO, isShellReady, fetchHubShell]);
    ...
    const fetchInventory = useCallback(async ({
        ...
        try {
            if (wantsProductPage) {
                ...
                const pageProducts = prodRes.products || [];
    ...
    const fetchSales = useCallback(async ({ force = false, mode = 'bootstrap' } = {}) => {
        ...
        try {
            ...
            const customerPromise = wantsFullList
    ...
    useEffect(() => {
        fetchersRef.current = { fetchHubShell, fetchFinance, fetchSales, fetchAnalytics, fetchInventory, fetchPurchases, fetchExpenses };
    });

    // Initial load: shell-first single bootstrap, then deferred purchases.
    ...
    useEffect(() => {
        ...
        queueMicrotask(() => {
            ...
            // Reset module tracking for the new tenant; never leak ready flags across businesses.
            setModuleReady({});
            setLoadingModules({});
            setIsDataLoaded(false);

            // Secondary modules are never in the shell cache — always reset on tenant change.
            setCustomers([]);
            setVendors([]);
            setQuotations([]);
            setSalesOrders([]);
            ...
            if (cached) {
                ...
                // No cache for this tenant/range — clear shell-owned state (avoid cross-tenant paint).
            ...
            // Zoho/Busy/Odoo-style: paint shell immediately; one bootstrap action hydrates Overview.
    ...
    // Idle-prefetch Visual Studio analytics after shell bootstrap (does not block Overview).
    useEffect(() => {
    ...
    }, [isDataLoaded, businessId, dateFromISO, dateToISO, queryClient]);
