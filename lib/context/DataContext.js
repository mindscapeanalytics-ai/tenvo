...
export function DataProvider({ children }) {
    ...
    const applyHubShellPayload = useCallback((payload, { markReady = true, paintedKey = null } = {}) => {
        if (!payload) return;
    ...
    const hydrateHubShellFromServer = useCallback((payload, meta = {}) => {
        ...
        applyHubShellPayload(payload, { markReady: true, paintedKey });
    ...
    }, [applyHubShellPayload, shellRangeKey]);
    ...
    const fetchHubShell = useCallback(async ({ force = false } = {}) => {
        ...
        try {
            if (!force) {
                ...
                if (cached && !isStale()) {
                    applyHubShellPayload(cached, { markReady: true, paintedKey });
                }
            }
            ...
            applyHubShellPayload(result, { markReady: true, paintedKey });
    ...
    useEffect(() => {
        ...
        const cached = readHubShellCache(cacheKey);
        if (cached) {
            applyHubShellPayload(cached, { markReady: true, paintedKey: key });
    ...
    useLayoutEffect(() => {
        ...
        if (cached) {
            ...
            // RSC hydrator already painted — keep Overview live; do not wipe moduleReady.
            setLoadingModules({});
        } else {
