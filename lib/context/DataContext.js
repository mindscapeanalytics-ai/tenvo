...
export function DataProvider({ children }) {
    ...
    // Track which businessId we've already loaded to prevent double-fetch
    // (BusinessContext can fire twice: once from localStorage cache, once from server sync)
    ...
    // Initial load: only re-run when business.id actually changes (not on every
    // fetch-function identity change). Guard against double-fetch from
    // BusinessContext's optimistic localStorage load + subsequent server sync.
    useEffect(() => {
        const id = business?.id;
        if (!id) {
            // Business cleared (logout / switch) — reset so next load triggers fresh fetch
            loadedBusinessIdRef.current = null;
            return;
        }
        // Same business already loaded — skip (handles the localStorage→server double-fire)
        if (loadedBusinessIdRef.current === id) return;
        loadedBusinessIdRef.current = id;

        setIsDataLoaded(false);
        const { fetchFinance, fetchSales, fetchAnalytics, fetchInventory, fetchPurchases, fetchExpenses } = fetchersRef.current;
        Promise.allSettled([
            fetchFinance(),
            fetchSales(),
            fetchAnalytics(),
