...
import { seedHubShellQueryCache } from '@/lib/dashboard/hubShellQuery';
import { prefetchHotHubTabsIdle } from '@/lib/utils/hubTabNavigation';
...
import { useHubShellQuery } from '@/lib/hooks/useHubShellQuery';
...
/** Align with useHubShellQuery staleTime — avoid double full bootstrap on SSR/cache paint. */
const HUB_SHELL_DEFERRED_REVALIDATE_MS = 45_000;
...
export function DataProvider({ children }) {
    ...
    /** Deferred soft-revalidate after SSR/cache paint (avoids double full bootstrap). */
    const shellDeferredRevalidateTimerRef = useRef(null);
    ...
    const hubShellQuery = useHubShellQuery({
        businessId,
        dateFromISO,
        dateToISO,
        enabled: Boolean(businessId),
    });
    const hubShellRefetch = hubShellQuery.refetch;
    ...
    const applyHubShellPayload = useCallback((payload, { markReady = true, paintedKey = null, businessId: payloadBusinessId = null } = {}) => {
        ...
        if (businessId && scopedBusinessId && String(scopedBusinessId) !== String(businessId)) {
            return;
        }
    ...
    const hydrateHubShellFromServer = useCallback((payload, meta = {}) => {
        ...
        seedHubShellQueryCache(queryClient, id, from, to, payload);
    ...
    }, [applyHubShellPayload, shellRangeKey, businessId, queryClient]);

    const fetchHubShell = useCallback(async ({ force = false } = {}) => {
        if (!businessId) return;
        const key = hubShellQueryKey(businessId, dateFromISO, dateToISO);
        if (force) {
            await queryClient.invalidateQueries({ queryKey: key });
        }
        return hubShellRefetch();
    }, [businessId, dateFromISO, dateToISO, queryClient, hubShellRefetch]);

    // React Query hub shell → local hub state (single SOT for bootstrap payload).
    const lastAppliedShellRef = useRef(null);
    useEffect(() => {
        if (!businessId || !hubShellQuery.data) return;
        const qk = hubShellQuery.data?._queryBusinessId;
        if (qk && String(qk) !== String(businessId)) return;
        const paintedKey = shellRangeKey(businessId, dateFromISO, dateToISO);
        const fingerprint = `${paintedKey}|${hubShellQuery.dataUpdatedAt ?? 0}`;
        if (lastAppliedShellRef.current === fingerprint) return;
        lastAppliedShellRef.current = fingerprint;
        applyHubShellPayload(hubShellQuery.data, {
            markReady: true,
            paintedKey,
            businessId,
        });
        ...
        hubShellQuery.data,
        hubShellQuery.dataUpdatedAt,
        businessId,
        dateFromISO,
        dateToISO,
        applyHubShellPayload,
        shellRangeKey,
    ]);
    ...
    useEffect(() => {
        ...
        const alreadyPainted =
            shellPaintedKeyRef.current === paintedKey || Boolean(hubShellQuery.data);
        if (hubShellQuery.isFetching && !alreadyPainted) {
        ...
        } else if (!hubShellQuery.isFetching) {
            setLoadingModules((prev) => ({
                ...prev,
                finance: false,
                sales: false,
    ...
    useEffect(() => {
        if (hubShellQuery.isError && businessId) {
            console.error('Hub shell bootstrap error:', hubShellQuery.error);
            moduleReadyRef.current = {
                ...moduleReadyRef.current,
                finance: true,
                sales: true,
    ...
    }, [hubShellQuery.isError, hubShellQuery.error, businessId]);
    ...
    const fetchFinance = useCallback(async ({ force = false } = {}) => {
        ...
        try {
            ...
            const metricsFromSnapshot = snapshot.success
                ...
                : null;
            // Shell owns Overview KPIs (SOT). Finance refresh updates finance snapshot / charts /
            // GL only — never overwrite painted shell metrics (POS/date use hubShell invalidate).
            if (metricsFromSnapshot) {
                setDashboardMetrics((prev) => prev || metricsFromSnapshot);
            }
    ...
    useEffect(() => {
        ...
        // Cache miss for new date range: layout effect owns SWR paint + immediate refetch.
        // Do NOT null KPIs here (races with layout keep-prior paint → blank Overview flash).
        salesListDepthRef.current = 'bootstrap';
    }, [businessId, dateFromISO, dateToISO, isShellReady, shellRangeKey]);
    ...
    const fetchInventory = useCallback(async ({
        ...
        if (moduleInFlightRef.current.inventory) {
            if (!force && !append && !loadAllPages) return;
            if (force || loadAllPages) {
        ...
        if (!force && !append && !loadAllPages && moduleReadyRef.current.inventory) {
        ...
        try {
            if (wantsProductPage) {
                const locPromise = getWarehouseLocationsAction(businessId);
                let pageProducts = [];
                let total = 0;
                let more = false;
                ...
                if (loadAllPages) {
                    // POS / full browse: drain pages (Busy/Zoho-style complete till catalog).
                    const all = [];
                    let offsetCursor = 0;
                    let pages = 0;
                    let hasMore = true;
                    while (hasMore && pages < 100) {
                        const prodRes = await getProductsAction(businessId, {
                        ...
                        const chunk = scopeProductsToBusiness(prodRes.products || [], businessId);
                        all.push(...chunk);
                        total = Number(prodRes.total ?? all.length);
                        hasMore = Boolean(prodRes.hasMore);
                        offsetCursor += pageLimit;
                        pages += 1;
                    ...
                    pageProducts = scopeProductsToBusiness(prodRes.products || [], businessId);
                    total = Number(prodRes.total ?? pageProducts.length);
                    more = Boolean(prodRes.hasMore);
                    nextOffset = append
                        ? productOffsetRef.current + pageProducts.length
                        : pageProducts.length;
                }

                const locRes = await locPromise;
                if (isStale()) return;

                setProducts((prev) => {
                    if (loadAllPages || !append) return pageProducts;
            ...
            if (wantsProductPage && !append) {
                // Keep painted catalog on refresh failure (POS/browse must not empty mid-session).
                // Only wipe when we had nothing to show yet.
                setProducts((prev) => (Array.isArray(prev) && prev.length > 0 ? prev : []));
                if (!inventoryCatalogReadyRef.current) {
                    setProductTotal(0);
                    setHasMoreProducts(false);
                }
            }
            setLocations((prev) => (Array.isArray(prev) && prev.length > 0 ? prev : []));
            if (wantsProductPage && !append && !inventoryCatalogReadyRef.current) {
                inventoryCatalogReadyRef.current = false;
            }
            moduleReadyRef.current.inventory = true;
            setModuleReady(prev => ({
    ...
    }, [businessId, dateFromISO, dateToISO, queryClient]);
    ...
    const upsertInvoiceInState = useCallback((invoice) => {
        ...
        const invBid = invoice.business_id || invoice.businessId;
        if (businessId && invBid && String(invBid) !== String(businessId)) return false;
        setInvoices((prev) => {
    ...
    const upsertCustomerInState = useCallback((customer) => {
        ...
        const custBid = customer.business_id || customer.businessId;
        if (businessId && custBid && String(custBid) !== String(businessId)) return false;
        setCustomers((prev) => {
            const list = Array.isArray(prev) ? prev : [];
    ...
    }, [businessId, fetchHubShell, fetchInventory, fetchSales, fetchPurchases, fetchManufacturing, fetchAnalytics, fetchPayroll, fetchApprovals, fetchExpenses]);
    ...
    useLayoutEffect(() => {
        ...
        if (alreadyReadyForKey) {
            // SSR/cache already painted this range — do NOT immediately re-run full bootstrap.
            // Mark loaded; defer soft-revalidate to staleTime window (matches RQ staleTime).
            if (shellSoftFetchKeyRef.current === paintedKey) return undefined;
            shellSoftFetchKeyRef.current = paintedKey;
            const generation = fetchGenerationRef.current;
            const { fetchPurchases } = fetchersRef.current;
            setIsDataLoaded(true);
            void fetchPurchases?.();
            if (shellDeferredRevalidateTimerRef.current) {
                clearTimeout(shellDeferredRevalidateTimerRef.current);
            }
            shellDeferredRevalidateTimerRef.current = setTimeout(() => {
                shellDeferredRevalidateTimerRef.current = null;
                if (fetchGenerationRef.current !== generation) return;
                if (shellSoftFetchKeyRef.current !== paintedKey) return;
                void Promise.resolve(hubShellRefetch()).catch(() => {});
            }, HUB_SHELL_DEFERRED_REVALIDATE_MS);
            return () => {
                if (shellDeferredRevalidateTimerRef.current) {
                    clearTimeout(shellDeferredRevalidateTimerRef.current);
                    shellDeferredRevalidateTimerRef.current = null;
        ...
        const generation = ++fetchGenerationRef.current;
        // Capture prior range key before we overwrite — identity change must refetch immediately.
        const priorPaintedKey = financeDateKeyRef.current;
        ...
        const warmTenantSwitch = tenantChanged && Boolean(cached);
        const identityChanged =
            tenantChanged ||
            (priorPaintedKey != null && priorPaintedKey !== paintedKey);
        shellSoftFetchKeyRef.current = paintedKey;
        lastAppliedShellRef.current = null;
        ...
        // Hard-clear inventory only on shop switch without warm cache.
        if (tenantChanged) {
            setCustomers([]);
            setVendors([]);
            setQuotations([]);
        ...
        if (cached) {
            ...
            setLoadingModules({});
            if (!tenantChanged) {
                // Date-only: keep prior KPIs painted until new shell arrives (SWR warm paint).
                // Drop range-scoped lists that would show wrong-range rows.
                setInvoices([]);
                setActivityFeed([]);
            } else {
        ...
        const { fetchPurchases } = fetchersRef.current;
        const paintedWarm = Boolean(cached) || serverAlreadyPainted;
        // Defer soft-revalidate only for cold entry same key (SSR/session already paid).
        // Date/tenant identity changes must refetch immediately (Zoho/Busy instant filter feel).
        const deferSoftRevalidate = paintedWarm && !identityChanged;
        ...
        if (deferSoftRevalidate) {
            // Avoid double full bootstrap on cold entry (SSR/session already paid once).
            setIsDataLoaded(true);
            void fetchPurchases?.();
            shellDeferredRevalidateTimerRef.current = setTimeout(() => {
                shellDeferredRevalidateTimerRef.current = null;
        ...
        Promise.resolve(hubShellRefetch())
            .catch(() => {})
            .finally(() => {
                if (fetchGenerationRef.current !== generation) return;
                setIsDataLoaded(true);
        ...
        return undefined;
    }, [businessId, dateFromISO, dateToISO, applyHubShellPayload, shellRangeKey, hubShellRefetch]);
    ...
    // Warm hot-tab JS chunks as soon as shell is painted (not only on HubTabProvider mount).
    useEffect(() => {
        if (!isShellReady || !businessId) return undefined;
        prefetchHotHubTabsIdle();
        return undefined;
    }, [isShellReady, businessId]);
