...
export function DataProvider({ children }) {
    ...
    const fetchInventory = useCallback(async ({ force = false, includeSerials } = {}) => {
        ...
        // Force / post-save refreshes load serials in one shot (Busy/Zoho grid accuracy).
        // First paint may omit serials, then upgrade in the background.
        const loadSerials = includeSerials === true || (includeSerials !== false && force);
        try {
            if (loadSerials) {
                const [prodRes, locRes] = await Promise.all([
                    productAPI.getAll(businessId, { includeSerials: true }),
            ...
            // Fast path: skip serial rows so Easy KPIs unlock sooner.
            const [prodRes, locRes] = await Promise.all([
